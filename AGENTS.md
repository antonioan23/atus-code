...
This file provides guidance to Atus Code when working with code in this
repository.
...
cd integration-tests && \
  cross-env ATUS_SANDBOX=false npx vitest run cli interactive
```
...
1. **Design doc for non-trivial work** — write one in `.atus-code/design/` if the
...
2. **Test plan for behavioral changes** — write an E2E test plan in
   `.atus-code/e2e-tests/` when the change affects user-observable behavior. Dry-run
   against the global `atus` CLI first to confirm the baseline.
...
Project artifacts live under `.atus-code/`:

| Directory               | Purpose                              |
| ----------------------- | ------------------------------------ |
| `.atus-code/design/`         | Design docs for planned features     |
| `.atus-code/e2e-tests/`      | E2E test plans and results           |
| `.atus-code/issues/`         | Issue drafts before filing on GitHub |
| `.atus-code/pr-drafts/`      | PR drafts before submitting          |
| `.atus-code/pr-reviews/`     | PR review notes                      |
| `.atus-code/investigations/` | Structured debugging journals        |
| `.atus-code/scripts/`        | Utility scripts                      |
