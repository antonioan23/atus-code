...
function InsightApp({ data }: { data: InsightData }) {
  ...
  const performExport = async () => {
    ...
    try {
      ...
      link.href = imgData;
      link.download = `atus-insights-card-${new Date().toISOString().slice(0, 10)}.png`;
      link.click();
    } catch (error) {
  ...
  const showFeatures =
    !!data.qualitative &&
    (hasMeaningfulArray(data.qualitative.improvements?.Atus_md_additions) ||
      ...
      <header className="insights-header">
        <div className="header-content">
          <div className="header-title-section">
            <h1 className="header-title">atus code Insights</h1>
