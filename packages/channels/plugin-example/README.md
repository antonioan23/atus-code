# @atus-code/channel-plugin-example

A reference channel plugin for Atus Code. It connects to a WebSocket server and routes messages through the full channel pipeline (access control, session routing, agent bridge).
...
```bash
npm install @atus-code/channel-plugin-example
```

### 2. Link it as a Atus Code extension

The package ships a `atus-extension.json` manifest, so it works as an extension out of the box:

```bash
atus extensions link ./node_modules/@atus-code/channel-plugin-example
```

### 3. Configure the channel

Add a channel entry to `~/.atus-code/settings.json`:
...
```bash
npx atus-channel-plugin-example-server
```

The server prints the HTTP and WebSocket URLs. You can customize ports with environment variables:

```bash
HTTP_PORT=8080 WS_PORT=8081 npx atus-channel-plugin-example-server
```
...
```bash
atus channel start my-plugin-test
```
...
4. Add a `atus-extension.json` manifest
