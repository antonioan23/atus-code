# @atus-code/channel-base

Base infrastructure for building Atus Code channel adapters. Provides the abstract base class, access control, session routing, and the ACP bridge that communicates with the agent.
...
```bash
npm install @atus-code/channel-base
```
...
```typescript
import { ChannelBase } from '@atus-code/channel-base';
import type {
  ChannelConfig,
  Envelope,
  AcpBridge,
} from '@atus-code/channel-base';
...
For a complete working example, see [`@atus-code/channel-plugin-example`](../plugin-example/).
...
| `AcpBridge`     | Spawns and communicates with the `atus-code --acp` agent process |
...
Manages the `atus-code --acp` child process and ACP sessions.
...
Persists pairing state to `~/.atus-code/channels/{channelName}-pairing.json` and `{channelName}-allowlist.json`.
...
- [`@atus-code/channel-plugin-example`](../plugin-example/) — working reference implementation
