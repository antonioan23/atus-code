...
describe('validateImagePath', () => {
  ...
  it('allows Windows paths inside the workspace directory', () => {
    const imagePath = 'D:\\WorkGroup\\AtusCode\\002\\hello.png';
    const workspaceDir = 'D:\\WorkGroup\\AtusCode\\002';
    mockRealpathSync.mockImplementation((p: string) => {
      if (p.includes('hello.png')) return imagePath;
      if (p.includes('AtusCode\\002')) return workspaceDir;
      return p;
    });
  ...
  it('rejects Windows paths in a sibling directory with the same prefix', () => {
    const imagePath = 'D:\\WorkGroup\\AtusCode\\0022\\hello.png';
    const workspaceDir = 'D:\\WorkGroup\\AtusCode\\002';
    mockRealpathSync.mockImplementation((p: string) => {
      if (p.includes('hello.png')) return imagePath;
      if (p.includes('AtusCode\\002')) return workspaceDir;
