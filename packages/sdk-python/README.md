...
Experimental Python SDK for programmatic access to atus code through the
...
- Python `>=3.10`
- External `atus` CLI installed and available in `PATH`
...
- `path_to_qwen_executable`: `atus`, an absolute binary path, or a `.js` CLI
  bundle
...
- `ProcessExitError`: `atus` exited with a non-zero code
...
- Uses external `atus` CLI via process transport
