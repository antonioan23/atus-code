...
import pytest
from atus_code_sdk.transport import build_cli_arguments, prepare_spawn_info
from atus_code_sdk.types import QueryOptions, TimeoutOptions
...
def test_prepare_spawn_info_uses_runtime_for_python_scripts(tmp_path: Path) -> None:
    script_path = tmp_path / "fake-atus.py"
...
def test_prepare_spawn_info_uses_node_for_javascript_files(tmp_path: Path) -> None:
    script_path = tmp_path / "fake-atus.js"
...
def test_prepare_spawn_info_keeps_plain_command_names() -> None:
    spawn_info = prepare_spawn_info("atus-custom")

    assert spawn_info.command == "atus-custom"
...
def test_prepare_spawn_info_defaults_to_qwen_when_none() -> None:
    ...
    assert spawn_info.command == "atus"
    assert spawn_info.args == []
