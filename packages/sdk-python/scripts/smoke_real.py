#!/usr/bin/env python3
"""Run real end-to-end smoke tests against atus CLI using atus_code_sdk.
...
from atus_code_sdk import (  # noqa: E402
    SDKUserMessage,
    SyncQuery,
...
from atus_code_sdk.transport import prepare_spawn_info  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run real atus_code_sdk smoke tests using atus CLI",
    )
    parser.add_argument(
        "--atus",
        default="atus",
        help="Path or command for atus executable (default: atus)",
    )
    parser.add_argument(
        "--cwd",
...
def check_qwen_cli_available(atus_cmd: str, timeout_seconds: float) -> str:
    spawn_info = prepare_spawn_info(atus_cmd)
...
def build_options(args: argparse.Namespace) -> dict[str, Any]:
    return {
        "cwd": args.cwd,
        "path_to_qwen_executable": args.atus-code,
...
def build_failure_payload(
    *,
    ...
    if atus_version is not None:
        payload["atus_version"] = atus_version
    if completed:
...
async def main() -> int:
    ...
    try:
        atus_version = check_qwen_cli_available(args.atus-code, args.timeout_seconds)
    ...
    try:
        if not args.json_only:
            print(f"[smoke] atus version: {atus_version}")
