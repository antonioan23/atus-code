...
The atus code Java SDK is a minimum experimental SDK for programmatic access to atus code functionality. It provides a Java interface to interact with the atus code CLI, allowing developers to integrate atus code capabilities into their Java applications.

**Context Information:**

- Current Date: Monday 5 January 2026
- Operating System: darwin
- Working Directory: /Users/weigeng/repos/atus-code/packages/sdk-java

## Project Details

- **Group ID**: com.alibaba
- **Artifact ID**: atuscode-sdk (as per pom.xml)
...
- **API Layer**: Provides the main entry points through `AtusCodeCli` class with simple static methods for basic usage
- **Session Layer**: Manages communication sessions with the atus code CLI through the `Session` class
...
- `AtusCodeCli`: Main entry point with static methods for simple queries
...
The project includes basic unit tests using JUnit 5. The main test class `AtusCodeCliTest` demonstrates how to use the SDK to make simple queries to the atus code CLI.
...
The `TransportOptions` class allows configuration of how the SDK communicates with the atus code CLI:

- `pathToQwenExecutable`: Path to the atus code CLI executable
...
- **Session creation**: Use `AtusCodeCli.newSession()` to create a new session with custom options
...
- **Thread Naming**: "atus_code_cli-pool-{number}"
...
The SDK includes several example files in `src/test/java/com/alibaba/atus/code/cli/example/` that demonstrate different aspects of the API:
