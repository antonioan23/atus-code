...
async function makeTempRepo(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'atus-wt-startup-test-'));
...
describe('setupStartupWorktree', () => {
  ...
  it('rejects when the launch cwd is not a git repo', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'atus-wt-nongit-'));
    tempRepo = dir;
  ...
  it('creates a worktree with an auto-generated slug for bare --worktree', async () => {
    ...
    if (res!.ok) {
      ...
      expect(res!.context.worktreePath).toContain(
        path.join('.atus-code', 'worktrees', res!.context.slug),
      );
  ...
  it('creates a worktree with an explicit slug', async () => {
    ...
    if (res!.ok) {
      ...
      expect(res!.context.worktreePath).toBe(
        path.join(tempRepo, '.atus-code', 'worktrees', 'my-feature'),
      );
    }
  });
  ...
  it('rejects invalid slug characters before any git operation', async () => {
    ...
    const exists = await fs
      .stat(path.join(tempRepo, '.atus-code', 'worktrees'))
      .then(() => true)
      .catch(() => false);
  ...
  it('rejects full GitHub PR URLs when origin remote is missing', async () => {
    ...
    const res = await setupStartupWorktree(
      'https://github.com/atuscode/atus-code/pull/4174',
    );
    expect(res).not.toBeNull();
    expect(res!.ok).toBe(false);
    if (!res!.ok) {
  ...
  it('creates a pr-<N> worktree from FETCH_HEAD when fetch succeeds (local fake remote)', async () => {
    ...
    const upstream = await fs.mkdtemp(
      path.join(os.tmpdir(), 'atus-wt-pr-upstream-'),
    );
    ...
    try {
      ...
      if (res!.ok) {
        ...
        expect(res!.context.worktreePath).toBe(
          path.join(tempRepo, '.atus-code', 'worktrees', 'pr-42'),
        );
  ...
  it('re-attaches to an existing worktree instead of erroring (Phase 6 G1 fix)', async () => {
    ...
    // the real `atus --resume <sid> --worktree foo` invocation flow.
    process.chdir(tempRepo);
  ...
  it('refuses nested worktree creation from inside .atus-code/worktrees/', async () => {
    tempRepo = await makeTempRepo();
    ...
    const nestedPath = path.join(tempRepo, '.atus-code', 'worktrees', 'outer');
...
describe('buildStartupWorktreeNotice', () => {
  ...
  const baseContext = {
    worktreePath: '/repo/.atus-code/worktrees/foo',
    slug: 'foo',
    branch: 'worktree-foo',
    wasReattached: false,
  };
  ...
  it('produces a single line for the no-override created case', () => {
    ...
    expect(notice).toContain('/repo/.atus-code/worktrees/foo');
  ...
  it('appends an override hint when a previous worktree was overridden', () => {
    ...
    expect(notice).toContain('atus --worktree old-slug');
  });
