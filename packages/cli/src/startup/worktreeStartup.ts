...
import type { Config, WorktreeSession } from '@atus-code/atus-code-core';
...
export async function setupStartupWorktree(
  ...
  // worktree creates `<otherRepo>/.atus-code/worktrees/<slug>/`, which is rarely
  // what the user wants and corrupts ownership tracking.
  if (/[\\/]\.atus-code[\\/]worktrees[\\/]/.test(launchCwd)) {
    return {
      ok: false,
  ...
  // case: user did `atus --worktree foo` previously, exited with Keep,
  // and now runs `atus --resume <sid> --worktree foo` to continue. The
...
export function buildStartupWorktreeNotice(
  ...
  if (override?.overrodeResumedWorktree && override.overriddenSlug) {
    return (
      ...
      `That worktree directory was left intact; re-attach with \`atus --worktree ${override.overriddenSlug}\` if needed.`
    );
  }
  return base;
}
