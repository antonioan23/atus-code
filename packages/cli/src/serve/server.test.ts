...
import { ApprovalMode, TrustGateError } from '@atus-code/atus-code-core';
import {
  ...
  describe('POST /session', () => {
    ...
    it('200 with the BridgeSession shape on success', async () => {
      ...
      const res = await request(app)
        .post('/session')
        .set('Host', `127.0.0.1:${baseOpts.port}`)
        .send({ cwd: '/work/a', modelServiceId: 'atus-prod' });
      expect(res.status).toBe(200);
      ...
      expect(bridge.calls).toEqual([
        { workspaceCwd: '/work/a', modelServiceId: 'atus-prod' },
      ]);
    });
...
describe('runQwenServe', () => {
  ...
  afterEach(async () => {
    ...
    delete process.env['ATUS_SERVER_TOKEN'];
  });
  ...
  it('does not mutate process.env when caller provides mcp budget options (#4247 R6 line 216)', async () => {
    ...
    delete process.env['ATUS_SERVE_MCP_CLIENT_BUDGET'];
    delete process.env['ATUS_SERVE_MCP_BUDGET_MODE'];
    ...
    expect(process.env['ATUS_SERVE_MCP_CLIENT_BUDGET']).toBeUndefined();
    expect(process.env['ATUS_SERVE_MCP_BUDGET_MODE']).toBeUndefined();
  });
  ...
  it('preserves pre-existing process.env values (no longer wipes globals on omit) (#4247 R6 line 216)', async () => {
    ...
    // operator had ATUS_SERVE_MCP_CLIENT_BUDGET exported in their
    ...
    process.env['ATUS_SERVE_MCP_CLIENT_BUDGET'] = '99';
    try {
      handle = await runQwenServe({
        hostname: '127.0.0.1',
        port: 0,
      ...
      expect(process.env['ATUS_SERVE_MCP_CLIENT_BUDGET']).toBe('99');
    } finally {
      delete process.env['ATUS_SERVE_MCP_CLIENT_BUDGET'];
    }
  });
  ...
  it('accepts ATUS_SERVER_TOKEN from the env when binding non-loopback', async () => {
    process.env['ATUS_SERVER_TOKEN'] = 'env-secret';
    handle = await runQwenServe({
      hostname: '0.0.0.0',
      port: 0,
      mode: 'http-bridge',
  ...
  it('--workspace flows end-to-end and surfaces on /capabilities (#3803 §02)', async () => {
    ...
    // making this a true E2E that doesn't require a real `atus --acp`
    // child.
    const bridge = fakeBridge();
    handle = await runQwenServe(
      {
  ...
  it('rejects --workspace pointing at a non-existent directory (BkUyD followup — boot-loud over opaque ENOENT)', async () => {
    ...
    // would then spawn a `atus --acp` child with that cwd and the
    ...
    await expect(
      runQwenServe({
        ...
        port: 0,
        mode: 'http-bridge',
        workspace: `/tmp/atus-serve-no-such-path-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      }),
  ...
  it('wires fsFactory + emit through to the read routes (#4175 PR 19 follow-up #2)', async () => {
    ...
    const wsRoot = await fsp.mkdtemp(
      path.join(os.tmpdir(), 'atus-runqwen-fs-'),
    );
  ...
  it('trust snapshot defaults to true (operator-chosen workspace)', async () => {
    ...
    const bridge = fakeBridge();
    const wsRoot = await fsp.mkdtemp(
      path.join(os.tmpdir(), 'atus-runqwen-trust-'),
    );
    try {
      const captured: BridgeEvent[] = [];
      handle = await runQwenServe(
  ...
  it('trust snapshot=false flows through deps.trustedWorkspace into the boundary (#4175 PR 19 follow-up #2)', async () => {
    ...
    const wsRoot = await fsp.mkdtemp(
      path.join(os.tmpdir(), 'atus-runqwen-untrust-'),
    );
    try {
...
describe('createServeApp ServeAppDeps.fsFactory wiring (#4175 PR 18)', () => {
  ...
  it('default fsFactory is built with trusted=false (writes refused)', async () => {
    ...
    const tmp = await import('node:fs').then((m) =>
      m.promises.mkdtemp(path.join(os.tmpdir(), 'atus-serve-default-trust-')),
    );
    try {
      const app = createServeApp(
        {
...
describe('auth device-flow routes', () => {
  ...
  function makeFakeProvider(): {
    ...
    return {
      provider: {
        providerId: 'atus-oauth' as const,
        async start() {
          starts += 1;
          return {
            deviceCode:
  ...
  it('POST /workspace/auth/device-flow returns 201 on fresh start with redacted body', async () => {
    ...
    const res = await request(app)
      ...
      .set('Host', `127.0.0.1:${baseOpts.port}`)
      .send({ providerId: 'atus-oauth' });
    expect(res.status).toBe(201);
    expect(res.body.providerId).toBe('atus-oauth');
  ...
  it('POST with unknown providerId returns 400 unsupported_provider', async () => {
    ...
    expect(res.body.supportedProviders).toContain('atus-oauth');
  });
  ...
  it('GET /workspace/auth/status surfaces pending flows and supported providers', async () => {
    ...
    expect(status.body.supportedDeviceFlowProviders).toContain('atus-oauth');
  ...
  it('sweeper-driven auto-expiry transitions a stale entry to status:error and surfaces over GET', async () => {
    ...
    const registry = new DeviceFlowRegistry({
      events: { publish: () => {} },
      resolveProvider: (id) => (id === 'atus-oauth' ? fakeProvider : undefined),
      now: () => now,
  ...
  it('GET /workspace/auth/device-flow/:id returns 400 invalid_client_id when X-Qwen-Client-Id is malformed (atus-latest review N3)', async () => {
    // PR #4291 follow-up review (atus-latest, N3): the GET handler's
  ...
  it('GET /workspace/auth/device-flow/:id returns userCode for an anonymously-started flow when the GET caller is also anonymous', async () => {
    // PR #4291 follow-up review (atus-latest, #3): the original
