...
/**
 * Device-flow authorization registry for `atus serve` (issue #4175 Wave 4
...
export const DEVICE_FLOW_SUPPORTED_PROVIDERS = ['atus-oauth'] as const;
...
export interface BrandedSecret<T extends string = string> {
   ...
   *  `BrandedSecret<'atus-oauth'>` is distinguishable from
...
export type DeviceFlowPollResult =
  ...
  | {
       ...
       *  `atusDeviceFlowProvider` is the canonical example. */
...
export interface DeviceFlowProvider {
   ...
   *
   * `atusDeviceFlowProvider` is the canonical example — see PR #4255
 ...
 * follow-up review (atus-latest): the catch block previously routed
...
export class DeviceFlowRegistry {
  ...
  private async runPollTick(
    ...
    //
    // PR #4291 follow-up review (atus-latest, #5): keep a reference to
    ...
    try {
      ...
      //
      // PR #4291 follow-up review (atus-latest, #2): the previous
    ...
    // PR #4291 follow-up review (atus-latest, #5): if our race timer
      ...
      void tracked.then(
        (latePollResult) => {
          this.deps.audit?.record({
            ...
            status: 'failed',
            errorKind: 'upstream_error',
            // PR #4291 follow-up review (atus-latest, N1): the late-
