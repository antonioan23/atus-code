...
class FakeProvider implements DeviceFlowProvider {
  readonly providerId: DeviceFlowProviderId = 'atus-oauth';
  startCount = 0;
  pollCount = 0;
...
function buildRegistry(provider: FakeProvider) {
  ...
  const registry = new DeviceFlowRegistry({
    ...
    resolveProvider: (id) => (id === 'atus-oauth' ? provider : undefined),
    now: env.now,
...
describe('DeviceFlowRegistry — start / public view', () => {
  ...
  it('idempotent take-over for the same providerId', async () => {
    const first = await registry.start({ providerId: 'atus-oauth' });
    expect(first.attached).toBe(false);
    expect(provider.startCount).toBe(1);
    const second = await registry.start({ providerId: 'atus-oauth' });
  ...
  it('concurrent start() for the same providerId coalesces — provider.start fires once', async () => {
    ...
    const [first, second, third] = await Promise.all([
      registry.start({ providerId: 'atus-oauth' }),
      registry.start({ providerId: 'atus-oauth' }),
      registry.start({ providerId: 'atus-oauth' }),
    ]);
    expect(provider.startCount).toBe(1);
...
describe('DeviceFlowRegistry — polling state machine', () => {
  ...
  it('does NOT import child_process or browser-launch helpers anywhere in the device-flow source path', () => {
    ...
    const sources = [
      ...
      fs.readFileSync(path.join(dir, 'atusDeviceFlowProvider.ts'), 'utf8'),
    ];
...
describe('DeviceFlowRegistry — authoritative timeouts (fold-in 7)', () => {
  it('start() rejects when a non-abortable provider.start() hangs past START_TIMEOUT_MS (#1)', async () => {
    ...
    try {
      const startPromise = registry.start({ providerId: 'atus-oauth' });
  ...
  it('poll() that hangs past POLL_TIMEOUT_MS surfaces as upstream_error and aborts the entry signal (follow-up review)', async () => {
    ...
    try {
      const { view } = await registry.start({ providerId: 'atus-oauth' });
      ...
      if (failed && failed.emission.type === 'failed') {
        ...
        // PR #4291 follow-up review (atus-latest, #2): the SSE/HTTP
      ...
      // PR #4291 follow-up review (atus code review summary):
  ...
  it('records lost_late_poll_after_timeout when provider.poll() resolves AFTER the registry race timeout (follow-up review #5)', async () => {
    // PR #4291 follow-up review (atus-latest, #5): symmetric with
    ...
    try {
      ...
      // PR #4291 follow-up review (atus-latest, N1): kind=pending is a
  ...
  it('records lost_late_poll_after_timeout with abort-driven hint when late resolution is kind=error (atus-latest review N1)', async () => {
    // PR #4291 follow-up review (atus-latest, N1): when the registry
  ...
  it('records lost_late_poll_after_timeout when provider.poll() REJECTS after the race timeout (atus-latest review N2)', async () => {
    // PR #4291 follow-up review (atus-latest, N2): the late observer
    ...
    const { registry, env, auditLines } = built;
    try {
      await registry.start({ providerId: 'atus-oauth' });
      env.clock.tick(DEVICE_FLOW_DEFAULT_INTERVAL_MS + 1);
      env.scheduler.flushDue(env.clock.now);
      await flushAsync();
      env.clock.tick(DEVICE_FLOW_POLL_TIMEOUT_MS + 1);
  ...
  it("does NOT double-audit when late rejection is the registry's own DeviceFlowPollTimeoutError (atus-latest review N2 guard)", async () => {
    // PR #4291 follow-up review (atus-latest, N2): the late-rejection
