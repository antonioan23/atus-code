...
import type { ApprovalMode } from '@atus-code/atus-code-core';
import {
  TrustGateError,
  getCurrentGeminiMdFilename,
} from '@atus-code/atus-code-core';
import type {
  CancelNotification,
  Client,
  PromptRequest,
 ...
 *   - One `atus --acp` child total; multiple sessions multiplex onto it
...
// `@atus-code/acp-bridge/bridgeTypes` in #4175 PR 22b. Imported AND
...
import type {
  ...
  HttpAcpBridge,
} from '@atus-code/acp-bridge/bridgeTypes';
export type {
  BridgeSpawnRequest,
  BridgeSession,
  BridgeRestoreSessionRequest,
...
// Bridge errors lifted to `@atus-code/acp-bridge/bridgeErrors` in
// #4175 PR 22b. `MAX_WORKSPACE_PATH_LENGTH` lifted to
// `@atus-code/acp-bridge/workspacePaths` in the same slice.
...
} from '@atus-code/acp-bridge/bridgeErrors';
import { MAX_WORKSPACE_PATH_LENGTH } from '@atus-code/acp-bridge/workspacePaths';
export {
  SessionNotFoundError,
  RestoreInProgressError,
  InvalidSessionScopeError,
...
// `@atus-code/acp-bridge` in #4175 PR 22a so `channels/base/AcpBridge.ts`
...
// https://github.com/atuscode/atus-code/pull/3889#issuecomment-4427773706

// `BridgeOptions` + `DaemonStatusProvider` lifted to
// `@atus-code/acp-bridge/bridgeOptions` in #4175 PR 22b/2 — the
...
} from '@atus-code/acp-bridge/bridgeOptions';
export type { BridgeOptions, DaemonStatusProvider };

/**
 * The single `atus --acp` child + the ACP connection on top of it,
...
function isServeDebugLoggingEnabled(): boolean {
  const value = process.env['ATUS_SERVE_DEBUG'];
...
function writeServeDebugLine(message: string): void {
  ...
  writeStderrLine(`atus serve debug: ${message}`);
}

// `InvalidPermissionOptionError` lifted to
// `@atus-code/acp-bridge/bridgeErrors` in #4175 PR 22b — see
...
// `@atus-code/acp-bridge/bridgeErrors` in #4175 PR 22b — see the
 ...
 *
 * Stage 1 trust model: the spawned `atus --acp` child runs as the same user
...
class BridgeClient implements Client {
  ...
  async requestPermission(
    ...
    if (entry.pendingPermissionIds.size >= this.maxPendingPerSession) {
      writeStderrLine(
        `atus serve: session ${entry.sessionId} exceeded ` +
    ...
    return await new Promise<RequestPermissionResponse>((resolve) => {
      ...
      if (this.permissionTimeoutMs > 0) {
        timer = setTimeout(() => {
          if (settled) return;
          writeStderrLine(
            `atus serve: session ${entry.sessionId} permission ` +
   ...
   * method is recognized today — `atus/notify/session/mcp-budget-event`
  ...
  async extNotification(
    ...
    if (method !== 'atus/notify/session/mcp-budget-event') return;
  ...
  private bufferEarlyEvent(
    ...
    if (
      ...
      writeStderrLine(
        `atus serve: dropping mcp guardrail extNotification ` +
...
export function createHttpAcpBridge(opts: BridgeOptions): HttpAcpBridge {
  ...
  /**
   * Get-or-create the daemon's single `atus --acp` channel (#3803 §02).
  ...
  async function ensureChannel(): Promise<ChannelInfo> {
    ...
    const promise = (async () => {
      ...
      if (aliveChannels.size > 2) {
        writeStderrLine(
          `atus serve: WARNING aliveChannels.size=${aliveChannels.size} ` +
      ...
      void channel.exited.then((exitInfo) => {
        ...
        // can't tell from `atus serve`'s own output that the agent
        // process is gone.
        //
        ...
        if (!shuttingDown) {
          writeStderrLine(
            `atus serve: channel exited (code=${exitInfo?.exitCode ?? 'none'}, signal=${exitInfo?.signalCode ?? 'none'}, ${sessions.length} session(s) torn down)`,
          );
        }
      ...
      try {
        await withTimeout(
          connection.initialize({
            ...
            clientInfo: { name: 'atus-serve-bridge', version: '0' },
          }),
          initTimeoutMs,
          'initialize',
        );
  ...
  return {
    ...
    async closeSession(sessionId, context) {
      ...
      writeStderrLine(
        `atus serve: closing session ${JSON.stringify(sessionId)}` +
          (originatorClientId
      ...
      if (ci && ci.sessionIds.size === 0 && ci.pendingRestoreIds.size === 0) {
        ...
        await ci.channel.kill().catch((err) => {
          writeStderrLine(
            `atus serve: closeSession channel kill failed for session ` +
    ...
    updateSessionMetadata(sessionId, metadata, context) {
      ...
      if (metadata.displayName !== undefined) {
        ...
        if (entry.displayName !== nextDisplayName) {
          ...
          writeStderrLine(
            `atus serve: updated session metadata ${JSON.stringify(sessionId)} ` +
    ...
    publishWorkspaceEvent(event) {
      ...
      // ATUS_SERVE_DEBUG would let a true regression succeed at the
      ...
      for (const entry of sessions) {
        try {
          ...
          if (shuttingDown) {
            ...
            writeStderrLine(`atus serve: ${detail}`);
          }
        }
      }
      if (sessions.length > 0 && successCount === 0 && !shuttingDown) {
        writeStderrLine(
          `atus serve: publishWorkspaceEvent type=${event.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`,
        );
      }
    },
    ...
    async getWorkspaceEnvStatus() {
      ...
      try {
        ...
        writeStderrLine(
          `atus serve: statusProvider.getEnvStatus failed; ` +
    ...
    async getWorkspacePreflightStatus() {
      ...
      if (!opts.statusProvider) {
        ...
        try {
          ...
          writeStderrLine(
            `atus serve: statusProvider.getDaemonPreflightCells failed; ` +
    ...
    async setSessionApprovalMode(sessionId, mode, opts, context) {
      // #4175 Wave 4 PR 17. Forwards through `atus/control/session/
      ...
      // #4282 fold-in 4 (atus-latest C1): validate the persist contract
      ...
      // #4282 fold-in 4 (atus-latest S2): when the change is persisted to
 ...
 * CLI's entry script in `--acp` mode. `process.argv[1]` resolves to the atus
 * entry script when launched via the `atus` bin shim.
 *
...
export const defaultSpawnChannelFactory: ChannelFactory = async (
  ...
  // Resolution order:
  //   1. `ATUS_CLI_ENTRY` env override — escape hatch for non-standard
  ...
  //   2. `process.argv[1]` — works when launched via the `atus` bin
  //      shim, which is the common path.
  // Fail loudly with an actionable error if neither resolves.
  const cliEntry = process.env['ATUS_CLI_ENTRY'] || process.argv[1];
  if (!cliEntry) {
  ...
  // `atus serve` or set `process.execPath` to a tsx-aware shim. Stage 1
  ...
  // `ANTHROPIC_API_KEY` / `ATUS_*` / `DASHSCOPE_API_KEY` / a custom
  ...
  // we MUST scrub is `ATUS_SERVER_TOKEN` (daemon-only auth that
  ...
  // `ATUS_SERVER_TOKEN` in overrides — defense in depth).
...
/**
 * Environment variables stripped from the spawned `atus --acp` child's
 ...
 *
 * Currently just `ATUS_SERVER_TOKEN`: the daemon's own bearer token,
