...
function buildApp(opts: {
  ...
  mountWorkspaceMemoryRoutes(app, {
    ...
    parseClientId: (req, res) => {
      const raw = req.get('x-atus-client-id');
...
describe('workspace memory routes', () => {
  ...
  beforeEach(async () => {
    tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'atus-serve-memory-'));
  ...
  describe('POST /workspace/memory', () => {
    ...
    it('writes to the global ~/.atus-code directory when scope=global', async () => {
      const bridge = buildBridgeStub();
      const app = buildApp({ bridge, boundWorkspace: workspace });
      const res = await request(app)
        .post('/workspace/memory')
    ...
    it('omits errorMessage + filePath in 500/413 responses unless ATUS_SERVE_DEBUG is on', async () => {
      ...
      // for the path. Setting ATUS_SERVE_DEBUG=1 enables both.
      const bridge = buildBridgeStub();
      const app = buildApp({ bridge, boundWorkspace: workspace });
      ...
      const prevDebug = process.env['ATUS_SERVE_DEBUG'];
      try {
        delete process.env['ATUS_SERVE_DEBUG'];
        ...
        // detail.
        process.env['ATUS_SERVE_DEBUG'] = '1';
        ...
        if (prevDebug === undefined) delete process.env['ATUS_SERVE_DEBUG'];
        else process.env['ATUS_SERVE_DEBUG'] = prevDebug;
