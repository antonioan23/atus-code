 ...
 *
 * `http-bridge` (Stage 1): per #3803 §02, one `atus --acp` child per
...
export interface ServeOptions {
   ...
   * `atus --acp` child via `connection.newSession()`.
   *
   ...
   * systemd / docker-compose / k8s / `atus-coordinator` reference
   ...
   * forwards `ATUS_SERVE_MCP_CLIENT_BUDGET` to the child's env so
...
export interface CapabilitiesEnvelope {
   ...
   * on incompatible frame changes" stance (see `atus-serve-protocol.md`
