 ...
 * (declared in `@atus-code/acp-bridge/bridgeOptions`). Production
 * `atus serve` wires this into `BridgeOptions.statusProvider` so the
...
import { canUseRipgrep } from '@atus-code/atus-code-core';
import {
...
/**
 * Construct the production `DaemonStatusProvider` for `atus serve`.
  ...
  const cliEntryCell = (): ServePreflightCell => {
    ...
    if (cliEntry) {
      return {
        ...
        detail: {
          path: cliEntry,
          source: process.env['ATUS_CLI_ENTRY']
            ? 'ATUS_CLI_ENTRY'
            : 'process.argv[1]',
        },
      };
    ...
    return {
      ...
      hint: 'Set ATUS_CLI_ENTRY to the absolute path of the atus entry script.',
      locality: 'daemon',
    };
  };
