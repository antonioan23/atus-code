 ...
 * plus the user's `~/.atus-code/` global. Read-only; returns
...
export function mountWorkspaceMemoryRoutes(
  ...
  app.get('/workspace/memory', async (_req, res) => {
    try {
      ...
      writeStderrLine(
        `atus serve: GET /workspace/memory failed: ${
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (err instanceof WorkspaceMemoryWriteTimeoutError) {
          writeStderrLine(
            `atus serve: POST /workspace/memory timeout — file lock at ` +
        ...
        if (err instanceof WorkspaceMemoryFileTooLargeError) {
          writeStderrLine(
            `atus serve: POST /workspace/memory refused — existing file ` +
          ...
          // gated behind ATUS_SERVE_DEBUG so production responses
          // don't include `/Users/<x>/.atus-code/...` in the body.
        ...
        writeStderrLine(
          `atus serve: POST /workspace/memory failed (scope=${scope} mode=${mode}): ${
            err instanceof Error ? (err.stack ?? err.message) : String(err)
          }`,
        );
        ...
        // ENOENT/EACCES messages) is gated behind `ATUS_SERVE_DEBUG`.
 ...
 * based rules (`.atus-code/rules/`) are also out of scope for v1.
 */
