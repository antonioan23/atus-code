...
export function mountWorkspaceAgentsRoutes(
  ...
  app.get('/workspace/agents', async (_req, res) => {
    try {
      // `force: true` re-walks `.atus-code/agents/` on every call so out-of-
      ...
      writeStderrLine(
        `atus serve: GET /workspace/agents failed: ${
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        writeStderrLine(
          `atus serve: POST /workspace/agents failed: ${
            err instanceof Error ? (err.stack ?? err.message) : String(err)
          }`,
        );
        res.status(500).json({
      ...
      if (!created) {
        ...
        // arrives.
        writeStderrLine(
          `atus serve: agent_create_reload_failed (name=${safeLogValue(config.name)} ` +
  ...
  app.get('/workspace/agents/:agentType', async (req, res) => {
    ...
    try {
      ...
      writeStderrLine(
        `atus serve: GET /workspace/agents/${safeLogValue(agentType)} failed: ${
          err instanceof Error ? (err.stack ?? err.message) : String(err)
        }`,
      );
      res.status(500).json({
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (err instanceof SubagentError) {
          ...
          if (err.code === SubagentErrorCode.FILE_ERROR) {
            ...
            // generic envelope; `ATUS_SERVE_DEBUG` re-enables the
        ...
        writeStderrLine(
          `atus serve: POST /workspace/agents/${safeLogValue(agentType)} failed: ${
            err instanceof Error ? (err.stack ?? err.message) : String(err)
          }`,
        );
        res.status(500).json({
      ...
      if (!updated) {
        ...
        writeStderrLine(
          `atus serve: agent_update_reload_failed (name=${safeLogValue(agentType)} ` +
  ...
  app.delete(
    ...
    async (req, res) => {
      ...
      try {
        ...
        writeStderrLine(
          `atus serve: DELETE /workspace/agents/${safeLogValue(agentType)} failed: ${
            err instanceof Error ? (err.stack ?? err.message) : String(err)
          }`,
        );
        res.status(500).json({
      ...
      if (remaining.length > 0) {
        writeStderrLine(
          `atus serve: DELETE /workspace/agents/${safeLogValue(agentType)} partial — ` +
...
function parseAgentConfig(
  ...
  // and write a project-level file at `<workspace>/.atus-code/agents/
...
export function createDaemonSubagentManager(
  ...
  const guarded = new Proxy(stub, {
    get(target, prop) {
      ...
      throw new Error(
        `atus serve workspace agents: SubagentManager touched Config.` +
    ...
    has(target, prop) {
      ...
      throw new Error(
        `atus serve workspace agents: SubagentManager probed Config.` +
