...
export function hostAllowlist(
  ...
  const allowedFor = (port: number): Set<string> => {
    ...
    // the URI scheme's default. http → 80, https → 443. The atus
...
export function createMutationGate(
  ...
  const strictDenier: RequestHandler = (_req: Request, res: Response) => {
    ...
    // different boot error. Configuring a token via `ATUS_SERVER_TOKEN`
    ...
    res.status(401).json({
      error:
        ...
        'token. Set ATUS_SERVER_TOKEN or pass --token to enable bearer ' +
        'auth.',
      code: 'token_required',
    });
  };
