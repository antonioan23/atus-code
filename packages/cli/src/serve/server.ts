...
import { APPROVAL_MODES, TrustGateError } from '@atus-code/atus-code-core';
...
export function createDefaultFsAuditEmit(): (event: BridgeEvent) => void {
  ...
  return (event: BridgeEvent) => {
    ...
    if (droppedCount === 1 || droppedCount % WARN_EVERY === 0) {
      ...
      writeStderrLine(
        `atus serve: fs audit emit is the default no-op — ${droppedCount} event(s) dropped so far. ` +
...
export interface ServeAppDeps {
  ...
  /**
   * Extra device-flow providers for tests / future extensions. Used by
   * tests that stub auth flows. Qwen OAuth has been removed in atus code;
   * the default provider list is now empty.
   */
  deviceFlowProviders?: DeviceFlowProvider[];
}

/**
 * Build the Express app for `atus serve`. Pure function — no side effects on
...
export function createServeApp(
  ...
  // Qwen OAuth has been removed in atus code. Add custom device-flow
  // providers via deps.deviceFlowProviders to extend auth flows.
  ...
  const demoHandler = (
    ...
    try {
      ...
      writeStderrLine(
        `atus serve: /demo render failed: ${err instanceof Error ? err.message : String(err)}`,
      );
  ...
  // arbitrary IP:port to confirm a `atus serve` is listening), so
  ...
  // (`docs/users/atus-serve.md` + `atus-serve-protocol.md`) clarify
  ...
  const healthHandler = (
    ...
    try {
      ...
      writeStderrLine(
        `atus serve: /health deep probe failed: ${err instanceof Error ? err.message : String(err)}`,
      );
  ...
  //
  // PR #4291 follow-up review (atus-latest, #4): GET now also runs
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      const clientId = parseClientIdHeader(req, res);
      if (clientId === null) return;
      // PR #4291 follow-up review (atus-latest, N4): when the
      ...
      // ATUS_SERVE_DEBUG-gated stderr (rather than unconditional
      ...
      // the symptom can flip ATUS_SERVE_DEBUG=1 and get the
      ...
      if (
        !callerIsInitiator &&
        process.env['ATUS_SERVE_DEBUG'] &&
        !['0', 'false', 'off', 'no'].includes(
          (process.env['ATUS_SERVE_DEBUG'] ?? '').trim().toLowerCase(),
        )
      ) {
        writeStderrLine(
          `atus serve debug: GET /workspace/auth/device-flow/${id} redacted verification fields — caller-clientId mismatch (initiator=${view.initiatorClientId ?? 'anonymous'}, caller=${clientId ?? 'anonymous'})`,
        );
      }
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      // #4282 fold-in 4 (atus-latest S1): match the
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      // #4282 fold-in 4 (atus-latest C3): trim before persistence so the
  ...
  app.get('/session/:id/events', (req, res) => {
    ...
    try {
      ...
      if (err instanceof SubscriberLimitExceededError) {
        writeStderrLine(
          `atus serve: subscriber limit reached for session ${sessionId} (limit=${err.limit}); rejecting new SSE client with 429`,
        );
    ...
    res.on('error', (err) => {
      ...
      writeStderrLine(
        `atus serve: SSE socket error (session ${sessionId}): ${err.message}`,
      );
      cleanup();
    });
  ...
  app.use(
    (
      ...
      writeStderrLine(
        `atus serve: unhandled error: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
      );
      if (!res.headersSent) {
...
const CLIENT_ID_HEADER = 'x-atus-client-id';
const MAX_CLIENT_ID_LENGTH = 128;
/** #4282 fold-in 2 (deepseek SV1) — see /workspace/tools/:name/enable. */
const MAX_TOOL_NAME_LENGTH = 256;
/** #4282 fold-in 4 (atus-latest S1) — see /workspace/mcp/:server/restart. */
...
function toDeviceFlowStateBody(
  ...
  // PR #4291 follow-up review (atus-latest, #3): the gate must accept
...
function parseMaxQueuedQuery(
  ...
  if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
    ...
    // `sendBridgeError`.
    writeStderrLine(
      `atus serve: rejected ?maxQueued ${safeLogValue(raw)} ` +
...
function parseLastEventId(raw: unknown): number | undefined {
  ...
  if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
    ...
    if (typeof raw === 'string' && raw.length > 0) {
      writeStderrLine(
        `atus serve: rejected Last-Event-ID ${safeLogValue(raw)} ` +
          `(not a decimal integer)`,
      );
    }
    return undefined;
...
function sendBridgeError(
  ...
  if (err instanceof WorkspaceMismatchError) {
    ...
    // readability.
    writeStderrLine(
      `atus serve: workspace_mismatch (POST /session): ` +
  ...
  writeStderrLine(
    `atus serve: bridge error${ctxStr}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
  );
