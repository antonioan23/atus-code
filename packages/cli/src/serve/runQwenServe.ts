...
const ATUS_SERVER_TOKEN_ENV = 'ATUS_SERVER_TOKEN';
...
/**
 * #4282 fold-in 4 (atus-latest C2). Per-workspace promise chain that
 ...
 *
 * Scope is INTRA-process: a separate `atus serve` invocation against
...
export async function runQwenServe(
  ...
  // Trim both sources. Common gotcha: `export ATUS_SERVER_TOKEN=$(cat
  ...
  const rawToken = optsIn.token ?? process.env[ATUS_SERVER_TOKEN_ENV];
  const token =
  ...
  if (!isLoopbackBind(opts.hostname) && !token) {
    throw new Error(
      `Refusing to bind ${opts.hostname}:${opts.port} without a bearer token. ` +
        `Set ${ATUS_SERVER_TOKEN_ENV} or pass --token, or rebind to loopback ` +
  ...
  if (opts.requireAuth && !token) {
    throw new Error(
      `Refusing to start with --require-auth set but no bearer token ` +
        `configured. Set ${ATUS_SERVER_TOKEN_ENV} or pass --token, or omit ` +
  ...
  // `atus --acp` child with that cwd and the agent would fail with an
  ...
  const childEnvOverrides: Record<string, string | undefined> = {
    ATUS_SERVE_MCP_CLIENT_BUDGET:
      opts.mcpClientBudget !== undefined
        ? String(opts.mcpClientBudget)
        : undefined,
    ATUS_SERVE_MCP_BUDGET_MODE: opts.mcpBudgetMode,
  };
  ...
  const bridge =
    ...
    createHttpAcpBridge({
      ...
      //
      // #4282 fold-in 4 (atus-latest C2): both persist callbacks run
  ...
  // `atus serve --hostname [::1]` would pass the loopback/token check
  ...
  return await new Promise<RunHandle>((resolve, reject) => {
    const server = app.listen(opts.port, listenHostname, () => {
      ...
      writeStdoutLine(
        `atus serve listening on ${url} (mode=${opts.mode}, ` +
      ...
      writeStderrLine(
        `atus serve: bound to workspace ${JSON.stringify(boundWorkspace)}`,
      );
      if (!token) {
        writeStderrLine(
          `atus serve: bearer auth disabled (loopback default). Set ${ATUS_SERVER_TOKEN_ENV} to enable.`,
        );
        ...
        writeStderrLine(
          'atus serve: --require-auth enabled (bearer token mandatory ' +
      ...
      const onSignal = async (signal: NodeJS.Signals) => {
        if (shuttingDown) {
          ...
          //
          // Bd1y6: synchronously SIGKILL every live `atus --acp`
          ...
          // `atus` processes in the operator's `ps` output.
          writeStderrLine(
            `atus serve: received ${signal} during drain — forcing exit`,
          );
          try {
            bridge.killAllSync();
          } catch (err) {
            writeStderrLine(
              `atus serve: force-kill error: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
          process.exit(1);
          return;
        }
        writeStderrLine(`atus serve: received ${signal}, draining...`);
        try {
          await handle.close();
          process.exit(0);
        } catch (err) {
          writeStderrLine(`atus serve: shutdown error: ${String(err)}`);
          process.exit(1);
        }
      };
          ...
          closePromise = new Promise<void>((res, rej) => {
            ...
            if (deviceFlowRegistry) {
              try {
                ...
                writeStderrLine(
                  `atus serve: device-flow registry dispose error: ${
            ...
            bridge
              ...
              .catch((err) => {
                writeStderrLine(
                  `atus serve: bridge shutdown error: ${String(err)}`,
                );
                bridgeShutdownError =
              ...
              .finally(() => {
                ...
                const forceTimer = setTimeout(() => {
                  writeStderrLine(
                    `atus serve: ${SHUTDOWN_FORCE_CLOSE_MS}ms listener-drain timeout reached; force-closing remaining connections`,
                  );
                  ...
                  secondaryTimer = setTimeout(() => {
                    writeStderrLine(
                      `atus serve: server.close did not fire ${SECONDARY_DEADLINE_MS}ms after force-close; resolving anyway`,
                    );
                    finish();
      ...
      server.on('error', (err) => {
        writeStderrLine(
          `atus serve: server error: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
      resolve(handle);
    });
