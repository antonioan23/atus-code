...
import { Ignore } from '@atus-code/atus-code-core';
import {
...
async function makeHarness(opts?: {
  ...
  const scratch = await fsp.mkdtemp(
    path.join(os.tmpdir(), `atus-wfs-${randomBytes(4).toString('hex')}-`),
  );
...
describe('WorkspaceFileSystem - audit always emits on body errors', () => {
  ...
  it('fs.denied audit message field is gated behind ATUS_AUDIT_RAW_PATHS (privacy default omits)', async () => {
    ...
    // raw-path logging via not-setting `ATUS_AUDIT_RAW_PATHS`.
...
describe('WorkspaceFileSystem - factory', () => {
  it('canonicalizes the workspace once at factory build', async () => {
    const scratch = await fsp.mkdtemp(
      path.join(
        os.tmpdir(),
        `atus-wfs-canon-${randomBytes(4).toString('hex')}-`,
      ),
    );
    try {
