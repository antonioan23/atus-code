...
describe('canonicalizeWorkspace', () => {
  ...
  beforeEach(async () => {
    ...
    scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `atus-fs-paths-${id}-`));
  });
...
describe('hasSuspiciousPathPattern', () => {
  ...
  it('accepts ordinary POSIX paths', () => {
    ...
    expect(hasSuspiciousPathPattern('.atusignore')).toBe(false);
...
describe('resolveWithinWorkspace', () => {
  ...
  beforeEach(async () => {
    ...
    scratch = await fsp.mkdtemp(
      path.join(os.tmpdir(), `atus-fs-resolve-${id}-`),
    );
