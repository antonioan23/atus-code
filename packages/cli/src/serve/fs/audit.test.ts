...
describe('createAuditPublisher', () => {
  ...
  it('respects ATUS_AUDIT_RAW_PATHS=1 via env when includeRawPaths is unset', () => {
    const original = process.env['ATUS_AUDIT_RAW_PATHS'];
    process.env['ATUS_AUDIT_RAW_PATHS'] = '1';
    try {
      const events: BridgeEvent[] = [];
      ...
      if (original === undefined) delete process.env['ATUS_AUDIT_RAW_PATHS'];
      else process.env['ATUS_AUDIT_RAW_PATHS'] = original;
    }
  });
