...
export interface FsAccessAuditPayload {
   ...
   */
  sessionId?: string;
  /** Workspace-relative path; only populated when ATUS_AUDIT_RAW_PATHS=1. */
  relPath?: string;
  sizeBytes?: number;
  truncated?: boolean;
...
function rawPathsEnabled(): boolean {
  return process.env['ATUS_AUDIT_RAW_PATHS'] === '1';
}
...
export function createAuditPublisher(
  ...
  return {
    recordAccess(ctx, record) {
      ...
      // context opt in via `ATUS_AUDIT_RAW_PATHS=1`.
    ...
    recordDenied(ctx, record) {
      ...
      // opt in via `ATUS_AUDIT_RAW_PATHS=1`.
