...
import { isWithinRoot } from '@atus-code/atus-code-core';
import { FsError, type FsErrorKind } from './errors.js';

// `canonicalizeWorkspace` and `MAX_WORKSPACE_PATH_LENGTH` lifted to
// `@atus-code/acp-bridge` in #4175 PR 22b — the bridge package owns the
...
} from '@atus-code/acp-bridge/workspacePaths';
          ...
          if (!isWithinRoot(canonicalTarget, boundCanonical)) {
            throw new FsError(
              ...
              `dangling symlink target escapes workspace: ${input}`,
              {
                ...
                // run with `ATUS_AUDIT_RAW_PATHS=1` and read it from
                // `relPath` / `message`.
                hint: 'symlink chain leaves the workspace; enable ATUS_AUDIT_RAW_PATHS for the resolved target',
              },
            );
          }
