...
export interface FsEntry {
  name: string;
  kind: 'file' | 'directory' | 'symlink' | 'other';
  /** True iff the entry matched a `.gitignore`/`.atusignore` rule. */
  ignored: boolean;
}
...
export interface CreateWorkspaceFileSystemFactoryDeps {
   ...
   */
  ignore?: Ignore;
  /** Override audit raw-path mode. Defaults to env `ATUS_AUDIT_RAW_PATHS=1`. */
...
class WorkspaceFileSystemImpl implements WorkspaceFileSystem {
  ...
  async edit(
    ...
    try {
      ...
      // a `.gitignore`d / `.atusignore`d file (build artifacts,
