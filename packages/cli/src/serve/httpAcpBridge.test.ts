...
import { ApprovalMode } from '@atus-code/atus-code-core';
...
describe('createHttpAcpBridge', () => {
  ...
  it('forwards childEnvOverrides to the channelFactory at spawn time (#4247 R6 line 216)', async () => {
    ...
    const bridge1 = makeBridge({
      ...
      childEnvOverrides: {
        ATUS_SERVE_MCP_CLIENT_BUDGET: '5',
        ATUS_SERVE_MCP_BUDGET_MODE: 'enforce',
      },
    });
    const bridge2 = makeBridge({
      channelFactory: factory,
      childEnvOverrides: {
        ATUS_SERVE_MCP_CLIENT_BUDGET: '20',
        ATUS_SERVE_MCP_BUDGET_MODE: 'warn',
      },
    });
  ...
  it('requests workspace status through the existing ACP channel', async () => {
      ...
      channelFactory: async () => {
        const h = makeChannel({
          extMethodImpl: (method) => {
            if (method === 'atus/status/workspace/mcp') {
              return {
                v: 1,
                workspaceCwd: WS_A,
                initialized: true,
            ...
            if (method === 'atus/status/workspace/skills') {
              return {
                v: 1,
                workspaceCwd: WS_A,
                initialized: true,
    ...
    expect(handles[0]?.agent.extMethodCalls.map((c) => c.method)).toEqual([
      'atus/status/workspace/mcp',
      'atus/status/workspace/skills',
      'atus/status/workspace/providers',
    ]);
  ...
  it('merges daemon cells with live ACP-side preflight cells when a channel is up', async () => {
    ...
    const bridge = makeBridge({
      channelFactory: async () => {
        const h = makeChannel({
          extMethodImpl: (method) => {
            if (method === 'atus/status/workspace/preflight') {
  ...
  it('requests session status through the existing ACP channel', async () => {
      ...
      channelFactory: async () => {
        const h = makeChannel({
          extMethodImpl: (method, params) => {
            if (method === 'atus/status/session/context') {
              return {
                v: 1,
    ...
    expect(handles[0]?.agent.extMethodCalls.map((c) => c.method)).toEqual([
      'atus/status/session/context',
      'atus/status/session/supported_commands',
    ]);
  ...
  it('creates fresh session per call under sessionScope:thread (Stage 1.5 multi-session: shares channel)', async () => {
    ...
    // workspace's `atus --acp` child. Only one `channelFactory` call.
  ...
  describe('BridgeClient file proxy (Stage 1: same-host trust)', () => {
    ...
    it('writeTextFile writes to local fs', async () => {
      ...
      const tmp = path.join(
        os.tmpdir(),
        `atus-bridge-write-${randomBytes(8).toString('hex')}.txt`,
      );
      try {
        await (
          conn as unknown as {
    ...
    it('writeTextFile leaves no .tmp turd in the target directory (BSA0D)', async () => {
      ...
      const dir = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'atus-bridge-atomic-'),
      );
    ...
    it('readTextFile rejects files past the size cap (BSA0E)', async () => {
      ...
      const sparse = path.join(
        os.tmpdir(),
        `atus-bridge-sparse-${randomBytes(8).toString('hex')}.bin`,
      );
    ...
    it('readTextFile rejects non-regular files even when size=0 (BX8YO)', async () => {
      ...
      const fifoPath = path.join(
        os.tmpdir(),
        `atus-bridge-fifo-${randomBytes(8).toString('hex')}`,
      );
    ...
    it('writeTextFile preserves symlinks (BX8Yw)', async () => {
      ...
      const { bridge, conn } = await setupForFs();
      const dir = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'atus-bridge-symlink-'),
      );
    ...
    it('writeTextFile preserves dangling symlinks (BfFvO)', async () => {
      ...
      const { bridge, conn } = await setupForFs();
      const dir = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'atus-bridge-dangling-'),
      );
    ...
    it('readTextFile returns full content by default', async () => {
      ...
      const tmp = path.join(
        os.tmpdir(),
        `atus-bridge-read-${randomBytes(8).toString('hex')}.txt`,
      );
      await fsp.writeFile(
        tmp,
        'line one\nline two\nline three\nline four',
    ...
    it('readTextFile slices via line/limit (ACP 1-based line)', async () => {
      ...
      const tmp = path.join(
        os.tmpdir(),
        `atus-bridge-slice-${randomBytes(8).toString('hex')}.txt`,
      );
  ...
  describe('setSessionApprovalMode (#4175 Wave 4 PR 17)', () => {
    /**
     * #4282 fold-in 4 (atus-latest C1). Build a channel factory whose
     * extMethod handler answers `atus/control/session/approval_mode`
    ...
    function approvalModeFactoryWithCallTracker(): {
      ...
      const factory: ChannelFactory = async () => {
        ...
        const agent = new FakeAgent({
          extMethodImpl: (method, params) => {
            calls.push({ method });
            if (method === 'atus/control/session/approval_mode') {
    ...
    it('throws BEFORE the ACP roundtrip when persist:true but no callback wired', async () => {
      ...
      expect(
        getCalls().some(
          (c) => c.method === 'atus/control/session/approval_mode',
        ),
      ).toBe(false);
      await bridge.shutdown();
    });
  ...
  describe('initWorkspace (#4175 Wave 4 PR 17)', () => {
    ...
    beforeEach(async () => {
      tmpWs = await fsp.mkdtemp(path.join(os.tmpdir(), 'atus-init-workspace-'));
    });
  ...
  // Translates `atus/notify/session/mcp-budget-event` into session-scoped
  ...
  describe('extNotification — MCP budget events (PR 14b)', () => {
    it('publishes mcp_budget_warning when the child fires the warning event', async () => {
      ...
      void capturedConn!.extNotification(
        'atus/notify/session/mcp-budget-event',
        {
          v: 1,
          sessionId: session.sessionId,
          kind: 'budget_warning',
    ...
    it('drops unknown extNotification methods, kinds, and missing sessionIds silently', async () => {
      ...
      // Unknown method — drop.
      void capturedConn!.extNotification('atus/notify/session/unknown-event', {
        sessionId: session.sessionId,
        kind: 'budget_warning',
      });
  ...
  describe('maxSessions cap (chiga0 Rec 3)', () => {
    ...
    it('Stage 1.5 multi-session: N sessions on same workspace share ONE channel', async () => {
      ...
      // sessions on one workspace pay for one `atus --acp` child, not
