...
export const SETTINGS_DIRECTORY_NAME = ATUS_DIR;

// Lazy getters: must NOT be top-level consts. `ATUS_HOME` may be resolved
// from `~/.env` or `~/.atus-code/.env` by `preResolveHomeEnvOverrides()` in
...
export const ENV_CORRUPTED_PATH = 'ATUS_CODE_SETTINGS_CORRUPTED_PATH';
export const ENV_WAS_RECOVERED = 'ATUS_CODE_SETTINGS_WAS_RECOVERED';

// ATUS_HOME and ATUS_RUNTIME_DIR control where global state (settings, OAuth
...
export function getSystemSettingsPath(): string {
  if (process.env['ATUS_CODE_SYSTEM_SETTINGS_PATH']) {
    return process.env['ATUS_CODE_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/AtusCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\atus-code\\settings.json';
  } else {
    return '/etc/atus-code/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['ATUS_CODE_SYSTEM_DEFAULTS_PATH']) {
    return process.env['ATUS_CODE_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
 ...
 * `.env` (which respects `ATUS_HOME`). When `ATUS_HOME` redirects elsewhere,
 * the legacy `<homedir>/.atus-code/.env` is also included so credentials users
...
function getUserLevelEnvPaths(): Set<string> {
  ...
  const legacyQwenEnv = path.normalize(path.join(homeDir, ATUS_DIR, '.env'));
  paths.add(legacyQwenEnv);
  return paths;
}

/**
 * Pre-resolves ATUS_HOME and ATUS_RUNTIME_DIR from user-level `.env` files
 ...
 * global state between `~/.atus-code/...` and `<ATUS_HOME>/...`.
 *
...
export function preResolveHomeEnvOverrides(): void {
  ...
  // rest of the storage layer; when ATUS_HOME is unset it equals
  // `<homedir>/.atus-code`, so path.dirname() recovers `<homedir>`.
  ...
  // ATUS_RUNTIME_DIR can be sourced from there (mirrors the VS Code
 ...
 *
 * Candidates are iterated most-specific-first (`~/.atus-code/.env` before
...
function getHomeEnvFallbackVars(): Record<string, string> {
  ...
  // When ATUS_HOME is set, skip ~/.env to avoid surprise cross-contamination
...
/**
 * Surfaces a one-shot warning when ATUS_HOME has been redirected but the
...
function detectQwenHomeRedirectWithoutMigration(
  ...
  // Compute the legacy path by briefly unsetting ATUS_HOME so Storage uses
  ...
  return (
    `ATUS_HOME points to "${activeQwenDir}" but no settings.json was found there. ` +
...
function findEnvFile(
  ...
  const globalQwenDir = Storage.getGlobalQwenDir();
  const legacyQwenDir = path.normalize(path.join(homeDir, ATUS_DIR));
  ...
  // ~/.atus-code/.env (only when ATUS_HOME redirects), then ~/.env.
  // Users who add `ATUS_HOME=` to an existing global env file shouldn't lose
  ...
  while (true) {
    if (currentDir === homeDir) {
      ...
      // Workspace step: prefer .atus-code/.env, then plain .env.
      const geminiEnvPath = path.join(currentDir, ATUS_DIR, '.env');
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    try {
      ...
      //   only these may set ATUS_HOME / ATUS_RUNTIME_DIR.
      // atusScoped: any `.env` whose immediate parent is `.atus-code` (including
      //   `<repo>/.atus-code/.env`) — exempt from the user `excludedEnvVars` list.
      const isHomeScopedEnvFile = userLevelPaths.has(normalizedEnvFilePath);
      const isQwenScopedEnvFile =
        isHomeScopedEnvFile ||
        path.basename(path.dirname(normalizedEnvFilePath)) === ATUS_DIR;
 ...
 * System Defaults → User (~/.atus-code/settings.json) → Workspace → System.
 */
export function loadSettings(
  workspaceDir: string = process.cwd(),
  consumeCorruptionEnvVars: boolean = true,
): LoadedSettings {
  // Apply any ATUS_HOME / ATUS_RUNTIME_DIR set in user-level `.env` files
  ...
  const allMigrationWarnings: string[] = [
    ...(atusHomeRedirectWarning ? [atusHomeRedirectWarning] : []),
