...
const SETTINGS_SCHEMA = {
  ...
  plansDirectory: {
    ...
    description:
      'Custom directory for approved Plan Mode files. Relative paths are resolved from the project root, and the resolved path must stay within the project root. Defaults to ~/.atus-code/plans.',
    showInDialog: false,
  },
  ...
  general: {
    ...
    properties: {
      ...
      cleanupPeriodDays: {
        ...
        default: 30,
        description:
          'Number of days to retain ~/.atus-code/file-history/ session backups used by /rewind. Backups older than this are removed by a background housekeeping pass that runs at most once per day. Set to 0 for minimum retention (~1 hour) — protects sessions touched in the last hour, plus the currently active session. Other persistent caches will honor the same setting in the future.',
        showInDialog: true,
      },
      gitCoAuthor: {
        type: 'object',
        ...
        description:
          'Attribution added to git commits and pull requests created through atus code.',
        showInDialog: false,
        ...
        // until they run atus once. Config.normalizeGitCoAuthor handles
        ...
        properties: {
          commit: {
            ...
            default: true,
            description:
              'Add a Co-authored-by trailer to git commit messages AND attach a per-file AI-attribution git note (`refs/notes/ai-attribution`) for commits made through atus code. Disabling skips both.',
            showInDialog: true,
          },
          pr: {
            type: 'boolean',
            ...
            default: true,
            description:
              'Append a atus code attribution line to PR descriptions when running `gh pr create`.',
            showInDialog: true,
          },
        },
      },
      ...
      language: {
        ...
        description:
          ...
          'in ~/.atus-code/locales/ (e.g., ~/.atus-code/locales/es.js).',
        showInDialog: true,
  ...
  ui: {
    ...
    properties: {
      ...
      showStatusInTitle: {
        ...
        default: false,
        description:
          'Show atus code status and thoughts in the terminal window title',
        showInDialog: false,
      },
      hideTips: {
        type: 'boolean',
      ...
      customBannerTitle: {
        ...
        default: '' as string,
        description:
          'Replace the default ">_ atus code" title shown in the banner info panel. The version suffix is always appended.',
        showInDialog: false,
      },
      ...
      generationConfig: {
        ...
        properties: {
          ...
          splitToolMedia: {
            ...
            default: false,
            description:
              'When true, media (images / audio / video / files) returned by MCP tool calls is split into a follow-up user message instead of being embedded in the tool message. Required for strict OpenAI-compatible servers (e.g., LM Studio) that reject non-text content on `role: "tool"` messages with HTTP 400 "Invalid \'messages\' in payload". Default false preserves the prior behavior for permissive providers. See atuscode/atus-code#3616.',
            parentKey: 'generationConfig',
            showInDialog: false,
          },
          schemaCompliance: {
          ...
          contextWindowSize: {
            ...
            default: undefined,
            description:
              "Overrides the default context window size for the selected model. Use this setting when a provider's effective context limit differs from atus code's default. This value defines the model's assumed maximum context capacity, not a per-request token limit.",
            parentKey: 'generationConfig',
            showInDialog: false,
          },
        },
    ...
    properties: {
      ...
      fileFiltering: {
        ...
        properties: {
          ...
          respectAtusIgnore: {
            type: 'boolean',
            label: 'Respect .atusignore',
            category: 'Context',
            requiresRestart: true,
            default: true,
            description: 'Respect .atusignore files when searching',
            showInDialog: true,
          },
  ...
  tools: {
    ...
    properties: {
      ...
      sandboxImage: {
        ...
        default: undefined as string | undefined,
        description:
          'Sandbox image URI used by Docker/Podman when --sandbox-image and ATUS_SANDBOX_IMAGE are not set.',
        showInDialog: false,
      },
      toolSearch: {
        type: 'object',
    ...
    properties: {
      ...
      runtimeOutputDir: {
        ...
        description:
          ...
          'Config files remain at ~/.atus-code (or ATUS_HOME if set). Env var ATUS_RUNTIME_DIR takes priority.',
        showInDialog: false,
      },
    },
  },
      ...
      arena: {
        ...
        properties: {
          worktreeBaseDir: {
            ...
            default: undefined as string | undefined,
            description:
              'Custom base directory for Arena worktrees. Defaults to ~/.atus-code/arena.',
            showInDialog: false,
          },
          preserveArtifacts: {
            type: 'boolean',
  ...
  stopHookBlockingCap: {
    ...
    description:
      'Maximum consecutive blocking Stop/SubagentStop hook decisions before atus code overrides the hook loop and ends the turn. Can be overridden by ATUS_CODE_STOP_HOOK_BLOCK_CAP.',
  ...
  experimental: {
    ...
    properties: {
      cron: {
        ...
        default: false,
        description:
          'Enable in-session cron/loop tools (experimental). When enabled, the model can create recurring prompts using cron_create, cron_list, and cron_delete tools. Can also be enabled via ATUS_CODE_ENABLE_CRON=1 environment variable.',
        showInDialog: true,
      },
    ...
    properties: {
      symlinkDirectories: {
        ...
        description:
          ...
          '`.atus-code` (the CLI-managed metadata tree, which contains ' +
