...
import { ATUS_DIR } from '@atus-code/atus-code-core';
...
vi.mock('@atus-code/atus-code-core', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@atus-code/atus-code-core')>();
  return {
    ...actual,
...
// must keep going through `getUserSettingsPath()` to pick up `ATUS_HOME`
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    it('should add version field to V2 settings without version and write to disk', () => {
      ...
      expect(writtenContent.model?.name).toBe('atus-coder');
    ...
    it('should correctly handle partially migrated settings without version field', () => {
      ...
      expect(writtenContent.model).toEqual({ name: 'atus-coder' });
    ...
    it('should not search dirname(atusDir)/.env when ATUS_HOME is set (#4466)', () => {
      const customHome = '/custom/atus/home';
      process.env['ATUS_HOME'] = customHome;
    ...
    it('should resolve ${VAR} from ~/.env when ATUS_HOME is not set (#4466)', () => {
    ...
    it('should prefer ~/.atus-code/.env over ~/.env for the same key (first-write-wins) (#4466)', () => {
      const atusEnvPath = path.join(path.dirname(USER_SETTINGS_PATH), '.env');
      const homeEnvPath = path.join(
        path.dirname(path.dirname(USER_SETTINGS_PATH)),
        '.env',
      );
      ...
      (mockFsExistsSync as Mock).mockImplementation(
        (p: fs.PathLike) =>
          p === USER_SETTINGS_PATH || p === atusEnvPath || p === homeEnvPath,
      );
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          if (p === USER_SETTINGS_PATH)
            return JSON.stringify(userSettingsContent);
          if (p === atusEnvPath) return 'PRECEDENCE_TOKEN=from_qwen_dir';
    ...
    it('should succeed with unresolved placeholder when .env read throws (#4466)', () => {
      ...
      (mockFsExistsSync as Mock).mockImplementation(
        (p: fs.PathLike) => p === USER_SETTINGS_PATH || p === atusEnvPath,
      );
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          if (p === USER_SETTINGS_PATH)
            return JSON.stringify(userSettingsContent);
          if (p === atusEnvPath) throw new Error('EACCES: permission denied');
          return '{}';
        },
      );
    ...
    describe('when ATUS_CODE_SYSTEM_SETTINGS_PATH is set', () => {
      const MOCK_ENV_SYSTEM_SETTINGS_PATH = '/mock/env/system/settings.json';

      beforeEach(() => {
        process.env['ATUS_CODE_SYSTEM_SETTINGS_PATH'] =
          MOCK_ENV_SYSTEM_SETTINGS_PATH;
      });

      afterEach(() => {
        delete process.env['ATUS_CODE_SYSTEM_SETTINGS_PATH'];
      });
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      const geminiEnvPath = path.resolve(path.join(ATUS_DIR, '.env'));
    ...
    describe('settings.env field', () => {
      ...
      it('should never set ATUS_HOME or ATUS_RUNTIME_DIR from settings.env', () => {
        ...
        const userSettingsContent: Settings = {
          env: {
            ATUS_HOME: '/redirected/by/settings',
            ATUS_RUNTIME_DIR: '/redirected/runtime',
            HARMLESS_VAR: 'ok',
          },
        };
        ...
        expect(process.env['ATUS_HOME']).toBeUndefined();
    ...
    describe('ATUS_HOME custom directory', () => {
      const originalQwenHome = process.env['ATUS_HOME'];

      beforeEach(() => {
        delete process.env['DEBUG'];
        delete process.env['DEBUG_MODE'];
        delete process.env['ATUS_HOME_TEST_VAR'];
      });
      ...
      afterEach(() => {
        if (originalQwenHome === undefined) {
          ...
          process.env['ATUS_HOME'] = originalQwenHome;
        }
        delete process.env['DEBUG'];
        delete process.env['DEBUG_MODE'];
        delete process.env['ATUS_HOME_TEST_VAR'];
      ...
      it('does not exclude DEBUG/DEBUG_MODE from .env in a ATUS_HOME dir not named .atus-code', () => {
        const customHome = '/tmp/atus-home-custom';
        process.env['ATUS_HOME'] = customHome;
        ...
        // because the .env lives inside the user-level ATUS_HOME directory.
        expect(process.env['DEBUG']).toEqual('true');
        expect(process.env['DEBUG_MODE']).toEqual('1');
        expect(process.env['ATUS_HOME_TEST_VAR']).toEqual('hello');
      });

      it('ignores ATUS_HOME and ATUS_RUNTIME_DIR set in a project .env', () => {
        delete process.env['ATUS_HOME'];
        delete process.env['ATUS_RUNTIME_DIR'];
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            ...
            if (p === projectEnvPath)
              return [
                'ATUS_HOME=/tmp/hijack',
                'ATUS_RUNTIME_DIR=/tmp/hijack-runtime',
                'OTHER_VAR=ok',
              ].join('\n');
            return '{}';
          },
      ...
      it('still honors ATUS_HOME from a user-level .env (~/.atus-code/.env)', () => {
        delete process.env['ATUS_HOME'];

        const cwdSpy = vi
          .spyOn(process, 'cwd')
          .mockReturnValue('/mock/home/user');
        const userQwenEnvPath = path.join('/mock/home/user', ATUS_DIR, '.env');
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === USER_SETTINGS_PATH) return JSON.stringify({});
            if (p === userQwenEnvPath) return 'ATUS_HOME=/tmp/from-user-env';
            return '{}';
          },
        );
        ...
        expect(process.env['ATUS_HOME']).toEqual('/tmp/from-user-env');
        cwdSpy.mockRestore();
      });

      it('does not exclude DEBUG/DEBUG_MODE from a workspace .atus-code/.env', () => {
        const cwdSpy = vi
          .spyOn(process, 'cwd')
          .mockReturnValue(MOCK_WORKSPACE_DIR);
        ...
        // Per docs, `.atus-code/.env` files are never filtered by excludedEnvVars,
      ...
      it('still blocks ATUS_HOME from a workspace .atus-code/.env', () => {
        delete process.env['ATUS_HOME'];
        delete process.env['ATUS_RUNTIME_DIR'];
        ...
        // A workspace `.atus-code/.env` is exempt from `excludedEnvVars` but must
      ...
      it('redirects user settings path when ATUS_HOME is set in ~/.atus-code/.env', () => {
        delete process.env['ATUS_HOME'];
        ...
        // The pre-pass propagates ATUS_HOME from ~/.atus-code/.env into
      ...
      it('warns when ATUS_HOME redirects but the legacy ~/.atus-code still has settings', () => {
        const customHome = '/tmp/atus-home-fresh';
        process.env['ATUS_HOME'] = customHome;
        ...
        // Active ATUS_HOME has nothing yet; legacy ~/.atus-code has settings.json.
        const customSettingsPath = path.join(customHome, 'settings.json');
        ...
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) => {
          const s = p.toString();
          // Legacy settings exist, active ATUS_HOME settings do not.
        ...
        const warningMatch = loaded.migrationWarnings.find((w) =>
          w.includes('ATUS_HOME points to'),
        );
        expect(warningMatch).toBeDefined();
        expect(warningMatch).toContain(customHome);
        expect(warningMatch).toContain(path.join('/mock/home/user', ATUS_DIR));
      });

      it('does not warn when ATUS_HOME points to a directory with settings.json', () => {
        const customHome = '/tmp/atus-home-migrated';
        process.env['ATUS_HOME'] = customHome;
        const customSettingsPath = path.join(customHome, 'settings.json');
      ...
      it('does not warn when ATUS_HOME is unset (default ~/.atus-code)', () => {
        delete process.env['ATUS_HOME'];
        const legacySettings = path.join(
          '/mock/home/user',
          ATUS_DIR,
      ...
      it('prefers ATUS_HOME/.env over ~/.env at the home-dir step', () => {
        const customHome = '/tmp/atus-home-custom';
        process.env['ATUS_HOME'] = customHome;
        const customGlobalEnvPath = path.join(customHome, '.env');
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === USER_SETTINGS_PATH) return JSON.stringify({});
            if (p === customGlobalEnvPath)
              return 'ATUS_HOME_TEST_VAR=fromQwenHome';
            if (p === homeEnvPath) return 'ATUS_HOME_TEST_VAR=fromHomeDir';
            return '{}';
          },
        );
        ...
        // ATUS_HOME/.env must win — without the precedence fix, ~/.env would
        // be returned by the walk-up before the ATUS_HOME fallback was ever
        // consulted.
        expect(process.env['ATUS_HOME_TEST_VAR']).toEqual('fromQwenHome');
      });

      it('falls back to legacy ~/.atus-code/.env for non-routing keys when <ATUS_HOME>/.env is absent', () => {
        // User keeps OPENAI_API_KEY in ~/.atus-code/.env and adds ATUS_HOME to the
        ...
        const cwdSpy = vi
          .spyOn(process, 'cwd')
          .mockReturnValue('/mock/home/user');
        const customHome = '/tmp/atus-home-fresh-fallback';
        const userQwenEnvPath = path.join('/mock/home/user', ATUS_DIR, '.env');
        const customSettingsPath = path.join(customHome, 'settings.json');
        ...
        // Only the legacy ~/.atus-code/.env exists; <ATUS_HOME>/.env, the active
        // settings.json under <ATUS_HOME>, and ~/.env all do not.
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
        ...
        expect(process.env['ATUS_HOME']).toEqual(customHome);
