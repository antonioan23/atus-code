...
import * as ServerConfig from '@atus-code/atus-code-core';
...
describe('parseArguments', () => {
  ...
  it('should accept --json-schema with no -p / positional when stdin is piped', async () => {
    // `echo "..." | atus --json-schema ...` — input arrives via the
...
describe('mergeExcludeTools', () => {
  ...
  it('should not auto-disable tool_search for non-deepseek models', async () => {
    process.argv = ['node', 'script.js', '--model', 'atus-max'];
    const argv = await parseArguments();
    const settings: Settings = {};
    const config = await loadCliConfig(settings, argv, undefined, []);
...
describe('Telemetry configuration via environment variables', () => {
  it('should prioritize ATUS_TELEMETRY_ENABLED over settings', async () => {
    vi.stubEnv('ATUS_TELEMETRY_ENABLED', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = { telemetry: { enabled: false } };
    const config = await loadCliConfig(settings, argv, undefined, []);
  ...
  it('should prioritize ATUS_TELEMETRY_TARGET over settings', async () => {
    vi.stubEnv('ATUS_TELEMETRY_TARGET', 'gcp');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should throw when ATUS_TELEMETRY_TARGET is invalid', async () => {
    vi.stubEnv('ATUS_TELEMETRY_TARGET', 'bogus');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should prioritize ATUS_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
    vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
    vi.stubEnv('ATUS_TELEMETRY_OTLP_ENDPOINT', 'http://gemini.env.com');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should prioritize ATUS_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
    vi.stubEnv('ATUS_TELEMETRY_OTLP_PROTOCOL', 'http');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
  ...
  it('should prioritize ATUS_TELEMETRY_LOG_PROMPTS over settings', async () => {
    vi.stubEnv('ATUS_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = { telemetry: { logPrompts: true } };
    const config = await loadCliConfig(settings, argv, undefined, []);
  ...
  it('should prioritize ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES over settings', async () => {
    vi.stubEnv('ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should prioritize ATUS_TELEMETRY_OUTFILE over settings', async () => {
    vi.stubEnv('ATUS_TELEMETRY_OUTFILE', '/gemini/env/telemetry.log');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
  ...
  it('should use settings value when ATUS_TELEMETRY_ENABLED is not set', async () => {
    vi.stubEnv('ATUS_TELEMETRY_ENABLED', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = { telemetry: { enabled: true } };
    const config = await loadCliConfig(settings, argv, undefined, []);
  ...
  it('should use settings value when ATUS_TELEMETRY_TARGET is not set', async () => {
    vi.stubEnv('ATUS_TELEMETRY_TARGET', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = {
      telemetry: { target: 'local' },
  ...
  it("should treat ATUS_TELEMETRY_ENABLED='1' as true", async () => {
    vi.stubEnv('ATUS_TELEMETRY_ENABLED', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig({}, argv, undefined, []);
    expect(config.getTelemetryEnabled()).toBe(true);
  ...
  it("should treat ATUS_TELEMETRY_ENABLED='0' as false", async () => {
    vi.stubEnv('ATUS_TELEMETRY_ENABLED', '0');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig(
  ...
  it("should treat ATUS_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
    vi.stubEnv('ATUS_TELEMETRY_LOG_PROMPTS', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig({}, argv, undefined, []);
    expect(config.getTelemetryLogPromptsEnabled()).toBe(true);
  ...
  it("should treat ATUS_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
    vi.stubEnv('ATUS_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig(
  ...
  it("should treat ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES='1' as true", async () => {
    vi.stubEnv('ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig({}, argv, undefined, []);
    expect(config.getTelemetryIncludeSensitiveSpanAttributes()).toBe(true);
  ...
  it("should treat ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES='false' as false", async () => {
    vi.stubEnv('ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const config = await loadCliConfig(
      { telemetry: { includeSensitiveSpanAttributes: true } },
...
describe('sandbox image resolution precedence', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(os.homedir).mockReturnValue('/mock/home/user');
    vi.stubEnv('GEMINI_API_KEY', 'test-api-key');
    delete process.env['ATUS_SANDBOX_IMAGE'];
  });
  ...
  it('uses --sandbox-image over env and settings', async () => {
    vi.stubEnv('ATUS_SANDBOX_IMAGE', 'env-image');
    process.argv = [
      'node',
      'script.js',
      '--sandbox',
  ...
  it('uses ATUS_SANDBOX_IMAGE over tools.sandboxImage', async () => {
    vi.stubEnv('ATUS_SANDBOX_IMAGE', 'env-image');
...
describe('loadCliConfig runtimeOutputDir', () => {
  const originalArgv = process.argv;
  const originalRuntimeEnv = process.env['ATUS_RUNTIME_DIR'];
  ...
  afterEach(() => {
    ...
    if (originalRuntimeEnv !== undefined) {
      process.env['ATUS_RUNTIME_DIR'] = originalRuntimeEnv;
    } else {
      delete process.env['ATUS_RUNTIME_DIR'];
    }
    vi.unstubAllEnvs();
  ...
  it('should resolve relative runtimeOutputDir against cwd', async () => {
    const argv = await parseArguments();
    const settings: Settings = {
      advanced: { runtimeOutputDir: '.atus-code' },
    };
  ...
  it('should let ATUS_RUNTIME_DIR env var take priority over settings', async () => {
