...
export interface CliArgs {
   ...
   * the CLI chdirs into `<repoRoot>/.atus-code/worktrees/<slug>/` and the entire
...
export async function parseArguments(): Promise<CliArgs> {
  ...
  if (
    rawArgv.length > 0 &&
    (rawArgv[0].endsWith('/dist/atus-cli/cli.js') ||
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('atus')
    .usage(
      'Usage: atus [options] [command]\n\natus code - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description: 'Proxy for atus code, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command('$0 [query..]', 'Launch atus code CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('worktree', {
          type: 'string',
          description:
            'Start the session inside a git worktree at <repoRoot>/.atus-code/worktrees/<slug>/. ' +
        ...
        .option('disabled-slash-commands', {
          ...
          description:
            ...
            'and ATUS_DISABLED_SLASH_COMMANDS. Matched case-insensitively ' +
        ...
        .check((argv: { [x: string]: unknown }) => {
          ...
          if (argv['jsonSchema']) {
            ...
            const hasPositionalQuery = Array.isArray(query)
              ? query.length > 0
              : !!query;
            // Allow stdin piping (`echo "..." | atus --json-schema ...`):
    ...
    // Register `atus serve` (Stage 1 daemon — see issue #3803)
...
export async function loadCliConfig(
  ...
  // Set runtime output directory from settings (env var ATUS_RUNTIME_DIR
  // is auto-detected inside getRuntimeBaseDir() at each call site).
  // Pass cwd so that relative paths like ".atus-code" resolve per-project.
  ...
  for (const name of (process.env['ATUS_DISABLED_SLASH_COMMANDS'] ?? '').split(
    ',',
  )) {
    addDisabled(name);
  }
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`atus --resume\` without an ID to choose from existing sessions.`;
