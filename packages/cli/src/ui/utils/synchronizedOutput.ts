...
export function terminalSupportsSynchronizedOutput(
  ...
  if (
    env['ATUS_CODE_DISABLE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['ATUS_CODE_SYNCHRONIZED_OUTPUT'] === '0'
  ) {
    return false;
  }

  if (
    env['ATUS_CODE_FORCE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['ATUS_CODE_SYNCHRONIZED_OUTPUT'] === '1'
  ) {
    return true;
  }
