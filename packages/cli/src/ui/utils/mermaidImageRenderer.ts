...
export function detectTerminalImageProtocol(
  ...
  if (env['ATUS_CODE_DISABLE_MERMAID_IMAGES'] === '1') {
    return null;
  }

  const forced = env['ATUS_CODE_MERMAID_IMAGE_PROTOCOL']?.toLowerCase();
...
function isMermaidImageRenderingDisabled(env: NodeJS.ProcessEnv): boolean {
  return env['ATUS_CODE_DISABLE_MERMAID_IMAGES'] === '1';
}
...
function unavailableImageRenderingDisabled(): MermaidImageUnavailableResult {
  return {
    kind: 'unavailable',
    reason:
      'Mermaid image rendering is disabled via ATUS_CODE_DISABLE_MERMAID_IMAGES.',
  };
}
...
export function renderMermaidImageSync({
  ...
  const imageRendering = env['ATUS_CODE_MERMAID_IMAGE_RENDERING'];
  if (
    imageRendering !== '1' &&
    ...
    return {
      kind: 'unavailable',
      reason:
        'Mermaid image rendering is disabled by default. Set ATUS_CODE_MERMAID_IMAGE_RENDERING=1 to enable external renderers.',
      showReason: false,
    };
  }
  ...
  if (!mmdc) {
    return {
      kind: 'unavailable',
      reason:
        'Mermaid CLI (mmdc) was not found. Install @mermaid-js/mermaid-cli, set ATUS_CODE_MERMAID_MMD_CLI, or set ATUS_CODE_MERMAID_ALLOW_NPX=1.',
    };
  }
...
function findMmdc(env: NodeJS.ProcessEnv): string | null {
  const explicit = env['ATUS_CODE_MERMAID_MMD_CLI'];
  ...
  if (
    env['ATUS_CODE_MERMAID_ALLOW_NPX'] === '1' &&
...
function findExecutable(
  ...
  const allowLocalRenderers =
    env['ATUS_CODE_MERMAID_ALLOW_LOCAL_RENDERERS'] === '1';
...
function renderPngWithMmdc(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-mermaid-'));
...
async function renderPngWithMmdcAsync(
  ...
  const tempDir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), 'atus-mermaid-'),
  );
  const inputPath = path.join(tempDir, 'diagram.mmd');
  const outputPath = path.join(tempDir, 'diagram.png');
  const renderWidth = getMermaidRenderWidth(env);
...
function getMermaidRenderWidth(env: NodeJS.ProcessEnv): number {
  const configuredWidth = Number(env['ATUS_CODE_MERMAID_RENDER_WIDTH']);
...
function getMermaidRenderTimeout(env: NodeJS.ProcessEnv): number {
  const configuredTimeout = Number(env['ATUS_CODE_MERMAID_RENDER_TIMEOUT_MS']);
...
function getMermaidCellAspectRatio(env: NodeJS.ProcessEnv): number {
  const configuredAspectRatio = Number(
    env['ATUS_CODE_MERMAID_CELL_ASPECT_RATIO'],
  );
