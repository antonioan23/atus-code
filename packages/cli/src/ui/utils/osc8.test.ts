...
describe('osc8 helpers', () => {
  ...
  describe('supportsHyperlinks', () => {
    ...
    it('FORCE_HYPERLINK=1 does NOT override non-TTY suppression', () => {
      ...
      // running `atus | cat` — escape bytes never go into a file/pipe.
      setTTY(false);
      process.env['FORCE_HYPERLINK'] = '1';
      expect(supportsHyperlinks()).toBe(false);
    });
    ...
    it('hard opt-outs (NO_COLOR/ATUS_DISABLE_HYPERLINKS) win over FORCE_HYPERLINK', () => {
      setTTY(true);
      process.env['FORCE_HYPERLINK'] = '1';
      process.env['NO_COLOR'] = '1';
      expect(supportsHyperlinks()).toBe(false);
      delete process.env['NO_COLOR'];
      process.env['ATUS_DISABLE_HYPERLINKS'] = '1';
      expect(supportsHyperlinks()).toBe(false);
    });
