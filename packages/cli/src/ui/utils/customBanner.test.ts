...
function makeSettings(opts: {
  ...
  return {
    ...
    user: opts.userUi
      ? file(
          { ui: opts.userUi },
          opts.userPath ?? '/home/u/.atus-code/settings.json',
        )
      : empty,
    workspace: opts.workspaceUi
      ? file(
          { ui: opts.workspaceUi },
          opts.workspacePath ?? '/repo/.atus-code/settings.json',
        )
      : empty,
...
describe('resolveCustomBanner', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-banner-'));
  });
  ...
  it('rejects {path} mixed with tier keys (mutually exclusive object branches)', () => {
    const out = resolveCustomBanner(
      makeSettings({
        ...
        userPath: '/home/u/.atus-code/settings.json',
      }),
    );
