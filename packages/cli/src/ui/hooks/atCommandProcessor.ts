...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const atusIgnored =
      respectFileIgnore.respectAtusIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectAtusIgnore: true,
      });

    if (gitIgnored || atusIgnored) {
      const reason =
        gitIgnored && atusIgnored ? 'both' : gitIgnored ? 'git' : 'atus';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and atus'
          : reason === 'git'
            ? 'git-ignored'
            : 'atus-ignored';
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['atus'].length) {
      messages.push(`Qwen-ignored: ${ignoredByReason['atus'].join(', ')}`);
    }
