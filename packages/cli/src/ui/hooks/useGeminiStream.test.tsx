...
describe('useGeminiStream', () => {
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.atus-code/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  describe('Tool-use summary generation', () => {
    ...
    it('fires generation with tool input/output when enabled', async () => {
      ...
      expect(options.model).toBe('atus-fast');
    ...
    it('should render descriptions from subject-bearing thought chunks', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            ...
            value: {
              subject: '',
              description: ' user mentioned globally installed atus,',
            },
          };
          yield {
            type: ServerGeminiEventType.Finished,
      ...
      await waitFor(() => {
        expect(mockAddItem).toHaveBeenCalledWith(
          expect.objectContaining({
            type: 'gemini_thought',
            text: 'The user mentioned globally installed atus,',
          }),
          expect.any(Number),
        );
      });
      expect(result.current.thought).toEqual({
        subject: 'Evaluating installation approach',
        description: 'The user mentioned globally installed atus,',
      });
    });
