...
describe('handleAtCommand', () => {
  ...
  describe('atus-ignore filtering', () => {
    it('should skip atus-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.atusignore'),
        'build/output.js',
      );
      const atusIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${atusIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${atusIgnoredFile} is atus-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nQwen-ignored: ${atusIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .atusignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.atusignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed atus-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.atusignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${atusIgnoredFile}`;
    ...
    expect(result).toMatchObject({
      processedQuery: [
        { text: `@${validFile} @${atusIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from ${validFile}:\n` },
