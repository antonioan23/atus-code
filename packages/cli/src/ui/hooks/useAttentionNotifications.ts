...
const NOTIFICATION_TITLE = 'atus code';
  ...
  useEffect(() => {
    if (
      ...
      const message = awaitingToolName
        ? `atus code needs your permission to use ${awaitingToolName}`
        : 'atus code is waiting for your input';
  ...
  useEffect(() => {
    ...
    if (streamingState === StreamingState.Idle) {
      ...
      if (wasLongTask && !isFocused && terminalBellEnabled) {
        sendNotification(
          {
            message: 'atus code is waiting for your input',
