...
import { AuthType, type Config } from '@atus-code/atus-code-core';
...
describe('availableModels', () => {
  ...
  describe('getAvailableModelsForAuthType', () => {
    ...
    it('should return hard-coded atus models for atus-oauth', () => {
      const models = getAvailableModelsForAuthType(AuthType.ATUS_OAUTH);
      expect(models.length).toBe(1);
      expect(models[0].id).toBe('coder-model');
      expect(models[0].isVision).toBe(true);
    });

    it('should use config models for atus-oauth when config is provided', () => {
      const mockConfig = {
