...
describe('modelCommand', () => {
  ...
  it('should return dialog action for ATUS_OAUTH auth type', async () => {
    const mockConfig = createMockConfig({
      model: 'test-model',
  ...
  it('should switch the main model directly in interactive mode when args are provided', async () => {
    ...
    mockContext = createMockCommandContext({
      invocation: { raw: '/model atus-max', name: 'model', args: 'atus-max' },
      services: {
        config: {
          ...
          getAvailableModelsForAuthType: vi
            .fn()
            .mockReturnValue([{ id: 'atus-max', label: 'Qwen Max' }]),
          switchModel,
        },
    ...
    const result = await modelCommand.action!(mockContext, 'atus-max');
  ...
  it('should not persist the model when direct model validation fails', async () => {
    ...
    expect(result).toEqual({
      ...
      messageType: 'error',
      content:
        "Model 'missing-model' is not available for auth type 'atus-oauth'.\n" +
        "No models are configured for auth type 'atus-oauth'.\n" +
  ...
  it('should not persist the model when direct model switching fails after validation', async () => {
    ...
    await expect(modelCommand.action!(mockContext, 'atus-max')).rejects.toThrow(
      'Refresh failed',
    );
    ...
    mockContext = createMockCommandContext({
      ...
      services: {
        config: {
          ...
          getAuthType: vi.fn().mockReturnValue(AuthType.ATUS_OAUTH),
          getAvailableModelsForAuthType: vi
            .fn()
            .mockReturnValue([{ id: 'gpt-4', label: 'GPT-4' }]),
          switchModel,
  ...
  describe('non-interactive mode', () => {
    it('should return current model without triggering dialog when no args', async () => {
      mockContext = createMockCommandContext({
        ...
        services: {
          config: {
            ...
            getModel: vi.fn().mockReturnValue('atus-max'),
          },
        },
      });
      ...
      expect(result).toEqual({
        type: 'message',
        messageType: 'info',
        content: expect.stringContaining('atus-max'),
      });
    ...
    it('should return current fast model without triggering dialog for --fast no args', async () => {
      mockContext = createMockCommandContext({
        ...
        services: {
          ...
          settings: {
            merged: { fastModel: 'atus-turbo' } as Record<string, unknown>,
          },
        },
      });
      ...
      expect(result).toEqual({
        type: 'message',
        messageType: 'info',
        content: expect.stringContaining('atus-turbo'),
      });
    });
  });
});
