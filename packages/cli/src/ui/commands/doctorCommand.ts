      ...
      if (shouldWriteHeapSnapshot) {
        ...
        try {
          ...
          if (isHighHeapPressure(latestDiagnostics)) {
            throw new Error(
              t(
                'Heap snapshot skipped: V8 heap pressure is already high, and writing a synchronous heap snapshot could make the process unresponsive or trigger OOM. Restart atus code first if it is unstable, or retry before memory pressure reaches the warning threshold.',
              ),
            );
          }
...
async function memoryDoctorAction(context: CommandContext, args = '') {
  ...
  try {
    const diagnostics = await collectMemoryDiagnostics({
      ...
      atusVersion: context.services.config?.getCliVersion(),
    });
  ...
  if (!stopResult.ok) {
    ...
    if (alreadyStopped) {
      const infoMsg =
        process.platform === 'win32'
          ? t(
              'CPU profile was stopped externally. Check ~/.atus-code/cpu-profiles/ for the output.',
            )
          : t(
              'CPU profile was stopped externally (e.g., via SIGUSR1). Check ~/.atus-code/cpu-profiles/ for the output.',
            );
      if (executionMode === 'interactive') {
