...
describe('bugCommand', () => {
  ...
  it('should generate the default GitHub issue URL', async () => {
    ...
    const atusCodeLine =
      GIT_COMMIT_INFO && !['N/A'].includes(GIT_COMMIT_INFO)
        ? `atus code: 0.1.0 (${GIT_COMMIT_INFO})`
        : 'atus code: 0.1.0';
    const expectedInfo = `${atusCodeLine}
...
Memory Usage: 100 MB`;
    const expectedUrl =
      'https://github.com/atuscode/atus-code/issues/new?template=bug_report.yml&title=A%20test%20bug&info=%0A' +
...
Memory Usage: 100 MB`;
    const expectedUrl =
      'https://github.com/atuscode/atus-code/issues/new?template=bug_report.yml&title=OpenAI%20bug&info=%0A' +
      encodeURIComponent(expectedInfo) +
      '%0A';
