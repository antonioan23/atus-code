...
export const GITHUB_WORKFLOW_PATHS = [
  'atus-dispatch/atus-dispatch.yml',
  'atus-assistant/atus-invoke.yml',
  'issue-triage/atus-triage.yml',
  'issue-triage/atus-scheduled-triage.yml',
  'pr-review/atus-review.yml',
];
...
// Add atus code specific entries to .gitignore file
export async function updateGitignore(gitRepoRoot: string): Promise<void> {
  const gitignoreEntries = ['.atus-code/', 'gha-creds-*.json'];
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/atuscode/atus-code-action/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/atuscode/atus-code-action/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
