...
export const summaryCommand: SlashCommand = {
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    const saveSummaryToDisk = async (
      ...
      // Ensure .atus-code directory exists
      const projectRoot = config.getProjectRoot();
      const atusDir = path.join(projectRoot, '.atus-code');
      try {
        await fsPromises.mkdir(atusDir, { recursive: true });
      } catch (_err) {
        // Directory might already exist, ignore error
      }

      // Save the summary to PROJECT_SUMMARY.md
      const summaryPath = path.join(atusDir, 'PROJECT_SUMMARY.md');
...
`;
      ...
      return {
        filePathForDisplay: '.atus-code/PROJECT_SUMMARY.md',
