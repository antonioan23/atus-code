  ...
  action: async (
    ...
    if (!config.isLspEnabled()) {
      return emitMessage(
        ...
        'info',
        t(
          'LSP is not enabled. Start atus code with `--experimental-lsp` to enable LSP support.',
        ),
      );
    }
    ...
    if (!client) {
      return emitMessage(
        ...
        'info',
        t(
          'LSP is enabled but no client is connected. Check debug logs under `${ATUS_RUNTIME_DIR:-~/.atus-code}/debug/` or see the LSP troubleshooting docs.',
        ),
      );
    }
