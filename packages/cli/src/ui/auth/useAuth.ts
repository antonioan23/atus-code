...
export type { AtusAuthState } from '../hooks/useQwenAuth.js';
...
export type AuthUiState = {
  ...
  atusAuthState: ReturnType<typeof useQwenAuth>['atusAuthState'];
};
...
export const useAuthCommand = (
  ...
  const { atusAuthState, cancelQwenAuth } = useQwenAuth(
    pendingAuthType,
    isAuthenticating,
  );
  ...
  const cancelAuthentication = useCallback(() => {
    if (isAuthenticating && pendingAuthType === AuthType.ATUS_OAUTH) {
      cancelQwenAuth();
    }
  ...
  // -- Validate ATUS_DEFAULT_AUTH_TYPE env var on mount --------------------

  useEffect(() => {
    const val = process.env['ATUS_DEFAULT_AUTH_TYPE'];
    const valid = [
      AuthType.ATUS_OAUTH,
      AuthType.USE_OPENAI,
