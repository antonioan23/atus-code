...
describe('generateCustomApiKeyEnvKey', () => {
  it('generates deterministic URL-based env key', () => {
    ...
    expect(key).toMatch(/^ATUS_CUSTOM_API_KEY_[A-Z0-9_]+$/);
...
describe('normalizeCustomModelIds', () => {
  it('splits comma-separated model IDs', () => {
    const result = normalizeCustomModelIds('atus/qwen3-coder,openai/gpt-4.1');
    expect(result).toEqual(['atus/qwen3-coder', 'openai/gpt-4.1']);
  });
  ...
  it('handles single model ID', () => {
    const result = normalizeCustomModelIds('atus/qwen3-coder');
    expect(result).toEqual(['atus/qwen3-coder']);
  });
});
