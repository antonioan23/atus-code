...
import { AuthType } from '@atus-code/atus-code-core';
...
describe('AuthDialog', { timeout: 15000 }, () => {
  ...
  beforeEach(() => {
    ...
    process.env['ATUS_DEFAULT_AUTH_TYPE'] = '';
    vi.clearAllMocks();
  });
  ...
  describe('GEMINI_API_KEY environment variable', () => {
    ...
    it('should not show the GEMINI_API_KEY message if ATUS_DEFAULT_AUTH_TYPE is set to something else', () => {
      process.env['GEMINI_API_KEY'] = 'foobar';
      process.env['ATUS_DEFAULT_AUTH_TYPE'] = AuthType.USE_OPENAI;
    ...
    it('should show the GEMINI_API_KEY message if ATUS_DEFAULT_AUTH_TYPE is set to use api key', () => {
      process.env['GEMINI_API_KEY'] = 'foobar';
      process.env['ATUS_DEFAULT_AUTH_TYPE'] = AuthType.USE_OPENAI;
  ...
  describe('ATUS_DEFAULT_AUTH_TYPE environment variable', () => {
    it('should select the auth type specified by ATUS_DEFAULT_AUTH_TYPE', () => {
      // ATUS_OAUTH is the only valid AuthType that can be selected via env var
      // API-KEY is not an AuthType enum value, so it cannot be selected this way
      process.env['ATUS_DEFAULT_AUTH_TYPE'] = AuthType.ATUS_OAUTH;
    ...
    it('should fall back to default if ATUS_DEFAULT_AUTH_TYPE is not set', () => {
      const settings: LoadedSettings = new LoadedSettings(
        {
          settings: {
            security: { auth: { selectedType: undefined } },
    ...
    it('should show an error and fall back to default if ATUS_DEFAULT_AUTH_TYPE is invalid', () => {
      process.env['ATUS_DEFAULT_AUTH_TYPE'] = 'invalid-auth-type';
      ...
      // Since the auth dialog doesn't show ATUS_DEFAULT_AUTH_TYPE errors anymore,
    ...
    async () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(frame).toContain('ATUS_CUSTOM_API_KEY_');
          expect(frame).toContain('atus/qwen3-coder');
