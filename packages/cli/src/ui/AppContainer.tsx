...
export const AppContainer = (props: AppContainerProps) => {
   ...
   *
   * In legacy blocking mode (`ATUS_CODE_LEGACY_MCP_BLOCKING=1`) MCP
  ...
  useEffect(() => {
    ...
    // `debugLogger.warn` so it shows up under `ATUS_CODE_DEBUG=1` and in
    ...
    async (choice: 'keep' | 'remove' | 'cancel') => {
      ...
      if (choice === 'remove' && activeWorktree) {
        try {
          ...
          // subdir but the worktree lives at `<repoRoot>/.atus-code/worktrees/`,
  ...
  // Update terminal title with atus code status and thoughts
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because atus code is already doing that elsewhere
  }, [
    streamingState,
    thought,
    settings.merged.ui?.showStatusInTitle,
