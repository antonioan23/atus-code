...
import { AtusLight } from './atus-light.js';
import { AtusDark } from './atus-dark.js';
...
export const DEFAULT_THEME: Theme = AtusDark;
...
class ThemeManager {
  ...
  private resolveAutoTheme(): Theme {
    ...
    return detected === 'light' ? AtusLight : AtusDark;
  }
  ...
  async resolveAutoThemeAsync(): Promise<void> {
    ...
    this.activeTheme = detected === 'light' ? AtusLight : AtusDark;
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    const atusThemes = builtInThemes.filter(
      (theme) => theme.name === AtusLight.name || theme.name === AtusDark.name,
    );
    ...
    return [...atusThemes, ...sortedOtherThemes];
  }
