...
import { MAIN_SOURCE } from '@atus-code/atus-code-core';
...
describe('<StatsDisplay />', () => {
  ...
  describe('Subagent source attribution', () => {
    ...
    it('labels main rows session-wide when a subagent uses a different model', () => {
      // Session has two models: glm-5 used only by main, atus-plus used only by
      // a subagent. Even though glm-5 has a single main source, it must still
      // render with `(main)` because the session-wide rule triggers on atus-plus.
      const metrics: SessionMetrics = {
        models: {
      ...
      expect(output).toContain('glm-5 (main)');
      expect(output).toContain('atus-plus (researcher)');
