...
const createSettings = (options?: {
  ...
  return {
    ...
    user: {
      settings: { ui },
      originalSettings: { ui },
      path: '/home/u/.atus-code/settings.json',
    },
...
const createMockConfig = (overrides = {}) => ({
  ...
  getTargetDir: vi.fn(() => '/projects/atus-code'),
  getMcpServers: vi.fn(() => ({})),
...
describe('<AppHeader />', () => {
  it('shows the working directory', () => {
    ...
    expect(lastFrame()).toContain('/projects/atus-code');
  });
  ...
  it('hides the header when screen reader is enabled', () => {
    ...
    expect(lastFrame()).not.toContain('/projects/atus-code');
    expect(lastFrame()).not.toContain('atus code');
  });

  it('shows the header with all info when banner is visible', () => {
    const { lastFrame } = renderWithProviders(createMockUIState());
    expect(lastFrame()).toContain('>_ atus code');
  ...
  it('hides the banner when ui.hideBanner is set, but keeps tips intact', () => {
    ...
    expect(lastFrame()).not.toContain('>_ atus code');
  ...
  it('renders custom banner title and inline ASCII art end-to-end through resolveCustomBanner', () => {
    ...
    expect(frame).not.toContain('>_ atus code');
