...
const defaultProps = {
  version: '1.0.0',
  authDisplayType: AuthDisplayType.ATUS_OAUTH,
  model: 'atus-coder-plus',
...
describe('<Header />', () => {
  ...
  it('displays auth type and model', () => {
    ...
    expect(lastFrame()).toContain('atus-coder-plus');
  });
  ...
  it('renders the custom subtitle in place of the blank spacer row', () => {
    ...
    const titleIdx = frame.indexOf('>_ atus code');
  ...
  it('keeps the blank spacer row when no subtitle is set (back-compat)', () => {
    ...
    expect(frame).toContain('>_ atus code');
