...
import { AuthType, DEFAULT_QWEN_MODEL } from '@atus-code/atus-code-core';
...
describe('<ModelDialog />', () => {
  ...
  it('passes all model options to DescriptiveRadioButtonSelect', () => {
    ...
    expect(props.items[0].value).toBe(
      `${AuthType.ATUS_OAUTH}::${DEFAULT_QWEN_MODEL}`,
    );
  ...
  it('initializes with the model from ConfigContext', () => {
    ...
    const atusModels = getFilteredQwenModels();
    const expectedIndex = atusModels.findIndex(
  ...
  it('blocks atus-oauth model selection with an error message (discontinued)', async () => {
    ...
    await childOnSelect(`${AuthType.ATUS_OAUTH}::${DEFAULT_QWEN_MODEL}`);

    // atus-oauth is discontinued — switchModel should NOT be called
  ...
  it('blocks switching to atus-oauth from another authType (discontinued)', async () => {
    const switchModel = vi.fn().mockResolvedValue(undefined);
    const getAuthType = vi.fn(() => AuthType.USE_OPENAI);
    const getAvailableModelsForAuthType = vi.fn((t: AuthType) => {
      if (t === AuthType.USE_OPENAI) {
  ...
  it('updates initialIndex when config context changes', () => {
    const mockGetModel = vi.fn(() => DEFAULT_QWEN_MODEL);
    const mockGetAuthType = vi.fn(() => 'atus-oauth');
    ...
    const atusModels = getFilteredQwenModels();
    const expectedCoderIndex = atusModels.findIndex(
      (m) => m.id === DEFAULT_QWEN_MODEL,
    );
