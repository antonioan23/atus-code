...
const MODEL_PROVIDERS_DOCUMENTATION_URL =
  'https://atuscode.github.io/atus-code-docs/en/users/configuration/settings/#modelproviders';
...
export function ArenaStartDialog({
  ...
  const modelItems = useMemo(() => {
    ...
    return selectableModels.map((model) => {
      ...
      const isQwenOauth = model.authType === AuthType.ATUS_OAUTH;
      return {
        key: token,
        value: token,
      ...
      {(hasDisabledQwenOauth || needsMoreModels) && (
        <Box marginTop={1} flexDirection="column">
          {hasDisabledQwenOauth && (
            <Text color={theme.status.warning}>
              {t('Note: atus-oauth models are not supported in Arena.')}
            </Text>
          )}
          {needsMoreModels && (
            <>
