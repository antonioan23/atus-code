...
vi.mock('node:fs', async (importOriginal) => {
  const actual = await importOriginal<Record<string, unknown>>();
  const mocks = {
    mkdtempSync: vi.fn().mockReturnValue('/tmp/atus-edit-mock'),
    writeFileSync: vi.fn(),
...
const expectedTmpFile = pathMod.join('/tmp/atus-edit-mock', 'buffer.txt');

describe('openInExternalEditor', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    (fs.mkdtempSync as Mock).mockReturnValue('/tmp/atus-edit-mock');
  ...
  it('should create temp file in private mkdtemp directory', async () => {
    ...
    expect(fs.mkdtempSync).toHaveBeenCalledWith(
      expect.stringContaining('atus-edit-'),
    );
  ...
  it('should clean up tmpDir and file in finally', async () => {
    ...
    expect(fs.rmSync).toHaveBeenCalledWith('/tmp/atus-edit-mock', {
      recursive: true,
      force: true,
    });
  });
