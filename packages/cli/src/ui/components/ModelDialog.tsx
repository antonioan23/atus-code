...
export function ModelDialog({
  ...
  const availableModelEntries = useMemo(() => {
    ...
    // Fixed order: atus-oauth first, then others in a stable order
    ...
    () =>
      availableModelEntries.map(
        ({ authType: t2, model, isRuntime, snapshotId }) => {
          ...
          const isQwenOAuth = t2 === AuthType.ATUS_OAUTH;
  ...
  const handleSelect = useCallback(
    async (selected: string) => {
      ...
      // Block selection of discontinued atus-oauth models
      ...
      const isQwenOAuthSelection =
        selected.startsWith(`${AuthType.ATUS_OAUTH}::`) ||
        (selected.startsWith('$runtime|') &&
          selected.split('|')[1] === AuthType.ATUS_OAUTH);
      const isRuntimeOAuthSelection = selected.startsWith(
        `$runtime|${AuthType.ATUS_OAUTH}|`,
      );
      ...
      try {
        ...
        await config.switchModel(selectedAuthType, modelId, {
          ...
          selectedAuthType === AuthType.ATUS_OAUTH
      ...
      {highlightedEntry && (
        <Box marginTop={1} flexDirection="column">
          <Box
            ...
            borderColor={theme.border.default}
          />
          {highlightedEntry.authType === AuthType.ATUS_OAUTH &&
