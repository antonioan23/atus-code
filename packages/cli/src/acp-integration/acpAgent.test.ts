...
vi.mock('@atus-code/atus-code-core', () => ({
  createDebugLogger: () => ({
    debug: vi.fn(),
    error: vi.fn(),
    warn: vi.fn(),
  ...
  QwenOAuth2Event: {},
  atusOAuth2Events: { on: vi.fn(), off: vi.fn() },
  MCPDiscoveryState: {
...
// ---------------------------------------------------------------------------
// Tests for AtusAgent.initialize() mcpCapabilities + newSession SSE/HTTP
// ---------------------------------------------------------------------------

describe('AtusAgent MCP SSE/HTTP support', () => {
  ...
  it('does not return discontinued atus-oauth as the only ACP auth option', async () => {
  ...
  it('status ext methods expose workspace snapshots without secrets', async () => {
    ...
    mockConfig = {
      ...
      getAuthType: vi.fn().mockReturnValue('atus'),
      getAllConfiguredModels: vi.fn().mockReturnValue([
        {
          id: 'atus-plus',
          label: 'Qwen Plus',
      ...
      getModel: vi.fn().mockReturnValue('atus-plus'),
    } as unknown as Config;
    ...
    expect(providers).toMatchObject({
      ...
      workspaceCwd: '/work/status',
      initialized: true,
      current: { authType: 'atus', modelId: 'atus-plus(atus)' },
      providers: [
        {
          kind: 'model_provider',
          status: 'ok',
          ...
          models: [
            {
              modelId: 'atus-plus(atus)',
              baseModelId: 'atus-plus',
              name: 'Qwen Plus',
              description: 'General coding model',
              contextLimit: 65_536,
              isCurrent: true,
  ...
  it('extMethod atus/status/workspace/preflight returns 6 ACP-side cells', async () => {
    mockConfig = {
      ...mockConfig,
      getTargetDir: vi.fn().mockReturnValue('/work/status'),
      getMcpServers: vi.fn().mockReturnValue({}),
  ...
  it('extMethod preflight surfaces SkillError as parse_error errorKind', async () => {
    const skillError = new (
      await import('@atus-code/atus-code-core')
  ...
  it('provider status marks current only for matching models', async () => {
    ...
    await expect(
      ...
      current: { authType: 'atus', modelId: 'missing-model(atus)' },
      providers: [
        {
          authType: 'atus',
          current: false,
  ...
  it('provider status uses runtime model ids for base id and token limit', async () => {
    mockConfig = {
      ...
      getAllConfiguredModels: vi.fn().mockReturnValue([
        {
          id: 'atus-plus',
          runtimeSnapshotId: 'runtime-atus-plus',
    ...
    await expect(
      ...
      current: { authType: 'atus', modelId: 'runtime-atus-plus(atus)' },
      providers: [
        {
          authType: 'atus',
          current: true,
          models: [
            {
              modelId: 'runtime-atus-plus(atus)',
              baseModelId: 'runtime-atus-plus',
              contextLimit: 128_000,
              isCurrent: true,
              isRuntime: true,
            },
    ...
    expect(vi.mocked(tokenLimit)).toHaveBeenCalledWith('runtime-atus-plus');
  ...
  it('per-session newSession surfaces MCP failures to stderr (round-7 fix: was silent before)', async () => {
    // Round-7 regression: `AtusAgent.initializeConfig()` (per-session ACP
...
describe('AtusAgent extMethod renameSession routing', () => {
  type AgentSideConnectionLike = { closed: Promise<void> };
  type AgentLike = {
    initialize: (args: Record<string, unknown>) => Promise<unknown>;
    newSession: (args: Record<string, unknown>) => Promise<unknown>;
...
// Tests for AtusAgent.loadSession() and AtusAgent.unstable_resumeSession()
...
// resume does not).
describe('AtusAgent loadSession / unstable_resumeSession', () => {
  let capturedAgentFactory:
