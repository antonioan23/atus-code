...
export class Session implements SessionContext {
  ...
  private async runToolCalls(
    ...
    // `ATUS_CODE_MAX_TOOL_CONCURRENCY` (default 10). Results are returned
  ...
  private async runTool(
    ...
    if (pm && !(await pm.isToolEnabled(fc.name as string))) {
      return earlyErrorResponse(
        new Error(
          `atus code requires permission to use "${fc.name}", but that permission was declined.`,
        ),
        fc.name,
      );
    }
        ...
        if (
          ...
          if (hooksEnabled && messageBus) {
            void fireNotificationHook(
              messageBus,
              `atus code needs your permission to use ${fc.name}`,
