 ...
 * here, or explicitly waived for non-env-based auth like atus-oauth). The
...
export async function runAcpAgent(
  ...
  let agentInstance: AtusAgent | undefined;
  const connection = new AgentSideConnection((conn) => {
    agentInstance = new AtusAgent(config, settings, argv, conn);
...
class AtusAgent implements Agent {
  ...
  async authenticate({ methodId }: AuthenticateRequest): Promise<void> {
    ...
    if (method === AuthType.ATUS_OAUTH) {
      atusOAuth2Events.once(QwenOAuth2Event.AuthUri, authUriHandler);
    }
    ...
    try {
      ...
      if (method === AuthType.ATUS_OAUTH) {
        atusOAuth2Events.off(QwenOAuth2Event.AuthUri, authUriHandler);
      }
    }
  }
  ...
  private buildWorkspaceMcpStatus(config: Config): ServeWorkspaceMcpStatus {
    try {
      ...
      try {
        ...
        process.stderr.write(
          `atus serve: getMcpClientAccounting failed: ` +
  ...
  private buildBudgetCells(
    ...
    const cell: ServeMcpBudgetStatusCell = {
      ...
      // and reads `ATUS_SERVE_MCP_CLIENT_BUDGET` independently.
    ...
    try {
      ...
      if (!authType) {
        return this.acpCell('auth', {
          ...
          hint: 'Run `atus` and complete the auth flow, or set a provider env var.',
      ...
      // No env-var registration → either OAuth-style auth (atus-oauth) or
  ...
  private async newSessionConfig(
    ...
    // events under `ATUS_CODE_LEGACY_MCP_BLOCKING=1` (synchronous
    ...
    if (
      ...
      config.setMcpBudgetEventCallback((event) => {
        ...
        void this.connection
          .extNotification('atus/notify/session/mcp-budget-event', {
            v: 1,
            sessionId: sid,
            ...event,
          })
  ...
  private async ensureAuthenticated(config: Config): Promise<void> {
    ...
    if (!selectedType) {
      throw RequestError.authRequired(
        ...
        'Use atus code CLI to authenticate first.',
      );
    }
