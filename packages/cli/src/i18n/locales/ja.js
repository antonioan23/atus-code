...
// Japanese translations for atus code CLI
...
export default {
  ...
  'for help on atus code': 'atus code のヘルプ',
  ...
  'List available atus code tools. Usage: /tools [desc]':
    '利用可能な atus code ツールを一覧表示。使い方: /tools [desc]',
  ...
  'Workspace is untrusted; workspace settings are ignored by the merged config. Run /trust first to persist skills changes here, or edit ~/.atus-code/settings.json directly to manage skills at user scope.':
    'ワークスペースが信頼されていないため、ワークスペース設定はマージ設定で無視されます。先に /trust を実行するか、~/.atus-code/settings.json を直接編集してユーザースコープでスキルを管理してください。',
  ...
  'All available skills are disabled. Edit ~/.atus-code/settings.json or .atus-code/settings.json (skills.disabled) to re-enable.':
    'すべての利用可能なスキルが無効化されています。~/.atus-code/settings.json または .atus-code/settings.json (skills.disabled) を編集して再有効化してください。',
  ...
  Bundled: '組み込み',
  'Available atus code CLI tools:': '利用可能な atus code CLI ツール:',
  ...
  'Project Level (.atus-code/agents/)': 'プロジェクトレベル (.atus-code/agents/)',
  'User Level (~/.atus-code/agents/)': 'ユーザーレベル (~/.atus-code/agents/)',
  ...
  'Generate with atus code (Recommended)': 'atus code で生成(推奨)',
  ...
  'To see changes, atus code must be restarted. Press r to exit and apply changes now.':
    '変更を確認するには atus code を再起動する必要があります。 r を押して終了し、変更を適用してください',
  'View and edit atus code settings': 'atus code の設定を表示・編集',
  Settings: '設定',
  'Vim Mode': 'Vim モード',
  'Output Format': '出力形式',
  'Hide Tips': 'ヒントを非表示',
  ...
  'Respect .atusignore': '.atusignore を優先',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with atus code.':
    '{{originSource}} から拡張機能をインストールしています。一部の機能は atus code で完全に動作しない可能性があります。',
  ...
  'IDE integration is not supported in your current environment. To use this feature, run atus code in one of these supported IDEs: VS Code or VS Code forks.':
    '現在の環境ではIDE連携はサポートされていません。この機能を使用するには、VS Code または VS Code 派生エディタで atus code を実行してください',
  ...
  // Commands - Hooks
  // ============================================================================
  'Manage atus code hooks': 'atus code のフックを管理する',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask atus code.':
    'このメニューは読み取り専用です。フックを追加または変更するには、settings.json を直接編集するか、atus code に尋ねてください。',
  ...
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask atus code.':
    'フックを再有効化するには、settings.json から "disableAllHooks" を削除するか、atus code に尋ねてください。',
  // Hooks - Source
  Project: 'プロジェクト',
  User: 'ユーザー',
  Skill: 'スキル',
  ...
  'Generate a project summary and save it to .atus-code/PROJECT_SUMMARY.md':
    'プロジェクトサマリーを生成し、.atus-code/PROJECT_SUMMARY.md に保存',
  ...
  'Run atus --debug to see error logs':
    'atus --debug を実行してエラーログを確認してください',
  ...
  'Invalid ATUS_DEFAULT_AUTH_TYPE value: "{{value}}". Valid values are: {{validValues}}':
    '無効な ATUS_DEFAULT_AUTH_TYPE 値: "{{value}}"。有効な値: {{validValues}}',
  // Dialogs - Model
  'Select Model': 'モデルを選択',
  Modality: 'モダリティ',
  ...
  "atus code won't ask before using allowed tools.":
    'atus code は許可されたツールを使用する前に確認しません。',
  'atus code will ask before using these tools.':
    'atus code はこれらのツールを使用する前に確認します。',
  'atus code is not allowed to use denied tools.':
    'atus code は拒否されたツールを使用できません。',
  ...
  'Checked in at .atus-code/settings.json': '.atus-code/settings.json にチェックイン',
  'User settings': 'ユーザー設定',
  'Saved in at ~/.atus-code/settings.json': '~/.atus-code/settings.json に保存',
  ...
  'atus code can read files in the workspace, and make edits when auto-accept edits is on.':
    'atus code はワークスペース内のファイルを読み取り、自動編集承認が有効な場合は編集を行えます。',
  'atus code will be able to read files in this directory and make edits when auto-accept edits is on.':
    'atus code はこのディレクトリ内のファイルを読み取り、自動編集承認が有効な場合は編集を行えます。',
  ...
  'You can run any shell commands from atus code using ! (e.g. !ls).':
    'atus code から ! を使って任意のシェルコマンドを実行できます（例: !ls）。',
  ...
  'You can resume a previous conversation by running atus --continue or atus --resume.':
    'atus --continue または atus --resume で前の会話を再開できます。',
  ...
  'Add a QWEN.md file to give atus code persistent project context.':
    'QWEN.md ファイルを追加すると、atus code に永続的なプロジェクトコンテキストを与えられます。',
  ...
  '  atus auth atus-oauth     - Authenticate with Qwen OAuth (discontinued)':
    '  atus auth atus-oauth     - Qwen OAuth で認証（終了）',
  'Or simply run:': 'または以下を実行:',
  '  atus auth                - Interactive authentication setup\n':
    '  atus auth                - インタラクティブ認証セットアップ\n',
  ...
  '  Run `atus auth coding-plan` to re-configure.\n':
    '  `atus auth coding-plan` を実行して再設定してください。\n',
  ...
  "Set up atus code's status line UI": 'atus code のステータスライン UI を設定',
