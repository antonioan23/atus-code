/**
 * @license
 * Copyright 2025 Atus Code team
 * SPDX-License-Identifier: Apache-2.0
 */

// Traditional Chinese (zh-TW) translations for atus code CLI
...
export default {
  ...
  'for help on atus code': '獲取 atus code 幫助',
  ...
  'List available atus code tools. Usage: /tools [desc]':
    '列出可用的 atus code 工具。用法：/tools [desc]',
  ...
  'Workspace is untrusted; workspace settings are ignored by the merged config. Run /trust first to persist skills changes here, or edit ~/.atus-code/settings.json directly to manage skills at user scope.':
    '目前工作區未受信任，工作區設定會被合併設定忽略。請先執行 /trust，或直接編輯 ~/.atus-code/settings.json 在使用者範圍管理技能。',
  ...
  'All available skills are disabled. Edit ~/.atus-code/settings.json or .atus-code/settings.json (skills.disabled) to re-enable.':
    '所有可用技能皆已停用。請編輯 ~/.atus-code/settings.json 或 .atus-code/settings.json（skills.disabled）以重新啟用。',
  ...
  Bundled: '內建',
  'Available atus code CLI tools:': '可用的 atus code CLI 工具：',
  ...
  'Are you sure you want to delete agent "{{name}}"?':
    '您確定要刪除智能體 "{{name}}" 嗎？',
  'Project Level (.atus-code/agents/)': '項目級 (.atus-code/agents/)',
  'User Level (~/.atus-code/agents/)': '用戶級 (~/.atus-code/agents/)',
  ...
  'Generate with atus code (Recommended)': '使用 atus code 生成（推薦）',
  ...
  'not updatable': '不可更新',
  error: '錯誤',
  'View and edit atus code settings': '查看和編輯 atus code 設置',
  Settings: '設置',
  'To see changes, atus code must be restarted. Press r to exit and apply changes now.':
    '要查看更改，必須重啟 atus code。按 r 退出並立即應用更改。',
  'Vim Mode': 'Vim 模式',
  ...
  'How is Atus doing this session? (optional)': 'Atus 這次表現如何？（可選）',
  Bad: '不滿意',
  Fine: '還行',
  Good: '滿意',
  Dismiss: '忽略',
  ...
  'Respect .atusignore': '遵守 .atusignore',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with atus code.':
    '您正在安裝來自 {{originSource}} 的擴展。某些功能可能無法完美兼容 atus code。',
  ...
  'IDE integration is not supported in your current environment. To use this feature, run atus code in one of these supported IDEs: VS Code or VS Code forks.':
    '您當前環境不支持 IDE 集成。要使用此功能，請在以下支持的 IDE 之一中運行 atus code：VS Code 或 VS Code 分支版本。',
  ...
  'Manage atus code hooks': '管理 atus code Hook',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask atus code.':
    '此選單為只讀。要添加或修改 Hook，請直接編輯 settings.json 或詢問 atus code。',
  ...
  'To add hooks, edit settings.json directly or ask Atus.':
    '要添加 Hook，請直接編輯 settings.json 或詢問 Atus。',
  ...
  'To modify or remove this hook, edit settings.json directly or ask Atus to help.':
    '要修改或刪除此 Hook，請直接編輯 settings.json 或詢問 Atus。',
  ...
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask atus code.':
    '要重新啟用 Hook，請從 settings.json 中刪除 "disableAllHooks" 或詢問 atus code。',
  Project: '項目',
  User: '用戶',
  Skill: '技能',
  System: '系統',
  ...
  'Right before atus code concludes its response': 'atus code 結束響應之前',
  ...
  'stdout shown to Atus': '向 Atus 顯示 stdout',
  ...
  'Run atus --debug to see error logs': '運行 atus --debug 查看錯誤日誌',
  ...
  'Generate a project summary and save it to .atus-code/PROJECT_SUMMARY.md':
    '生成項目摘要並保存到 .atus-code/PROJECT_SUMMARY.md',
  ...
  'Qwen OAuth (removed) - use API key': 'Qwen OAuth 認證',
  'Please visit this URL to authorize:': '請訪問此 URL 進行授權：',
  'Waiting for authorization': '等待授權中',
  'Time remaining:': '剩餘時間：',
  'Qwen OAuth (removed) - use API key Timeout': 'Qwen OAuth 認證超時',
  'OAuth token expired (over {{seconds}} seconds). Please select authentication method again.':
    'OAuth 令牌已過期（超過 {{seconds}} 秒）。請重新選擇認證方法',
  'Press any key to return to authentication type selection.':
    '按任意鍵返回認證類型選擇',
  'Qwen OAuth has been removed': '正在等待 Qwen OAuth 認證...',
  ...
  'Invalid ATUS_DEFAULT_AUTH_TYPE value: "{{value}}". Valid values are: {{validValues}}':
    '無效的 ATUS_DEFAULT_AUTH_TYPE 值："{{value}}"。有效值為：{{validValues}}',
  'Select Model': '選擇模型',
  'API Key': 'API Key',
  '(default)': '(默認)',
  '(not set)': '(未設置)',
  ...
  'Checked in at .atus-code/settings.json': '保存在 .atus-code/settings.json',
  'User settings': '用戶設置',
  'Saved in at ~/.atus-code/settings.json': '保存在 ~/.atus-code/settings.json',
  ...
  'You can run any shell commands from atus code using ! (e.g. !ls).':
    '在 atus code 中使用 ! 可運行任意 shell 命令（例如 !ls）。',
  ...
  'You can resume a previous conversation by running atus --continue or atus --resume.':
    '運行 atus --continue 或 atus --resume 可繼續之前的會話。',
  ...
  'Add a QWEN.md file to give atus code persistent project context.':
    '添加 QWEN.md 檔案，為 atus code 提供持久的項目上下文。',
  ...
  'Agent powering down. Goodbye!': 'atus code 正在關閉，再見！',
  ...
  '↑/↓: Navigate | Enter: Select | Esc: Cancel':
    '↑/↓: 導航 | Enter: 選擇 | Esc: 取消',
  'Authenticate (Qwen OAuth removed; use API key)': '使用 Qwen OAuth 進行認證',
  ...
  '  atus auth atus-oauth     - Authenticate with Qwen OAuth (discontinued)':
    '  atus auth atus-oauth     - 使用 Qwen OAuth 登錄（已停用）',
  ...
  "Set up atus code's status line UI": '配置 atus code 的狀態欄',
  ...
  'Rollback on Windows requires manual intervention. Rename atus-code.old to atus-code in your installation directory.':
    '在 Windows 上回滾需要手動操作。請將安裝目錄中的 atus-code.old 重新命名為 atus-code。',
