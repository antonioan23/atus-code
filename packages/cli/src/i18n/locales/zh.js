...
// Chinese translations for atus code CLI
...
export default {
  ...
  'for help on atus code': '获取 atus code 帮助',
  ...
  'Workspace is untrusted; workspace settings are ignored by the merged config. Run /trust first to persist skills changes here, or edit ~/.atus-code/settings.json directly to manage skills at user scope.':
    '当前工作区未受信任，工作区设置会被合并配置忽略。请先执行 /trust，或直接编辑 ~/.atus-code/settings.json 在用户范围管理技能。',
  ...
  'All available skills are disabled. Edit ~/.atus-code/settings.json or .atus-code/settings.json (skills.disabled) to re-enable.':
    '所有可用技能均已禁用。请编辑 ~/.atus-code/settings.json 或 .atus-code/settings.json（skills.disabled）以重新启用。',
  ...
  // ============================================================================
  'Project Level (.atus-code/agents/)': '项目级 (.atus-code/agents/)',
  'User Level (~/.atus-code/agents/)': '用户级 (~/.atus-code/agents/)',
  ...
  'Generate with atus code (Recommended)': '使用 atus code 生成（推荐）',
  ...
  // ============================================================================
  'View and edit atus code settings': '查看和编辑 atus code 设置',
  Settings: '设置',
  'To see changes, atus code must be restarted. Press r to exit and apply changes now.':
    '要查看更改，必须重启 atus code。按 r 退出并立即应用更改。',
  // ============================================================================
  // Settings Labels
  // ============================================================================
  'Vim Mode': 'Vim 模式',
  ...
  'How is Atus doing this session? (optional)': 'Atus 这次表现如何？（可选）',
  Bad: '不满意',
  Fine: '还行',
  Good: '满意',
  Dismiss: '忽略',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with atus code.':
    '您正在安装来自 {{originSource}} 的扩展。某些功能可能无法完美兼容 atus code。',
  ...
  'IDE integration is not supported in your current environment. To use this feature, run atus code in one of these supported IDEs: VS Code or VS Code forks.':
    '您当前环境不支持 IDE 集成。要使用此功能，请在以下支持的 IDE 之一中运行 atus code：VS Code 或 VS Code 分支版本。',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask atus code.':
    '此菜单为只读。要添加或修改 Hook，请直接编辑 settings.json 或询问 atus code。',
  ...
  'To add hooks, edit settings.json directly or ask Atus.':
    '要添加 Hook，请直接编辑 settings.json 或询问 Atus。',
  ...
  'To modify or remove this hook, edit settings.json directly or ask Atus to help.':
    '要修改或删除此 Hook，请直接编辑 settings.json 或询问 Atus。',
  // Hooks - Disabled Step
  ...
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask atus code.':
    '要重新启用 Hook，请从 settings.json 中删除 "disableAllHooks" 或询问 atus code。',
  // Hooks - Source
  Project: '项目',
  User: '用户',
  Skill: '技能',
  ...
  'Right before atus code concludes its response': 'atus code 结束响应之前',
  ...
  'stdout shown to Atus': '向 Atus 显示 stdout',
  ...
  'Run atus --debug to see error logs': '运行 atus --debug 查看错误日志',
  ...
  'Generate a project summary and save it to .atus-code/PROJECT_SUMMARY.md':
    '生成项目摘要并保存到 .atus-code/PROJECT_SUMMARY.md',
  ...
  'Qwen OAuth (removed) - use API key': 'Qwen OAuth 认证',
  'Please visit this URL to authorize:': '请访问此 URL 进行授权：',
  'Waiting for authorization': '等待授权中',
  'Time remaining:': '剩余时间：',
  'Qwen OAuth (removed) - use API key Timeout': 'Qwen OAuth 认证超时',
  'OAuth token expired (over {{seconds}} seconds). Please select authentication method again.':
    'OAuth 令牌已过期（超过 {{seconds}} 秒）。请重新选择认证方法',
  'Press any key to return to authentication type selection.':
    '按任意键返回认证类型选择',
  'Qwen OAuth has been removed': '正在等待 Qwen OAuth 认证...',
  ...
  'Invalid ATUS_DEFAULT_AUTH_TYPE value: "{{value}}". Valid values are: {{validValues}}':
    '无效的 ATUS_DEFAULT_AUTH_TYPE 值："{{value}}"。有效值为：{{validValues}}',
  // ============================================================================
  // Dialogs - Model
  // ============================================================================
  'Select Model': '选择模型',
  ...
  'You can run any shell commands from atus code using ! (e.g. !ls).':
    '在 atus code 中使用 ! 可运行任意 shell 命令（例如 !ls）。',
  ...
  'You can resume a previous conversation by running atus --continue or atus --resume.':
    '运行 atus --continue 或 atus --resume 可继续之前的会话。',
  ...
  'Add a QWEN.md file to give atus code persistent project context.':
    '添加 QWEN.md 文件，为 atus code 提供持久的项目上下文。',
  ...
  // Exit Screen / Stats
  // ============================================================================
  'Agent powering down. Goodbye!': 'atus code 正在关闭，再见！',
  ...
  '↑/↓: Navigate | Enter: Select | Esc: Cancel':
    '↑/↓: 导航 | Enter: 选择 | Esc: 取消',
  'Authenticate (Qwen OAuth removed; use API key)': '使用 Qwen OAuth 进行认证',
  ...
  '  atus auth atus-oauth     - Authenticate with Qwen OAuth (discontinued)':
    '  atus auth atus-oauth     - 使用 Qwen OAuth 登录（已停用）',
  ...
  "Set up atus code's status line UI": '配置 atus code 的状态栏',
  ...
  'Rollback on Windows requires manual intervention. Rename atus-code.old to atus-code in your installation directory.':
    '在 Windows 上回滚需要手动操作。请将安装目录中的 atus-code.old 重命名为 atus-code。',
