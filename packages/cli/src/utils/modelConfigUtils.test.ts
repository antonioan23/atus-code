...
describe('modelConfigUtils', () => {
  describe('getAuthTypeFromEnv', () => {
    ...
    it('should return ATUS_OAUTH when ATUS_OAUTH is set', () => {
      process.env['ATUS_OAUTH'] = 'true';

      expect(getAuthTypeFromEnv()).toBe(AuthType.ATUS_OAUTH);
    });
    ...
    it('should prioritize ATUS_OAUTH over other auth types when explicitly set', () => {
      process.env['ATUS_OAUTH'] = 'true';
      process.env['OPENAI_API_KEY'] = 'test-key';
      process.env['OPENAI_MODEL'] = 'gpt-4';
      process.env['OPENAI_BASE_URL'] = 'https://api.openai.com';

      // ATUS_OAUTH is checked first, so it should be returned even when other auth vars are set
      expect(getAuthTypeFromEnv()).toBe(AuthType.ATUS_OAUTH);
    });
  ...
  describe('resolveCliGenerationConfig', () => {
    ...
    // Edge Case 2: ATUS_MODEL is used as final fallback when OPENAI_MODEL is not set
    it('Edge Case 2: ATUS_MODEL should be used as fallback when OPENAI_MODEL is not set', () => {
      const argv = {};
      const atusProvider: ProviderModelConfig = {
        id: 'atus-env-model',
        name: 'Qwen Env Model',
      };
      const settings = makeMockSettings({
      ...
      vi.mocked(resolveModelConfig).mockReturnValue({
        config: { model: 'atus-env-model', apiKey: '', baseUrl: '' },
        sources: {},
        warnings: [],
      });
      ...
      resolveCliGenerationConfig({
        ...
        settings,
        selectedAuthType,
        env: { ATUS_MODEL: 'atus-env-model' },
      });

      expect(vi.mocked(resolveModelConfig)).toHaveBeenCalledWith(
        expect.objectContaining({
          modelProvider: atusProvider,
        }),
      );
    });

    // Edge Case 3: OPENAI_MODEL over ATUS_MODEL when both are set and settings.model.name is not set
    it('Edge Case 3: OPENAI_MODEL should win over ATUS_MODEL when both set', () => {
      const argv = {};
    ...
    describe('[Regression] model precedence', () => {
      ...
      it('[Regression] ATUS_MODEL as fallback when OPENAI_MODEL not set', () => {
        const settings = makeMockSettings({ model: { name: undefined } });
        const selectedAuthType = AuthType.USE_OPENAI;
        const env = { ATUS_MODEL: 'atus-model', OPENAI_API_KEY: 'key' };

        vi.mocked(resolveModelConfig).mockImplementation(() => ({
          config: { model: 'atus-model', apiKey: 'key', baseUrl: '' },
          sources: { model: { kind: 'env' as const, envKey: 'ATUS_MODEL' } },
          warnings: [],
        }));
        ...
        // ATUS_MODEL should be passed to resolveModelConfig
        const callArgs = vi.mocked(resolveModelConfig).mock.calls[0][0];
        expect(callArgs.env?.['ATUS_MODEL']).toBe('atus-model');
      });
