 ...
 *
 * Activated by setting ATUS_CODE_PROFILE_STARTUP=1. When enabled, collects
 * high-resolution timestamps at key phases of CLI startup and writes a JSON
 * report to ~/.atus-code/startup-perf/ on finalization.
 *
 ...
 * reports. Set ATUS_CODE_PROFILE_STARTUP_OUTER=1 to also profile the outer
...
import type { StartupEventAttrs } from '@atus-code/atus-code-core';
...
export function initStartupProfiler(): void {
  ...
  if (process.env['ATUS_CODE_PROFILE_STARTUP'] !== '1') {
    return;
  }

  const inSandboxChild = !!process.env['SANDBOX'];
  const outerOptIn = process.env['ATUS_CODE_PROFILE_STARTUP_OUTER'] === '1';
  ...
  // Disable with ATUS_CODE_PROFILE_STARTUP_NO_HEAP=1 when measuring the
  // Heisenberg overhead of the heap call itself.
  captureHeap = process.env['ATUS_CODE_PROFILE_STARTUP_NO_HEAP'] !== '1';
  finalized = false;
...
export function finalizeStartupProfile(sessionId?: string): void {
  ...
  try {
    const dir = path.join(os.homedir(), '.atus-code', 'startup-perf');
