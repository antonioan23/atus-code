...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/atuscode/atus-code-action/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest atus-code-action release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest atus-code-action release on GitHub.`,
    );
  }
};
