...
describe('apiPreconnect', () => {
  beforeEach(() => {
    ...
    delete process.env['http_proxy'];
    delete process.env['ATUS_CODE_DISABLE_PRECONNECT'];
  ...
  describe('shouldSkipPreconnect', () => {
    it('should skip when NODE_EXTRA_CA_CERTS is set', () => {
      ...
      preconnectApi('atus-oauth', { proxy: 'http://proxy.example.com:8080' });
  ...
  describe('preconnect behavior', () => {
    it('should use default baseUrl for atus-oauth', () => {
      preconnectApi('atus-oauth', {
        proxy: 'http://proxy.example.com:8080',
      });
      expect(mockFetch).toHaveBeenCalledWith(
    ...
    it('should skip when ATUS_CODE_DISABLE_PRECONNECT is set', () => {
      process.env['ATUS_CODE_DISABLE_PRECONNECT'] = '1';
      preconnectApi('atus-oauth', { proxy: 'http://proxy.example.com:8080' });
      expect(mockFetch).not.toHaveBeenCalled();
    });
