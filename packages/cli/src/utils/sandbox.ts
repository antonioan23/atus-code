...
import type { Config, SandboxConfig } from '@atus-code/atus-code-core';
import {
  FatalSandboxError,
  Storage,
  isSubpath,
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'atus-code-sandbox';
const SANDBOX_NETWORK_NAME = 'atus-code-sandbox';
const SANDBOX_PROXY_NAME = 'atus-code-sandbox-proxy';
...
function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : process.env['DEBUG']
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which atus)`
        : 'atus';
...
export async function start_sandbox(
  ...
  if (config.command === 'sandbox-exec') {
    ...
    // on missing dirs and a custom ATUS_HOME / ATUS_RUNTIME_DIR may not exist
    // yet on first run.
    const atusDir = Storage.getGlobalQwenDir();
    const runtimeDir = Storage.getRuntimeBaseDir();
    fs.mkdirSync(atusDir, { recursive: true });
    ...
    const args = [
      ...
      '-D',
      `ATUS_DIR=${fs.realpathSync(atusDir)}`,
      '-D',
    ...
    // start and set up proxy if ATUS_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env['ATUS_SANDBOX_PROXY_COMMAND'];
  ...
  // determine full path for atus-code to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under atus-code repo
  //
  // note this can only be done with binary linked from atus-code repo
  if (process.env['BUILD_SANDBOX']) {
    if (!gcPath.includes('atus-code/packages/')) {
      throw new FatalSandboxError(
        'Cannot build sandbox using installed atus code binary; ' +
          'run `npm link ./packages/cli` under AtusCode-cli repo to switch to linked binary.',
      );
    } else {
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            ATUS_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the atus-code repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify atus-code-dev@service.alibaba.com if the issue persists.';
    throw new FatalSandboxError(
  ...
  // Mount user settings at /home/node/.atus-code and at the canonical host path
  // used by ATUS_HOME, unless that host path is already covered by a broader
  ...
  // Pass ATUS_HOME so the sandboxed CLI resolves the global atus dir to the
  // same path the host did, instead of relying on the /home/node/.atus-code mount
  // being the default fallback.
  args.push('--env', `ATUS_HOME=${userSettingsDirContainerPath}`);

  // Mount the runtime base dir and pass ATUS_RUNTIME_DIR when it diverges
  // from the global atus dir; otherwise the existing user-settings mount
  // already covers it.
  ...
  if (!runtimeSameAsUserSettings) {
    args.push('--env', `ATUS_RUNTIME_DIR=${runtimeBaseDirContainerPath}`);
  }
  ...
  // ATUS_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env['ATUS_SANDBOX_PROXY_COMMAND'];
  ...
  const isIntegrationTest =
    process.env['ATUS_CODE_INTEGRATION_TEST'] === 'true';
  let containerName;
  if (isIntegrationTest) {
    containerName = `atus-code-integration-test-${randomBytes(4).toString(
      'hex',
    )}`;
  ...
  // copy ATUS_CODE_TEST_VAR for integration tests
  if (process.env['ATUS_CODE_TEST_VAR']) {
    args.push(
      '--env',
      `ATUS_CODE_TEST_VAR=${process.env['ATUS_CODE_TEST_VAR']}`,
    );
  }
  ...
  // tests that need to access host's ~/.atus-code (e.g., --resume functionality)
  ...
  if (useCurrentUser) {
    ...
    // allowing tests that need to access host's ~/.atus-code (e.g., --resume) to work.
    ...
    // finally switch to that user to run the atus process. This is
