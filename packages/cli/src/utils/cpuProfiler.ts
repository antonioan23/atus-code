 ...
 *
 * Three trigger modes:
 * 1. Environment variable: ATUS_CODE_CPU_PROFILE=1 — records from process start to exit
 ...
 *
 * Output: ~/.atus-code/cpu-profiles/atus-code-cpu-<pid>-<timestamp>.cpuprofile
 ...
 * When ATUS_CODE_CPU_PROFILE=1, also starts recording immediately.
 */
export function initCpuProfiler(): void {
  ...
  const enabled = process.env['ATUS_CODE_CPU_PROFILE'] === '1';
  if (!enabled) return;
...
export async function stopCpuProfile(options?: {
  ...
  try {
    ...
    const filePath = path.join(
      outputDir,
      `atus-code-cpu-${process.pid}-${formatTimestamp(now)}.cpuprofile`,
    );
...
function defaultOutputDir(): string {
  return path.join(os.homedir(), '.atus-code', 'cpu-profiles');
}
...
function cleanupOldProfiles(outputDir: string, maxProfiles: number): void {
  ...
  try {
    profiles = fs
      ...
      .filter(
        (name) =>
          name.startsWith('atus-code-cpu-') && name.endsWith('.cpuprofile'),
      )
