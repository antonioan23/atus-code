...
describe('memoryDiagnostics', () => {
  ...
  it('surfaces high heap pressure with actionable recommendations', () => {
    ...
    expect(report).toContain('restart atus code to recover memory');
  ...
  it('writes heap snapshots to a diagnostics directory with stable filenames', () => {
    const outputDir = path.join(os.tmpdir(), 'atus-memory-diagnostics-test');
    ...
    expect(writtenPath).toBe(
      path.join(
        outputDir,
        `atus-code-heap-${process.pid}-2026-05-15T12-00-00-000Z.heapsnapshot`,
      ),
    );
  });
  ...
  it('refuses heap snapshots when estimated heap dump would leave little free disk', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-disk-${process.pid}`,
    );
  ...
  it('rate-limits repeated heap snapshot writes in the same directory', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-rate-limit-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it('keeps only the newest heap snapshots after writing', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-cleanup-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
    ...
    const oldSnapshot = path.join(
      outputDir,
      `atus-code-heap-${process.pid}-2026-05-15T11-00-00-000Z.heapsnapshot`,
    );
    const newerSnapshot = path.join(
      outputDir,
      `atus-code-heap-${process.pid}-2026-05-15T11-30-00-000Z.heapsnapshot`,
    );
  ...
  it('orders heap snapshot cleanup by modification time across process ids', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-mtime-cleanup-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
    fs.mkdirSync(outputDir, { recursive: true });
    ...
    const newerLowPidSnapshot = path.join(
      outputDir,
      'atus-code-heap-9-2026-05-15T12-30-00-000Z.heapsnapshot',
    );
    const olderHighPidSnapshot = path.join(
      outputDir,
      'atus-code-heap-12345-2026-05-15T12-00-00-000Z.heapsnapshot',
    );
  ...
  it('uses filename timestamps to break equal-mtime cleanup ties', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-equal-mtime-cleanup-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
    fs.mkdirSync(outputDir, { recursive: true });
    ...
    const olderSnapshot = path.join(
      outputDir,
      'atus-code-heap-1-2026-05-15T11-00-00-000Z.heapsnapshot',
    );
    const newerSnapshot = path.join(
      outputDir,
      'atus-code-heap-1-2026-05-15T12-00-00-000Z.heapsnapshot',
    );
  ...
  it('keeps successful heap snapshot writes when cleanup fails', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-cleanup-fail-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
    fs.mkdirSync(outputDir, { recursive: true });
    const brokenSymlink = path.join(
      outputDir,
      'atus-code-heap-999-2026-05-15T11-00-00-000Z.heapsnapshot',
    );
  ...
  it('removes partial heap snapshot files after a failed write', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-partial-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
    let partialPath = '';
  ...
  it('creates heap snapshot directories and files with private permissions', () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-private-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it('refuses heap snapshot writes when free disk space cannot be read', async () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-statfs-fallback-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
    vi.resetModules();
  ...
  it('falls back to process heap total when V8 heap statistics are unavailable for snapshot sizing', async () => {
    const outputDir = path.join(
      os.tmpdir(),
      `atus-memory-diagnostics-estimate-fallback-${process.pid}`,
    );
    fs.rmSync(outputDir, { recursive: true, force: true });
    vi.resetModules();
