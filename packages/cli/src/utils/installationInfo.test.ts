...
import { isGitRepository } from '@atus-code/atus-code-core';
...
describe('getInstallationInfo', () => {
  ...
  it('should detect standalone installs and avoid npm auto-update', () => {
    setPlatform('linux');
    const installDir = '/Users/test/.local/lib/atus-code';
  ...
  it('should detect Windows standalone installs and avoid npm auto-update', () => {
    setPlatform('win32');
    const installDir = 'C:/Users/test/AppData/Local/atus-code';
    const cliPath = `${installDir}/lib/cli.js`;
    process.argv[1] = cliPath;
    mockedRealPathSync.mockReturnValue(cliPath);
    mockedExistsSync.mockImplementation((candidate) =>
    ...
    mockedLstatSync.mockImplementation((candidate) => {
      if (
        [`${installDir}/bin/atus.cmd`, `${installDir}/node/node.exe`].includes(
  ...
  it('should ignore standalone-like installs with symlinked runtime files', () => {
    ...
    mockedLstatSync.mockImplementation((candidate) => {
      if (candidate === path.join(installDir, 'bin', 'atus')) {
        return symlinkStats();
      }
      return fileStats();
    });
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@atus-code/atus-code/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g @atus-code/atus-code@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@atus-code/atus-code/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(
      'yarn global add @atus-code/atus-code@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @atus-code/atus-code@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
