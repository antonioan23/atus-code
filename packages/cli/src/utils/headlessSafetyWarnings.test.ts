...
import { ApprovalMode, type Config } from '@atus-code/atus-code-core';
import {
...
describe('getHeadlessYoloSafetyWarning', () => {
  ...
  it('does not warn when SANDBOX env is set to the value the sandbox transport actually writes', () => {
    ...
    expect(
      getHeadlessYoloSafetyWarning(cfg, { SANDBOX: 'atus-code-sandbox' }),
    ).toBeNull();
  ...
  it('respects the explicit suppression env var when set to 1 or true', () => {
    ...
    expect(
      getHeadlessYoloSafetyWarning(cfg, {
        ATUS_CODE_SUPPRESS_YOLO_WARNING: 'true',
      }),
    ).toBeNull();
  });

  it('does NOT suppress when ATUS_CODE_SUPPRESS_YOLO_WARNING is 0 / false / empty', () => {
    const cfg = makeConfig(ApprovalMode.YOLO, undefined);
    for (const val of ['0', 'false', '', 'no']) {
      expect(
        getHeadlessYoloSafetyWarning(cfg, {
          ATUS_CODE_SUPPRESS_YOLO_WARNING: val,
        }),
