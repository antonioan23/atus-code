...
import { createDebugLogger } from '@atus-code/atus-code-core';
import { verifySignature } from './standalone-update-verify.js';

const debugLogger = createDebugLogger('STANDALONE_UPDATE');

const OSS_BASE =
  'https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/releases/atus-code';
const GITHUB_BASE = 'https://github.com/atuscode/atus-code/releases/download';
...
async function verifyChecksum(
  ...
  // key and publishes SHA256SUMS.sig, set ATUS_REQUIRE_SIGNATURE=1 to enforce.
  // Until then, verification is best-effort (passes when .sig exists, warns when not).
  const requireSig = process.env['ATUS_REQUIRE_SIGNATURE'] === '1';
  ...
  if (sigResponse) {
    ...
    throw new Error(
      'SHA256SUMS.sig not found and ATUS_REQUIRE_SIGNATURE=1 is set',
    );
  } else {
    debugLogger.warn(
      'SHA256SUMS.sig not available — update integrity relies on SHA256 checksum only. ' +
        'Set ATUS_REQUIRE_SIGNATURE=1 to enforce signature verification.',
    );
  }
...
function atomicReplace(
  ...
  if (os.platform() === 'win32') {
    ...
    const logFile = path.join(path.dirname(standaloneDir), 'atus-update.log');
    ...
    // 3. Log success/failure to atus-update.log for post-mortem (the bat
...
/**
 * Ensures ~/.local/bin/atus exists and points to the standalone install.
...
export function ensureBinWrapper(standaloneDir: string, target: string): void {
  ...
  try {
    ...
    if (target.startsWith('win')) {
      ...
      const wrapperPath = path.join(binDir, 'atus.cmd');
      if (!fs.existsSync(wrapperPath)) {
        const content = `@echo off\r\ncall "${standaloneDir}\\bin\\atus.cmd" %*\r\n`;
        fs.writeFileSync(wrapperPath, content);
      }
    } else {
      assertSafeForShellEmbed(standaloneDir, 'standaloneDir');
      const wrapperPath = path.join(binDir, 'atus');
      if (!fs.existsSync(wrapperPath)) {
        const content = `#!/bin/sh\nexec "${standaloneDir}/bin/atus" "$@"\n`;
 ...
 * Mirrors the logic in install-atus-standalone.sh maybe_update_shell_path.
 */
export function ensurePathInShellRc(binDir: string): void {
  ...
  if (shell.endsWith('/zsh')) {
    ...
    // Match install-atus-standalone.sh's maybe_update_shell_path logic.
  ...
  try {
    ...
    const marker = '# Added by atus code standalone installer';
...
export async function performStandaloneUpdate(
  ...
  if (fs.existsSync(manifestPath)) {
    ...
    throw new Error(
      `${standaloneDir} exists but is not a atus code standalone install. Remove it manually to proceed.`,
    );
  } else {
  ...
  const lockPath = path.join(parentDir, '.atus-code-update.lock');
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-code-update-'));
  let extractDir: string;
  let updateResult: 'done' | 'deferred' | undefined;
  try {
    extractDir = fs.mkdtempSync(path.join(parentDir, '.atus-code-update-'));
  } catch (err) {
  ...
  try {
    ...
    const newInstallDir = path.join(extractDir, 'atus-code');
    if (!fs.existsSync(path.join(newInstallDir, 'manifest.json'))) {
      throw new Error(
        'Extracted archive does not contain expected atus-code directory',
      );
    }
    ...
    if (fs.existsSync(oldDir)) {
      if (isFirstTimeMigration) {
        ...
        try {
          ...
          fs.writeFileSync(
            path.join(oldDir, '.atus-code-rollback-info.json'),
...
export function rollbackStandaloneUpdate(
  ...
  const lockPath = path.join(path.dirname(standaloneDir), '.atus-code-update.lock');
  try {
    const pidStr = fs.readFileSync(lockPath, 'utf-8').trim();
    const pid = parseInt(pidStr, 10);
    if (!Number.isNaN(pid) && isProcessAlive(pid)) {
