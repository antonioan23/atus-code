...
const ENV_KEYS = ['NODE_ENV', 'DEBUG', 'ATUS_DEBUG'] as const;
...
describe('initializeWarningHandler', () => {
  ...
  it('keeps warnings visible when ATUS_DEBUG is set', () => {
    process.env['ATUS_DEBUG'] = '1';
    initializeWarningHandler();
    emit(
      makeWarning(
        'MaxListenersExceededWarning',
  ...
  it('a child process with the handler installed does not print suppressed AbortSignal warnings to stderr', async () => {
    ...
    try {
      const script = `
        ...
        delete process.env.DEBUG; delete process.env.ATUS_DEBUG;
