 ...
 * For bug reports, removes 'atus-' or 'atus-code-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'atus-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false): string {
  ...
  // For bug reports, remove atus- prefix
  if (stripPrefix) {
    return sandbox.replace(/^atus-(?:code-)?/, '');
  }
