...
function defaultHeapSnapshotDir(): string {
  return path.join(os.homedir(), '.atus-code', 'memory-snapshots');
}
...
function cleanupOldHeapSnapshots(
  ...
  const snapshots = fs
    ...
    .filter(
      (name) =>
        name.startsWith('atus-code-heap-') && name.endsWith('.heapsnapshot'),
    )
...
function extractHeapSnapshotTimestamp(filePath: string): string {
  return (
    path
      .basename(filePath)
      .match(/^atus-code-heap-\d+-(.+)\.heapsnapshot$/)?.[1] ?? ''
  );
}
...
export function writeMemoryHeapSnapshot({
  ...
  const filePath = path.join(
    outputDir,
    `atus-code-heap-${process.pid}-${formatSnapshotTimestamp(now)}.heapsnapshot`,
  );
...
function buildMemoryInsights(diagnostics: MemoryDiagnostics): MemoryInsights {
  ...
  if (diagnostics.v8.unavailable) {
    ...
    recommendations.push(
      'Re-run /doctor memory after restarting atus code; if V8 diagnostics remain unavailable, include this report when filing an issue.',
    );
  }
  ...
  if (heapIsHigh) {
    ...
    recommendations.push(
      'If the CLI is sluggish or near OOM, restart atus code to recover memory, then capture a heap snapshot before the next restart to identify retained objects.',
    );
  }
