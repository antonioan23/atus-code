...
const DEFAULT_BASE_URLS: Record<string, string> = {
  ...
  'atus-oauth': 'https://coding.dashscope.aliyuncs.com',
 ...
 *
 * @param authType - Authentication type (openai, atus-oauth, anthropic, etc.)
...
export function preconnectApi(
  ...
  // Check if disabled
  if (process.env['ATUS_CODE_DISABLE_PRECONNECT'] === '1') {
  ...
  try {
    ...
    undiciFetch(targetUrl, {
      ...
      headers: {
        'User-Agent': 'AtusCode-Preconnect/1.0',
      },
      dispatcher,
    })
      .then(() => {
