...
describe('handleAutoUpdate', () => {
  ...
  it('should show manual update message when enableAutoUpdate is false', () => {
    ...
    mockGetInstallationInfo.mockReturnValue({
      updateCommand: 'npm i -g @atus-code/atus-code@latest',
      updateMessage:
        'Please run npm i -g @atus-code/atus-code@latest to update',
      isGlobal: true,
  ...
  it('should emit "update-failed" when the update process fails', async () => {
    ...
    expect(emitSpy).toHaveBeenCalledWith('update-failed', {
      message:
        'Automatic update failed. Please try updating manually. (command: npm i -g @atus-code/atus-code@2.0.0, stderr: An error occurred)',
    });
  });
  ...
  it('should use the "@nightly" tag for nightly updates', async () => {
    ...
    expect(mockSpawn).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([
        ...
        'npm i -g @atus-code/atus-code@nightly',
      ]),
      {
        stdio: 'pipe',
      },
...
describe('handleAutoUpdate — standalone path', () => {
  ...
  it('calls performStandaloneUpdate and does NOT spawn npm', async () => {
    mockGetInstallationInfo.mockReturnValue({
      ...
      isGlobal: false,
      isStandalone: true,
      standaloneDir: '/home/user/.local/lib/atus-code',
      packageManager: PackageManager.NPM,
    });
