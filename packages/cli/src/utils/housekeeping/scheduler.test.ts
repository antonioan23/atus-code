...
describe('_needsCatchUpForTesting', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-scheduler-test-'));
...
describe('_runHousekeepingForTesting', () => {
  ...
  beforeEach(() => {
    atusHome = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-scheduler-test-'));
    fileHistoryRoot = path.join(atusHome, FILE_HISTORY_DIR);
    vi.stubEnv('ATUS_HOME', atusHome);
  });

  afterEach(() => {
    vi.unstubAllEnvs();
    fs.rmSync(atusHome, { recursive: true, force: true });
  });
  ...
  it('whitelists the current session via lazy getSessionId()', async () => {
    ...
    expect(
      fs.existsSync(path.join(atusHome, _FILE_HISTORY_MARKER_FOR_TESTING)),
    ).toBe(true);
  });
  ...
  it('re-reads sessionId on every pass (defends against /clear)', async () => {
    ...
    fs.rmSync(path.join(atusHome, _FILE_HISTORY_MARKER_FOR_TESTING));
