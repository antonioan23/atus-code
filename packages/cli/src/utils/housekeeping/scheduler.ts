...
// atus for a while or every session has been < 10 min — either way we have
  ...
  await runThrottledOnce(
    {
      ...
      markerPath: join(atusDir, FILE_HISTORY_MARKER),
      lockPath: join(atusDir, FILE_HISTORY_MARKER + '.lock'),
    },
    async () => {
