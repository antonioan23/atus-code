...
function isDebugMode(): boolean {
  ...
  return truthy(process.env['DEBUG']) || truthy(process.env['ATUS_DEBUG']);
}
 ...
 * debug mode (NODE_ENV=development, or DEBUG / ATUS_DEBUG set), all
...
export function initializeWarningHandler(): void {
  ...
  installedHandler = (warning: Error) => {
    // Evaluate isDebugMode() per warning so DEBUG / ATUS_DEBUG can be
