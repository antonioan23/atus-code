...
import { createDebugLogger, isGitRepository } from '@atus-code/atus-code-core';
...
const STANDALONE_UNIX_INSTALLER =
  'https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-atus-standalone.sh';
const STANDALONE_WINDOWS_INSTALLER =
  'https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-atus-standalone.ps1';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^atus-code$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @atus-code/atus-code@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @atus-code/atus-code@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @atus-code/atus-code@latest';
      return {
    ...
    if (!npmPrefixWritable && isAutoUpdateEnabled) {
      ...
      // which installs to ~/.local/lib/atus-code/ (user-writable)
      ...
      if (!installRoot || installRoot === '/') {
        ...
        return {
          ...
          isGlobal: true,
          updateMessage:
            'Update requires sudo. Run: sudo npm install -g @atus-code/atus-code@latest',
        };
      }
    ...
    const updateCommand = 'npm install -g @atus-code/atus-code@latest';
    return {
      packageManager: PackageManager.NPM,
      isGlobal: true,
      updateCommand,
...
function isStandaloneInstallDir(installDir: string): boolean {
  try {
    ...
    if (
      manifest.name !== '@atus-code/atus-code' ||
    ...
    return (
      fs.existsSync(atusBin) &&
      fs.existsSync(nodeBin) &&
      isStandaloneRuntimeFile(atusBin) &&
