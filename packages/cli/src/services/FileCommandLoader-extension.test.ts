...
describe('FileCommandLoader - Extension Commands Support', () => {
  ...
  const projectCommandsDir = path.join(projectRoot, '.atus-code', 'commands');
  ...
  it('should load commands from extension with config.commands path', async () => {
    ...
    mock({
      ...
      [projectCommandsDir]: {},
      [extensionDir]: {
        'atus-extension.json': JSON.stringify(extensionConfig),
        'custom-cmds': {
          'test.md':
  ...
  it('should load commands from multiple extensions in alphabetical order', async () => {
    const ext1Dir = path.join(projectRoot, '.atus-code', 'extensions', 'ext-b');
    const ext2Dir = path.join(projectRoot, '.atus-code', 'extensions', 'ext-a');
