...
export const buildRemovalNotice = (): string =>
  [
    '',
    yellow(t('⚠  atus auth has been removed.')),
    '',
    `  ${cyan(t('Interactive'))}   →  ${t('run atus and use /auth to configure providers')}`,
    ...
    `  ${cyan(t('Qwen OAuth'))}    →  ${t('run atus interactively and use /auth; OAuth cannot be configured with env vars alone')}`,
    `  ${cyan(t('Scripted'))}      →  ${t('edit ~/.atus-code/settings.json, or run atus interactively once')}`,
    '',
...
export const authCommand: CommandModule = {
  command: 'auth',
  describe: t('Configure authentication (use API key or Atus Proxy)'),
  builder: (yargs: Argv) => {
