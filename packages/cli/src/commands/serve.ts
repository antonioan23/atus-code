...
// here would tax every `atus` invocation (interactive, mcp, channel, etc.)
// with ~50ms of cold ESM resolution. The runtime import is deferred to the
// handler below so it only loads when the user actually runs `atus serve`.
...
export const serveCommand: CommandModule<unknown, ServeArgs> = {
  command: 'serve',
  describe:
    'Run atus code as a local HTTP daemon (Stage 1 experimental: --http-bridge)',
  builder: (yargs: Argv) =>
    yargs
      .option('port', {
        type: 'number',
      ...
      .option('token', {
        type: 'string',
        description:
          'Bearer token required on every request. Falls back to the ATUS_SERVER_TOKEN env var.',
      })
      .option('max-sessions', {
        type: 'number',
        default: 20,
      ...
      .option('workspace', {
        ...
        description:
          ...
          'For multi-workspace deployments, run one `atus serve` per workspace ' +
      ...
      .option('require-auth', {
        ...
        description:
          ...
          '127.0.0.1. Requires --token or ATUS_SERVER_TOKEN. /health also ' +
      ...
      .option('http-bridge', {
        ...
        default: true,
        description:
          'Stage 1 mode: one `atus --acp` child per daemon (the daemon binds to ' +
  ...
  handler: async (argv) => {
    if (!argv['http-bridge']) {
      writeStderrLine(
        'atus serve: --no-http-bridge (native mode) is not yet implemented; ' +
    ...
    if (argv.token) {
      ...
      writeStderrLine(
        'atus serve: --token is visible in the process command line; ' +
          'prefer the ATUS_SERVER_TOKEN env var for any non-trivial ' +
          'deployment.',
      );
    }
    ...
    if (mcpClientBudget !== undefined) {
      if (
        ...
        writeStderrLine(
          'atus serve: --mcp-client-budget must be a positive integer.',
        );
        process.exit(1);
      }
    }
    if (mcpBudgetMode === 'enforce' && mcpClientBudget === undefined) {
      writeStderrLine(
        'atus serve: --mcp-budget-mode=enforce requires --mcp-client-budget=N.',
      );
      process.exit(1);
    }
    ...
    if (mcpClientBudget !== undefined) {
      ...
      writeStderrLine(
        `atus serve: --mcp-client-budget=${mcpClientBudget} mode=${resolvedMcpMode}` +
    ...
    // ATUS_CODE_SUPPRESS_YOLO_WARNING=1.
    try {
      ...
      const suppress = process.env['ATUS_CODE_SUPPRESS_YOLO_WARNING'];
    ...
    try {
      ...
      writeStderrLine(
        `atus serve: ${err instanceof Error ? err.message : String(err)}`,
      );
      process.exit(1);
    }
    await blockForever();
