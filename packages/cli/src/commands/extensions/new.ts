...
import { resolveBundleDir } from '@atus-code/atus-code-core';
...
async function handleNew(args: NewArgs) {
  try {
    if (args.template) {
      ...
      await writeFile(
        join(args.path, 'atus-extension.json'),
        JSON.stringify(manifest, null, 2),
      );
      writeStdoutLine(`Successfully created new extension at ${args.path}.`);
    }
    writeStdoutLine(
      `You can install this using "atus extensions link ${args.path}" to test it out.`,
    );
  } catch (error) {
