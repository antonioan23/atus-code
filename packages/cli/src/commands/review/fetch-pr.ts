...
// `atus review fetch-pr`: prepare a PR review's working state in a single
// deterministic pass.
//
//   1. Clean any stale worktree / branch from a previously interrupted run
//      so the new run starts fresh.
//   2. `git fetch <remote> pull/<n>/head:atus-review/pr-<n>` — pull the PR
...
//      `.atus-code/tmp/review-pr-<n>` so subsequent steps can run in isolation.
