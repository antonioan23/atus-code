...
// Centralised path constants and helpers for the `atus review` subcommands.
...
export const REVIEW_TMP_DIR = join('.atus-code', 'tmp');
export const REVIEWS_DIR = join('.atus-code', 'reviews');
export const REVIEW_CACHE_DIR = join('.atus-code', 'review-cache');
...
export function reviewBranch(prNumber: string | number): string {
  return `atus-review/pr-${prNumber}`;
}

/**
 * Per-target side-file path (review JSON, PR context, presubmit report).
 *
 * Files live under `.atus-code/tmp/` rather than the OS temp dir so the path is
