...
// CI / build status, existing atus code comment classification, and the
...
async function runPresubmit(args: PresubmitArgs): Promise<void> {
  ...
  const atusComments = allComments.filter((c) =>
    /via atus code \/review/.test(c.body ?? ''),
  );
