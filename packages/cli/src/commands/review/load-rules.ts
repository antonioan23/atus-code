...
// `atus review load-rules`: read project-specific code-review rules from
...
// PR cannot inject `.atus-code/review-rules.md` content that bypasses scrutiny.
// Sources, in order:
//
//   1. `.atus-code/review-rules.md`
...
function loadCombined(baseRef: string): {
  ...
  // 1. Qwen-native rules.
  const atusRules = showFile(baseRef, '.atus-code/review-rules.md');
  if (atusRules) {
    sections.push(`### From .atus-code/review-rules.md\n\n${atusRules.trim()}`);
    loaded.push('.atus-code/review-rules.md');
  }
  ...
  const atusMd = showFile(baseRef, 'QWEN.md');
  if (atusMd) {
    const section = extractCodeReviewSection(atusMd);
    if (section) {
...
export const loadRulesCommand: CommandModule = {
  ...
  describe:
    'Read project review rules from the base branch (.atus-code/review-rules.md, .github/copilot-instructions.md, AGENTS.md, QWEN.md) and write a combined Markdown file',
  builder: (yargs) =>
    yargs
      .positional('base_ref', {
        type: 'string',
