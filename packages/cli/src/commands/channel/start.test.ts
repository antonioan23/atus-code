...
vi.mock('@atus-code/channel-base', () => ({
...
const invokeStartHandler = async (
  ...
  await handler({ _: [], $0: 'atus', ...args } as StartCommandArgs);
};

const mockParsedChannelConfig = {
  cwd: '/tmp/atus-channel-test',
  model: 'atus-test-model',
  sessionScope: 'user',
  type: 'telegram',
};
...
beforeEach(() => {
  ...
  mockFindCliEntryPath.mockReturnValue('/tmp/atus-cli-entry.js');
