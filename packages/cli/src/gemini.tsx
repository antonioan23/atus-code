...
function getNodeMemoryArgs(isDebugMode: boolean): string[] {
  ...
  if (process.env['ATUS_CODE_NO_RELAUNCH']) {
    return [];
  }
...
export async function main() {
  ...
  // `isStartupProfilerEnabled()` so that when ATUS_CODE_PROFILE_STARTUP is
  ...
  if (process.argv.includes('--bare')) {
    process.env[ATUS_CODE_SIMPLE_ENV_VAR] = '1';
  }
  ...
  // to run atus code. It is now safe to perform expensive initialization that
...

  {
    ...
    if (config.isInteractive()) {
      ...
      // can't catch this case (`atus --json-schema '...'` on a TTY with
      ...
      if (failedMcpServers.length > 0) {
        writeStderrLine(
          `Warning: MCP server(s) failed to start: ${failedMcpServers.join(', ')}. ` +
            ...
            `Re-run with ATUS_CODE_DEBUG=1 to see per-server reasons.`,
        );
      }
