...
/**
 * Wire types for the `atus serve` daemon HTTP API.
 *
...
export interface DaemonCapabilities {
   ...
   * (see `atus-serve-protocol.md`) makes additive optionality the
...
export interface DaemonMcpBudgetStatusCell extends DaemonStatusCell {
   ...
   * scope values, drop don't fail." See `atus-serve-protocol.md`.
   */
 ...
 *
 * `project` / `user` / `builtin` are the levels the `atus serve`
...
export type DaemonAuthProviderId = 'atus-oauth' | (string & {});
