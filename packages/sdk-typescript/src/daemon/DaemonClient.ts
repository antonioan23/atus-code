...
/**
 * SDK-side HTTP client for the `atus serve` daemon. Sibling to
 * `ProcessTransport`: ProcessTransport drives a stdio child running
 * `atus --input-format stream-json`; DaemonClient hits the daemon's HTTP
...
export class DaemonClient {
   ...
   * workspace root or `~/.atus-code`.
   *
   * v1 discovers files at the bound workspace ROOT only, plus the
   * user's global `~/.atus-code` directory — it does NOT walk parent
