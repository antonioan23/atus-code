 ...
 * - WebSocket: tcp
 *
 * This interface aligns with MCPServerConfig in @atus-code/atus-code-core.
 */
...
export interface QueryOptions {
   ...
   * This takes precedence over the environment variables `OPENAI_MODEL` and `ATUS_MODEL`
   * @example 'atus-max', 'atus-plus', 'atus-turbo'
   */
  model?: string;
   ...
   * - Command name (no path separators): `'atus'` -> executes from PATH
   ...
   * - Native binary: `'/path/to/atus'` -> executes directly
   *
   * Runtime detection:
   ...
   * - `{ type: 'preset', preset: 'atus_code', append?: string }`:
   *   uses atus code's built-in prompt as the base and optionally appends extra
   ...
   * - 'atus-oauth': Use Qwen OAuth authentication
   *
   * Though we support 'atus-oauth', it's not recommended to use it in the SDK.
   * Because the credentials are stored in `~/.atus-code` and may need to refresh periodically.
   */
  authType?: AuthType;
