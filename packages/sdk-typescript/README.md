...
A minimum experimental TypeScript SDK for programmatic access to atus code.
...
```typescript
import { query } from '@atus-code/sdk';
...
Creates a new query session with the atus code.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'atus-max'`, `'atus-plus'`, `'atus-turbo'`). Takes precedence over `OPENAI_MODEL` and `ATUS_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                                 |
| `pathToQwenExecutable`   | `string`                                       | Auto-detected    | Path to the atus code executable. Supports multiple formats: `'atus'` (native binary from PATH), `'/path/to/atus'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `ATUS_CODE_CLI_PATH` env var, `~/.volta/bin/atus`, `~/.npm-global/bin/atus`, `/usr/local/bin/atus`, `~/.local/bin/atus`, `~/node_modules/.bin/atus`, `~/.yarn/bin/atus`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the atus code process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                  |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in atus code system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                  |
...
| `authType`               | `'openai' \| 'atus-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'atus-oauth'` in SDK is not recommended as credentials are stored in `~/.atus-code` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                          |
...
```typescript
const query = atus.query('Your prompt', {
  timeout: {
...
running `atus serve` daemon over HTTP + SSE. It binds one daemon session so TUI,
...
const daemon = new DaemonClient({
  ...
  token: process.env['ATUS_SERVER_TOKEN'],
});
...
```typescript
import { query, type SDKUserMessage } from '@atus-code/sdk';
...
```typescript
import { query, type CanUseTool } from '@atus-code/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@atus-code/sdk';
...
```typescript
import { query, isAbortError } from '@atus-code/sdk';
...
```typescript
import { AbortError, isAbortError } from '@atus-code/sdk';
...
#### atus code Installation Required

Version 0.1.0 requires [atus code](https://github.com/atuscode/atus-code) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install atus code globally
npm install -g atus-code@^0.4.0
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate atus code installation is needed.
