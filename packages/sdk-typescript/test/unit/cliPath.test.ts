...
describe('CLI Path Utilities', () => {
  ...
  describe('findBundledCliPath', () => {
    ...
    it('should throw descriptive error when bundled CLI not found', () => {
      ...
      expect(() => findBundledCliPath()).toThrow('Bundled atus CLI not found');
  ...
  describe('prepareSpawnInfo', () => {
    describe('auto-detection (no spec provided)', () => {
      ...
      it('should throw when bundled CLI not found', () => {
        ...
        expect(() => prepareSpawnInfo()).toThrow('Bundled atus CLI not found');
      });
    });

    describe('command name detection', () => {
      it('should detect command names without path separators', () => {
        const result = prepareSpawnInfo('atus');
      ...
      it('should detect command names on Windows', () => {
        const result = prepareSpawnInfo('atus.exe');

        expect(result).toEqual({
          command: 'atus.exe',
          args: [],
          type: 'native',
          originalInput: 'atus.exe',
        });
      });

      it('should reject invalid command name characters', () => {
        expect(() => prepareSpawnInfo('atus@invalid')).toThrow(
          "Invalid command name 'atus@invalid'. Command names should only contain letters, numbers, dots, hyphens, and underscores.",
        );
      });

      it('should accept valid command names', () => {
        expect(() => prepareSpawnInfo('atus')).not.toThrow();
        expect(() => prepareSpawnInfo('atus-code')).not.toThrow();
        expect(() => prepareSpawnInfo('atus_code')).not.toThrow();
        expect(() => prepareSpawnInfo('atus.exe')).not.toThrow();
    ...
    describe('native executables', () => {
      ...
      it('should prepare spawn info for native binary path', () => {
        const result = prepareSpawnInfo('/usr/local/bin/atus');

        expect(result).toEqual({
          command: path.resolve('/usr/local/bin/atus'),
          args: [],
          type: 'native',
          originalInput: '/usr/local/bin/atus',
        });
      });
    });
    ...
    describe('file path resolution', () => {
      it('should resolve absolute file paths', () => {
        ...
        const result = prepareSpawnInfo('/absolute/path/to/atus');

        expect(result.command).toBe(path.resolve('/absolute/path/to/atus'));
  ...
  describe('Windows path handling', () => {
    ...
    it('should handle Windows native executables', () => {
      const windowsPath = 'C:\\Program Files\\atus\\atus.exe';
      const result = prepareSpawnInfo(windowsPath);
