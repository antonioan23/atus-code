...
describe('daemon event schema', () => {
  ...
  it('preserves memory_changed snapshot when no agent_changed follows', () => {
    const state = reduceDaemonSessionEvent(createDaemonSessionViewState(), {
      ...
      data: {
        scope: 'global',
        filePath: '/home/.atus-code/QWEN.md',
        mode: 'replace',
        bytesWritten: 100,
      },
    });
...
describe('PR 21 — auth device-flow events', () => {
  ...
  it('rejects malformed device-flow data via type guards', () => {
    expect(
      asKnownDaemonEvent({
        ...
        data: {
          deviceFlowId: 'x',
          providerId: 'atus-oauth' /* missing expiresAt */,
        },
      }),
    ).toBeUndefined();
  ...
  it('reduceDaemonAuthEvent: started → throttled → authorized projects per-provider state', () => {
    ...
    const flow = state.flows['atus-oauth'];
  ...
  it('reduceDaemonAuthEvent: failed event always projects status:error + errorKind (aligned with daemon)', () => {
    ...
    expect(expired.flows['atus-oauth']?.status).toBe('error');
    expect(expired.flows['atus-oauth']?.errorKind).toBe('expired_token');
    ...
    expect(denied.flows['atus-oauth']?.status).toBe('error');
    expect(denied.flows['atus-oauth']?.errorKind).toBe('access_denied');
    ...
    expect(persistFailed.flows['atus-oauth']?.status).toBe('error');
    expect(persistFailed.flows['atus-oauth']?.errorKind).toBe('persist_failed');
  });
  ...
  it('reduceDaemonAuthEvent ignores stale events that do not match the current flow', () => {
    ...
    expect(stale.flows['atus-oauth']?.status).toBe('pending');
  });
  ...
  it('reduceDaemonAuthEvent rejects out-of-order frames (fold-in 8 #2 monotonicity)', () => {
    ...
    expect(state.flows['atus-oauth']?.status).toBe('authorized');
    expect(state.flows['atus-oauth']?.lastSeenEventId).toBe(10);
    ...
    expect(replayedStale.flows['atus-oauth']?.status).toBe('authorized');
    expect(replayedStale.flows['atus-oauth']?.lastSeenEventId).toBe(10);
    expect(replayedStale.flows['atus-oauth']?.errorKind).toBeUndefined();
    ...
    expect(replayedStartedStale.flows['atus-oauth']?.deviceFlowId).toBe(
      'flow-A',
    );
    expect(replayedStartedStale.flows['atus-oauth']?.status).toBe('authorized');
  });
  ...
  it('reduceDaemonAuthEvent passes synthetic frames (no envelope id) through the gate', () => {
    ...
    expect(state.flows['atus-oauth']?.status).toBe('cancelled');
  });
