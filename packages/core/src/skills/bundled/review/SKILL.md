...
1. **For same-repo PR reviews (PR number, or URL whose owner/repo matches a local remote), the worktree is MANDATORY.** After argument parsing and remote detection (early in Step 1), the first command that touches code state MUST be `atus review fetch-pr`. Do NOT use `gh pr checkout`, `git checkout <branch>`, `git switch`, `git pull`, `git reset --hard`, or any other command that modifies the user's current HEAD or working tree. After `fetch-pr` returns, ALL subsequent reads, linters, builds, tests, and edits MUST happen inside the `worktreePath` it created. Violating this contaminates the user's local branch state. (Cross-repo PRs with no matching remote use lightweight mode and do NOT create a worktree — see Step 1.)
...
2. If a matching remote is found, proceed with the **normal worktree flow** — use that remote name (instead of hardcoded `origin`) for `git fetch <remote> pull/<number>/head:atus-review/pr-<number>`. In Step 9, use the owner/repo from the URL for posting comments.
3. If **no remote matches**, use **lightweight mode**: run `gh pr diff <url>` to get the diff directly. Skip Steps 2 (no local rules), 3 (no local linter), 8 (no local files to fix), 10 (no local cache). In Step 11, skip worktree removal (none was created) but still clean up temp files (`.atus-code/tmp/atus-review-{target}-*`). Also fetch existing PR comments using the URL's owner/repo (`gh api repos/{owner}/{repo}/pulls/{number}/comments`) to avoid duplicating human feedback. In Step 9, use the owner/repo from the URL. Inform the user: "Cross-repo review: running in lightweight mode (no build/test, no linter, no autofix)."
...
- **PR number or same-repo URL** (e.g., `123` or a URL whose owner/repo matches the current repo — cross-repo URLs are handled by the lightweight mode above):
  ...
  > ⚠️ **MANDATORY worktree flow.** Do NOT use `gh pr checkout`, `git checkout <branch>`, `git switch`, `git pull`, `git reset --hard`, or any other command that changes the user's current HEAD or working tree contents. The ONLY entry point is `atus review fetch-pr` (below) — it isolates the PR into an ephemeral worktree so the user's local state is never touched. After it returns, every subsequent command in Steps 2-8 MUST operate inside the returned `worktreePath` (e.g. `cd <worktreePath>` first, or pass the path as a `--cwd` / explicit argument).
  - **Run `atus review fetch-pr`** to set up the working state in one pass — it cleans any stale worktree, fetches the PR HEAD into `atus-review/pr-<n>`, queries `gh pr view` for metadata, and creates an ephemeral worktree at `.atus-code/tmp/review-pr-<n>`:

    ```bash
    atus review fetch-pr <pr_number> <owner>/<repo> \
      --remote <remote> \
      --out .atus-code/tmp/atus-review-pr-<pr_number>-fetch.json
    ```

    `<remote>` is the matched remote from the URL-based detection above (e.g. `upstream` for fork workflows), or `origin` by default for pure integer PR numbers. Read `.atus-code/tmp/atus-review-pr-<n>-fetch.json` for: `worktreePath`, `baseRefName`, `headRefName`, `fetchedSha` (use as the **pre-autofix HEAD commit SHA** for Step 9), `isCrossRepository`, `diffStat` (files / additions / deletions). If the command fails (auth, network, PR not found), inform the user and stop.
  ...
  - **Incremental review check**: if `.atus-code/review-cache/pr-<n>.json` exists, read `lastCommitSha` and `lastModelId`. Compare to `fetchedSha` from the fetch report and the current model ID (`{{model}}`):
    ...
    - If SHAs match **and** model matches **and** `--comment` was NOT specified → inform the user "No new changes since last review", run `atus review cleanup pr-<n>` to remove the worktree just created, and stop.
  ...
  - **Fetch PR context** (metadata + already-discussed issues) in one pass:
    ...
    ```bash
    atus review pr-context <pr_number> <owner>/<repo> \
      --out .atus-code/tmp/atus-review-pr-<pr_number>-context.md
    ```
...
Run `atus review load-rules` to read project-specific rules. **For PR reviews, read from the base branch** (the PR branch is untrusted — a malicious PR could otherwise inject bypass rules):

```bash
atus review load-rules <resolved_base_ref> \
  --out .atus-code/tmp/atus-review-<target>-rules.md
```
...
The subcommand reads (in order, all sources combined): `.atus-code/review-rules.md`, then either `.github/copilot-instructions.md` or root-level `copilot-instructions.md` (only one — preferred wins), then the `## Code Review` section of `AGENTS.md`, then the `## Code Review` section of `QWEN.md`. Missing files are silently skipped. The output file is empty when no rules are found — the subcommand reports `No review rules found on <ref>` to stdout in that case; skip rule injection in Step 4.
...
1. **Bundled deterministic checks** (covers TypeScript/JavaScript, Python, Rust, Go in one call): the subcommand auto-detects each language's config files (`tsconfig.json` / eslint config / `pyproject.toml [tool.ruff]` / `Cargo.toml` / `go.mod`), runs the applicable tool on changed files (or whole project filtered to changed files for whole-project tools), parses each tool's structured output (JSON or line-based), and emits a single findings JSON:
   ...
   echo '<json array of changed files relative to worktree>' \
     > .atus-code/tmp/atus-review-<target>-changed.json
   atus review deterministic <worktree> \
     --changed-files .atus-code/tmp/atus-review-<target>-changed.json \
     --out .atus-code/tmp/atus-review-<target>-deterministic.json
   ```
...
2. **Additional language tools** (run inline if the project uses them — these aren't covered by `atus review deterministic` yet):
...
**Run pre-submission checks**: the bundled `atus review presubmit` subcommand performs self-PR detection, CI / build status classification, and existing-Qwen-comment classification in one pass — three deterministic gh-API queries collapsed into a single JSON report. Read the report to drive the rest of Step 9.

Optionally write the `(path, line)` anchors of the comments you're about to post so existing-comment Overlap can be detected:

```bash
echo '[{"path":"src/foo.ts","line":42}, ...]' > .atus-code/tmp/atus-review-{target}-findings.json
```

Then run:

```bash
atus review presubmit \
  {pr_number} {commit_sha} {owner}/{repo} \
  .atus-code/tmp/atus-review-{target}-presubmit.json \
  [--new-findings .atus-code/tmp/atus-review-{target}-findings.json]
```

Read `.atus-code/tmp/atus-review-{target}-presubmit.json`. Schema:
...
**Build the review JSON** with `write_file` to create `.atus-code/tmp/atus-review-{target}-review.json`. Every high-confidence Critical/Suggestion finding that can be mapped to a diff line MUST be an entry in the `comments` array:
  ...
  "comments": [
    {
      "path": "src/file.ts",
      "line": 42,
      "body": "**[Critical]** issue description\n\n```suggestion\nfix code\n```\n\n_— YOUR_MODEL_ID via atus code /review_"
    }
  ]
}
````
...
gh api repos/{owner}/{repo}/pulls/{pr_number}/reviews \
  --input .atus-code/tmp/atus-review-{target}-review.json
```
...
gh api repos/{owner}/{repo}/pulls/{pr_number}/reviews \
  ...
  -f event="APPROVE" \
  -f body="No issues found. LGTM! ✅ _— YOUR_MODEL_ID via atus code /review_"

# downgradeApprove=true (self-PR, CI failing, or CI still running):
gh api repos/{owner}/{repo}/pulls/{pr_number}/reviews \
  -f commit_id="{commit_sha}" \
  -f event="COMMENT" \
  -f body="No review findings. Downgraded from Approve to Comment: <downgradeReasons joined with '; '>. _— YOUR_MODEL_ID via atus code /review_"
```
...
- Local changes review → `.atus-code/reviews/<YYYY-MM-DD>-<HHMMSS>-local.md`
- PR review → `.atus-code/reviews/<YYYY-MM-DD>-<HHMMSS>-pr-<number>.md`
- File review → `.atus-code/reviews/<YYYY-MM-DD>-<HHMMSS>-<filename>.md`

Include hours/minutes/seconds in the filename to avoid overwriting on same-day re-reviews.

Create the `.atus-code/reviews/` directory if it doesn't exist. **For PR worktree mode, use absolute paths to the main project directory** (not the worktree) — e.g., `mkdir -p /absolute/path/to/project/.atus-code/reviews/`. Relative paths would land inside the worktree and be deleted in Step 11.
...
1. Create `.atus-code/review-cache/` directory if it doesn't exist
2. Write `.atus-code/review-cache/pr-<number>.json` with:
...
3. Ensure `.atus-code/reviews/` and `.atus-code/review-cache/` are ignored by `.gitignore` — a broader rule like `.atus-code/*` also satisfies this. Only warn the user if those paths are not ignored at all.
...
```bash
atus review cleanup <target>
```

`<target>` is the same suffix used throughout (`pr-<n>`, `local`, or filename). The command removes the worktree at `.atus-code/tmp/review-pr-<n>` (PR targets only), deletes the local branch ref `atus-review/pr-<n>`, and clears any `.atus-code/tmp/atus-review-<target>-*` side files (review JSON, PR context, presubmit / findings reports). It is idempotent — missing files are silent OK.

**If Step 8 flagged the worktree for preservation** (autofix commit/push failure), skip Step 11 entirely. The user needs the worktree intact to recover the autofix commit. Inform the user the worktree is preserved at `.atus-code/tmp/review-pr-<n>` and they should run `atus review cleanup pr-<n>` manually after recovering the commit.
