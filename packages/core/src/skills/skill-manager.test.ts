...
describe('SkillManager', () => {
  ...
  beforeEach(() => {
    ...
    mockParseYaml.mockImplementation((yamlString: string) => {
      ...
      if (yamlString.includes('name: shared-skill')) {
        const desc = yamlString.includes('From atus dir')
          ? 'From atus dir'
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.atus-code/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.atus-code/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.atus-code/skills/test-skill/SKILL.md';
...
`;
  ...
  describe('listSkills', () => {
    beforeEach(() => {
      ...
      const userQwenSkillsDir = path.join('/home/user', '.atus-code', 'skills');
...
Body`);
    ...
    it('should deduplicate same-name skills across provider dirs within a level', async () => {
      // Override readdir to return the same skill name from both .atus-code and .agents dirs
      vi.mocked(fs.readdir).mockReset();
      const projectQwenDir = path.join('/test/project', '.atus-code', 'skills');
      ...
      vi.mocked(fs.readFile).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.includes('.atus-code') && pathStr.includes('shared-skill')) {
          return Promise.resolve(
            `---\nname: shared-skill\ndescription: From atus dir\n---\nQwen content`,
          );
        }
      ...
      // Only one instance should remain, from .atus-code (first in PROVIDER_CONFIG_DIRS)
      expect(skills).toHaveLength(1);
      expect(skills[0].name).toBe('shared-skill');
      expect(skills[0].description).toBe('From atus dir');
    });
  ...
  describe('getSkillsBaseDirs', () => {
    it('should return all project-level base dirs', () => {
      ...
      expect(baseDirs).toContain(path.join('/test/project', '.atus-code', 'skills'));
    ...
    it('should return all user-level base dirs', () => {
      ...
      expect(baseDirs).toHaveLength(2);
      expect(baseDirs).toContain(path.join('/home/user', '.atus-code', 'skills'));
  ...
  describe('bundled skills', () => {
    ...
    const projectDirSegment = path.join('.atus-code', 'skills');
    const userDirSegment = path.join('.atus-code', 'skills');
...
Symlinked skill content`);
  ...
  describe('file watchers', () => {
    it('should pass ignored function and shallow depth to chokidar', async () => {
      const projectSkillsDir = path.join('/test/project', '.atus-code', 'skills');
