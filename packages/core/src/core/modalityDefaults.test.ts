...
describe('defaultModalities', () => {
  ...
  describe('Qwen', () => {
    it('returns image + video for atus-vl-max', () => {
      const m = defaultModalities('atus-vl-max');
      expect(m.image).toBe(true);
    ...
    it('returns text-only for atus-turbo', () => {
      expect(defaultModalities('atus-turbo')).toEqual({});
    });
  });
