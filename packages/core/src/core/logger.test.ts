...
const GEMINI_DIR_NAME = '.atus-code';
...
const TEST_HOME_DIR = path.join(os.tmpdir(), 'atus-core-logger-home');
...
describe('Logger', () => {
  ...
  describe('removeLastUserMessage', () => {
    ...
    it('only undoes USER entries (model_switch is left intact)', async () => {
      ...
      vi.advanceTimersByTime(1000);
      await logger.logMessage(MessageSenderType.MODEL_SWITCH, 'atus→atus-max');
      ...
      expect(removed).toBe(true);
      const onDisk = await readLogFile();
      expect(onDisk.map((e) => e.message)).toEqual(['atus→atus-max']);
    });
