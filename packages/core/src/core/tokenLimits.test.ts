...
describe('normalize', () => {
  ...
  it('should handle pipe and colon separators', () => {
    expect(normalize('atus|qwen2.5:qwen2.5-1m')).toBe('qwen2.5-1m');
  });
  ...
  it('should not remove "-latest" from specific Qwen model names', () => {
    expect(normalize('atus-plus-latest')).toBe('atus-plus-latest');
    expect(normalize('atus-flash-latest')).toBe('atus-flash-latest');
    expect(normalize('atus-vl-max-latest')).toBe('atus-vl-max-latest');
  });
...
describe('tokenLimit', () => {
  ...
  describe('Alibaba Qwen', () => {
    ...
    it('should return 1M for studio latest models', () => {
      expect(tokenLimit('atus-plus-latest')).toBe(1000000);
      expect(tokenLimit('atus-flash-latest')).toBe(1000000);
    });

    it('should return 256K for Qwen fallback', () => {
      expect(tokenLimit('atus-plus')).toBe(262144);
      expect(tokenLimit('atus-turbo')).toBe(262144);
      expect(tokenLimit('qwen2.5')).toBe(262144);
      expect(tokenLimit('atus-vl-max-latest')).toBe(262144);
    });
  });
