 ...
 * https://github.com/atuscode/atus-code/issues/3786
 */
 ...
 * `x-api-key` auth and a truthful `AtusCode` User-Agent (avoids identity
...
export class AnthropicContentGenerator implements ContentGenerator {
  ...
  private buildHeaders(useProxyIdentity: boolean): Record<string, string> {
    ...
    // truthful AtusCode User-Agent so usage isn't misattributed to Claude
    ...
    const userAgent = useProxyIdentity
      ...
      : `AtusCode/${version} (${process.platform}; ${process.arch})`;
   ...
   * `enableCacheControl` in-place on the atus-oauth path (without
   * recreating the ContentGenerator); non-atus-oauth providers refresh
  ...
  private async buildRequest(
    ...
    // on the atus-oauth path, where `Config.handleModelChange()`
    // hot-updates `enableCacheControl` in place without recreating the
    // ContentGenerator. (Non-atus-oauth providers refresh via generator
  ...
  private buildSamplingParameters(request: GenerateContentParameters): {
    ...
    if (userMaxTokens !== undefined && userMaxTokens !== null) {
      ...
      const envVal = process.env['ATUS_CODE_MAX_OUTPUT_TOKENS'];
