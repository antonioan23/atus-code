...
export enum AuthType {
  USE_OPENAI = 'openai',
  ATUS_OAUTH = 'atus-oauth', // deprecated: Qwen OAuth removed in Atus Code; kept for type compat
  USE_GEMINI = 'gemini',
...
export type ContentGeneratorConfig = {
  ...
  // providers). See atuscode/atus-code#3616.
...
export async function createContentGenerator(
  ...
  } else if (authType === AuthType.ATUS_OAUTH) {
    // Qwen OAuth has been removed in Atus Code. Use API-key based auth
    // (USE_OPENAI with a DashScope-compatible base URL, or one of the
    // providers configured under modelProviders).
    throw new Error(
      'Qwen OAuth is no longer supported in Atus Code. ' +
        'Set authType to USE_OPENAI with a base URL and API key, ' +
        'or use the OpenAI-compatible DashScope provider.',
    );
