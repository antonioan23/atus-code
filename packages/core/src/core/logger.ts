...
export class Logger {
  private atusDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.atusDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.atusDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.atusDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.atusDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.atusDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.atusDir) {
      this.debugLogger.error(
