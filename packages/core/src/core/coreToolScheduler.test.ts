  ...
  it('applies canonical legacy tool names to the deny-list fallback', async () => {
    ...
    if (completedCall.status === 'error') {
      ...
      expect(completedCall.response.error?.message).toBe(
        'Atus Code requires permission to use edit, but that permission was declined.',
      );
    }
    ...
    it('should return permission error for excluded tools instead of "not found" message', async () => {
      ...
      if (completedCall.status === 'error') {
        ...
        expect(errorMessage).toBe(
          'Atus Code requires permission to use write_file, but that permission was declined.',
        );
...
describe('CoreToolScheduler telemetry spans', () => {
  ...
  it('ModifyWithEditor !editorType stamps modify_with_editor_unavailable on tool span (#4321)', async () => {
    ...
    expect(
      toolSpan?.spanAttributes['atus-code.tool.modify_with_editor_unavailable'],
    ).toBe(true);
...
describe('Fire hook functions integration', () => {
  ...
  describe('Concurrent tool execution', () => {
    ...
    const origEnv = process.env['ATUS_CODE_MAX_TOOL_CONCURRENCY'];
    beforeEach(() => {
      delete process.env['ATUS_CODE_MAX_TOOL_CONCURRENCY'];
    });
    afterEach(() => {
      if (origEnv !== undefined) {
        process.env['ATUS_CODE_MAX_TOOL_CONCURRENCY'] = origEnv;
      } else {
        delete process.env['ATUS_CODE_MAX_TOOL_CONCURRENCY'];
      }
    });
