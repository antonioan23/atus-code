...
const debugLogger = createDebugLogger('ATUS_CODE_CHAT');
 ...
 * note above `ORPHAN_TOOL_USE_REPAIR_REASON`. atus-code analogue of
...
const SESSION_START_CONTEXT_SENTINEL_END = '</atus:session-start-context>';
    ...
    return (async function* () {
      try {
        ...
        const hasUserMaxTokensOverride =
          ...
          !!process.env['ATUS_CODE_MAX_OUTPUT_TOKENS'];
    ...
    const streamResponse = await retryWithBackoff(apiCall, {
      ...
      heartbeatFn: (info) => {
        process.stderr.write(
          `[atus-code] Waiting for API capacity... attempt ${info.attempt}, retry in ${Math.ceil(info.remainingMs / 1000)}s\n`,
        );
      },
      onRetry: (info) => {
        logApiRetry(
