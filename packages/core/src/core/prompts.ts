...
import { ATUS_DIR } from '../config/storage.js';
...
export function getCoreSystemPrompt(
  ...
  // if ATUS_SYSTEM_MD is set (and not 0|false), override system prompt from file
  // default path is .atus-code/system.md (project-level), can be overridden via ATUS_SYSTEM_MD
  let systemMdEnabled = false;
  let systemMdPath = path.resolve(path.join(ATUS_DIR, 'system.md'));
  // Resolve the environment variable to get either a path or a switch value.
  const systemMdResolution = resolvePathFromEnv(process.env['ATUS_SYSTEM_MD']);
  ...
  const basePrompt = systemMdEnabled
    ...
    : `
You are atus code, an interactive CLI agent developed by Alibaba Group, specializing in software engineering tasks. Your primary goal is to help users safely and efficiently, adhering strictly to the following instructions and utilizing your available tools.
...
`.trim();
  ...
  // if ATUS_WRITE_SYSTEM_MD is set (and not 0|false), write base system prompt to file
  const writeSystemMdResolution = resolvePathFromEnv(
    process.env['ATUS_WRITE_SYSTEM_MD'],
  );
...
const atusCoderToolCallExamples = `
# Examples (Illustrating Tone and Workflow)
<example>
user: 1 + 2
model: 3
...
function getToolCallExamples(model?: string): string {
  ...
  const toolCallStyle = process.env['ATUS_CODE_TOOL_CALL_STYLE'];
  if (toolCallStyle) {
    switch (toolCallStyle.toLowerCase()) {
      case 'atus-coder':
        return atusCoderToolCallExamples;
      case 'atus-vl':
        return atusVlToolCallExamples;
      case 'general':
        return generalToolCallExamples;
      default:
        debugLogger.warn(
          `Unknown ATUS_CODE_TOOL_CALL_STYLE value: ${toolCallStyle}. Using model-based detection.`,
        );
        break;
    }
  }
  ...
  if (model && model.length < 100) {
    // Match atus*-coder patterns (e.g., qwen3-coder, qwen2.5-coder, atus-coder)
    if (/atus[^-]*-coder/i.test(model)) {
      return atusCoderToolCallExamples;
    }
    // Match atus*-vl patterns (e.g., atus-vl, qwen2-vl, qwen3-vl)
    if (/atus[^-]*-vl/i.test(model)) {
      return atusVlToolCallExamples;
    }
...
const INSIGHT_PROMPTS: Record<InsightPromptType, string> = {
  analysis: `Analyze this atus code session and extract structured facets.
...
4. If very short or just warmup, use warmup_minimal for goal_category`,
  ...
  impressive_workflows: `Analyze this atus code usage data and identify what's working well for this user. Use second person ("you").
...
Include 3 impressive workflows.`,
  ...
  project_areas: `Analyze this atus code usage data and identify project areas.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "areas": [
    {"name": "Area name", "session_count": N, "description": "2-3 sentences about what was worked on and how atus code was used."}
  ]
}

Include 4-5 areas. Skip internal QC operations.`,

  future_opportunities: `Analyze this atus code usage data and identify future opportunities.
...
Include 3 opportunities. Think BIG - autonomous workflows, parallel agents, iterating against tests.`,
  ...
  friction_points: `Analyze this atus code usage data and identify friction points for this user. Use second person ("you").
...
Include 3 friction categories with 2 examples each.`,
  ...
  memorable_moment: `Analyze this atus code usage data and find a memorable moment.
...
Find something genuinely interesting or amusing from the session summaries.`,
  ...
  improvements: `Analyze this atus code usage data and suggest improvements.
...
1. **MCP Servers**: Connect Qwen to external tools, databases, and APIs via Model Context Protocol.
   - How to use: Run \`atus mcp add --transport http <server-name> <http-url>\`
   - Good for: database queries, Slack integration, GitHub issue lookup, connecting to internal APIs
   - Example: "To connect to GitHub, run \`atus mcp add --header "Authorization: Bearer your_github_mcp_pat" --transport http github https://api.githubcopilot.com/mcp/\` and set the AUTHORIZATION header with your PAT. Then you can ask Qwen to query issues, PRs, or repos."

2. **Custom Skills**: Reusable prompts you define as markdown files that run with a single /command.
   - How to use: Create \`.atus-code/skills/commit/SKILL.md\` with instructions. Then type \`/commit\` to run it.
...
3. **Headless Mode**: Run Qwen non-interactively from scripts and CI/CD.
   - How to use: \`atus -p "fix lint errors"\`
...
IMPORTANT for Atus_md_additions: PRIORITIZE instructions that appear MULTIPLE TIMES in the user data. If user told Qwen the same thing in 2+ sessions (e.g., 'always run tests', 'use TypeScript'), that's a PRIME candidate - they shouldn't have to repeat themselves.

IMPORTANT for features_to_try: Pick 2-3 from the QC FEATURES REFERENCE above. Include 2-3 items for each category.`,

  interaction_style: `Analyze this atus code usage data and describe the user's interaction style.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "narrative": "2-3 paragraphs analyzing HOW the user interacts with atus code. Use second person 'you'. Describe patterns: iterate quickly vs detailed upfront specs? Interrupt often or let Qwen run? Include specific examples. Use **bold** for key insights.",
  "key_pattern": "One sentence summary of most distinctive interaction style"
}
`,

  at_a_glance: `You're writing an "At a Glance" summary for a atus code usage insights report for atus code users. The goal is to help them understand their usage and improve how they can use Qwen better, especially as models improve.
...
3. **Quick wins to try** - Specific atus code features they could try from the examples below, or a workflow technique if you think it's really compelling. (Avoid stuff like "Ask Qwen to confirm before taking actions" or "Type out more context up front" which are less compelling.)
