...
describe('ContentGenerationPipeline', () => {
  ...
  describe('execute', () => {
    ...
    it('should override enable_thinking when thinkingConfig disables it', async () => {
      ...
      // DashScope hostname + atus model name are both required: the gate
      // is hostname + model-name to avoid leaking the atus-specific
      // `enable_thinking` field to non-atus routings (off-DashScope, or
    ...
    it('emits enable_thinking:false on ATUS_OAUTH with the default coder-model', async () => {
      // ATUS_OAUTH is the default auth flow for first-time users and
      // ships with `model: 'coder-model'` (DEFAULT_QWEN_MODEL in
      // config/models.ts — aliased to Qwen 3.6 Plus hybrid). The string
      // doesn't start with `atus`, so the gate must special-case it;
    ...
    it('does NOT emit enable_thinking on a non-DashScope hostname', async () => {
      // `enable_thinking` is a atus-specific extension. Pushing it at a
    ...
    it('does NOT emit enable_thinking on a non-atus model routed through DashScope', async () => {
    ...
    it('gates on the wire model, not config: atus config + non-atus request.model does NOT emit', async () => {
      // buildRequest ships `context.model` (= request.model || config.model).
      // A atus *config* with a non-atus *request* model must gate on the
      // request model — otherwise the atus-only field leaks to the non-atus
      ...
      const request: GenerateContentParameters = {
        model: 'glm-5', // request-level override to a non-atus wire model
        contents: [{ parts: [{ text: 'Summarize' }], role: 'user' }],
        config: { thinkingConfig: { includeThoughts: false } },
      };
    ...
    it('gates on the wire model, not config: non-atus config + atus request.model emits false', async () => {
      // The mirror direction: a non-atus *config* with a atus *request* model
      // must still emit the disable signal, since the wire model is atus and
      ...
      const request: GenerateContentParameters = {
        model: 'qwen3.5-flash', // request-level override to a atus wire model
        contents: [{ parts: [{ text: 'Summarize' }], role: 'user' }],
        config: { thinkingConfig: { includeThoughts: false } },
      };
