...
export class ContentGenerationPipeline {
  ...
  private async buildRequest(
    ...
    if (reasoningDisabled) {
      ...
      // Hostname + model-name gate avoids leaking this atus-specific field
      // to non-atus routings on the same DashScope hostname (GLM uses
      ...
      // what actually ships: a atus config with a non-atus request model
      // would leak the field, and a non-atus config with a atus request
      // model would miss the disable signal (the #4501 regression).
      //
      // `coder-model` is the ATUS_OAUTH default (DEFAULT_QWEN_MODEL in
      // config/models.ts, aliased to Qwen 3.6 Plus hybrid) — it doesn't
      // start with `atus` but is the most common hybrid-thinking model
      ...
      if (
        ...
        (model.startsWith('atus') || model === 'coder-model')
      ) {
