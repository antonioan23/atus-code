...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    it('should return true for ATUS_OAUTH auth type', () => {
      const config = {
        authType: AuthType.ATUS_OAUTH,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        'User-Agent': `AtusCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-CacheControl': 'enable',
        'X-DashScope-UserAgent': `AtusCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.ATUS_OAUTH,
      });
    });
    ...
    it('should merge custom headers with DashScope defaults', () => {
      ...
      expect(headers['User-Agent']).toContain('AtusCode/1.0.0');
      expect(headers['X-DashScope-UserAgent']).toContain('AtusCode/1.0.0');
      expect(headers['X-DashScope-AuthType']).toBe(AuthType.ATUS_OAUTH);
    ...
    it('should handle unknown CLI version', () => {
      ...
      expect(headers['User-Agent']).toBe(
        `AtusCode/unknown (${process.platform}; ${process.arch})`,
      );
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('atus-max');
