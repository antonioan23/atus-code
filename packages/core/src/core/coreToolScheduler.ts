...
export class CoreToolScheduler {
      ...
      for (const reqInfo of requestsToProcess) {
        ...
        if (pm && !(await pm.isToolEnabled(canonicalName))) {
          ...
          const ruleInfo = matchingRule
            ...
            : '';
          const permissionErrorMessage = `Atus Code requires permission to use "${reqInfo.name}", but that permission was declined.${ruleInfo}`;
          newToolCalls.push({
            status: 'error',
            request: reqInfo,
            response: createErrorResponse(
        ...
        if (!pm) {
          ...
          if (excludeTools && excludeTools.length > 0) {
            ...
            if (excludedMatch) {
              const permissionErrorMessage = `Atus Code requires permission to use ${excludedMatch}, but that permission was declined.`;
              newToolCalls.push({
                status: 'error',
                request: reqInfo,
                response: createErrorResponse(
        ...
        try {
          ...
          if (
            ...
            if (isNonInteractiveDeny) {
              const errorMessage = `Atus Code requires permission to use "${reqInfo.name}", but that permission was declined (non-interactive mode cannot prompt for confirmation).`;
              this.setStatusInternal(
                reqInfo.callId,
                'error',
                createErrorResponse(
            ...
            if (hooksEnabled && messageBus) {
              fireNotificationHook(
                messageBus,
                `Atus Code needs your permission to use ${reqInfo.name}`,
          ...
          if (toolSpan) {
            try {
              toolSpan.setAttributes({
                'atus-code.tool.modify_with_editor_unavailable': true,
              });
            } catch {
  ...
  private async runConcurrently(
    ...
    const parsed = parseInt(
      process.env['ATUS_CODE_MAX_TOOL_CONCURRENCY'] || '',
      10,
    );
