...
describe('createContentGeneratorConfig', () => {
  ...
  it('should preserve provided fields and set authType for ATUS_OAUTH', () => {
    const cfg = createContentGeneratorConfig(mockConfig, AuthType.ATUS_OAUTH, {
      model: 'coder-model',
      apiKey: 'ATUS_OAUTH_DYNAMIC_TOKEN',
    });
    expect(cfg.authType).toBe(AuthType.ATUS_OAUTH);
    expect(cfg.model).toBe('coder-model');
    expect(cfg.apiKey).toBe('ATUS_OAUTH_DYNAMIC_TOKEN');
  });

  it('should not warn or fallback for ATUS_OAUTH (resolution handled by ModelConfigResolver)', () => {
    const warnSpy = vi
      .spyOn(console, 'warn')
