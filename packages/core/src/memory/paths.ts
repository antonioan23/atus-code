...
import { ATUS_DIR, sanitizeCwd } from '../utils/paths.js';
 ...
 * Defaults to the global atus dir (`~/.atus-code` or `$ATUS_HOME`);
 * overridable via ATUS_CODE_MEMORY_BASE_DIR for tests.
 */
export function getMemoryBaseDir(): string {
  if (process.env['ATUS_CODE_MEMORY_BASE_DIR']) {
    return process.env['ATUS_CODE_MEMORY_BASE_DIR'];
  }
...
export function getAutoMemoryRoot(projectRoot: string): string {
  ...
  let result: string;
  if (process.env['ATUS_CODE_MEMORY_LOCAL'] === '1') {
    result = path.join(projectRoot, ATUS_DIR, AUTO_MEMORY_DIRNAME);
  } else {
    const canonicalRoot =
