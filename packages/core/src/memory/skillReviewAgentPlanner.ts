...
async function evaluateScopedDecision(
  ...
  switch (ctx.toolName) {
    ...
    case ToolNames.WRITE_FILE: {
      ...
      // anywhere under `.atus-code/skills/**` — SkillManager would ignore them
 ...
 * the agent uses when picking `.atus-code/skills/<name>/SKILL.md`).
 *
...
export async function buildTaskPrompt(projectRoot: string): Promise<string> {
  ...
  return [
    ...
    "Each skill lives at .atus-code/skills/<name>/SKILL.md. Skills you create MUST include 'source: auto-skill' in the frontmatter:",
    '',
    '---',
    'name: <skill-name>',
