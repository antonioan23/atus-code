...
/**
 * Maximum output length (10,000 characters as per atus code spec)
 */
...
export class HttpHookRunner {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        // Per atus code spec: Non-2xx status is a non-blocking error
        ...
        if (
          ...
          // Timeout or abort is a non-blocking error per atus code spec
          debugLogger.warn(
        ...
        // Connection failure is a non-blocking error per atus code spec
        debugLogger.warn(
   ...
   * Per atus code spec: output is capped at 10,000 characters
   */
  ...
  private normalizeOutput(
    ...
    if ('systemMessage' in json && typeof json['systemMessage'] === 'string') {
      // Apply output length limit per atus code spec
