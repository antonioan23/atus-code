...
export const EXTENSIONS_DIRECTORY_NAME = path.join(ATUS_DIR, 'extensions');
export const EXTENSIONS_CONFIG_FILENAME = 'atus-extension.json';
export const INSTALL_METADATA_FILENAME = '.atus-extension-install.json';
  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".atus-code" in markdown files
  ...
  try {
    ...
    for (const file of scriptFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".atus-code" in shell scripts
        // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
        // Avoid matching URLs, comments, or string literals containing .claude
        ...
        if (finalScriptContent !== content) {
          ...
          debugLogger.debug(
            `Updated transcript format and replaced .claude with .atus-code in shell script: ${filePath}`,
          );
        }
      } catch (error) {
        debugLogger.warn(
