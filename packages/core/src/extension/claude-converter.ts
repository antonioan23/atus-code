...
/**
 * Converter for Claude Code plugins to atus code format.
 */
...
export function convertClaudeAgentConfig(
  ...
  if (claudeAgent.permissionMode) {
    ...
    atusAgent['approvalMode'] = mapped;
  }
  if (claudeAgent.hooks) {
    atusAgent['hooks'] = claudeAgent.hooks;
  }
  if (claudeAgent.skills && claudeAgent.skills.length > 0) {
    atusAgent['skills'] = claudeAgent.skills;
  }
  if (claudeAgent.disallowedTools && claudeAgent.disallowedTools.length > 0) {
    atusAgent['disallowedTools'] = claudeAgent.disallowedTools;
  }
...
async function convertAgentFiles(agentsDir: string): Promise<void> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const atusAgent = convertClaudeAgentConfig(claudeAgent);

      // Build new frontmatter (excluding systemPrompt as it goes in body)
      const newFrontmatter: Record<string, unknown> = {};
      for (const [key, value] of Object.entries(atusAgent)) {
      ...
      const systemPrompt = (atusAgent['systemPrompt'] as string) || body.trim();
      const newContent = `---
${newYaml}
---
...
/**
 * Converts a Claude plugin config to atus code format.
...
/**
 * Converts a complete Claude plugin package to atus code format.
 * Creates a new temporary directory with:
 * 1. Converted atus-extension.json
...
export async function convertClaudePluginPackage(
  ...
  try {
    ...
    const atusConfig = convertClaudeToQwenConfig(mergedConfig);

    // Step 11: Write atus-extension.json
    const atusConfigPath = path.join(tmpDir, 'atus-extension.json');
    fs.writeFileSync(
      atusConfigPath,
      JSON.stringify(atusConfig, null, 2),
      'utf-8',
    );
