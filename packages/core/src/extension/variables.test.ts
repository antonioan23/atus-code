...
describe('substituteHookVariables', () => {
  ...
  it('should handle multiple event types with hooks', () => {
    const basePath = '/home/user/.atus-code/extensions/my-extension';
    ...
    expect(
      (result!['PreToolUse']![0].hooks![0] as { command: string }).command,
    ).toBe('/home/user/.atus-code/extensions/my-extension/scripts/pre-start.sh');
    expect(result!['UserPromptSubmit']).toHaveLength(1);
    expect(
      (result!['UserPromptSubmit']![0].hooks![0] as { command: string })
        .command,
    ).toBe('/home/user/.atus-code/extensions/my-extension/setup/install.py');
  });
...
describe('performVariableReplacement', () => {
  ...
  it('should replace .claude with .atus-code in markdown files', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('.atus-code/loop.local.md');
    expect(result).toContain('rm .atus-code/loop.local.md');
    expect(result).toContain('$HOME/.atus-code/cache');
    expect(result).toContain('./.atus-code/local');
  ...
  it('should replace .claude with .atus-code in shell scripts', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('~/.atus-code/cache');
    expect(result).toContain('./.atus-code/local');
    expect(result).toContain('.atus-code/config');
    // Note: URLs are also being replaced in current implementation
    expect(result).toContain('https://example.com/.atus-code/page');
  });
