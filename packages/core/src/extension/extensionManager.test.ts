...
const EXTENSIONS_DIRECTORY_NAME = path.join(ATUS_DIR, 'extensions');
...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'atus-code-test-home-'),
    );
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(tempHomeDir, 'atus-code-test-workspace-'),
    );
  ...
  describe('hooks loading and processing', () => {
    it('should load hooks from atus-extension.json', async () => {
      const extensionDir = path.join(userExtensionsDir, 'hooks-extension');
      fs.mkdirSync(extensionDir, { recursive: true });

      // Create atus-extension.json with hooks
    ...
    it('should load hooks from hooks/hooks.json when not in main config', async () => {
      ...
      // Create atus-extension.json without hooks
    ...
    it('should substitute ${CLAUDE_PLUGIN_ROOT} variable in hooks', async () => {
      ...
      // Create atus-extension.json with hooks using ${CLAUDE_PLUGIN_ROOT}
      const configWithHooks = {
    ...
    it('should load hooks from config.hooks string path', async () => {
      ...
      // Create atus-extension.json with hooks as string path
