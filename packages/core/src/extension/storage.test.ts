...
describe('ExtensionStorage', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(Storage).mockImplementation(
      () =>
        ({
          getExtensionsDir: () => path.join(mockHomeDir, '.atus-code', 'extensions'),
        }) as any, // eslint-disable-line @typescript-eslint/no-explicit-any
    );
    vi.mocked(Storage.getUserExtensionsDir).mockReturnValue(
      path.join(mockHomeDir, '.atus-code', 'extensions'),
    );
  ...
  it('should return the correct user extensions directory', () => {
    const expectedDir = path.join(mockHomeDir, '.atus-code', 'extensions');
    expect(ExtensionStorage.getUserExtensionsDir()).toBe(expectedDir);
  });

  it('should create a temporary directory', async () => {
    const mockTmpDir = '/tmp/atus-extension-123';
