...
describe('Telemetry SDK', () => {
  ...
  it('should route OpenTelemetry diagnostics to debug log instead of console output', async () => {
    ...
    const previousDebugLogFileEnv = process.env['ATUS_DEBUG_LOG_FILE'];
    try {
      process.env['ATUS_DEBUG_LOG_FILE'] = '1';
      ...
      if (previousDebugLogFileEnv === undefined) {
        delete process.env['ATUS_DEBUG_LOG_FILE'];
      } else {
        process.env['ATUS_DEBUG_LOG_FILE'] = previousDebugLogFileEnv;
      }
    }
  });
  ...
  describe('Resource attributes', () => {
    ...
    it('always sets service.name and service.version from runtime', () => {
      ...
      expect(attrs['service.name']).toBe('atus-code');
    ...
    it('user-provided service.name wins over default', () => {
      vi.spyOn(mockConfig, 'getTelemetryResourceAttributes').mockReturnValue({
        'service.name': 'atus-code-ci',
      });
      initializeTelemetry(mockConfig);
      expect(getResourceAttributes()['service.name']).toBe('atus-code-ci');
    });
    ...
    it('empty-string service.name from settings falls back to default', () => {
      ...
      initializeTelemetry(mockConfig);
      expect(getResourceAttributes()['service.name']).toBe('atus-code');
    });
