...
/**
 * ALS for the active `atus-code.subagent` span. Child LLM/tool/hook spans
...
function ttlFor(ctx: SpanContext): number {
  if (ctx.type === 'subagent') {
    const kind = ctx.attributes['atus-code.subagent.invocation_kind'];
    if (
      typeof kind === 'string' &&
    ...
    if (!ctx.ended) {
      ...
      try {
        ctx.span.setAttributes({
          ...
          'atus-code.span.duration_ms': ageMs,
          ...
          ...(ctx.type === 'subagent'
            ? {
                ...
                // only get the generic atus-code.span.duration_ms
                // above). wenshao @ #4410.
                'atus-code.subagent.duration_ms': ageMs,
              }
            : {}),
        });
      } catch (error) {
...
export function startInteractionSpan(
  ...
  const attributes: Attributes = {
    ...
    'atus-code.prompt_id': options.promptId,
    'atus-code.message_type': options.messageType,
    'atus-code.model': options.model,
    'atus-code.approval_mode': config.getApprovalMode(),
...
export function startLLMRequestSpan(model: string, promptId: string): Span {
  ...
  const attributes: Attributes = {
    'atus-code.model': model,
    'atus-code.prompt_id': promptId,
    ...
    // (atus-code.model) remains authoritative; gen_ai.* is a compat layer
 ...
 * `gen_ai.conversation.id`) alongside vendor `atus-code.subagent.*` keys.
...
export function startSubagentSpan(opts: StartSubagentSpanOptions): Span {
  ...
  const attributes: Attributes = {
    ...
    // Vendor (atus-code-specific). Dual-emit id/name so dashboards already
    // querying spec keys still work.
    'atus-code.subagent.id': opts.agentId,
    'atus-code.subagent.name': opts.subagentName,
    'atus-code.subagent.invocation_kind': opts.invocationKind,
    'atus-code.subagent.is_built_in': opts.isBuiltIn,
    'atus-code.subagent.depth': opts.depth,
  };
  ...
  if (opts.parentAgentId !== undefined) {
    attributes['atus-code.subagent.parent_agent_id'] = opts.parentAgentId;
  }
  if (opts.invokingRequestId !== undefined) {
    attributes['atus-code.subagent.invoking_request_id'] =
...
export function endSubagentSpan(
  ...
  try {
    ...
    const endAttributes: Attributes = {
      ...
      'atus-code.subagent.duration_ms': duration,
      'atus-code.subagent.status': metadata.status,
    };
    if (metadata.terminateReason !== undefined) {
      endAttributes['atus-code.subagent.terminate_reason'] =
        metadata.terminateReason;
    }
    if (metadata.resultSummaryPresent !== undefined) {
      endAttributes['atus-code.subagent.result_summary_present'] =
