...
import { AtusLogger } from './atus-logger/atus-logger.js';
...
export function logStartSession(
  ...
  AtusLogger.getInstance(config)?.logStartSessionEvent(event);
...
export function logUserPrompt(config: Config, event: UserPromptEvent): void {
  AtusLogger.getInstance(config)?.logNewPromptEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserRetry(config: Config, event: UserRetryEvent): void {
  AtusLogger.getInstance(config)?.logRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolCall(config: Config, event: ToolCallEvent): void {
  ...
  AtusLogger.getInstance(config)?.logToolCallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolOutputTruncated(
  ...
  AtusLogger.getInstance(config)?.logToolOutputTruncatedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFileOperation(
  ...
  AtusLogger.getInstance(config)?.logFileOperationEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiRequest(config: Config, event: ApiRequestEvent): void {
  // AtusLogger.getInstance(config)?.logApiRequestEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFlashFallback(
  ...
  AtusLogger.getInstance(config)?.logFlashFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logRipgrepFallback(
  ...
  AtusLogger.getInstance(config)?.logRipgrepFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiError(config: Config, event: ApiErrorEvent): void {
  ...
  AtusLogger.getInstance(config)?.logApiErrorEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiCancel(config: Config, event: ApiCancelEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  AtusLogger.getInstance(config)?.logApiCancelEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiResponse(config: Config, event: ApiResponseEvent): void {
  ...
  AtusLogger.getInstance(config)?.logApiResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logLoopDetected(
  ...
  AtusLogger.getInstance(config)?.logLoopDetectedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logLoopDetectionDisabled(
  ...
  AtusLogger.getInstance(config)?.logLoopDetectionDisabledEvent();
}
...
export function logNextSpeakerCheck(
  ...
  AtusLogger.getInstance(config)?.logNextSpeakerCheck(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSlashCommand(
  ...
  AtusLogger.getInstance(config)?.logSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logIdeConnection(
  ...
  AtusLogger.getInstance(config)?.logIdeConnectionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logConversationFinishedEvent(
  ...
  AtusLogger.getInstance(config)?.logConversationFinishedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logChatCompression(
  ...
  AtusLogger.getInstance(config)?.logChatCompressionEvent(event);
...
export function logKittySequenceOverflow(
  ...
  AtusLogger.getInstance(config)?.logKittySequenceOverflowEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logMalformedJsonResponse(
  ...
  AtusLogger.getInstance(config)?.logMalformedJsonResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logInvalidChunk(
  ...
  AtusLogger.getInstance(config)?.logInvalidChunkEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetry(
  ...
  AtusLogger.getInstance(config)?.logContentRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetryFailure(
  ...
  AtusLogger.getInstance(config)?.logContentRetryFailureEvent(event);
  if (!isTelemetrySdkInitialized()) return;
 ...
 *   1. AtusLogger RUM ingestion (Aliyun internal stats)
...
export function logApiRetry(config: Config, event: ApiRetryEvent): void {
  AtusLogger.getInstance(config)?.logApiRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSubagentExecution(
  ...
  AtusLogger.getInstance(config)?.logSubagentExecutionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logModelSlashCommand(
  ...
  AtusLogger.getInstance(config)?.logModelSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logHookCall(config: Config, event: HookCallEvent): void {
  // Log to AtusLogger for RUM telemetry only
  AtusLogger.getInstance(config)?.logHookCallEvent(event);
}
...
export function logExtensionInstallEvent(
  ...
  AtusLogger.getInstance(config)?.logExtensionInstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionUninstall(
  ...
  AtusLogger.getInstance(config)?.logExtensionUninstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export async function logExtensionUpdateEvent(
  ...
  AtusLogger.getInstance(config)?.logExtensionUpdateEvent(event);
...
export function logExtensionEnable(
  ...
  AtusLogger.getInstance(config)?.logExtensionEnableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionDisable(
  ...
  AtusLogger.getInstance(config)?.logExtensionDisableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logAuth(config: Config, event: AuthEvent): void {
  AtusLogger.getInstance(config)?.logAuthEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSkillLaunch(config: Config, event: SkillLaunchEvent): void {
  AtusLogger.getInstance(config)?.logSkillLaunchEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserFeedback(
  ...
  uiTelemetryService.addEvent(uiEvent);
  config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
  AtusLogger.getInstance(config)?.logUserFeedbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionStarted(
  ...
  AtusLogger.getInstance(config)?.logArenaSessionStartedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaAgentCompleted(
  ...
  AtusLogger.getInstance(config)?.logArenaAgentCompletedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionEnded(
  ...
  AtusLogger.getInstance(config)?.logArenaSessionEndedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
