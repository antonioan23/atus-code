...
describe('LogToSpanProcessor', () => {
  ...
  describe('bridge skip-list (#3731 Phase 3)', () => {
    it('skips atus-code.subagent_execution when native subagent span is active', async () => {
      mockIsInNativeSubagentSpan = true;
      const logRecord = {
        body: 'subagent started',
        hrTime: [2000, 0] as [number, number],
        attributes: {
          'event.name': 'atus-code.subagent_execution',
    ...
    it('bridges subagent_execution when no native span is active (e.g. runForkedAgent)', async () => {
      ...
      expect(exportedSpans).toHaveLength(1);
      expect(exportedSpans[0].name).toBe('atus-code.subagent_execution');
    });

    it('still bridges other events normally (e.g. atus-code.tool_call)', async () => {
      const logRecord = {
        body: 'tool call',
        hrTime: [3000, 0] as [number, number],
        attributes: {
          'event.name': 'atus-code.tool_call',
          tool_name: 'read_file',
        },
      } as unknown as ReadableLogRecord;
      ...
      expect(exportedSpans).toHaveLength(1);
      expect(exportedSpans[0].name).toBe('atus-code.tool_call');
    });
  });
