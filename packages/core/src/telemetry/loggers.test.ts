...
describe('loggers', () => {
  ...
  describe('logChatCompression', () => {
    beforeEach(() => {
      ...
      vi.spyOn(AtusLogger.prototype, 'logChatCompressionEvent');
    });

    it('logs the chat compression event to AtusLogger', () => {
      ...
      expect(AtusLogger.prototype.logChatCompressionEvent).toHaveBeenCalledWith(
        event,
      );
    });
  ...
  describe('logRipgrepFallback', () => {
    ...
    beforeEach(() => {
      vi.spyOn(AtusLogger.prototype, 'logRipgrepFallbackEvent');
    });
    ...
    it('should log ripgrep fallback event', () => {
      ...
      expect(AtusLogger.prototype.logRipgrepFallbackEvent).toHaveBeenCalled();
  ...
  describe('logSkillLaunch', () => {
    ...
    beforeEach(() => {
      vi.spyOn(AtusLogger.prototype, 'logSkillLaunchEvent');
    });

    it('forwards the event to AtusLogger and emits an OTLP record', () => {
      const event = new SkillLaunchEvent('test-skill', true, 'prompt-id-42');

      logSkillLaunch(mockConfig, event);

      expect(AtusLogger.prototype.logSkillLaunchEvent).toHaveBeenCalledWith(
        event,
      );
    ...
    it('forwards to AtusLogger even when OTLP SDK is not initialized', () => {
  ...
  describe('logMalformedJsonResponse', () => {
    beforeEach(() => {
      vi.spyOn(AtusLogger.prototype, 'logMalformedJsonResponseEvent');
    });
    ...
    it('logs the event to Clearcut and OTEL', () => {
      ...
      expect(
        AtusLogger.prototype.logMalformedJsonResponseEvent,
  ...
  describe('logExtensionInstall', () => {
    ...
    beforeEach(() => {
      vi.spyOn(AtusLogger.prototype, 'logExtensionInstallEvent');
    });
    ...
    it('should log extension install event', () => {
      ...
      expect(
        AtusLogger.prototype.logExtensionInstallEvent,
      ).toHaveBeenCalledWith(event);
  ...
  describe('logExtensionUninstall', () => {
    ...
    beforeEach(() => {
      vi.spyOn(AtusLogger.prototype, 'logExtensionUninstallEvent');
    });
    ...
    it('should log extension uninstall event', () => {
      ...
      expect(
        AtusLogger.prototype.logExtensionUninstallEvent,
      ).toHaveBeenCalledWith(event);
  ...
  describe('logExtensionEnable', () => {
    ...
    beforeEach(() => {
      vi.spyOn(AtusLogger.prototype, 'logExtensionEnableEvent');
    });
    ...
    it('should log extension enable event', () => {
      ...
      expect(AtusLogger.prototype.logExtensionEnableEvent).toHaveBeenCalledWith(
        event,
      );
  ...
  describe('logExtensionDisable', () => {
    ...
    beforeEach(() => {
      vi.spyOn(AtusLogger.prototype, 'logExtensionDisableEvent');
    });
    ...
    it('should log extension disable event', () => {
      ...
      expect(
        AtusLogger.prototype.logExtensionDisableEvent,
      ).toHaveBeenCalledWith(event);
  ...
  describe('logHookCall', () => {
    ...
    beforeEach(() => {
      vi.spyOn(AtusLogger, 'getInstance').mockReturnValue(
        mockQwenLogger as unknown as AtusLogger,
      );
      mockQwenLogger.logHookCallEvent.mockClear();
    });

    it('should log a successful hook call to AtusLogger', () => {
    ...
    it('should handle when AtusLogger is not available', () => {
      vi.spyOn(AtusLogger, 'getInstance').mockReturnValue(undefined);
      ...
      // Should not throw when AtusLogger is not available
    ...
    it('should pass the exact event object to AtusLogger', () => {
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        'test-hook.sh',
  ...
  describe('logApiRetry (Phase 4b)', () => {
    ...
    it('fans out to all 3 sinks: AtusLogger, OTel log, and metric counter', () => {
      const mockConfig = makeFakeConfig({ sessionId: 'test-session-id' });
      ...
      expect(logRecord.attributes['event.name']).toBe('atus-code.api_retry');
    ...
    it('skips logger.emit and metric counter when SDK is not initialized (AtusLogger still called)', () => {
      vi.spyOn(sdk, 'isTelemetrySdkInitialized').mockReturnValue(false);
      const mockConfig = makeFakeConfig({ sessionId: 'test-session-id' });
      const event = buildEvent();
      logApiRetry(mockConfig, event);
