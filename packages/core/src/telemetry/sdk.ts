...
export function initializeTelemetry(config: Config): void {
  ...
  if (attrWarnings.length > 0) {
    ...
    console.warn(
      `[atus-code telemetry] ${attrWarnings.length} resource attribute issue(s):`,
    );
      ...
      if (logsUrl) {
        ...
        logToSpanProcessor = new LogToSpanProcessor(
          new OTLPTraceExporterHttp({ url: tracesUrl }),
          {
            ...
            // disabled file logging via ATUS_DEBUG_LOG_FILE=0, debugLogger.warn
  ...
  sdk = new NodeSDK({
    ...
    instrumentations: [
      ...
      // and the trace tree terminates at the atus-code process boundary.
