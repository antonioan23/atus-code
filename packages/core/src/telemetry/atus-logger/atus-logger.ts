...
export class AtusLogger {
  private static instance: AtusLogger;
  ...
  private constructor(config: Config) {
    this.config = config;
    this.debugLogger = createDebugLogger('ATUS_LOGGER');
  ...
  static getInstance(config?: Config): AtusLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!AtusLogger.instance) {
      AtusLogger.instance = new AtusLogger(config);
    }

    return AtusLogger.instance;
  }
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `AtusLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      this.debugLogger.error('AtusLogger: Failed to enqueue log event.', error);
    }
  }
  ...
  readSourceInfo(): string {
    try {
      ...
      // Also check legacy ~/.atus-code/source.json when ATUS_HOME is set,
      // since the installer writes to ~/.atus-code/ regardless of the env var.
      const legacyPath = path.join(os.homedir(), '.atus-code', 'source.json');
      const candidates =
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        'AtusLogger: Flush already in progress, marking pending flush.',
      );
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `AtusLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
