...
import { AtusLogger, TEST_ONLY } from './atus-logger.js';
...
const makeFakeConfig = (overrides: Partial<Config> = {}): Config => {
  const defaults = {
    ...
    getAuthType: () => AuthType.ATUS_OAUTH,
    getMcpServers: () => ({}),
    getModel: () => 'test-model',
...
describe('AtusLogger', () => {
  ...
  beforeEach(() => {
    ...
    (AtusLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = AtusLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = AtusLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(AtusLogger);
    });

    it('is a singleton', () => {
      const logger1 = AtusLogger.getInstance(mockConfig);
      const logger2 = AtusLogger.getInstance(mockConfig);
  ...
  describe('logHookCallEvent', () => {
    ...
    it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
      ...
      const logger = AtusLogger.getInstance(configWithLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should not include error when telemetry log prompts disabled', () => {
      ...
      (AtusLogger as unknown as { instance: undefined }).instance = undefined;
      const logger = AtusLogger.getInstance(configWithoutLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should sanitize hook name to remove sensitive information', () => {
      ...
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        '/home/user/.atus-code/hooks/check-secrets.sh --api-key=secret123',
        { tool_name: 'read_file' },
        100,
        true,
      );
