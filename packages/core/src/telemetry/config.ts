...
export async function resolveTelemetrySettings(options: {
  ...
  const enabled =
    argv.telemetry ??
    parseBooleanEnvFlag(env['ATUS_TELEMETRY_ENABLED']) ??
    settings.enabled;

  const rawTarget =
    (argv.telemetryTarget as string | TelemetryTarget | undefined) ??
    env['ATUS_TELEMETRY_TARGET'] ??
  ...
  const otlpEndpoint =
    ...
    env['ATUS_TELEMETRY_OTLP_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_ENDPOINT'] ??
    settings.otlpEndpoint;

  const rawProtocol =
    (argv.telemetryOtlpProtocol as string | undefined) ??
    env['ATUS_TELEMETRY_OTLP_PROTOCOL'] ??
  ...
  const logPrompts =
    ...
    parseBooleanEnvFlag(env['ATUS_TELEMETRY_LOG_PROMPTS']) ??
    settings.logPrompts;

  const includeSensitiveSpanAttributes =
    parseBooleanEnvFlag(
      env['ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES'],
    ) ??
    settings.includeSensitiveSpanAttributes ??
    false;

  const outfile =
    argv.telemetryOutfile ?? env['ATUS_TELEMETRY_OUTFILE'] ?? settings.outfile;

  // Per-signal endpoint overrides (HTTP only).
  // Priority: ATUS_ env var > standard OTEL_ env var > settings.json
  const otlpTracesEndpoint =
    env['ATUS_TELEMETRY_OTLP_TRACES_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_TRACES_ENDPOINT'] ??
    settings.otlpTracesEndpoint;

  const otlpLogsEndpoint =
    env['ATUS_TELEMETRY_OTLP_LOGS_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_LOGS_ENDPOINT'] ??
    settings.otlpLogsEndpoint;

  const otlpMetricsEndpoint =
    env['ATUS_TELEMETRY_OTLP_METRICS_ENDPOINT'] ??
