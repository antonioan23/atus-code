...
import { AtusIgnoreParser } from './atusIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('AtusIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'atusignore-test-'));
  });
  ...
  describe('when .atusignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .atusignore', () => {
      const parser = new AtusIgnoreParser(projectRoot);
  ...
  describe('when .atusignore does not exist', () => {
