    ...
    if (isHomeDirectory) {
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (
          ...
          currentDir === path.join(resolvedHome, ATUS_DIR)
        ) {
          break;
        }
 ...
 * Also loads path-based context rules from `.atus-code/rules/` directories.
...
export async function loadServerHierarchicalMemory(
  ...
  // `<projectRoot>/.atus-code/QWEN.local.md`. Loaded after all hierarchical
  ...
  //   * `cwd === homedir` resolves the slot path to `~/.atus-code/QWEN.local.md`,
  ...
  // Load path-based context rules from .atus-code/rules/ directories.
  const {
