 ...
 * Requires the user to opt in via ATUS_CODE_UNATTENDED_RETRY — we intentionally
...
export function isUnattendedMode(): boolean {
  const val = process.env['ATUS_CODE_UNATTENDED_RETRY'];
...
export async function retryWithBackoff<T>(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      if (authType === AuthType.ATUS_OAUTH && isQwenQuotaExceededError(error)) {
        throw new Error(
          `Qwen OAuth free tier has been discontinued as of 2026-04-15.\n\n` +
            `To continue using atus code, try one of these alternatives:\n` +
