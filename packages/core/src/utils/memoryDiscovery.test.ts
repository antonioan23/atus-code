...
describe('loadServerHierarchicalMemory', () => {
  ...
  describe('when untrusted', () => {
    ...
    it('loads context from outside the untrusted workspace', async () => {
      ...
      const filepath = path.join(homedir, ATUS_DIR, DEFAULT_CONTEXT_FILENAME);
  ...
  it('should skip implicit global, project, and rule discovery in explicit-only mode', async () => {
    await createTestFile(
      path.join(homedir, ATUS_DIR, DEFAULT_CONTEXT_FILENAME),
      'global context',
    );
    await createTestFile(
      path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
    ...
    await createTestFile(
      path.join(projectRoot, ATUS_DIR, 'rules', 'baseline.md'),
      'project rule',
    );
  ...
  it('should load only the global custom context file if present and filename is changed', async () => {
    ...
    const customContextFile = await createTestFile(
      path.join(homedir, ATUS_DIR, customFilename),
  ...
  describe('QWEN.local.md (project-local context file)', () => {
    // The local-context-file slot is anchored at `<projectRoot>/.atus-code/`, where
    ...
    it('loads .atus-code/QWEN.local.md from project root when present', async () => {
      const localFile = await createTestFile(
        path.join(projectRoot, ATUS_DIR, 'QWEN.local.md'),
    ...
    it('silently ignores absent .atus-code/QWEN.local.md', async () => {
      await createTestFile(
        path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
        'project content',
      );
    ...
    it('does not search .atus-code/QWEN.local.md in CWD subdirectories', async () => {
      // A `.atus-code/QWEN.local.md` placed inside a nested directory (not the
      // project root) must NOT be picked up — the slot is single, fixed,
      // and lives at <projectRoot>/.atus-code/QWEN.local.md.
      await createTestFile(
        path.join(cwd, ATUS_DIR, 'QWEN.local.md'),
    ...
    it('skips QWEN.local.md when cwd === homedir without .git (avoids global-dir collision)', async () => {
      ...
      // would-be slot path resolves to `<homedir>/.atus-code/QWEN.local.md` —
      ...
      await createTestFile(
        path.join(homedir, ATUS_DIR, 'QWEN.local.md'),
      ...
      // Allowed: global QWEN.md / AGENTS.md in ~/.atus-code/ may still load via
