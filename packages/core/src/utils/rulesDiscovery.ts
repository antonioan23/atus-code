...
//
// Discovers .atus-code/rules/ files (recursively) with optional YAML frontmatter.
...
/**
 * Discover and load rule files from a single `.atus-code/rules/` directory.
...
export function formatRules(rules: RuleFile[], projectRoot: string): string {
  return rules
    .map((rule) => {
      ...
      // display paths should match — otherwise Windows shows `.atus-code\rules\foo.md`
      // and Linux shows `.atus-code/rules/foo.md`, which is confusing in diffs/tests.
...
/**
 * Load rules from both global (`~/.atus-code/rules/`) and project-level
 * (`.atus-code/rules/`) directories.
 *
...
export async function loadRules(
  ...
  // 1. Global rules: <ATUS_HOME or ~/.atus-code>/rules/
  ...
  // 2. Project-level rules: <projectRoot>/.atus-code/rules/  (trusted only)
  //    Skip if it resolves to the same directory as global rules.
  if (folderTrust) {
    const projectRulesDir = path.join(projectRoot, ATUS_DIR, 'rules');
