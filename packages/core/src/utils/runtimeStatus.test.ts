...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'atus-runtime-status-'));
});
...
describe('writeRuntimeStatus', () => {
  it('writes the expected fields', async () => {
    ...
    expect(data.atus_version).toBe('0.15.3');
  });

  it('defaults pid to process.pid and atus_version to null', async () => {
    ...
    expect(data.atus_version).toBeNull();
  });
...
describe('readRuntimeStatus', () => {
  it('round-trips a written record', async () => {
    ...
    expect(status!.atusVersion).toBe('0.15.3');
  });
