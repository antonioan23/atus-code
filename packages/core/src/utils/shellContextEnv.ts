...
export function getShellContextEnvVars(): Record<string, string> {
  ...
  const sessionId = process.env['ATUS_CODE_SESSION_ID'];
  if (sessionId) {
    env['ATUS_CODE_SESSION_ID'] = sessionId;
  }

  // For agent/prompt IDs: explicitly set empty string when no ALS context
  // exists, so that stale values inherited from a parent atus-code process
  // (via process.env spread) are overwritten rather than leaked.
  const agentId = getCurrentAgentId();
  env['ATUS_CODE_AGENT_ID'] = agentId ?? '';

  const promptId = promptIdContext.getStore();
  env['ATUS_CODE_PROMPT_ID'] = promptId ?? '';
