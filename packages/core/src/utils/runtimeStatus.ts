 ...
 *
 * Runtime status sidecar for an active interactive atus code session.
 *
 ...
 *
 *     "Which atus code session is the running PID X serving?"
 *
...
/** Snapshot of a live atus code session process for external observers. */
...
export async function writeRuntimeStatus(
  ...
  const payload: RuntimeStatusOnDisk = {
    ...
    atus_version: fields.atusVersion ?? null,
  };
 ...
 * dataclass.
 *
 * Note: a returned record only proves that *some* atus code process
...
export async function readRuntimeStatus(
  ...
  const atusVersion = obj['atus_version'];
  ...
  if (atusVersion !== null && typeof atusVersion !== 'string') return null;
 ...
 *
 * Intentionally **not** called on `/quit` — when the atus-code process
