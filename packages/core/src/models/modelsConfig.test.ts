...
describe('ModelsConfig', () => {
  ...
  it('should always force Qwen OAuth apiKey placeholder when applying model defaults', async () => {
    ...
    const modelsConfig = new ModelsConfig({
      initialAuthType: AuthType.ATUS_OAUTH,
      generationConfig: {
        apiKey: 'manual-key-should-not-leak',
      },
    });

    // Switching within atus-oauth triggers applyResolvedModelDefaults().
    await modelsConfig.switchModel(AuthType.ATUS_OAUTH, 'coder-model');

    const gc = currentGenerationConfig(modelsConfig);
    expect(gc.apiKey).toBe('ATUS_OAUTH_DYNAMIC_TOKEN');
  ...
  it('should use default model for new authType when switching from different authType with env vars', () => {
    ...
    // User switches to atus-oauth via AuthDialog
    // refreshAuth calls syncAfterAuthRefresh with the current model (gpt-4o)
    // which doesn't exist in atus-oauth registry, so it should use default
    modelsConfig.syncAfterAuthRefresh(AuthType.ATUS_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default atus-oauth model (coder-model), not the OPENAI model
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('ATUS_OAUTH_DYNAMIC_TOKEN');
    expect(gc.apiKeyEnvKey).toBeUndefined();
  });

  it('should clear manual credentials when switching from USE_OPENAI to ATUS_OAUTH', () => {
    ...
    // User switches to atus-oauth
    // Since authType is not USE_OPENAI, manual credentials should be cleared
    // and default atus-oauth model should be applied
    modelsConfig.syncAfterAuthRefresh(AuthType.ATUS_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default atus-oauth model, not preserve manual OpenAI credentials
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('ATUS_OAUTH_DYNAMIC_TOKEN');
    // baseUrl should be set to atus-oauth default, not preserved from manual OpenAI config
  ...
  describe('getAllConfiguredModels', () => {
    it('should return all models across all authTypes and put atus-oauth first', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // atus-oauth models should be ordered first
      const firstNonQwenIndex = allModels.findIndex(
        (m) => m.authType !== AuthType.ATUS_OAUTH,
      );
      expect(firstNonQwenIndex).toBeGreaterThan(0);
      expect(
        allModels
          .slice(0, firstNonQwenIndex)
          .every((m) => m.authType === AuthType.ATUS_OAUTH),
      ).toBe(true);
      expect(
        allModels
      ...
      // Should include atus-oauth models (hard-coded)
      const atusModels = allModels.filter(
        (m) => m.authType === AuthType.ATUS_OAUTH,
      );
      expect(atusModels.length).toBeGreaterThan(0);
    ...
    it('should return empty array when no models are registered', () => {
      ...
      // Should still include atus-oauth models (hard-coded)
    ...
    it('should support filtering by authTypes and still put atus-oauth first when included', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // Filter: OpenAI only (should not include atus-oauth)
      ...
      // Filter: include atus-oauth but request it later -> still ordered first
    ...
    it('should handle reload with empty config', async () => {
      ...
      expect(
        modelsConfig
          ...
          .filter((m) => m.authType !== 'atus-oauth').length,
      ).toBeGreaterThan(0);

      // Reload with empty config
      modelsConfig.reloadModelProvidersConfig({});

      // Only atus-oauth models should remain
      const models = modelsConfig.getAllConfiguredModels();
      expect(models.every((m) => m.authType === 'atus-oauth')).toBe(true);
    });

    it('should preserve atus-oauth models after reload', () => {
      const modelsConfig = new ModelsConfig({
        modelProvidersConfig: {
          openai: [{ id: 'gpt-4', name: 'GPT-4' }],
        },
      ...
      const initialQwenModels = modelsConfig
        .getAllConfiguredModels()
        .filter((m) => m.authType === 'atus-oauth');

      modelsConfig.reloadModelProvidersConfig({
        gemini: [{ id: 'gemini-pro', name: 'Gemini Pro' }],
      });

      // atus-oauth models should still exist
      const atusModelsAfterReload = modelsConfig
        .getAllConfiguredModels()
        .filter((m) => m.authType === 'atus-oauth');
      expect(atusModelsAfterReload.length).toBe(initialQwenModels.length);
    });
