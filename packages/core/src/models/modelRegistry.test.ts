...
describe('ModelRegistry', () => {
  describe('initialization', () => {
    it('should always include hard-coded atus-oauth models', () => {
      const registry = new ModelRegistry();

      const atusModels = registry.getModelsForAuthType(AuthType.ATUS_OAUTH);
      expect(atusModels.length).toBe(ATUS_OAUTH_MODELS.length);
      expect(atusModels[0].id).toBe('coder-model');
    });

    it('should initialize with empty config', () => {
      const registry = new ModelRegistry();
      expect(registry.getModelsForAuthType(AuthType.ATUS_OAUTH).length).toBe(
        ATUS_OAUTH_MODELS.length,
      );
    ...
    it('should ignore atus-oauth models in config (hard-coded)', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        'atus-oauth': [
          {
            id: 'custom-atus',
      ...
      // Should still use hard-coded atus-oauth models
      const atusModels = registry.getModelsForAuthType(AuthType.ATUS_OAUTH);
      expect(atusModels.length).toBe(ATUS_OAUTH_MODELS.length);
      expect(atusModels.find((m) => m.id === 'custom-atus')).toBeUndefined();
    });
  });
  ...
  describe('getDefaultModelForAuthType', () => {
    it('should return coder-model for atus-oauth', () => {
      const registry = new ModelRegistry();
  ...
  describe('default base URLs', () => {
    it('should apply default dashscope URL for atus-oauth', () => {
      const registry = new ModelRegistry();
      const model = registry.getModel(AuthType.ATUS_OAUTH, 'coder-model');
  ...
  describe('reloadModels', () => {
    ...
    it('should preserve hard-coded atus-oauth models after reload', () => {
      const registry = new ModelRegistry({
        openai: [{ id: 'gpt-4', name: 'GPT-4' }],
      });
      ...
      expect(
        registry.getModel(AuthType.ATUS_OAUTH, 'coder-model'),
      ).toBeDefined();
    });
    ...
    it('should ignore atus-oauth models in reload config', () => {
      const registry = new ModelRegistry();

      registry.reloadModels({
        'atus-oauth': [{ id: 'custom-atus', name: 'Custom Qwen' }],
      });

      // atus-oauth should still use hard-coded models
      const atusModels = registry.getModelsForAuthType(AuthType.ATUS_OAUTH);
      expect(atusModels.length).toBe(ATUS_OAUTH_MODELS.length);
      expect(atusModels.find((m) => m.id === 'custom-atus')).toBeUndefined();
    });
