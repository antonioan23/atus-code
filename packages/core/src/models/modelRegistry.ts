...
import { ATUS_OAUTH_MODELS } from './constants.js';
import { createDebugLogger } from '../utils/debugLogger.js';

const debugLogger = createDebugLogger('MODEL_REGISTRY');

export { ATUS_OAUTH_MODELS } from './constants.js';
...
export class ModelRegistry {
  ...
  constructor(modelProvidersConfig?: ModelProvidersConfig) {
    ...
    // Always register atus-oauth models (hard-coded, cannot be overridden)
    this.registerAuthTypeModels(AuthType.ATUS_OAUTH, ATUS_OAUTH_MODELS);
    ...
    if (modelProvidersConfig) {
      for (const [rawKey, models] of Object.entries(modelProvidersConfig)) {
        ...
        // Skip atus-oauth as it uses hard-coded models
   ...
   * For atus-oauth, returns the coder model.
   ...
   * Preserves hard-coded atus-oauth models.
   */
  reloadModels(modelProvidersConfig?: ModelProvidersConfig): void {
    // Clear existing user-configured models (preserve atus-oauth)
