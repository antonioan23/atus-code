...
/**
 * Applies ATUS_CODE_API_TIMEOUT_MS env override if modelProvider has not set a timeout.
...
function applyTimeoutEnvOverride(
  ...
  const raw = env['ATUS_CODE_API_TIMEOUT_MS'];
  ...
  if (Number.isFinite(parsed) && parsed > 0) {
    ...
    sources['timeout'] = {
      kind: 'env',
      envKey: 'ATUS_CODE_API_TIMEOUT_MS',
    };
  }
}
...
export function resolveModelConfig(
  ...
  // ---- Env override: ATUS_CODE_API_TIMEOUT_MS ----
...
function resolveQwenOAuthConfig(
  ...
  const allowedModels = new Set<string>(ATUS_OAUTH_ALLOWED_MODELS);
