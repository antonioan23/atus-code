...
export class ModelsConfig {
  ...
  // One-shot flag for atus-oauth credential caching
   ...
   * - atus-oauth models are always ordered first.
  ...
  getAllConfiguredModels(authTypes?: AuthType[]): AvailableModel[] {
    ...
    // Force atus-oauth to the front (if requested / defaulted in).
    const orderedAuthTypes: AuthType[] = [];
    if (uniqueAuthTypes.includes(AuthType.ATUS_OAUTH)) {
      orderedAuthTypes.push(AuthType.ATUS_OAUTH);
    }
    for (const authType of uniqueAuthTypes) {
      if (authType !== AuthType.ATUS_OAUTH) {
  ...
  async setModel(
    ...
    // Special case: atus-oauth model switch - hot update in place
    // coder-model supports vision capabilities and can be hot-updated
    if (
      this.currentAuthType === AuthType.ATUS_OAUTH &&
      ...
      if (this.onModelChange) {
        await this.onModelChange(AuthType.ATUS_OAUTH, false);
      }
      return;
    }
  ...
  async switchModel(
    ...
    const rollbackSnapshot = this.createStateSnapshotForRollback();
    if (authType === AuthType.ATUS_OAUTH && options?.requireCachedCredentials) {
  ...
  private applyResolvedModelDefaults(model: ResolvedModelConfig): void {
    ...
    // replaced later.)
    if (this.currentAuthType === AuthType.ATUS_OAUTH) {
      this._generationConfig.apiKey = 'ATUS_OAUTH_DYNAMIC_TOKEN';
      this.generationConfigSources['apiKey'] = {
        kind: 'computed',
  ...
  private getRuntimeModelOption(): AvailableModel | undefined {
    ...
    return {
      ...
      /**
       * `isVision` is for automatic switching of atus-oauth vision model.
