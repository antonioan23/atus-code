 ...
 * `process.env['ATUS_CODE_SESSION_ID']`, preventing throwaway instances
...
vi.mock('../telemetry/index.js', () => ({
  AtusLogger: vi.fn().mockImplementation(() => ({
...
describe('Config sessionEnvClaimed guard', () => {
  ...
  beforeEach(() => {
    originalEnv = process.env['ATUS_CODE_SESSION_ID'];
  ...
  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env['ATUS_CODE_SESSION_ID'] = originalEnv;
    } else {
      delete process.env['ATUS_CODE_SESSION_ID'];
    }
    vi.resetModules();
  ...
  it('first Config sets process.env ATUS_CODE_SESSION_ID to its sessionId', async () => {
    const { Config } = await import('./config.js');
    const config = new Config({ ...baseParams });

    expect(process.env['ATUS_CODE_SESSION_ID']).toBe(config.getSessionId());
  });
  ...
  it('subsequent Config does not overwrite the env var set by the first', async () => {
    ...
    expect(process.env['ATUS_CODE_SESSION_ID']).toBe(firstSessionId);
    expect(process.env['ATUS_CODE_SESSION_ID']).not.toBe(
  ...
  it('startNewSession updates env var to the new session ID', async () => {
    ...
    expect(process.env['ATUS_CODE_SESSION_ID']).toBe(originalSessionId);

    // Simulate /clear or session switch
    config.startNewSession('new-session-uuid-123');

    expect(process.env['ATUS_CODE_SESSION_ID']).toBe('new-session-uuid-123');
    expect(process.env['ATUS_CODE_SESSION_ID']).not.toBe(originalSessionId);
  });
});
