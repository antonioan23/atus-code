...
export interface ChatCompressionSettings {
   ...
   * Env override: `ATUS_IMAGE_TOKEN_ESTIMATE`.
   */
   ...
   * Env override: `ATUS_COMPACT_MAX_RECENT_FILES`.
   */
   ...
   * Env override: `ATUS_COMPACT_MAX_RECENT_IMAGES`.
   */
   ...
   * Env override: `ATUS_COMPACT_SCREENSHOT_TRIGGER` (`1`/`true`/`0`/`false`).
   */
   ...
   * Env override: `ATUS_COMPACT_SCREENSHOT_THRESHOLD`.
   */
 ...
 * data atus-code writes into outbound LLM API requests.
 *
 ...
 * the atus-code process and INTO third-party LLM provider request
  ...
  const pickBool = (v: unknown, fieldName: string): boolean => {
    ...
    if (typeof v === 'string') {
      ...
      if (!knownDisable.includes(lowered)) {
        ...
        // can see the actual coercion in ATUS_DEBUG_LOG_FILE.
...
export type ExtensionOriginSource = 'AtusCode' | 'Claude' | 'Gemini';
...
export interface WorktreeSettings {
   ...
   * resolve to git-internal paths (`.git`, `.atus-code`) are also rejected
...
export interface AgentsCollabSettings {
  ...
  arena?: {
    /** Custom base directory for Arena worktrees (default: ~/.atus-code/arena) */
...
export interface ConfigParameters {
   ...
   * the `ATUS_DISABLED_SLASH_COMMANDS` environment variable.
   */
   ...
   * When set, atus code outputs structured JSON events to this fd while
   ...
   * Only meaningful in headless mode (`atus -p`).
   */
  ...
  /** Glob patterns to exclude from .atus-code/rules/ loading. */
...
// ATUS_CODE_SESSION_ID env var. Prevents throwaway Config instances from
// overwriting the real session's ID while still allowing nested atus-code
...
export class Config {
   ...
   * `ATUS_CODE_LEGACY_MCP_BLOCKING=1` and racing against background
  ...
  constructor(params: ConfigParameters) {
    ...
    // rather than checking env existence — otherwise a nested atus-code
    // launched from within a session would inherit the parent's ID and
    // never claim its own.
    if (!sessionEnvClaimed && process.env) {
      process.env['ATUS_CODE_SESSION_ID'] = this.sessionId;
    ...
    this.gitCoAuthor = {
      ...
      name: 'Atus-Coder',
      email: 'atus-coder@alibabacloud.com',
    };
    ...
    this.fileFiltering = {
      ...
      respectAtusIgnore: params.fileFiltering?.respectAtusIgnore ?? true,
  ...
  async initialize(options?: ConfigInitializeOptions): Promise<void> {
    ...
    // behavior with `ATUS_CODE_LEGACY_MCP_BLOCKING=1` — kept ≥ 1 release as
    // an escape hatch.
    const legacyBlockingMcp =
      process.env['ATUS_CODE_LEGACY_MCP_BLOCKING'] === '1';
    ...
    // scan `<subdir>/.atus-code/worktrees/` — which never exists — and the
    // sweep would silently be a no-op forever.
    if (!this.getBareMode()) {
      void (async () => {
        try {
          // Resolve the repo top-level FIRST. The previous code bailed
          // on `fs.access(<targetDir>/.atus-code/worktrees)` before resolving,
          ...
          const worktreesDir = path.join(root, '.atus-code', 'worktrees');
          try {
   ...
   * - `ATUS_CODE_LEGACY_MCP_BLOCKING=1` is set (MCP already discovered
  ...
  startNewSession(
    ...
    // atus --prompt-interactive) must not delete a sibling shell's
  ...
  private async handleModelChange(
    ...
    // Hot update path: only supported for atus-oauth.
    // For other auth types we always refresh to recreate the ContentGenerator.
    //
    // Rationale:
    // - Non-atus providers may need to re-validate credentials / baseUrl / envKey.
    ...
    if (authType === AuthType.ATUS_OAUTH && !requiresRefresh) {
      const { config, sources } = resolveContentGeneratorConfigWithSources(
        this,
        authType,
        this.modelsConfig.getGenerationConfig(),
      ...
      // Hot-update fields (atus-oauth models share the same auth + client).
   ...
   * For atus-oauth model switches that are hot-update safe, this may update in place.
   *
  ...
  isCronEnabled(): boolean {
    ...
    if (process.env['ATUS_CODE_ENABLE_CRON'] === '1') return true;
    return this.cronEnabled;
  }

  isForkSubagentEnabled(): boolean {
    if (process.env['ATUS_CODE_ENABLE_FORK_SUBAGENT'] === '1') return true;
   ...
   *
   * Env overrides (either direction): `ATUS_CODE_EMIT_TOOL_USE_SUMMARIES=0`
   * to force off, `=1` to force on.
   */
  getEmitToolUseSummaries(): boolean {
    const env = process.env['ATUS_CODE_EMIT_TOOL_USE_SUMMARIES'];
  ...
  getFileFilteringRespectQwenIgnore(): boolean {
    return this.fileFiltering.respectAtusIgnore;
  }

  getFileFilteringOptions(): FileFilteringOptions {
    return {
      respectGitIgnore: this.fileFiltering.respectGitIgnore,
      respectAtusIgnore: this.fileFiltering.respectAtusIgnore,
    };
  }
  ...
  async createToolRegistry(
    ...
    // `atus [--bare] --json-schema X -p "..."` to loop until
