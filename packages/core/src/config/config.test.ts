...
describe('Server Config (config.ts)', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(AtusLogger.prototype, 'logStartSessionEvent').mockImplementation(
      async () => undefined,
    );
  ...
  describe('MemoryPressure configuration environment', () => {
    ...
    it('applies valid memory pressure env overrides', async () => {
      process.env['ATUS_MEMORY_PRESSURE_SOFT'] = '0.3';
      process.env['ATUS_MEMORY_PRESSURE_HARD'] = '0.6';
      process.env['ATUS_MEMORY_PRESSURE_CRITICAL'] = '0.9';
    ...
    it('falls back to defaults and warns on strict env parse failures', async () => {
      ...
      process.env['ATUS_MEMORY_PRESSURE_SOFT'] = '0.3extra';
      process.env['ATUS_MEMORY_PRESSURE_HARD'] = '0.6';
      process.env['ATUS_MEMORY_PRESSURE_CRITICAL'] = '0.9';
    ...
    it('falls back to defaults and warns on invalid threshold ordering', async () => {
      const stderrSpy = mockStderrWrite();
      process.env['ATUS_MEMORY_PRESSURE_SOFT'] = '0.7';
    ...
    it.each(['NaN', 'Infinity', '0'])(
      ...
      async (value) => {
        const stderrSpy = mockStderrWrite();
        process.env['ATUS_MEMORY_PRESSURE_SOFT'] = value;
    ...
    it('enables explicit GC when requested by env', async () => {
      process.env['ATUS_MEMORY_ENABLE_GC'] = '1';
    ...
    it('child Config monitors inherit the parent memory pressure config snapshot', async () => {
      ...
      process.env['ATUS_MEMORY_PRESSURE_SOFT'] = '0.9';
      process.env['ATUS_MEMORY_PRESSURE_HARD'] = '0.95';
      process.env['ATUS_MEMORY_PRESSURE_CRITICAL'] = '0.97';
      const child = Object.create(parent) as Config;
      mockMemoryRatio(0.35);
  ...
  describe('initialize', () => {
    ...
    it('honors ATUS_CODE_LEGACY_MCP_BLOCKING=1 by running MCP discovery inline', async () => {
      const originalLegacy = process.env['ATUS_CODE_LEGACY_MCP_BLOCKING'];
      ...
      try {
        ...
        if (originalLegacy === undefined) {
          delete process.env['ATUS_CODE_LEGACY_MCP_BLOCKING'];
        } else {
          process.env['ATUS_CODE_LEGACY_MCP_BLOCKING'] = originalLegacy;
        }
      }
    });
  ...
  describe('model switching optimization (ATUS_OAUTH)', () => {
    it('should switch atus-oauth model in-place without refreshing auth when safe', async () => {
      const config = new Config(baseParams);
      ...
      // Establish initial atus-oauth content generator config/content generator.
      await config.refreshAuth(AuthType.ATUS_OAUTH);
      ...
      await config.switchModel(AuthType.ATUS_OAUTH, 'coder-model');
  ...
  describe('Usage Statistics', () => {
    ...
    it('logs the session start event', async () => {
      ...
      expect(AtusLogger.prototype.logStartSessionEvent).toHaveBeenCalledOnce();
    });
  });
  ...
  // setter (proves AtusAgent calls it pre-`initialize`); the manager
...
describe('Model Switching and Config Updates', () => {
  ...
  it('should update contextWindowSize when switching models with hot-update', async () => {
    ...
    ).handleModelChange(AuthType.ATUS_OAUTH, false);

    // Verify all fields are updated
    const updatedConfig = config.getContentGeneratorConfig();
    expect(updatedConfig['model']).toBe('atus-max');
  ...
  it('should trigger full refresh when switching to non-atus-oauth provider', async () => {
    const config = new Config(baseParams);

    // Initialize with atus-oauth
    const initialConfig: ContentGeneratorConfig = {
      ['model']: 'qwen3-coder-plus',
      ['authType']: AuthType.ATUS_OAUTH,
      ['apiKey']: 'test-key',
  ...
  describe('runtime ContentGenerator view (AsyncLocalStorage)', () => {
    ...
    it('resolves getters to the runtime view inside the frame, instance fields outside', async () => {
      ...
      expect(config.getAuthType()).toBe(AuthType.ATUS_OAUTH);
