...
import { getProjectHash, ATUS_DIR, sanitizeCwd } from '../utils/paths.js';
import { FatalConfigError } from '../utils/errors.js';

export { ATUS_DIR } from '../utils/paths.js';
export const GOOGLE_ACCOUNTS_FILENAME = 'google_accounts.json';
export const OAUTH_FILE = 'oauth_creds.json';
export const SKILL_PROVIDER_CONFIG_DIRS = ['.atus-code', '.agents'];
...
export class Storage {
   ...
   *              Pass the project root so that relative values like ".atus-code" resolve
   ...
   *
   * Priority: ATUS_RUNTIME_DIR env var > setRuntimeBaseDir() value > getGlobalQwenDir()
   * @returns Absolute path to the runtime output base directory
   */
  static getRuntimeBaseDir(): string {
    const envDir = process.env['ATUS_RUNTIME_DIR'];
    if (envDir) {
      return (
  ...
  static getGlobalQwenDir(): string {
    ...
    if (!homeDir) {
      return path.join(os.tmpdir(), '.atus-code');
    }
    return path.join(homeDir, ATUS_DIR);
  }
  ...
  getQwenDir(): string {
    return path.join(this.targetDir, ATUS_DIR);
  }
  ...
  getExtensionsConfigPath(): string {
    return path.join(this.getExtensionsDir(), 'atus-extension.json');
  }
  ...
  /**
   * Returns the user-level extensions directory (~/.atus-code/extensions/).
   * Extensions installed at user scope are stored here, as opposed to
   * project-level extensions which live in <project>/.atus-code/extensions/.
   */
