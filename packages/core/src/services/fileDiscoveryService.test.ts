...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .atusignore patterns even when not in a git repo', async () => {
      await createTestFile('.atusignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.atusignore', 'logs/');
    });

    it('should filter out git-ignored and atus-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectAtusIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectAtusIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldQwenIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.atusignore', '*.log');
    });
    ...
    it('should return true for atus-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);
    ...
    it('should return false for non-atus-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);
