...
export function resolveSlimmingConfig(
  ...
  return {
    imageTokenEstimate: resolveNumber(
      process.env['ATUS_IMAGE_TOKEN_ESTIMATE'],
...
export function estimatePartChars(
  ...
  // Tool results in atus-code carry media on `functionResponse.parts`
 ...
 * atus-code attaches media here (see
...
function transformPart(part: Part, stats: SlimStats): Part {
  ...
  // Walk into functionResponse.parts (atus-code's nested-media carrier
...
function isNonImageMime(mime: string): boolean {
  ...
  // placeholder. audio/video are rare on atus-code's tool surface and
