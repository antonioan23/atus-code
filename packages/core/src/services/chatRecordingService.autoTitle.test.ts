 ...
 */
function mockOk(title: string, modelUsed = 'atus-turbo'): void {
...
describe('ChatRecordingService - auto-title trigger', () => {
    ...
    mockConfig = {
      ...
      storage: {
        getProjectTempDir: vi
          .fn()
          .mockReturnValue('/test/project/root/.atus-code/tmp/hash'),
        getProjectDir: vi
          .fn()
          .mockReturnValue('/test/project/root/.atus-code/projects/test-project'),
      },
  ...
  it('does not trigger when ATUS_DISABLE_AUTO_TITLE is set', async () => {
    const prev = process.env['ATUS_DISABLE_AUTO_TITLE'];
    process.env['ATUS_DISABLE_AUTO_TITLE'] = '1';
    try {
      chatRecordingService.recordAssistantTurn({
        model: 'atus-plus',
        message: [{ text: 'reply' }],
      ...
      if (prev === undefined) delete process.env['ATUS_DISABLE_AUTO_TITLE'];
      else process.env['ATUS_DISABLE_AUTO_TITLE'] = prev;
    }
  });

  it('still triggers when ATUS_DISABLE_AUTO_TITLE is falsy ("0")', async () => {
    mockOk('Fix login button');
    const prev = process.env['ATUS_DISABLE_AUTO_TITLE'];
    process.env['ATUS_DISABLE_AUTO_TITLE'] = '0';
    try {
      chatRecordingService.recordAssistantTurn({
        model: 'atus-plus',
        message: [{ text: 'reply' }],
  ...
  it('respects a late /rename that lands while the LLM call is in flight', async () => {
    ...
    resolveLlm({ ok: true, title: 'Auto Title', modelUsed: 'atus-turbo' });
    await flushMicrotasks();
