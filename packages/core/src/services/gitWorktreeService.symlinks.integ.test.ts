...
describe('GitWorktreeService.createUserWorktree() — symlinkDirectories', () => {
  ...
  beforeEach(async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'atus-wt-symlinks-'));
  ...
  it('rejects paths that traverse outside the repo root', async () => {
    ...
    const siblingDir = path.join(path.dirname(repoRoot), 'atus-wt-sibling');
    ...
    const result = await service.createUserWorktree('traverse', 'main', {
      symlinkDirectories: ['../atus-wt-sibling'],
    });

    try {
      expect(result.success).toBe(true);
      const dest = path.join(result.worktree!.path, '..', 'atus-wt-sibling');
      // No symlink was created inside the worktree directory.
      const stat = await fs
        .lstat(path.join(result.worktree!.path, 'atus-wt-sibling'))
        .catch(() => null);
  ...
  it('rejects paths inside .atus-code (security guard)', async () => {
    // `.atus-code` is CLI metadata: symlinking `.atus-code/worktrees` would create
    // a worktrees-inside-worktrees loop; symlinking `.atus-code/projects` or
    // `.atus-code/tmp` would alias session metadata users have no legitimate
    // reason to share across worktrees. Guard rejects the whole subtree.
    await fs.mkdir(path.join(repoRoot, '.atus-code'), { recursive: true });
    await fs.writeFile(path.join(repoRoot, '.atus-code', 'projects'), 'data');

    const service = new GitWorktreeService(repoRoot);
    const result = await service.createUserWorktree('reject-atus', 'main', {
      symlinkDirectories: ['.atus-code/projects'],
    });
    expect(result.success).toBe(true);

    const wt = result.worktree!.path;
    // No symlink at <worktree>/.atus-code/projects.
    const wrote = await fs
      .lstat(path.join(wt, '.atus-code', 'projects'))
      .then((s) => s.isSymbolicLink())
      .catch(() => false);
    expect(wrote).toBe(false);
  });
  ...
  it('refuses sources whose realpath escapes the repo root or lands in .git/.atus-code (committed-symlink bypass)', async () => {
    // Round-7 security fix: the lexical `isWithinRoot(sourceAbs, …)` and
    // `.git`/`.atus-code` blocklist checks DON'T resolve symlinks, so a symlink
    ...
    const outsideDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'atus-wt-outside-'),
    );
  ...
  describe('Phase D-3: fetchPullRequestRef error messages', () => {
    ...
    it('handles "no such ref" when origin is reachable but the PR does not exist', async () => {
      ...
      const upstream = await fs.mkdtemp(
        path.join(os.tmpdir(), 'atus-wt-pr-no-such-ref-'),
      );
