 ...
 * out at runtime: `ATUS_DISABLE_AUTO_TITLE=1` (or any truthy-ish value)
...
function autoTitleDisabledByEnv(): boolean {
  const v = process.env['ATUS_DISABLE_AUTO_TITLE'];
  if (!v) return false;
 ...
 *
 * File location: ~/.atus-code/tmp/<project_id>/chats/
 *
...
export class ChatRecordingService {
  ...
  private maybeTriggerAutoTitle(): void {
    ...
    // Headless/one-shot CLI flows (`atus -p "…"`, cron, CI scripts) run a
