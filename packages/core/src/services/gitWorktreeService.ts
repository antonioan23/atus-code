 ...
 * `.atus-code/worktrees/.gitignore`) so it cannot leak into commits.
 */
export const WORKTREE_SESSION_FILE = '.atus-code-session';
 ...
 * Changing this value affects the on-disk layout (`~/.atus-code/<WORKTREES_DIR>/`)
...
export class GitWorktreeService {
   ...
   * from — otherwise running `atus` from a monorepo subdirectory would
   * scatter `.atus-code/worktrees/` under each subdirectory instead of
  ...
  // `<projectRoot>/.atus-code/worktrees/<slug>` rather than under the
  // session-scoped Arena baseDir.
  // ──────────────────────────────────────────────────────────────────────

  /**
   * Returns the directory holding all general-purpose worktrees for this
   * repo: `<projectRoot>/.atus-code/worktrees`.
   */
  getUserWorktreesDir(): string {
    return path.join(this.sourceRepoPath, '.atus-code', WORKTREES_DIR);
  }
   ...
   * and `<repoRoot>/.atus-code/worktrees/foo` already exists on disk, we
   ...
   *    <repo>/.atus-code/worktrees/foo`) git walks up to the outer `.git`
  ...
  /**
   * Creates a general-purpose worktree at `<projectRoot>/.atus-code/worktrees/<slug>`
  ...
  async createUserWorktree(
    ...
    try {
      ...
      // touches an existing user-managed `.atus-code/.gitignore`.
  ...
  private async symlinkConfiguredDirectories(
    ...
    const atusDirAbs = path.join(repoRootAbs, '.atus-code');
    ...
    for (const raw of configured) {
      ...
      // common-dir, and a symlink would shadow it). The whole `.atus-code`
      // tree is also off-limits: linking `.atus-code` (parent) would
      // recursively pull `.atus-code/worktrees` into the new worktree,
      // recreating the loop; linking `.atus-code/worktrees` directly
      // creates the same loop more obviously; and `.atus-code/projects`
      // / `.atus-code/tmp` are CLI metadata users have no legitimate
      // reason to share across worktrees.
      // `gitDirAbs` / `atusDirAbs` are canonical (derived from the
      ...
      if (isWithinRoot(sourceAbs, atusDirAbs)) {
        debugLogger.warn(
          `symlinkConfiguredDirectories: refusing path "${raw}" — ` +
            `the .atus-code tree is CLI-managed; symlinking any of it could ` +
      ...
      // into `.git`, an outside dir, or `.atus-code`. Without this step a
      ...
      if (isWithinRoot(realSource, atusDirAbs)) {
        debugLogger.warn(
          `symlinkConfiguredDirectories: refusing path "${raw}" — real source ${realSource} resolves inside .atus-code`,
        );
        continue;
      }
  ...
  /**
   * Ensures `<projectRoot>/.atus-code/.gitignore` ignores the worktrees
  ...
  private async ensureWorktreesGitignored(): Promise<void> {
    try {
      const atusDir = path.join(this.sourceRepoPath, '.atus-code');
      await fs.mkdir(atusDir, { recursive: true });
      const gitignorePath = path.join(atusDir, '.gitignore');
