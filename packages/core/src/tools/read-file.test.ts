  ...
  beforeEach(async () => {
    ...
    const mockConfigInstance = {
      ...
      storage: {
        ...
        getUserSkillsDirs: () => [path.join(os.homedir(), '.atus-code', 'skills')],
      },
  ...
  describe('execute', () => {
    ...
    describe('with FileReadCache', () => {
      ...
      it('records an auto-memory read in the cache so a follow-up Edit can pass enforcement', async () => {
        ...
        // read. Put a file under .atus-code/<auto-memory>/ via
        // ATUS_CODE_MEMORY_LOCAL=1 and assert recordRead happened.
        const previousLocal = process.env['ATUS_CODE_MEMORY_LOCAL'];
        process.env['ATUS_CODE_MEMORY_LOCAL'] = '1';
        try {
          ...
          if (previousLocal === undefined) {
            delete process.env['ATUS_CODE_MEMORY_LOCAL'];
          } else {
            process.env['ATUS_CODE_MEMORY_LOCAL'] = previousLocal;
          }
        }
      });
      ...
      it('completely bypasses the cache when getFileReadCacheDisabled() is true', async () => {
        ...
        const disabledConfig = {
          ...
          storage: {
            ...
            getProjectDir: () => path.join(tempRootDir, '.project'),
            getUserSkillsDirs: () => [
              path.join(os.homedir(), '.atus-code', 'skills'),
            ],
          },
          getTruncateToolOutputThreshold: () => 2500,
          getTruncateToolOutputLines: () => 500,
    ...
    describe('with .atusignore', () => {
      beforeEach(async () => {
        await fsp.writeFile(
          path.join(tempRootDir, '.atusignore'),
          ['foo.*', 'ignored/'].join('\n'),
        );
      });

      it('should throw error if path is ignored by a .atusignore pattern', async () => {
        const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
        await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
        const params: ReadFileToolParams = {
          file_path: ignoredFilePath,
        };
        const expectedError = `File path '${ignoredFilePath}' is ignored by .atusignore pattern(s).`;
