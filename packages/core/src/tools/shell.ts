...
function findAttributableCommitSegment(
  ...
  for (const sub of splitCommands(command)) {
    ...
    if (start < 0) {
      ...
      // when ATUS_DEBUG_LOG_FILE is set.
      debugLogger.warn(
...
> {
  ...
  async execute(
    ...
    // Add co-author to git commit commands and atus code attribution to
    ...
    // ATUS_DEBUG_LOG_FILE entry the user has likely never set up.
   ...
   * PR body. Format: "🤖 Generated with atus code (N-shotted by Atus-Coder)"
   * when at least one user prompt has been recorded since the last commit;
   * otherwise just "🤖 Generated with atus code".
   *
  ...
  private addAttributionToPR(command: string): string {
    ...
    const generator = gitCoAuthorSettings.name ?? 'Atus-Coder';

    const attribution =
      shots > 0
        ? `\n\n🤖 Generated with atus code (${shots}-shotted by ${generator})`
        : `\n\n🤖 Generated with atus code`;
    ...
    // skip in `ATUS_DEBUG_LOG_FILE`. Inline-body rewriting is the
