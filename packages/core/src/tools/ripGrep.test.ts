...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass .atusignore to ripgrep when respected', async () => {
      await fs.writeFile(
    ...
    it('should include .atusignore matches when disabled in config', async () => {
      await fs.writeFile(path.join(tempRootDir, '.atusignore'), 'kept.txt\n');
      await fs.writeFile(path.join(tempRootDir, 'kept.txt'), 'keep me');
      Object.assign(mockConfig, {
        getFileFilteringOptions: () => ({
          respectGitIgnore: true,
          respectAtusIgnore: false,
        }),
      });
  ...
  describe('multi-directory workspace', () => {
    ...
    it('should load .atusignore from each workspace directory', async () => {
      const secondDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'grep-tool-second-'),
      );
      await fs.writeFile(path.join(secondDir, '.atusignore'), 'ignored.txt\n');
      await fs.writeFile(
        path.join(tempRootDir, '.atusignore'),
        'other-ignored.txt\n',
      );
      ...
      // Verify both .atusignore files were passed
      ...
      expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.atusignore'));
      expect(ignoreFileArgs).toContain(path.join(secondDir, '.atusignore'));
