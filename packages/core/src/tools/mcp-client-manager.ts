 ...
 * `atus/status/workspace/mcp` ext-method. `subprocessCount` is the
 ...
 * doesn't pass one. Daemon-mode (`atus serve`) sets these when
 * spawning the `atus --acp` child; standalone `atus` invocations
 * leave them unset and get `{ budgetMode: 'off' }` — the historical
 * behavior, no enforcement.
 *
 * `ATUS_SERVE_MCP_CLIENT_BUDGET` — positive integer; non-numeric /
 *   zero / negative / NaN are silently ignored (treated as unset).
 * `ATUS_SERVE_MCP_BUDGET_MODE` — `enforce|warn|off`. Defaults to
 *   `warn` when a budget is set, `off` otherwise.
 */
function readBudgetFromEnv(): McpBudgetConfig {
  const rawBudget = process.env['ATUS_SERVE_MCP_CLIENT_BUDGET'];
  const rawMode = process.env['ATUS_SERVE_MCP_BUDGET_MODE'];
  ...
  if (rawBudget !== undefined && rawBudget !== '') {
    ...
    if (Number.isFinite(parsed) && Number.isInteger(parsed) && parsed > 0) {
      ...
      // like `ATUS_SERVE_MCP_CLIENT_BUDGET=abc` previously fell
      ...
      process.stderr.write(
        `atus serve: ignoring invalid ATUS_SERVE_MCP_CLIENT_BUDGET=` +
  ...
  if (rawMode === 'enforce' || rawMode === 'warn' || rawMode === 'off') {
    ...
    if (rawMode !== undefined && rawMode !== '') {
      ...
      // typo is visible.
      process.stderr.write(
        `atus serve: ignoring invalid ATUS_SERVE_MCP_BUDGET_MODE=` +
  ...
  // `ATUS_SERVE_MCP_BUDGET_MODE=enforce` in a Docker Compose / k8s
  ...
  if (
    ...
    process.stderr.write(
      `atus serve: ATUS_SERVE_MCP_BUDGET_MODE=${budgetMode} requires ` +
        `ATUS_SERVE_MCP_CLIENT_BUDGET=N; downgrading to off. ` +
...
export class McpClientManager {
  ...
  constructor(
    ...
    // set by `atus serve --mcp-client-budget=N --mcp-budget-mode=X`
    // when spawning the ACP child. Standalone `atus` invocations
    ...
    if (
      ...
      process.stderr.write(
        `atus serve: McpClientManager constructed with budgetMode=${resolvedMode} ` +
  ...
  private refuseAndLog(
    ...
    process.stderr.write(
      `atus serve: MCP server '${serverName}' refused (budget exhausted, ` +
