...
const enterWorktreeDescription = `Creates an isolated git worktree at \`<projectRoot>/.atus-code/worktrees/<slug>\` and returns its absolute path so subsequent file edits, shell commands, and other tools can operate inside it.
...
> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    ...
    // already inside `.atus-code/worktrees/<slug>/`, a fresh worktree would
    // be provisioned at `<repo>/.atus-code/worktrees/<new>/` — but the
    ...
    // `.atus-code/worktrees` somewhere in cwd qualifies. We also forbid
    ...
    // monorepo subdirectory would scatter `.atus-code/worktrees/` under each
    ...
    // `<repoRoot>/.atus-code/worktrees/`, regardless of which subdirectory
    ...
    // atus from a feature branch but the main working tree still has
    // `main` checked out).
