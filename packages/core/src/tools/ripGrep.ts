...
/**
 * Per-process cache for `.atusignore` discovery. The same directories show
 ...
 * `atusIgnore`: dir → string | null (cached `.atusignore` path or null)
 *
 * **Known staleness window:** a `.atusignore` created mid-session, or a
...
const atusIgnoreCache = new Map<string, string | null>();
...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    // Add file exclusions from .gitignore and .atusignore
    const filteringOptions = this.getFileFilteringOptions();
    if (!filteringOptions.respectGitIgnore) {
      rgArgs.push('--no-ignore-vcs');
    }

    if (filteringOptions.respectAtusIgnore) {
      // Load .atusignore from each workspace directory, not just the primary one
      ...
      for (const searchPath of paths) {
        ...
        let atusIgnorePath = atusIgnoreCache.get(dir);
        if (atusIgnorePath === undefined) {
          const candidate = path.join(dir, '.atusignore');
          atusIgnorePath = fs.existsSync(candidate) ? candidate : null;
          atusIgnoreCache.set(dir, atusIgnorePath);
          trimCache(atusIgnoreCache);
        }
        if (atusIgnorePath && !seenIgnoreFiles.has(atusIgnorePath)) {
          rgArgs.push('--ignore-file', atusIgnorePath);
          seenIgnoreFiles.add(atusIgnorePath);
        }
      }
    }
  ...
  private getFileFilteringOptions(): FileFilteringOptions {
    ...
    return {
      ...
      respectAtusIgnore:
        options?.respectAtusIgnore ??
        DEFAULT_FILE_FILTERING_OPTIONS.respectAtusIgnore,
    };
  }
