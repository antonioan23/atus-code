...
describe('computer-use constants', () => {
  ...
  beforeEach(() => {
    originalEnv = process.env['ATUS_COMPUTER_USE_PACKAGE'];
    delete process.env['ATUS_COMPUTER_USE_PACKAGE'];
  });
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env['ATUS_COMPUTER_USE_PACKAGE'] = originalEnv;
    }
  });
  ...
  describe('PINNED_OPEN_COMPUTER_USE_PACKAGE_NAME', () => {
    it('is the scoped atuscode fork package', () => {
      expect(PINNED_OPEN_COMPUTER_USE_PACKAGE_NAME).toBe(
        '@atus-code/open-computer-use',
      );
    });
  });
  ...
  describe('resolveComputerUsePackageSpec', () => {
    ...
    it('honors ATUS_COMPUTER_USE_PACKAGE override', () => {
      process.env['ATUS_COMPUTER_USE_PACKAGE'] =
        '@atus-code/open-computer-use@0.99.99';
      expect(resolveComputerUsePackageSpec()).toBe(
        '@atus-code/open-computer-use@0.99.99',
      );
    });
    ...
    it('reads env var at call time (not at module load)', () => {
      ...
      process.env['ATUS_COMPUTER_USE_PACKAGE'] = 'spec-a';
      expect(resolveComputerUsePackageSpec()).toBe('spec-a');
      process.env['ATUS_COMPUTER_USE_PACKAGE'] = 'spec-b';
