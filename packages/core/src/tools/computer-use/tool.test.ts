...
describe('ComputerUseTool', () => {
  beforeEach(() => {
    ...
    process.env['ATUS_COMPUTER_USE_AUTO_APPROVE'] = '1';
  });

  afterEach(() => {
    delete process.env['ATUS_COMPUTER_USE_AUTO_APPROVE'];
  });
...
describe('ComputerUseInvocation confirmation pathway', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'atus-cu-tool-'));
    mockHome = tmpHome;
    ComputerUseClient.setSharedForTest(undefined);
    vi.clearAllMocks();
  });
  ...
  // approval. With ATUS_COMPUTER_USE_AUTO_APPROVE unset, the bootstrap fallback
