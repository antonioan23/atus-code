...
describe('install-state', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(pathJoin(tmpdir(), 'atus-cu-test-'));
  });
  ...
  it('round-trips state', async () => {
    await saveInstallState(tmpHome, {
      approvedPackageSpec: '@atus-code/open-computer-use@^0.3.0',
  ...
  it('isPackageSpecApproved returns false when no state', async () => {
    expect(
      await isPackageSpecApproved(
        tmpHome,
        '@atus-code/open-computer-use@^0.3.0',
      ),
    ).toBe(false);
  });
  ...
  it('isPackageSpecApproved returns false when version differs', async () => {
    ...
    expect(
      await isPackageSpecApproved(
        tmpHome,
        '@atus-code/open-computer-use@^0.4.0',
      ),
    ).toBe(false);
  });
  ...
  it('installStatePathFor returns correct path', () => {
    ...
    expect(path).toBe(
      pathJoin(tmpHome, '.atus-code', 'computer-use', 'installed.json'),
    );
  });
});
