 ...
 *      Requires @atus-code/open-computer-use >= 0.2.2 for permission-status.
 */
...
export interface BootstrapDeps {
   ...
   * ATUS_COMPUTER_USE_AUTO_APPROVE=1 env-var fallback; the interactive
 ...
 * window-free command.
 *
 * Requires `@atus-code/open-computer-use@>=0.2.2`. On older pinned packages
    ...
    promptInstallApproval: async (spec) => {
      process.stderr.write(
        `\n[Computer Use] First-time install\n` +
          ...
          `Set ATUS_COMPUTER_USE_AUTO_APPROVE=1 to skip this prompt.\n`,
      );
...
export async function runBootstrap(
  ...
  // must restart atus-code to re-enter the permission flow. This is
