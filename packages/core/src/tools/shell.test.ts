...
describe('ShellTool', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      storage: {
        getUserSkillsDirs: vi.fn().mockReturnValue(['/test/dir/.atus-code/skills']),
        getProjectTempDir: vi.fn().mockReturnValue('/tmp/atus-temp'),
  ...
  describe('build', () => {
    ...
    it('should throw an error for a directory within the user skills directory', async () => {
      expect(() =>
        shellTool.build({
          command: 'ls',
          directory: '/test/dir/.atus-code/skills/my-skill',
          is_background: false,
        }),
      ).toThrow(
    ...
    it('should throw an error for the user skills directory itself', async () => {
      expect(() =>
        shellTool.build({
          command: 'ls',
          directory: '/test/dir/.atus-code/skills',
          is_background: false,
        }),
      ).toThrow(
        'Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.',
    ...
    it('should resolve directory path before checking user skills directory', async () => {
      expect(() =>
        shellTool.build({
          command: 'ls',
          directory: '/test/dir/.atus-code/skills/../skills/my-skill',
          is_background: false,
        }),
      ).toThrow(
        'Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.',
      ...
      it('appends the hint AFTER truncation (so it survives `truncateToolOutput`)', async () => {
        ...
        const spy = vi
          ...
          .mockResolvedValue({
            ...
            outputFile: '/tmp/atus-temp/shell_mocked.output',
          });
    ...
    describe('addCoAuthorToGitCommit', () => {
      it('should add co-author to git commit with double quotes', async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(
          expect.stringContaining(
            'Co-authored-by: Atus-Coder <atus-coder@alibabacloud.com>',
          ),
          expect.any(String),
          expect.any(Function),
          expect.any(AbortSignal),
      ...
      it('should add Co-authored-by trailer to the LAST -m when multiple are present', async () => {
        ...
        expect(observed).toMatch(
          /-m\s+"Body line 1\s+Co-authored-by: Atus-Coder <atus-coder@alibabacloud\.com>"/s,
        );
 ...
 spanning multiple lines"`;
    ...
    describe('addAttributionToPR', () => {
      ...
      // warning to surface the skip when ATUS_DEBUG_LOG_FILE is set.
      it.each([
        ...
        async (_label, command) => {
          ...
          const observed = mockShellExecutionService.mock.calls[0][0] as string;
          expect(observed).toBe(command);
          expect(observed).not.toContain('Generated with atus code');
        },
      );
      ...
      it('should append attribution to `gh pr new --body "..."` (alias form)', async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(
          expect.stringContaining('Generated with atus code'),
          expect.any(String),
          expect.any(Function),
          expect.any(AbortSignal),
          false,
      ...
      it('should NOT match -b in earlier non-gh segments of a compound', async () => {
        ...
        expect(observed).toMatch(
          /gh pr create --title "x" --body "summary[\s\S]*Generated with atus code"/,
        );
      });
      ...
      it('should pick the OUTER --body when an inner -b appears in body text', async () => {
        ...
        expect(cmd).toMatch(
          /--body "docs mention -b 'flag' here[\s\S]*Generated with atus code"/,
        );
        expect(cmd).not.toMatch(
          /-b 'flag[\s\S]*Generated with atus code[\s\S]*' here"/,
        );
      });
      ...
      it('should target the LAST --body when gh pr create has multiple', async () => {
        ...
        const cmd = calls[calls.length - 1]?.[0] as string;
        expect(cmd).toMatch(
          /--body "ignored" --body "real summary[\s\S]*Generated with atus code/,
        );
        // The trailer must NOT be inside the first --body.
        expect(cmd).not.toMatch(
          /--body "ignored[\s\S]*Generated with atus code[\s\S]*" --body/,
        );
      });
      ...
      it('should NOT rewrite when gh pr create appears only inside quoted text', async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(
          expect.not.stringContaining('Generated with atus code'),
          expect.any(String),
          expect.any(Function),
          expect.any(AbortSignal),
          false,
      ...
      it("should match the full body across '\\\\'' apostrophe escapes", async () => {
        ...
        // middle of it.
        expect(observed).toMatch(
          /don'\\''t break me[\s\S]*Generated with atus code/,
        );
      });
    });
