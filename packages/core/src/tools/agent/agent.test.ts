...
describe('AgentTool', () => {
  ...
  const mockSubagents: SubagentConfig[] = [
    {
      ...
      level: 'project',
      filePath: '/project/.atus-code/agents/file-search.md',
    },
    {
      name: 'code-review',
      ...
      level: 'user',
      filePath: '/home/user/.atus-code/agents/code-review.md',
    },
  ];
  ...
  beforeEach(async () => {
    ...
    config = {
      ...
      storage: {
        getProjectDir: vi.fn().mockReturnValue('/test/project/.atus-code'),
      },
  ...
  describe('isolation — round-7 parent-dirty guard', () => {
    it('refuses isolation when parent has uncommitted edits', async () => {
      ...
      const repo = await fs.mkdtemp(
        pathMod.join(os.tmpdir(), 'atus-iso-dirty-'),
      );
      try {
    ...
    it('would allow isolation when parent is clean (sanity)', async () => {
      ...
      const { execFileSync } = await import('node:child_process');
      const repo = await fs.mkdtemp(
        pathMod.join(os.tmpdir(), 'atus-iso-clean-'),
      );
      try {
        execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
        execFileSync('git', ['config', 'user.email', 't@e.com'], {
      ...
      const newSubagents: SubagentConfig[] = [
        {
          ...
          level: 'project',
          filePath: '/project/.atus-code/agents/new-agent.md',
        },
      ];
      ...
      const newSubagents: SubagentConfig[] = [
        {
          ...
          level: 'project',
          filePath: '/project/.atus-code/agents/test-agent.md',
        },
      ];
  ...
  describe('AgentToolInvocation', () => {
    ...
    describe('atus-code.subagent span outcome (#4410 wenshao)', () => {
      beforeEach(() => {
  ...
  describe('Agent-level background: true', () => {
    ...
    const bgSubagent: SubagentConfig = {
      ...
      level: 'project',
      filePath: '/project/.atus-code/agents/monitor.md',
      background: true,
    };
