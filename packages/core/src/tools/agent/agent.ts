...
export interface AgentParams {
   ...
   * `<projectRoot>/.atus-code/worktrees/agent-<7hex>` and instructs the agent to
...
export const TOOL_REGISTRY_REBUILT: unique symbol = Symbol.for(
  'atus-code:tool-registry-rebuilt',
);
    ...
    const initialSchema = {
      ...
      properties: {
        ...
        isolation: {
          ...
          enum: ['worktree'],
          description:
            "Isolation mode. 'worktree' creates a temporary git worktree under <projectRoot>/.atus-code/worktrees/agent-<7hex> so the agent works on an isolated copy of the repo. The worktree is auto-removed if the agent makes no changes; otherwise the worktree path and branch are returned in the result.",
        },
      },
...
class AgentToolInvocation extends BaseToolInvocation<AgentParams, ToolResult> {
  ...
  /**
   * Wrap a subagent body in `atus-code.subagent` span lifecycle.
   *
  ...
  private async runSubagentWithHooks(
    ...
    opts: {
      ...
      updateOutput?: (output: ToolResultDisplay) => void;
      /**
       * Optional sink the atus-code.subagent span wrapper passes in so this
  ...
  async execute(
    ...
    try {
      ...
      if (this.params.isolation === 'worktree') {
        ...
        // running inside a worktree (cwd contains `.atus-code/worktrees/`),
        ...
        if (/\.atus-code[\\/]worktrees[\\/]/.test(cwd)) {
        ...
        // launches still gather worktrees under `<repoRoot>/.atus-code/...`,
      ...
      if (shouldRunInBackground) {
        ...
        // atus-code.subagent span (#3731 Phase 3) — background is
      ...
      if (isFork) {
        ...
        // Wrap fork body in atus-code.subagent span (#3731 Phase 3). Forks
      ...
      // Wrap in atus-code.subagent span (#3731 Phase 3). Foreground
      // invocations are child spans of the AGENT tool's `atus-code.tool`
