...
export class ArenaManager {
  ...
  constructor(config: Config, callbacks: ArenaCallbacks = {}) {
    ...
    // Use the user-configured base dir, or default to ~/.atus-code/arena.
    this.arenaBaseDir =
  ...
  private buildAgentSpawnConfig(agent: ArenaAgentState): AgentSpawnConfig {
    ...
    if (model.apiKey) {
      env['ATUS_API_KEY'] = model.apiKey;
    }
    if (model.baseUrl) {
      env['ATUS_BASE_URL'] = model.baseUrl;
    }
   ...
   * `~/.atus-code/worktrees/<sessionId>/`.  The directory contains:
