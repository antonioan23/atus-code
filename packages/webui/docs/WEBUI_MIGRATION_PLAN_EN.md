...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@atus-code/webui` package
...
```typescript
// @atus-code/webui/src/context/PlatformContext.ts
...
```javascript
// @atus-code/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
module.exports = {
  presets: [require('@atus-code/webui/tailwind.preset')],
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@atus-code/webui/dist/**/*.js',
  ],
};
```
...
```
Developer ──► @atus-code/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@atus-code/webui';
// or import { FileIcon } from '@atus-code/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@atus-code/webui`
...
│     └── Edit files in @atus-code/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@atus-code/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@atus-code/webui`
