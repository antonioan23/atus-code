...
import { SkillError } from '@atus-code/atus-code-core';
...
/**
 * Raised by `defaultSpawnChannelFactory` when neither `ATUS_CLI_ENTRY` nor
  ...
  constructor() {
    super(
      'Cannot determine CLI entry path for spawning the ACP child: ' +
        'process.argv[1] is empty and ATUS_CLI_ENTRY is unset. ' +
        'Set ATUS_CLI_ENTRY to the absolute path of the atus entry ' +
        'script (e.g. `export ATUS_CLI_ENTRY=$(which atus)`) to override.',
    );
...
export const SERVE_STATUS_EXT_METHODS = {
  workspaceMcp: 'atus/status/workspace/mcp',
  workspaceSkills: 'atus/status/workspace/skills',
  workspaceProviders: 'atus/status/workspace/providers',
  workspaceMemory: 'atus/status/workspace/memory',
  workspaceAgents: 'atus/status/workspace/agents',
  workspacePreflight: 'atus/status/workspace/preflight',
  sessionContext: 'atus/status/session/context',
  sessionSupportedCommands: 'atus/status/session/supported_commands',
} as const;
...
export const SERVE_CONTROL_EXT_METHODS = {
  sessionApprovalMode: 'atus/control/session/approval_mode',
  workspaceMcpRestart: 'atus/control/workspace/mcp/restart',
} as const;
...
export interface ServeMcpBudgetStatusCell extends ServeStatusCell {
   ...
   * cap independently. See `atus-serve-protocol.md` "PR 14 v1
...
export interface ServeWorkspaceMemoryFile {
   ...
   * `~/.atus-code/QWEN.md` style entries. Helps adapters render scope chips.
   */
...
export interface ServeWorkspaceMemoryStatus {
   ...
   */
  fileCount: number;
  /** Baseline path-rule count from `.atus-code/rules/`. */
  ruleCount: number;
  errors?: ServeStatusCell[];
}
 ...
 * Mirrors `DaemonAgentLevel` in `@atus-code/sdk` so route + SDK
...
export function mapDomainErrorToErrorKind(
  ...
  // `SkillError` is defined in `@atus-code/atus-code-core/skills`; same
  ...
  // the one resolved here through acp-bridge's `@atus-code/atus-code-core`
  ...
  // `TrustGateError` is defined in `@atus-code/atus-code-core/config`; we
