 ...
 * for the ACP bridge factory. Lifted to `@atus-code/acp-bridge` in #4175 PR
 ...
 * these cells; production `atus serve` provides
...
export interface BridgeOptions {
   ...
   * Reference:
   * https://github.com/atuscode/atus-code/pull/3889#issuecomment-4427875644
   */
  sessionScope?: 'single' | 'thread';
  /** Channel factory; defaults to spawning `atus --acp` as a child process. */
   ...
   *
   * Operators tune via `atus serve --event-ring-size <n>`. Cost
   ...
   *
   * Mode A in-process consumers (`atus --serve`, future) typically
