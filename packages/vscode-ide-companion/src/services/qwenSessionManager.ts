...
import { getProjectHash } from '@atus-code/atus-code-core';
import { getRuntimeBaseDir } from '../utils/paths.js';
import type { AtusSession } from './atusSessionReader.js';
 ...
 */
export class AtusSessionManager {
  /**
  ...
  async loadSession(
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        console.log(`[AtusSessionManager] Session file not found: ${filePath}`);
        return null;
      }
      ...
      console.log(`[AtusSessionManager] Session loaded: ${filePath}`);
      return session;
    } catch (error) {
      console.error('[AtusSessionManager] Failed to load session:', error);
      return null;
    }
  }
   ...
   */
  async listSessions(workingDir: string): Promise<AtusSession[]> {
    try {
      const sessionDir = this.getSessionDir(workingDir);
      ...
      for (const file of files) {
        try {
          ...
          console.error(
            `[AtusSessionManager] Failed to read session file ${file}:`,
            error,
          );
        }
      }
      ...
      console.error('[AtusSessionManager] Failed to list sessions:', error);
      return [];
    }
  }
  ...
  async deleteSession(sessionId: string, workingDir: string): Promise<boolean> {
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        console.log(`[AtusSessionManager] Session deleted: ${filePath}`);
        return true;
      }

      return false;
    } catch (error) {
      console.error('[AtusSessionManager] Failed to delete session:', error);
      return false;
    }
  }
}
