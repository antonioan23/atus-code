...
export interface AtusSession {
  sessionId: string;
  projectHash: string;
  startTime: string;
  lastUpdated: string;
  messages: AtusMessage[];
  filePath?: string;
  messageCount?: number;
...
export class AtusSessionReader {
    ...
    try {
      ...
      if (!allProjects && workingDir) {
        ...
        if (!fs.existsSync(tmpDir)) {
          console.log('[AtusSessionReader] Tmp directory not found:', tmpDir);
          return [];
        }
      ...
      console.error('[AtusSessionReader] Failed to get sessions:', error);
      return [];
    }
  }
   ...
   */
  private async readSessionsFromDir(chatsDir: string): Promise<AtusSession[]> {
    const sessions: AtusSession[] = [];
    ...
    for (const file of jsonSessionFiles) {
      ...
      try {
        ...
        const session = JSON.parse(content) as AtusSession;
        session.filePath = filePath;
        sessions.push(session);
      } catch (error) {
        console.error(
          '[AtusSessionReader] Failed to read session file:',
          filePath,
          error,
        );
      }
    ...
    for (const file of jsonlSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[AtusSessionReader] Failed to read JSONL session file:',
          filePath,
          error,
        );
      }
   ...
   */
  getSessionTitle(session: AtusSession): string {
  ...
  private async readJsonlSession(
    ...
    try {
      ...
      console.error(
        '[AtusSessionReader] Failed to parse JSONL session:',
        error,
      );
      return null;
    }
  ...
  async deleteSession(sessionId: string, workingDir: string): Promise<boolean> {
    try {
      ...
      console.error('[AtusSessionReader] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(
    ...
    try {
      ...
      console.error('[AtusSessionReader] Failed to rename session:', error);
      return false;
    }
  }
}
