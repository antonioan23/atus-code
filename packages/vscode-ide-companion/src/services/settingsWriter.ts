 ...
 * Handles bidirectional sync between VSCode Settings and ~/.atus-code/settings.json.
 */
...
/**
 * Mode for ~/.atus-code/settings.json — owner-only read/write. The unified install
...
/** Directory mode for ~/.atus-code — owner-only. */
...
/**
 * Values extracted from ~/.atus-code/settings.json for populating VSCode Settings.
 */
export interface AtusSettingsForVSCode {
 ...
 * so that `JSON.parse` can handle ~/.atus-code/settings.json files that
...
/**
 * Read ~/.atus-code/settings.json. Returns {} if the file is missing.
...
function readSettings(): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(
      `Cannot parse ~/.atus-code/settings.json (${message}). ` +
...
/**
 * Write ~/.atus-code/settings.json atomically (temp file + rename), creating the
...
function writeSettings(settings: Record<string, unknown>): void {
  ...
  try {
    ...
    // target (EPERM/EBUSY). The temp file otherwise lingers in ~/.atus-code
...
// ---------------------------------------------------------------------------
// Write: VSCode Settings → ~/.atus-code/settings.json
// ---------------------------------------------------------------------------

/**
 * Write Coding Plan configuration to ~/.atus-code/settings.json.
...
/**
 * Write model providers from VSCode Settings (key-value map) to ~/.atus-code/settings.json.
...
/**
 * Create a ProviderSettingsAdapter backed by ~/.atus-code/settings.json.
...
/**
 * Apply a ProviderInstallPlan to ~/.atus-code/settings.json.
...
// ---------------------------------------------------------------------------
// Read: ~/.atus-code/settings.json → VSCode Settings
// ---------------------------------------------------------------------------

/**
 * Read ~/.atus-code/settings.json and extract values for VSCode Settings UI.
 ...
 */
export function readQwenSettingsForVSCode(): AtusSettingsForVSCode | null {
...
/**
 * Clear persisted auth credentials from ~/.atus-code/settings.json.
...
export function clearPersistedAuth(): void {
  try {
    ...
    // (ATUS_CUSTOM_API_KEY_*) keys would linger on disk after the user
