...
import { AtusSessionReader, type AtusSession } from './atusSessionReader.js';
import { AtusSessionManager } from './atusSessionManager.js';
import type {
  ChatMessage,
  PlanEntry,
  ToolCallUpdateData,
...
import {
  AtusConnectionHandler,
  type AtusConnectionResult,
} from '../services/atusConnectionHandler.js';
import { AtusSessionUpdateHandler } from './atusSessionUpdateHandler.js';
...
export class AtusAgentManager {
  private connection: AcpConnection;
  private sessionReader: AtusSessionReader;
  private sessionManager: AtusSessionManager;
  private connectionHandler: AtusConnectionHandler;
  private sessionUpdateHandler: AtusSessionUpdateHandler;
  ...
  // Callback storage
  private callbacks: AtusAgentCallbacks = {};
  ...
  constructor() {
    ...
    this.sessionReader = new AtusSessionReader();
    this.sessionManager = new AtusSessionManager();
    this.connectionHandler = new AtusConnectionHandler();
    this.sessionUpdateHandler = new AtusSessionUpdateHandler({});
    ...
    this.connection.onSessionUpdate = (data: SessionNotification) => {
      ...
      try {
        ...
        console.warn('[AtusAgentManager] Rehydration routing failed:', err);
      }
    ...
    this.connection.onEndTurn = (reason?: string) => {
      try {
        ...
        console.warn('[AtusAgentManager] onEndTurn callback error:', err);
      }
    };
    ...
    this.connection.onAuthenticateUpdate = (
      ...
      try {
        ...
        console.warn(
          '[AtusAgentManager] onAuthenticateUpdate callback error:',
          err,
        );
      }
    };
    ...
    this.connection.onInitialized = (init: unknown) => {
      try {
        ...
        console.warn('[AtusAgentManager] onInitialized parse error:', err);
      }
    };
    ...
    this.connection.onDisconnected = (
      ...
      console.log(
        `[AtusAgentManager] Process disconnected (code: ${code}, signal: ${signal})`,
      );
  ...
  async connect(
    ...
    if (res.availableModels && res.availableModels.length > 0) {
      ...
      console.log(
        '[AtusAgentManager] Emitting availableModels from connect():',
  ...
  async reconnect(
    ...
    console.log('[AtusAgentManager] Attempting reconnection...');
    try {
  ...
  async setApprovalModeFromUi(
    ...
    try {
      ...
      console.error('[AtusAgentManager] Failed to set mode:', err);
      throw err;
    }
  }
  ...
  async setModelFromUi(modelId: string): Promise<ModelInfo | null> {
    try {
      ...
      console.error('[AtusAgentManager] Failed to set model:', err);
      throw err;
    }
  }
  ...
  async validateCurrentSession(): Promise<boolean> {
    try {
      ...
      console.warn('[AtusAgentManager] Session validation failed:', error);
  ...
  async getSessionList(): Promise<Array<Record<string, unknown>>> {
    console.log(
      '[AtusAgentManager] Getting session list with version-aware strategy',
    );

    try {
      console.log(
        '[AtusAgentManager] Attempting to get session list via ACP method',
      );
      const response = await this.connection.listSessions();
      console.log('[AtusAgentManager] ACP session list response:', response);

      const res: unknown = response;
      const items = extractSessionListItems(res);

      console.log(
        '[AtusAgentManager] Sessions retrieved via ACP:',
        res,
        items.length,
      );
      if (items.length > 0) {
      ...
      console.warn(
        '[AtusAgentManager] ACP session list failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log('[AtusAgentManager] Getting session list from file system');
      const sessions = await this.sessionReader.getAllSessions(undefined, true);
      console.log(
        '[AtusAgentManager] Session list from file system (all projects):',
        sessions.length,
      );

      const result = sessions.map(
        (session: AtusSession): Record<string, unknown> => ({
      ...
      console.log(
        '[AtusAgentManager] Sessions retrieved from file system:',
        result.length,
      );
      return result;
    } catch (error) {
      console.error(
        '[AtusAgentManager] Failed to get session list from file system:',
        error,
      );
      return [];
    }
  ...
  async getSessionListPaged(params?: {
    ...
    try {
      ...
      console.warn('[AtusAgentManager] Paged ACP session list failed:', error);
    ...
    try {
      ...
      console.error('[AtusAgentManager] File system paged list failed:', error);
  ...
  async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
    try {
      try {
        ...
        console.log(
          '[AtusAgentManager] Session list item for filePath lookup:',
          item,
        );
        if (
          typeof item === 'object' &&
        ...
        console.warn('[AtusAgentManager] JSONL read path lookup failed:', e);
      }
      ...
      console.error(
        '[AtusAgentManager] Failed to get session messages:',
        error,
      );
      return [];
    }
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    try {
      ...
      console.error('[AtusAgentManager] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(sessionId: string, title: string): Promise<boolean> {
    try {
      ...
      console.error('[AtusAgentManager] Failed to rename session:', error);
      return false;
    }
  }
  ...
  private async readJsonlMessages(filePath: string): Promise<ChatMessage[]> {
    ...
    try {
      ...
      console.log(
        '[AtusAgentManager] JSONL records read:',
        records.length,
        filePath,
      );
      ...
      console.log(
        '[AtusAgentManager] JSONL messages reconstructed:',
        msgs.length,
      );
      return msgs;
    } catch (err) {
      console.warn('[AtusAgentManager] Failed to read JSONL messages:', err);
      return [];
    }
  }
  ...
  async loadSessionViaAcp(
    ...
    try {
      ...
      console.log(
        '[AtusAgentManager] Rehydration start for session:',
        sessionId,
      );
      console.log(
        '[AtusAgentManager] Attempting session/load via ACP for session:',
        sessionId,
      );
      ...
      console.log(
        '[AtusAgentManager] Session load succeeded. Response:',
      ...
      console.error(
        '[AtusAgentManager] Session load via ACP failed for session:',
        sessionId,
      );
      console.error('[AtusAgentManager] Error type:', error?.constructor?.name);
      console.error('[AtusAgentManager] Error message:', errorMessage);
      ...
      if (error && typeof error === 'object') {
        ...
        if ('error' in error) {
          ...
          if (acpError.error) {
            console.error(
              '[AtusAgentManager] ACP error code:',
              acpError.error.code,
            );
            console.error(
              '[AtusAgentManager] ACP error message:',
              acpError.error.message,
            );
          }
        } else {
          console.error('[AtusAgentManager] Non-ACPIf error details:', error);
        }
      }
      ...
      console.log('[AtusAgentManager] Rehydration end for session:', sessionId);
  ...
  async loadSession(sessionId: string): Promise<ChatMessage[] | null> {
    console.log(
      '[AtusAgentManager] Loading session with version-aware strategy:',
      sessionId,
    );

    try {
      console.log(
        '[AtusAgentManager] Attempting to load session via ACP method',
      );
      await this.loadSessionViaAcp(sessionId);
      console.log('[AtusAgentManager] Session loaded successfully via ACP');
      ...
      console.warn(
        '[AtusAgentManager] ACP session load failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log(
        '[AtusAgentManager] Loading session messages from file system',
      );
      const messages = await this.loadSessionMessagesFromFile(sessionId);
      console.log(
        '[AtusAgentManager] Session messages loaded successfully from file system',
      );
      return messages;
    } catch (error) {
      console.error(
        '[AtusAgentManager] Failed to load session messages from file system:',
        error,
      );
      return null;
    }
  ...
  private async loadSessionMessagesFromFile(
    ...
    try {
      console.log(
        '[AtusAgentManager] Loading session from file system:',
        sessionId,
      );
      ...
      if (!session) {
        console.log(
          '[AtusAgentManager] Session not found in file system:',
          sessionId,
        );
        return null;
      }
      ...
      console.error(
        '[AtusAgentManager] Session load from file system failed:',
        error,
      );
      throw error;
    }
  ...
  async createNewSession(
    ...
    if (!forceNew && this.connection.currentSessionId) {
      console.log(
        '[AtusAgentManager] createNewSession: reusing existing session',
    ...
    if (this.sessionCreateInFlight) {
      console.log(
        '[AtusAgentManager] createNewSession: session creation already in flight',
      );
      if (!forceNew) {
    ...
    console.log('[AtusAgentManager] Creating new session...');
    ...
    this.sessionCreateInFlight = (async () => {
      try {
        ...
        try {
          ...
          console.log(
            '[AtusAgentManager] newSession returned:',
          ...
          if (requiresAuth) {
            if (!autoAuthenticate) {
              console.warn(
                '[AtusAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.',
              );
              throw err;
            }
            console.warn(
              '[AtusAgentManager] session/new requires authentication. Retrying with authenticate...',
            );
            try {
              // Let CLI handle authentication - it's the single source of truth
              await this.connection.authenticate(authMethod);
              console.log(
                '[AtusAgentManager] createNewSession Authentication successful. Retrying session/new...',
              );
              ...
              console.error(
                '[AtusAgentManager] Re-authentication failed:',
                reauthErr,
              );
              throw reauthErr;
            }
        ...
        console.log(
          '[AtusAgentManager] New session created with ID:',
          newSessionId,
        );
        return newSessionId;
      } finally {
  ...
  async cancelCurrentPrompt(): Promise<void> {
    console.log('[AtusAgentManager] Cancelling current prompt');
