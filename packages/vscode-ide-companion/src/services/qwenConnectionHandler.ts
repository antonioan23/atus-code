...
export interface AtusConnectionResult {
 ...
 */
export class AtusConnectionHandler {
  /**
  ...
  async connect(
    ...
    console.log(`[AtusAgentManager] 🚀 CONNECT() CALLED - ID: ${connectId}`);
    ...
    if (proxyUrl) {
      ...
      console.log(
        '[AtusAgentManager] Using proxy from VSCode settings:',
        proxyUrl,
      );
    }
    ...
    for (let attempt = 1; attempt <= maxConnectAttempts; attempt++) {
      try {
        console.log(
          `[AtusAgentManager] Connecting to ACP process (attempt ${attempt}/${maxConnectAttempts})...`,
        );
        await connection.connect(cliEntryPath!, workingDir, extraArgs);
        console.log('[AtusAgentManager] ACP process connected successfully');
        break;
      } catch (connectError) {
        console.error(
          `[AtusAgentManager] Connect attempt ${attempt} failed:`,
        ...
        console.log(`[AtusAgentManager] Retrying connect in ${delay}ms...`);
    ...
    if (!sessionRestored) {
      console.log(
        '[AtusAgentManager] no sessionRestored, Creating new session...',
      );

      try {
        console.log(
          '[AtusAgentManager] Creating new session (letting CLI handle authentication)...',
        );
        ...
        if (
          ...
          console.log(
            '[AtusAgentManager] Extracted availableModels from session/new:',
        ...
        console.log('[AtusAgentManager] New session created successfully');
        sessionCreated = true;
      } catch (sessionError) {
        const needsAuth =
        ...
        if (needsAuth) {
          requiresAuth = true;
          console.log(
            '[AtusAgentManager] Session creation requires authentication; waiting for user-triggered login.',
          );
        } else {
          console.log(
            `\n⚠️ [SESSION FAILED] newSessionWithRetry threw error\n`,
          );
          console.log(`[AtusAgentManager] Error details:`, sessionError);
          throw sessionError;
        }
      }
    } else {
    ...
    console.log(`\n========================================`);
    console.log(`[AtusAgentManager] ✅ CONNECT() COMPLETED SUCCESSFULLY`);
    console.log(`========================================\n`);
    return {
      sessionCreated,
      requiresAuth,
  ...
  private async newSessionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `[AtusAgentManager] Creating session (attempt ${attempt}/${maxRetries})...`,
        );
        const res = await connection.newSession(workingDir);
        console.log('[AtusAgentManager] Session created successfully');
        return res;
      } catch (error) {
        lastError = error;
        const errorMessage = getErrorMessage(error);
        console.error(
          `[AtusAgentManager] Session creation attempt ${attempt} failed:`,
          errorMessage,
        );
        ...
        if (requiresAuth) {
          if (!autoAuthenticate) {
            console.log(
              '[AtusAgentManager] Authentication required but auto-authentication is disabled. Propagating error.',
            );
            throw error;
          }
          console.log(
            '[AtusAgentManager] Qwen requires authentication. Authenticating and retrying session/new...',
          );
          try {
            ...
            console.log(
              '[AtusAgentManager] newSessionWithRetry Authentication successful',
            );
            // Retry immediately after successful auth
            const res = await connection.newSession(workingDir);
            console.log(
              '[AtusAgentManager] Session created successfully after auth',
            );
            return res;
          } catch (authErr) {
            console.error(
        ...
        console.log(`[AtusAgentManager] Retrying in ${delay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
