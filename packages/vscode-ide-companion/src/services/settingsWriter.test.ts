...
import { AuthType, type ProviderInstallPlan } from '@atus-code/atus-code-core';
...
describe('settingsWriter', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-vscode-settings-'));
    settingsPath = path.join(tempDir, '.atus-code', 'settings.json');
    ...
    it('wipes preset, custom, and subscription-plan env keys without touching unrelated env', () => {
      ...
      const initial = {
        env: {
          ...
          ATUS_CUSTOM_API_KEY_OPENAI_HTTPS_API_FOO_COM_ABC123DEF456:
            'sk-custom-1',
          ATUS_CUSTOM_API_KEY_ANTHROPIC_HTTPS_API_BAR_COM_DEAD0BEEF000:
            'sk-custom-2',
      ...
      const after = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
      // Every preset + subscription + OPENAI + every ATUS_CUSTOM_API_KEY_*
