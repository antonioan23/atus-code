  ...
  private async setupChildProcessHandlers(): Promise<void> {
    ...
    this.child!.stderr?.on('data', (data: Buffer) => {
      ...
      if (
        ...
        console.error(`[ACP atus]:`, message);
      } else {
        console.log(`[ACP atus]:`, message);
      }
    });
    ...
    this.child!.on('exit', (code: number | null, signal: string | null) => {
      console.error(
        `[ACP atus] Process exited with code: ${code}, signal: ${signal}`,
      );
    ...
    this.sdkConnection = new ClientSideConnection(
      (_agent: Agent): Client => ({
        ...
        extNotification: async (
          ...
          } else if (method === '_atuscode/slash_command') {
  ...
  async listSessions(options?: {
    ...
    try {
      ...
      if (options?.size !== undefined) {
        ...
        // other atus code ACP extensions.
