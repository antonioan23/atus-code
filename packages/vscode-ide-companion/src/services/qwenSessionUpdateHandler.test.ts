...
import type { AtusAgentCallbacks } from '../types/chatTypes.js';

describe('AtusSessionUpdateHandler', () => {
  let handler: AtusSessionUpdateHandler;
  let mockCallbacks: AtusAgentCallbacks;
  ...
  beforeEach(() => {
    ...
    handler = new AtusSessionUpdateHandler(mockCallbacks);
  });
  ...
  describe('plan handling', () => {
    ...
    it('falls back to stream chunk when onPlan is not set', () => {
      const handlerWithStream = new AtusSessionUpdateHandler({
        onStreamChunk: vi.fn(),
      });
  ...
  describe('available_commands_update handling', () => {
    ...
    it('does not call callback when onAvailableCommands is not set', () => {
      const handlerWithoutCallback = new AtusSessionUpdateHandler({});
