...
const CLI_IDE_COMPANION_IDENTIFIER = 'atuscode.atus-code-vscode-ide-companion';
const INFO_MESSAGE_SHOWN_KEY = 'atusCodeInfoMessageShown';
export const DIFF_SCHEME = 'atus-diff';
  ...
  try {
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Atus Code Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Atus Code Companion');
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer('atusCode.chat', {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('atus-code.copyMessage', () =>
      sendCopyToActive('copyMessage'),
    ),
    vscode.commands.registerCommand('atus-code.copyAllMessages', () =>
      sendCopyToActive('copyAllMessages'),
    ),
    vscode.commands.registerCommand('atus-code.copyLastReply', () =>
  ...
  context.subscriptions.push(
    ...
    (vscode.commands.registerCommand('atus.diff.accept', (uri?: vscode.Uri) => {
    ...
    vscode.commands.registerCommand('atus.diff.cancel', (uri?: vscode.Uri) => {
      const docUri = uri ?? vscode.window.activeTextEditor?.document.uri;
      if (docUri && docUri.scheme === DIFF_SCHEME) {
    ...
    vscode.commands.registerCommand('atus.diff.closeAll', async () => {
      try {
        await diffManager.closeAll();
      } catch (err) {
        console.warn('[Extension] atus.diff.closeAll failed:', err);
      }
    }),
    vscode.commands.registerCommand('atus.diff.suppressBriefly', async () => {
      try {
        diffManager.suppressFor(1200);
      } catch (err) {
        console.warn('[Extension] atus.diff.suppressBriefly failed:', err);
      }
    }),
  );
  ...
  if (!context.globalState.get(INFO_MESSAGE_SHOWN_KEY) && infoMessageEnabled) {
    void vscode.window.showInformationMessage(
      'Atus Code Companion extension successfully installed.',
    );
      ...
      async (
        ...
        if (!workspaceFolders || workspaceFolders.length === 0) {
          vscode.window.showInformationMessage(
            'No folder open. Please open a folder to run Atus Code.',
          );
          return;
        }
        ...
        if (workspaceFolders.length === 1) {
          ...
          selectedFolder = await vscode.window.showWorkspaceFolderPick({
            placeHolder: 'Select a folder to run Atus Code in',
          });
        }
        ...
        if (selectedFolder) {
          ...
          const terminalOptions: vscode.TerminalOptions = {
            name: `Atus Code (${selectedFolder.name})`,
          ...
          if (isWindows) {
            ...
            atusCmd = `ELECTRON_RUN_AS_NODE=1 ${baseCmd}`;
          }

          const terminal = vscode.window.createTerminal(terminalOptions);
          terminal.show();
          terminal.sendText(atusCmd);
        }
      },
    ),
    vscode.commands.registerCommand('atus-code.showNotices', async () => {
