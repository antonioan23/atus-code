...
import type { ChatMessage } from '../../services/atusAgentManager.js';
...
import { stripZeroWidthSpaces } from '@atus-code/webui';
import {
  exportSessionToFile,
...
export class SessionMessageHandler extends BaseMessageHandler {
        ...
        try {
          const modelId =
            typeof data?.modelId === 'string' && data.modelId.trim().length > 0
              ? data.modelId.trim()
              : undefined;
          await vscode.commands.executeCommand('atusCode.openNewChatTab', {
  ...
  private async promptAuth(message: string): Promise<boolean> {
    ...
    if (result === 'Configure') {
      if (this.authHandler) {
        ...
        await vscode.commands.executeCommand('atus-code.auth');
      }
      return true;
    }
    return false;
    ...
    if (editTargetTurnIndex !== undefined) {
      ...
      if (!this.agentManager.isConnected) {
        await this.promptAuth(
          'You need to configure your provider to use atus code.',
        );
        return;
      }
      ...
      if (!this.agentManager.currentSessionId) {
        try {
          ...
          if (this.shouldPromptAuth(createErr)) {
            await this.promptAuth(
              'Your session has expired or is invalid. Please configure your provider to continue using atus code.',
            );
            return;
          }
  ...
  private async handleRenameQwenSession(
    ...
    try {
      ...
      // Matches SESSION_TITLE_MAX_LENGTH from @atus-code/atus-code-core/sessionService
