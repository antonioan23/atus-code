...
describe('parseAcpModelId', () => {
  ...
  it('marks runtime snapshots and still strips the trailing wrapper', () => {
    expect(
      parseAcpModelId('$runtime|atus-oauth|qwen3-coder-plus(atus-oauth)'),
    ).toEqual({
      baseModelId: '$runtime|atus-oauth|qwen3-coder-plus',
      authType: 'atus-oauth',
      isRuntime: true,
    });
  });
...
describe('isDiscontinuedModel', () => {
  it('flags a non-runtime Qwen OAuth registry entry as discontinued', () => {
    expect(isDiscontinuedModel('qwen3-coder-plus(atus-oauth)')).toBe(true);
  });

  it('does NOT flag a runtime Qwen OAuth snapshot as discontinued', () => {
    expect(
      isDiscontinuedModel('$runtime|atus-oauth|qwen3-coder-plus(atus-oauth)'),
    ).toBe(false);
  });
...
describe('ATUS_OAUTH_AUTH_TYPE', () => {
  it('matches the encoded value used by the ACP server', () => {
    expect(ATUS_OAUTH_AUTH_TYPE).toBe('atus-oauth');
  });
});
