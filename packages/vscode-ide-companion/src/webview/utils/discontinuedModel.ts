 ...
 * e.g. `qwen3-coder-plus(atus-oauth)`. Runtime model snapshots are additionally
...
export const ATUS_OAUTH_AUTH_TYPE = 'atus-oauth';
...
export function isDiscontinuedModel(modelId: string): boolean {
  ...
  return parsed.authType === ATUS_OAUTH_AUTH_TYPE && !parsed.isRuntime;
}
