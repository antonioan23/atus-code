...
import { AtusAgentManager } from '../../services/atusAgentManager.js';
...
export function resolveQwenCliEntryPath(
  ...
  return vscode.Uri.joinPath(extensionUri, 'dist', 'atus-cli', 'cli.js').fsPath;
}
...
export class WebViewProvider {
  ...
  private agentManager: AtusAgentManager;
  ...
  constructor(
    ...
    this.agentManager = new AtusAgentManager();
    ...
    const configChangeDisposable = vscode.workspace.onDidChangeConfiguration(
      async (e) => {
        ...
        if (authSettingsChanged && !this.isSyncingToVSCode) {
          console.log(
            '[WebViewProvider] Auth-related atus-code settings changed by user, syncing...',
          );
          ...
          if (synced && this.agentInitialized) {
            ...
            e.affectsConfiguration('atus-code.apiKey')
          ) {
            // Only de-auth when atus-code.apiKey itself was cleared.
    ...
    // Note: Tool call updates are handled in handleSessionUpdate within AtusAgentManager
      ...
      async (request: RequestPermissionRequest) => {
        ...
        return new Promise((resolve) => {
          ...
          const handler = (message: PermissionResponseMessage) => {
            ...
            // Always close open atus-diff editors after any permission decision
            void vscode.commands.executeCommand('atus.diff.closeAll');
            ...
            if (isCancel) {
              ...
              void vscode.commands.executeCommand('atus.diff.suppressBriefly');
            }
          };
    ...
    newPanel.webview.onDidReceiveMessage(
      async (message: { type: string; data?: unknown }) => {
        ...
        if (message.type === 'updatePanelTitle') {
          ...
          if (panelRef) {
            panelRef.title = title ? truncatePanelTitle(title) : 'atus code';
          }
          return;
        }
        if (this.handleNewChatByContext(message)) {
  ...
  /**
   * Sync VSCode extension settings (atus-code.*) to ~/.atus-code/settings.json
  ...
  private async syncVSCodeSettingsToQwenConfig(): Promise<boolean> {
    const config = vscode.workspace.getConfiguration('atus-code');
    ...
    try {
      ...
      console.log(
        `[WebViewProvider] Synced VSCode settings → ~/.atus-code/settings.json (provider=${provider})`,
      );
      return true;
    } catch (error) {
  ...
  /**
   * Sync ~/.atus-code/settings.json values back to VSCode Settings UI.
  ...
  private async syncQwenConfigToVSCodeSettings(): Promise<void> {
    try {
      const atusSettings = readQwenSettingsForVSCode();
      if (!atusSettings) {
        return;
      }

      console.log(
        '[WebViewProvider] Syncing ~/.atus-code/settings.json → VSCode settings',
      );
      ...
      if (
        config.get<string>('provider', 'coding-plan') !== atusSettings.provider
      ) {
        updates.push(config.update('provider', atusSettings.provider, target));
      }
      if (
        config.get<'china' | 'global'>('codingPlanRegion', 'china') !==
        atusSettings.codingPlanRegion
      ) {
        updates.push(
          config.update(
            'codingPlanRegion',
      ...
      if (updates.length === 0) {
        console.log(
          '[WebViewProvider] VSCode settings already match ~/.atus-code/settings.json',
        );
        return;
      }
      ...
      console.error(
        '[WebViewProvider] Failed to sync atus config to VSCode settings:',
        error,
      );
    }
  }
  ...
  /**
   * Attempt to restore authentication state and initialize connection.
   * On startup, sync ~/.atus-code/settings.json → VSCode settings so the Settings UI
   * reflects existing non-secret CLI config, then attempt a connection.
   * Writing back to ~/.atus-code/settings.json happens through the auth flow and
   ...
   * Writes provider config to ~/.atus-code/settings.json and reconnects.
   * Mirrors the CLI's `atus auth coding-plan` / `atus auth` flow.
   */
  private async handleAuthInteractive(
    providerConfig: import('@atus-code/atus-code-core').ProviderConfig,
    inputs: import('@atus-code/atus-code-core').ProviderSetupInputs,
  ): Promise<void> {
    if (!inputs.apiKey) {
      this.sendMessageToWebView({
        type: 'authError',
   ...
   * the atus code panel (or sidebar view) when clicked.
   */
  private notifyUser(message: string): void {
    void vscode.window
      .showInformationMessage(`atus code: ${message}`, 'Show')
      .then((action) => {
        if (action === 'Show') {
          const panel = this.panelManager.getPanel();
          if (panel) {
            ...
            void vscode.commands.executeCommand('atus-code.focusChat');
          }
        }
      });
  ...
  /**
   * Whether the user can currently see the atus code panel.
  ...
  /** Whether the atus-code.notifications setting is enabled. */
