...
export class PanelManager {
  ...
  async createPanel(): Promise<boolean> {
    ...
    // First, check if there's an existing atus code group
    const existingGroup = this.findExistingAtusCodeGroup();

    if (existingGroup) {
      // If atus code webview already exists in a locked group, create the new panel in that same group
      console.log(
        '[PanelManager] Found existing atus code group, creating panel in same group',
      );
      ...
      // If no existing atus code group, create a new group to the right of the active editor group
      try {
  ...
  /**
   * Find the group and view column where the existing atus code webview is located
   * @returns The found group and view column, or undefined if not found
   */
  private findExistingAtusCodeGroup():
    ...
    for (const group of vscode.window.tabGroups.all) {
      for (const tab of group.tabs) {
        ...
        if (
          ...
          input.viewType === 'mainThreadWebview-atusCode.chat'
        ) {
          // Found an existing atus code tab
          console.log('[PanelManager] Found existing atus code group:', {
  ...
  captureTab(): void {
    ...
    setTimeout(() => {
      ...
      const match = allTabs.find((t) => {
        ...
        const sameViewType = isWebview && input.viewType === 'atusCode.chat';
