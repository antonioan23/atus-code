...
vi.mock('@atus-code/atus-code-core', async () => {
  const actual = await vi.importActual<
    typeof import('@atus-code/atus-code-core')
...
vi.mock('../../services/atusAgentManager.js', () => ({
  AtusAgentManager: class {
    isConnected = false;
...
describe('resolveQwenCliEntryPath', () => {
  it('uses the source dev entry when the extension runs in development mode', () => {
    const tempRoot = mkdtempSync(path.join(tmpdir(), 'atus-vscode-dev-'));
    try {
  ...
  it('uses the bundled CLI outside development mode', () => {
    ...
    ).toBe('/extension-root/dist/atus-cli/cli.js');
  });
});
...
describe('WebViewProvider settings sync', () => {
  ...
  it('only syncs non-secret VS Code settings from ~/.atus-code/settings.json', async () => {
  ...
  it('ignores non-auth atus-code setting changes', async () => {
    const provider = new WebViewProvider(
      { subscriptions: [] } as never,
      { fsPath: '/extension-root' } as never,
    );
    ...
    await configChangeHandler?.(createConfigChangeEvent('atus-code'));

    expect(syncSpy).not.toHaveBeenCalled();
  });

  it('reacts to auth-related atus-code setting changes', async () => {
    const provider = new WebViewProvider(
      { subscriptions: [] } as never,
      { fsPath: '/extension-root' } as never,
    );
    ...
    await configChangeHandler?.(
      createConfigChangeEvent('atus-code', 'atus-code.apiKey'),
    );
  ...
  it('does not de-auth when non-apiKey auth settings change on an api-key provider', async () => {
    ...
    await configChangeHandler?.(
      createConfigChangeEvent('atus-code', 'atus-code.codingPlanRegion'),
    );
...
describe('Notification & dot indicator', () => {
  ...
  it('shows orange dot and notification when a long task completes while panel is not active', async () => {
    ...
    expect(mockShowInformationMessage).toHaveBeenCalledWith(
      'atus code: Waiting for your input.',
      'Show',
    );
  });
  ...
  it('shows blue dot and notification for permission requests when panel is not active', async () => {
    ...
    expect(mockShowInformationMessage).toHaveBeenCalledWith(
      'atus code: Needs your permission to use Bash.',
      'Show',
    );
  });
