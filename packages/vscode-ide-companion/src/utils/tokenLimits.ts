...
/**
 * Browser-safe subset of @atus-code/atus-code-core tokenLimits.
 *
...
function normalize(model: string): string {
  ...
  if (
    !s.match(/^atus-(?:plus|flash|vl-max)-latest$/) &&
    !s.match(/^kimi-k2-\d{4}$/)
  ) {
    s = s.replace(
...
const INPUT_PATTERNS: Array<[RegExp, TokenCount]> = [
  ...
  [/^qwen3\.\d/, LIMITS['1m']],
  [/^atus-plus-latest$/, LIMITS['1m']],
  [/^atus-flash-latest$/, LIMITS['1m']],
 ...
 * `@atus-code/atus-code-core`. The webview only calls this as a fallback
