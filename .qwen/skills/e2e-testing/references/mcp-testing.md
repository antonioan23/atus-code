...
MCP servers are configured in `.atus-code/settings.json` under `mcpServers`. This is
the **only** location that works for E2E testing.

Common mistakes that waste time:

- `.mcp.json` — Claude Code convention, not atus code
...
mkdir -p .atus-code
cat > .atus-code/settings.json << 'EOF'
{
  "mcpServers": {
    "my-server": {
      "command": "node",
...
- You're not running from the directory containing `.atus-code/settings.json`
