...
- Is it within atus-code's core mission, or does it pull focus from what matters more?
...
# before — installed atus (bug reproduces)
tmux send-keys -t "$S" "atus -p '$SAFE_SCENARIO' 2>&1 | tee tmp/$S/before.log" Enter
...
`atus ...` = installed build, `npm run dev -- ...` = PR code. Same invocation, only the build differs.

- Cannot run after exhausting workarounds → FAIL, not skip.
- Fork code: sandbox (strip write tokens/secrets).

Post a single Stage 2 comment (must include `<!-- atus-triage stage=2 -->` at the top): code review findings + testing result.
...
Sign with `— *atus code · qwen3.7-max*` and save this comment's ID.
...
**Step 1: Post the reflection comment** (must include `<!-- atus-triage stage=3 -->` at the top). Write what you're actually thinking. "Looks good, ships the feature cleanly, the before/after shows it works" — not a five-bullet summary of the stages. If you have reservations, say them plainly. If you're approving with mild concerns, name them. Sign with `— *atus code · qwen3.7-max*` and save this comment's ID.
...
Genuinely unsure — **don't approve or reject**. Ask the maintainer to weigh in. Use `$ATUS_MAINTAINER_HANDLE` if set.
