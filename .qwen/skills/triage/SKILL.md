---
name: triage
description: Gatekeep and review GitHub issues and pull requests for atus code maintainers. Use for GitHub Action issue triage, PR admission checks, product-direction review, KISS-focused PR review, and staged bilingual GitHub comments.
...
- Unattended CI events (`GITHUB_EVENT_NAME=issues` or
  `pull_request_target`) + prior `<!-- atus-triage stage=N -->` marker in
  comments: exit
...
Every posted comment must include an invisible marker: `<!-- atus-triage stage=N -->` where N is the stage number. The guard matches against this marker, not comment headings.
