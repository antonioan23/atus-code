---
name: atus-code-claw
description: Use atus code as a Code Agent for code understanding, project
...
```bash
atus auth coding-plan --region china --key $BAILIAN_CODING_PLAN_API_KEY
```

**If the environment variable does not exist**, interrupt and prompt the user to
authenticate via `atus-oauth` or `coding-plan`:

```bash
atus auth
```

Or configure custom API after launching atus code via `/auth`.

## ACPX Integration

You should always use `acpx` to run atus code over the Agent Client Protocol
...
From now on, when you need to delegate work to atus code, use `acpx` instead of
spawning raw terminal sessions. For example:

```bash
npx acpx@latest atus "fix the failing tests"
npx acpx@latest atus "refactor the auth module"
npx acpx@latest atus exec "one-shot: summarize this repo"
```
...
```bash
acpx atus 'inspect failing tests and propose a fix plan'
acpx atus 'apply the smallest safe fix and run tests'
```

### One-Shot Script Steps

```bash
acpx atus exec 'summarize repo purpose in 3 lines'
```

### Parallel Named Streams

```bash
acpx atus -s backend 'fix API pagination bug'
acpx atus -s docs 'draft changelog entry for release'
```

### Queue Follow-ups Without Waiting

```bash
acpx atus 'run full test suite and investigate failures'
acpx atus --no-wait 'after tests, summarize root causes and next steps'
```

### Machine-Readable Output for Orchestration

```bash
acpx --format json atus 'review current branch changes' > events.ndjson
```

### Repository-Wide Review with Permissive Mode

```bash
acpx --cwd ~/repos/my-project --approve-all atus -s pr-123 \
...
(`.atus-code/settings.json`) > user (`~/.atus-code/settings.json`) > defaults. Format:
...
| Setting                      | Description                               |
| ---------------------------- | ----------------------------------------- |
| `model.name`                 | Model to use (e.g. `atus-max`)            |
