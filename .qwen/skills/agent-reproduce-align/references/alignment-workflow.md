...
The alignment phase starts after atus code has a candidate implementation. Use it to create a tight loop: run the selected reference agent and atus code, compare traces, patch the target, and rerun only the failing scenario.
...
2. Patch only the likely owner module in atus code.
