...
Use this skill when atus code already has a candidate implementation and needs evidence-based parity with a selected reference agent: `codex` or `claude-code`. The goal is not byte-for-byte equality; it is matching the observable contract that matters for the feature.
...
1. Re-state the parity target:
   ...
   - must-match fields
2. Run the reference agent and atus code in separate capture directories with the same scenario.
...
7. Patch atus code, rerun the smallest failing scenario, and repeat.
...
```sh
.atus-code/skills/agent-reproduce-align/scripts/normalize_trace.py \
...
```sh
.atus-code/skills/agent-reproduce-align/scripts/compare_traces.py \
  .repro-runs/reference/normalized.json \
  .repro-runs/atus/normalized.json
```
...
.atus-code/skills/agent-reproduce-align/scripts/run_pair_capture.sh \
...
- Stop when atus code passes the user-visible scenario and the remaining trace differences are documented as intentional.

## Done Criteria

- Reference-agent and atus code traces for the same scenario exist locally.
...
- atus code tests or smoke commands cover the fixed behavior.
- Any remaining mismatch is written down in the task notes or atus code docs when it affects users.
