...
Run atus code in a real tmux TUI session as a user would: navigate dialogs,
...
```bash
eval "$(bash .atus-code/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh \
...
```bash
bash .atus-code/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh help
```
...
bundle. Use the globally installed `atus` only when reproducing a user-reported
