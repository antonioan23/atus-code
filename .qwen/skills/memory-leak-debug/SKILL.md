...
description: Diagnose memory leaks in the atus code CLI using heap snapshots and
...
Diagnose memory leaks in the atus code Node.js CLI by capturing heap snapshots
...
```bash
HELPER=.atus-code/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh
eval "$(bash "$HELPER" start memleak . \
  env ATUS_CODE_NO_RELAUNCH=true NODE_OPTIONS=--heapsnapshot-signal=SIGUSR2 \
  npm run dev)"
...
- `ATUS_CODE_NO_RELAUNCH=true` prevents the CLI from spawning a child process,
...
```bash
NODE_PID=$(bash .atus-code/skills/memory-leak-debug/scripts/find-leaf-node.sh "<session-name>")
```

To profile the production bundle instead (e.g., verifying tree-shaking):
`npm run bundle` first, then use
`env ATUS_CODE_NO_RELAUNCH=true node --heapsnapshot-signal=SIGUSR2 dist/cli.js`
