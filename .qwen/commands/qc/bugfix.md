...
Create `.atus-code/issues/` if it doesn't exist, then pipe the issue directly into a
markdown file using `gh`:

```bash
mkdir -p .atus-code/issues
...
_Pending — to be filled by the test engineer._
' > .atus-code/issues/issue-<number>.md
```
...
`.atus-code/issues/issue-<number>.md` for the issue details, then assess and
...
`.atus-code/issues/issue-<number>.md`** to get the reproduction report. If the status
...
`test-engineer` agent again and tell it to read `.atus-code/issues/issue-<number>.md`
