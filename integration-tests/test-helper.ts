...
export class TestRig {
  ...
  getDefaultTimeout() {
    ...
    if (env['ATUS_SANDBOX']) return 30000; // 30s in containers
  ...
  setup(
    ...
    const atusDir = join(this.testDir, '.atus-code');
    mkdirSync(atusDir, { recursive: true });
    ...
    const settings = {
      ...
      sandbox: env.ATUS_SANDBOX !== 'false' ? env.ATUS_SANDBOX : false,
      ...options.settings, // Allow tests to override/add settings
    };
    writeFileSync(
      join(atusDir, 'settings.json'),
  ...
  /**
   * The command and args to use to invoke Atus Code CLI. Allows us to switch
   * between using the bundled gemini.js (the default) and using the installed
   * 'atus' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const command = isNpmReleaseTest ? 'atus' : 'node';
    ...
    return this.poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `atus-code.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'atus-code.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'atus-code.tool_call'
      ) {
  ...
  readLastApiRequest(): Record<string, unknown> | null {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === 'atus-code.api_request',
    );
      ...
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `atus-code.${metricName}`) {
              return metric;
            }
          }
        }
