...
class RunConfig:
    ...
    worktree_base: Path = field(default_factory=lambda: Path.home() / ".atus-code" / "worktrees")
...
class RunRecord:
    ...
    model: str
    status: RunStatus
    auth_type: Optional[str] = None  # e.g. "anthropic" for atus --auth-type
...
class GitWorktreeManager:
    ...
    async def collect_session_log(self, worktree_dir: Path, output_dir: Path) -> Optional[Tuple[Path, str, Path]]:
        ...
        ~/.atus-code/projects/{projectId}/chats/{sessionId}.jsonl
        ...
        atus_dir = Path.home() / ".atus-code"
        chats_dir = atus_dir / "projects" / project_id / "chats"
...
class AtusRunner:
    ...
    async def run(
        ...
        for prompt_index, prompt_text in enumerate(task.prompts, start=1):
            ...
            env["ATUS_CODE_ROOT"] = str(worktree_dir_resolved)
    ...
    def _build_command(self, run: RunRecord, prompt_text: str, use_continue: bool = False) -> List[str]:
        """Build the atus CLI command for a single prompt."""
        cmd = ["atus"]
...
def load_config(config_path: Path) -> RunConfig:
    ...
    return RunConfig(
        ...
        worktree_base=Path(data.get("worktree_base", "~/.atus-code/worktrees")).expanduser(),
...
async def execute_single_run(
    ...
    try:
        ...
        await atus_runner.run(run, worktree_dir, output_dir)
...
async def run_all(config: RunConfig, console: Console) -> ExecutionState:
    ...
    atus_runner = AtusRunner(config, console)
    ...
    async def execute_with_limit(run: RunRecord):
        async with semaphore:
            await execute_single_run(
                run, config, tracker, worktree_manager, atus_runner, console
            )
