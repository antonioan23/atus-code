...
/**
 * `atus serve` daemon — streaming / multi-client / recovery integration.
 *
 ...
 *
 *   1. Real `atus --acp` child crash → daemon publishes `session_died`,
...
import { DaemonClient, parseSseStream } from '@atus-code/sdk';
import type { DaemonEvent, DaemonSessionSummary } from '@atus-code/sdk';
...
// note in atus-serve-routes.test.ts for full rationale.
const CLI_BIN =
...
describeLLM('atus serve — child-crash recovery (real SIGKILL)', () => {
  it('publishes session_died after the atus --acp child is SIGKILL-ed', async () => {
    const session = await client.createOrAttachSession({
      workspaceCwd: REPO_ROOT,
    });

    // Find the daemon's direct `--acp` child PID.
    const childPids = execSync(`pgrep -P ${daemon.pid} -f "atus.*--acp"`, {
      encoding: 'utf8',
    })
      .trim()
      .split('\n')
...
describeLLM('atus serve — multi-client first-responder permission', () => {
  it('fans out permission_request to both subscribers; only one vote wins', async () => {
    ...
    const tmp = `/tmp/atus-serve-mc-${Date.now()}.txt`;
...
describeLLM('atus serve — Last-Event-ID resume', () => {
