...
(IS_SANDBOX ? describe.skip : describe)('acp integration', () => {
  ...
  it('returns internal error details when model auth is required', async () => {
    ...
    try {
      ...
      // Choose a atus-oauth model to trigger auth-required path deterministically.
      const atusOauthModel = newSession.models.availableModels.find((model) =>
        model.modelId.includes('atus-oauth'),
      );
      expect(atusOauthModel).toBeDefined();
      await expect(
        sendRequest('session/set_config_option', {
          sessionId: newSession.sessionId,
          configId: 'model',
          value: atusOauthModel!.modelId,
        }),
