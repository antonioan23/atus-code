...
/**
 * `atus serve` daemon — HTTP route + middleware integration tests.
 *
 ...
 * (which itself spawns real `atus --acp` children), then probe the HTTP
 ...
 * in `atus-serve-streaming.test.ts` and skip when no auth is set.
 */
...
describe('atus serve — bearer auth (timing-safe compare)', () => {
  ...
  it('/health exempt: missing Authorization header → 200', async () => {
    ...
    // docs/developers/atus-serve-protocol.md so a future middleware
...
describe('atus serve — CORS browser-origin denial', () => {
...
describe('atus serve — capabilities envelope', () => {
...
describe('atus serve — POST /session validation + concurrent coalescing', () => {
...
describe('atus serve — POST /permission/:requestId validation', () => {
...
describe('atus serve — SSE Content-Type guard (SDK side)', () => {
...
describe('atus serve — Last-Event-ID strict parsing', () => {
...
describe('atus serve — cancel + list', () => {
...
describe('atus serve — DELETE /session/:id', () => {
...
describe('atus serve — PATCH /session/:id/metadata', () => {
