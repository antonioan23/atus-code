...
/**
 * `atus serve` daemon — performance baseline harness.
 *
 ...
 * also skipped because the daemon's `atus --acp` child and its MCP
...
const SKIP =
  ...
  Boolean(
    process.env['ATUS_SANDBOX'] &&
      process.env['ATUS_SANDBOX']!.toLowerCase() !== 'false',
  );

// Read iteration tunings from env (documented in #4175 PR 1 plan).
const HEAVY = process.env['ATUS_BASELINE_HEAVY'] === '1';
const PROMPT_ITERATIONS = Number(
  process.env['ATUS_BASELINE_PROMPT_ITERATIONS'] ?? (HEAVY ? 100 : 20),
);
const RSS_SAMPLE_INTERVAL_MS = Number(
  process.env['ATUS_BASELINE_RSS_SAMPLE_INTERVAL_MS'] ?? 100,
);
const RSS_SAMPLE_DURATION_MS = Number(
  process.env['ATUS_BASELINE_RSS_SAMPLE_DURATION_MS'] ??
    (HEAVY ? 15_000 : 5_000),
);
...
const HAS_PROMPT_LATENCY_CREDENTIAL =
  process.env['ATUS_BASELINE_ENABLE_PROMPT_LATENCY'] === '1' ||
  PROMPT_LATENCY_CREDENTIAL_ENV_KEYS.some((key) => Boolean(process.env[key])) ||
  Object.entries(process.env).some(
    ([key, value]) => key.startsWith('ATUS_CUSTOM_API_KEY_') && Boolean(value),
  );
const SKIP_PROMPT_LATENCY =
  process.env['ATUS_BASELINE_SKIP_PROMPT_LATENCY'] === '1' ||
...
function makeTempWorkspace(label: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), `atus-baseline-${label}-`));
  return dir;
}
...
(SKIP ? describe.skip : describe)(
  ...
  () => {
    ...
    describe('MCP child amplification (P1 baseline)', () => {
      ...
      // measurement.
      //
      // Architectural note (PR 22a): a `atus serve` ACP child runs
      ...
      if (SKIP_PROMPT_LATENCY) {
        it('prompt latency skipped (no model credential env)', () => {
          snapshot.promptLatency = {
            ...
            skipped: true,
            skipReason:
              'No recognized model credential env var is set; prompt latency requires real model access. Set ATUS_BASELINE_ENABLE_PROMPT_LATENCY=1 to force-run with non-env auth.',
          };
...
function renderMarkdown(s: SnapshotShape): string {
  const fmt = (p: Percentiles | null | undefined): string =>
    p
      ...
      : 'n/a';
  return [
    `# atus serve daemon — perf baseline`,
    ``,
