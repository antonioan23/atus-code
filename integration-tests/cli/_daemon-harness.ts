...
/**
 * Reusable helpers for `atus serve` daemon tests:
 *
 *   - `spawnDaemon` lifts the inline `beforeAll` boot pattern from
 *     `atus-serve-routes.test.ts` / `atus-serve-streaming.test.ts` into one
 ...
 *     `atus-serve-streaming.test.ts:144`, with optional filtered subtree
...
import { DaemonClient, type SubscribeOptions } from '@atus-code/sdk';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

/**
 * Default workspace and CLI binary resolution mirrors the existing
 * `atus-serve-routes.test.ts` constants so callers that copy/paste between
...
export async function spawnDaemon(
  ...
  // atus-serve-routes.test.ts: capture the timer handle so a successful
...
/**
 * Write a `.atus-code/settings.json` into `workspaceCwd` so the daemon picks up
...
export function writeWorkspaceSettings(
  ...
  const settingsDir = path.join(workspaceCwd, '.atus-code');
 ...
 * `atus-serve-streaming.test.ts:144`. When `pgrepOpts.mcpFilter` is
 ...
 * `atus --acp` process between the daemon-facing ACP child and stdio MCP
 * servers.
 *
 * `pgrepOpts.acpFilter` defaults to `'atus.*--acp'` (matches the spawned
 * `atus --acp` child); pass an override only if a future bridge changes
...
export function countDescendants(
  ...
  const acpFilter = pgrepOpts.acpFilter ?? 'atus.*--acp';
