...
export interface SDKTestHelperOptions {
  /**
   * Optional settings for .atus-code/settings.json
   */
  settings?: Record<string, unknown>;
  /**
   * Whether to create .atus-code/settings.json
   */
...
export class SDKTestHelper {
  ...
  async setup(
    ...
    // Optionally create .atus-code/settings.json for CLI configuration
    if (options.createQwenConfig !== false) {
      const atusDir = join(this.testDir, '.atus-code');
      await mkdir(atusDir, { recursive: true });
