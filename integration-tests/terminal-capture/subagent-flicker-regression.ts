 ...
 *
 * Useful env:
 *   ATUS_TUI_E2E_REPO=/path/to/atus-code
 *   ATUS_TUI_E2E_OUT=/tmp/atus-tui-subagent-flicker
 *   ATUS_TUI_E2E_MAX_CLEAR_PAIRS=10       (default: 10)
 *   ATUS_TUI_E2E_MAX_CLEAR_SCREEN=20      (default: 20)
 *   ATUS_TUI_E2E_MAX_ERASE_LINE=460       (default: 460 — separates fix from
 *                                          no-fix; reverting the fix raises
 *                                          this counter to ~469)
 *   ATUS_TUI_E2E_SUBAGENT_TOOL_CALLS=5
 */
...
function chatCompletionId(suffix: string): string {
  return `chatcmpl-atus-tui-subagent-${suffix}-${Date.now()}`;
}
...
function buildMainAgentToolCall(packageJsonPath: string) {
  return {
    id: 'chatcmpl-atus-tui-subagent-dispatch',
...
async function startFakeOpenAIServer(
  ...
  const verbose = process.env['ATUS_TUI_E2E_VERBOSE'] === '1';
...
function atusArgs(baseUrl: string): string[] {
...
async function main(): Promise<void> {
  ...
  const repoRoot = resolve(process.env['ATUS_TUI_E2E_REPO'] ?? defaultRepoRoot);
  const defaultOut = join(
    tmpdir(),
    'atus-tui-subagent-flicker',
    basename(repoRoot),
  );
  const outputDir = resolve(process.env['ATUS_TUI_E2E_OUT'] ?? defaultOut);
  const maxClearPairs = envNumber('ATUS_TUI_E2E_MAX_CLEAR_PAIRS', 10);
  const maxClearScreen = envNumber('ATUS_TUI_E2E_MAX_CLEAR_SCREEN', 20);
  ...
  const maxEraseLine = envNumber('ATUS_TUI_E2E_MAX_ERASE_LINE', 460);
  const subagentToolCalls = envNumber('ATUS_TUI_E2E_SUBAGENT_TOOL_CALLS', 5);
  ...
  // Sandbox HOME to keep ~/.atus-code settings out of the run.
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    FORCE_COLOR: '1',
    NODE_NO_WARNINGS: '1',
    ATUS_CODE_DISABLE_SYNCHRONIZED_OUTPUT: '1',
    ATUS_CODE_NO_RELAUNCH: '1',
    // Intentionally NOT setting ATUS_CODE_SIMPLE so the agent tool stays in
    // the registry — see comment in atusArgs() above.
    ATUS_SANDBOX: 'false',
    TERM: 'xterm-256color',
    HOME: homeDir,
    USERPROFILE: homeDir,
  ...
  try {
    await terminal.spawn('node', atusArgs(fakeServer.baseUrl));
