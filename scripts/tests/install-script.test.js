...
describe('installation scripts', () => {
  it('keeps the Linux/macOS installer lightweight', () => {
    const script = readScript(
      'scripts/installation/install-atus-standalone.sh',
    );
    ...
    expect(script).not.toMatch(/^\s*exec\s+atus\s*$/m);
    ...
    expect(script).toContain('@atus-code/atus-code@latest');
    expect(script).toContain('Installing atus code version:');
    ...
    expect(script).toContain('uninstall-atus-standalone.sh');
  ...
  it('supports code-server-style standalone install on Linux/macOS', () => {
    ...
    expect(script).toContain('atus-code-${target}');
    ...
    expect(script).not.toContain('ln -sf "${INSTALL_LIB_DIR}/bin/atus"');
    ...
    expect(script).toContain('atus-code/node/bin/node');
    expect(script).toContain(
      'Archive contains symlinks or hardlinks; refusing to install',
    );
    expect(script).toContain('not a atus code standalone install');
    expect(script).toContain(
    ...
    expect(script).not.toContain('atus_install_$$');
    expect(script).toContain(
    ...
    expect(script).toContain(
      'ATUS_INSTALL_GITHUB_REPO must be in owner/repo format',
    );
    ...
    expect(script).toContain('# atus code PATH block begin');
    expect(script).toContain('# atus code PATH block end');
  ...
  it('keeps the Windows installer lightweight', () => {
    const script = readScript(
      'scripts/installation/install-atus-standalone.bat',
    );
    ...
    expect(script).not.toMatch(/^\s*call\s+atus\s*$/m);
    ...
    expect(script).toContain('@atus-code/atus-code@latest');
    expect(script).toContain('Installing atus code version:');
    expect(script).toContain('atus code');
    expect(script).toContain(
      'atus code !INSTALLED_VERSION! installed successfully, to start:',
    );
    expect(script).toContain('cd ^<project^>');
    expect(script).toContain('uninstall-atus-standalone.ps1');
    expect(script).toContain('ATUS_VERSION_POINTER_FILE');
    expect(script).toContain('ATUS_NORMALIZED_VERSION_FILE');
    expect(script).toContain('NORMALIZED_VERSION_FILE');
    expect(script).toContain(
      '[IO.File]::ReadAllText($env:ATUS_VERSION_POINTER_FILE)',
    );
    expect(script).toContain(
      '[IO.File]::WriteAllText($env:ATUS_NORMALIZED_VERSION_FILE',
    );
    expect(script).toContain('set "ATUS_VERSION_VALUE=!VERSION!"');
    expect(script).toContain(
  ...
  it('supports code-server-style standalone install on Windows', () => {
    ...
    expect(script).toContain('atus-code-!TARGET!.zip');
    expect(script).toContain(
    ...
    expect(script).toContain('$env:ATUS_DOWNLOAD_URL');
    expect(script).toContain('$env:ATUS_ARCHIVE_FILE');
    expect(script).toContain(
      'if defined ATUS_INSTALL_ROOT set "INSTALL_BASE=!ATUS_INSTALL_ROOT!"',
    );
    expect(script).not.toContain('%ATUS_INSTALL_ROOT%');
    expect(script).toContain('set "ATUS_VALIDATE_INSTALL_BASE=!INSTALL_BASE!"');
    expect(script).toContain(
      'installer options contain unsafe command characters',
    );
    expect(script).not.toContain('-EncodedCommand');
    expect(script).toContain('ATUS_VALIDATE_OPTIONS_SCRIPT');
    ...
    expect(script).toContain('$curl = $env:ATUS_INSTALL_CURL_EXE');
    expect(script).toContain('ATUS_INSTALL_CURL_EXE');
    ...
    expect(script).toContain(
      '$request = [Net.WebRequest]::Create($env:ATUS_CHECK_URL)',
    );
    ...
    expect(script).toContain('ATUS_INSTALL_GITHUB_REPO');
    expect(script).toContain(
      'ATUS_INSTALL_GITHUB_REPO must be in owner/repo format',
    );
    expect(script).toContain(
    ...
    expect(script).toContain('ATUS_INSTALL_ROOT');
    expect(script).toContain('npm fallback also failed');
    ...
    expect(script).toContain("'ATUS_INSTALL_VERSION'");
    expect(script.indexOf('$rawNames = @(')).toBeLessThan(
      script.indexOf('set "ATUS_VALIDATE_VERSION=!VERSION!"'),
    );
    expect(script).toContain('set "ARCHIVE_NAME=atus-code-!TARGET!.zip"');
  ...
  it('checks out the Windows standalone batch installer with CRLF line endings', () => {
    ...
    expect(attrs).toContain(
      'scripts/installation/install-atus-standalone.bat: eol: crlf',
    );
  ...
  it('prepends fake Windows tools to both PATH casings', () => {
    const fakeBin = 'C:\\atus-test-bin';

    const env = prependWindowsPath(fakeBin);

    expect(env.PATH).toMatch(/^C:\\atus-test-bin;/);
    expect(env.Path).toMatch(/^C:\\atus-test-bin;/);
  });

  it('creates a fake Windows curl command script', () => {
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-curl-helper-'));

    try {
      const fakeCurl = createFakeWindowsCurlCommand(tmpDir);

      expect(fakeCurl).toBe(path.join(tmpDir, 'curl.cmd'));
      expect(readScript(fakeCurl)).toContain('ATUS_FAKE_CURL_LOG');
      expect(readScript(fakeCurl)).toContain(
        '/releases/atus-code/latest/VERSION',
      );
  ...
  it('injects Windows processor overrides directly into cmd commands', () => {
    const prepared = prepareWindowsCommand(
      'call "C:\\tools\\install-atus-standalone.bat"',
      {
        Path: 'C:\\fake-bin',
    ...
    expect(prepared.command).toBe(
      'set "PROCESSOR_ARCHITECTURE=AMD64" && set "PROCESSOR_ARCHITEW6432=" && call "C:\\tools\\install-atus-standalone.bat"',
    );
  ...
  it('creates PowerShell validation scripts with a ps1 extension', () => {
    ...
    expect(script).toContain(
      "($env:ATUS_TEMP_FILE_PREFIX + '-' + [IO.Path]::GetRandomFileName() + $env:ATUS_TEMP_FILE_EXTENSION)",
    );
  });
});
...
describe('release-script-utils', () => {
  it('parses SHA256SUMS with BOM, empty lines, and CRLF', async () => {
    ...
    const checksums = parseSha256Sums(
      `\uFEFF${'a'.repeat(64)}  install-atus-standalone.sh\n\n${'b'.repeat(64)} *install-atus-standalone.bat\r\n${'c'.repeat(64)}  install-atus-standalone.ps1\n`,
    );

    expect(checksums.get('install-atus-standalone.sh')).toBe('a'.repeat(64));
    expect(checksums.get('install-atus-standalone.bat')).toBe('b'.repeat(64));
    expect(checksums.get('install-atus-standalone.ps1')).toBe('c'.repeat(64));
  });
  ...
  it('rejects malformed SHA256SUMS entries', async () => {
    ...
    expect(() =>
      parseSha256Sums('short-hash  install-atus-standalone.sh\n'),
  ...
  it('rejects duplicate SHA256SUMS entries', async () => {
    ...
    expect(() =>
      parseSha256Sums(
        `${first}  install-atus-standalone.sh\n${second}  install-atus-standalone.sh\n`,
      ),
    ).toThrow(/Duplicate SHA256SUMS entry for: install-atus-standalone\.sh/);
  });
...
describe('standalone release packaging', () => {
  it('defines a standalone packaging script', () => {
    ...
    expect(releaseScript).toContain('atus-code-${atusTarget}');
    ...
    expect(hostedInstallScript).toContain(
      "output: 'install-atus-standalone.sh'",
    );
    expect(hostedInstallScript).toContain(
      "output: 'install-atus-standalone.bat'",
    );
    expect(hostedInstallScript).toContain(
      "output: 'install-atus-standalone.ps1'",
    );
    ...
    expect(releaseVerifyScript).not.toContain("'atus-code-win-x64.zip'");
  ...
  it('validates standalone release checksum output', async () => {
    ...
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-release-test-'));

    try {
      const lines = RELEASE_TARGETS.map(({ atusTarget }) => {
        const extension = atusTarget === 'win-x64' ? 'zip' : 'tar.gz';
        return `${'a'.repeat(64)}  atus-code-${atusTarget}.${extension}`;
      });
      ...
      writeFileSync(
        ...
        `${lines.join('\n')}\n${'b'.repeat(64)}  atus-code-extra.tar.gz\n`,
      );
  ...
  it('installer scripts honor --version for hosted entrypoints', () => {
    ...
    expect(installShellSource).toContain(
      'VERSION="${ATUS_INSTALL_VERSION:-latest}"',
    );
    ...
    const installPowerShellSource = readScript(
      'scripts/installation/install-atus-standalone.ps1',
    );
    expect(installPowerShellSource).toContain('install-atus-standalone.bat');
    ...
    expect(installPowerShellSource).toContain('ATUS_INSTALL_VERSION');
  ...
  it('PowerShell hosted entrypoint refreshes the current Windows shell', () => {
    ...
    expect(installPowerShellSource).toContain('atus code current-session shim');
    expect(installPowerShellSource).toContain(
      'TEMP environment variable is not set',
    );
    expect(installPowerShellSource).toMatch(
      /function Get-AtusInstallBinDir \{[\s\S]*ATUS_INSTALL_BIN_DIR[\s\S]*return Join-Path \(Get-AtusInstallBase\) 'bin'[\s\S]*\}/,
    );
    expect(installPowerShellSource).toContain(
    ...
    expect(installPowerShellSource).toContain('ATUS_NO_MODIFY_PATH');
    expect(installPowerShellSource).not.toContain('doskey.exe');

    expect(installBatchSource).toContain('ATUS_INSTALLER_PARENT_POWERSHELL');
    expect(installBatchSource).toContain(
  ...
  it('stages hosted installation assets with checksums', async () => {
    ...
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-hosted-install-'));

    try {
      await buildHostedInstallationAssets(tmpDir);

      const installSh = path.join(tmpDir, 'install-atus-standalone.sh');
      const installBat = path.join(tmpDir, 'install-atus-standalone.bat');
      const installPs1 = path.join(tmpDir, 'install-atus-standalone.ps1');
      const uninstallSh = path.join(tmpDir, 'uninstall-atus-standalone.sh');
      const uninstallPs1 = path.join(tmpDir, 'uninstall-atus-standalone.ps1');
      ...
      expect(HOSTED_INSTALLATION_ASSET_NAMES).toEqual([
        'install-atus-standalone.sh',
        'install-atus-standalone.bat',
        'install-atus-standalone.ps1',
        'uninstall-atus-standalone.sh',
        'uninstall-atus-standalone.ps1',
      ]);
      expect(HOSTED_INSTALLATION_ASSETS.map(({ output }) => output)).toEqual(
        HOSTED_INSTALLATION_ASSET_NAMES,
      );
      expect(readScript(installSh)).toBe(
        readScript('scripts/installation/install-atus-standalone.sh'),
      );
      expect(readScript(installBat)).toBe(
        readScript('scripts/installation/install-atus-standalone.bat').replace(
          /\r?\n/g,
          '\r\n',
        ),
      );
      expect(readScript(installPs1)).toBe(
        readScript('scripts/installation/install-atus-standalone.ps1'),
      );
      expect(readScript(uninstallSh)).toBe(
        readScript('scripts/installation/uninstall-atus-standalone.sh'),
      );
      expect(readScript(uninstallPs1)).toBe(
        readScript('scripts/installation/uninstall-atus-standalone.ps1'),
      );
      ...
      expect(checksums).toMatch(
        /^[0-9a-f]{64} {2}install-atus-standalone\.sh$/m,
      );
      expect(checksums).toMatch(
        /^[0-9a-f]{64} {2}install-atus-standalone\.bat$/m,
      );
      expect(checksums).toMatch(
        /^[0-9a-f]{64} {2}install-atus-standalone\.ps1$/m,
      );
      expect(checksums).toMatch(
        /^[0-9a-f]{64} {2}uninstall-atus-standalone\.sh$/m,
      );
      expect(checksums).toMatch(
        /^[0-9a-f]{64} {2}uninstall-atus-standalone\.ps1$/m,
      );
      ...
      ).rejects.toThrow(/Checksum mismatch for install-atus-standalone\.sh/);
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it('rejects hosted installer sources without pinned hosted behavior', async () => {
    ...
    const tmpRoot = mkdtempSync(path.join(tmpdir(), 'atus-hosted-root-'));
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-hosted-install-'));
    ...
    try {
      ...
      writeFileSync(
        path.join(sourceDir, 'install-atus-standalone.sh'),
        '#!/usr/bin/env bash\n' +
          'VERSION="${ATUS_INSTALL_VERSION:-stable}"\n' +
          'case "$1" in --version) shift; VERSION="$1" ;; esac\n',
      );
      writeFileSync(
        path.join(sourceDir, 'install-atus-standalone.bat'),
        '@echo off\r\nset "VERSION=latest"\r\n',
      );
      writeFileSync(
        path.join(sourceDir, 'install-atus-standalone.ps1'),
        "# --version vX.Y.Z\n$env:ATUS_INSTALL_VERSION = 'latest'\n",
      );
      writeFileSync(
        path.join(sourceDir, 'uninstall-atus-standalone.sh'),
        '#!/usr/bin/env bash\nis_qwen_standalone_install_dir() { return 0; }\n',
      );
      writeFileSync(
        path.join(sourceDir, 'uninstall-atus-standalone.ps1'),
        'function Test-AtusStandaloneInstallDir { return $true }\n',
      );

      await expect(
        buildHostedInstallationAssets(tmpDir, { root: tmpRoot }),
      ).rejects.toThrow(
        /install-atus-standalone\.sh default install version must be 'latest'/,
      );
    } finally {
  ...
  it('rejects hosted installer sources without real version parsing', async () => {
    ...
    try {
      ...
      await expect(
        buildHostedInstallationAssets(tmpDir, { root: tmpRoot }),
      ).rejects.toThrow(/install-atus-standalone\.sh.*--version parser/);
    } finally {
      rmSync(tmpRoot, { recursive: true, force: true });
      rmSync(tmpDir, { recursive: true, force: true });
    }
  ...
  it('rejects hosted ps1 shim with a hardcoded version pin', async () => {
    ...
    try {
      ...
      writeFileSync(
        path.join(sourceDir, 'install-atus-standalone.bat'),
        '@echo off\r\nset "VERSION=%ATUS_INSTALL_VERSION%"\r\nif "%VERSION%"=="" set "VERSION=latest"\r\nset "VERSION=latest"\r\nif "%~1"=="--version" set "VERSION=%~2"\r\n',
      );
      // The ps1 shim has every required behavior pattern but also contains
      // a hardcoded $env:ATUS_INSTALL_VERSION assignment, which must be
      // rejected by the forbidden-patterns guard.
      writeFileSync(
        path.join(sourceDir, 'install-atus-standalone.ps1'),
        '# ATUS_INSTALL_VERSION documentation\n' +
          '$env:ATUS_INSTALL_VERSION = "v0.1.0"\n' +
      ...
      writeFileSync(
        ...
        'function Test-AtusStandaloneInstallDir { return $true }\n' +
          ...
          '$env:ATUS_UNINSTALL_PURGE = ""\n',
      );

      await expect(
        buildHostedInstallationAssets(tmpDir, { root: tmpRoot }),
      ).rejects.toThrow(
        /install-atus-standalone\.ps1 must not contain.*no hardcoded ATUS_INSTALL_VERSION assignment/,
      );
    } finally {
      rmSync(tmpRoot, { recursive: true, force: true });
      rmSync(tmpDir, { recursive: true, force: true });
  ...
  it('allows hosted ps1 shim that only documents ATUS_INSTALL_VERSION in comments', async () => {
    const { buildHostedInstallationAssets } = await import(
      hostedInstallationScriptUrl
    );
    const tmpRoot = mkdtempSync(path.join(tmpdir(), 'atus-hosted-root-'));
    ...
    try {
      ...
      // ("$env:ATUS_INSTALL_VERSION = 'vX.Y.Z'") as a `#` comment; the
  ...
  it('verifies release asset directory contents and checksums', async () => {
    ...
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-release-verify-'));
  ...
  it('rejects missing release archives and unexpected checksum entries', async () => {
    ...
    try {
      ...
      await expect(verifyReleaseDirectory(tmpDir)).rejects.toThrow(
        /Missing release asset: atus-code-/,
      );

      writeStandaloneReleaseAssets(tmpDir, EXPECTED_STANDALONE_ARCHIVE_NAMES);
      writeStandaloneReleaseChecksums(tmpDir, [
        ...EXPECTED_STANDALONE_ARCHIVE_NAMES,
        'atus-code-extra.tar.gz',
      ]);
      await expect(verifyReleaseDirectory(tmpDir)).rejects.toThrow(
        /Unexpected release asset checksum: atus-code-extra\.tar\.gz/,
      );
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  ...
  it('rejects unexpected files and non-file release assets', async () => {
    ...
    try {
      ...
      await expect(verifyReleaseDirectory(tmpDir)).rejects.toThrow(
        /Release asset is not a regular file: atus-code-/,
      );
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  ...
  it('verifies release asset URLs from SHA256SUMS', async () => {
    ...
    try {
      await expect(
        verifyReleaseBaseUrl('https://example.com/atus-code/v0.0.0', {
    ...
    expect(fetchedUrls).toContainEqual([
      'https://example.com/atus-code/v0.0.0/SHA256SUMS',
      'GET',
      true,
    ]);
    for (const assetName of EXPECTED_STANDALONE_ARCHIVE_NAMES) {
      expect(fetchedUrls).toContainEqual([
        `https://example.com/atus-code/v0.0.0/${assetName}`,
        'GET',
        true,
      ]);
    }
    ...
    for (const [url] of fetchedUrls) {
      expect(url).not.toMatch(/install-atus\.(sh|bat|ps1)$/);
  ...
  it('rejects remote release archives whose downloaded hash differs', async () => {
    ...
    ).rejects.toThrow(/Checksum mismatch for atus-code-/);
  });
  ...
  it('downloads release archive bodies instead of relying on HEAD probes', async () => {
    ...
    for (const assetName of EXPECTED_STANDALONE_ARCHIVE_NAMES) {
      const assetUrl = `https://example.com/atus-code/v0.0.0/${assetName}`;
  ...
  it('reports a single error when every asset URL is unavailable', async () => {
    ...
    try {
      await expect(
        ...
        new RegExp(
          `All ${EXPECTED_STANDALONE_ARCHIVE_NAMES.length} release asset URLs are unavailable; check --base-url: https://example\\.com/atus-code/v0\\.0\\.0/`,
        ),
      );
    } finally {
      warnSpy.mockRestore();
  ...
  it('prints explicit release asset paths for GitHub release upload', async () => {
    const { EXPECTED_RELEASE_ASSET_NAMES, EXPECTED_STANDALONE_ARCHIVE_NAMES } =
      await import(installationReleaseVerificationScriptUrl);
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-release-list-'));
  ...
  it('rejects a runtime archive without a Node executable', () => {
    ...
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-package-test-'));
  ...
  it('packages a win-x64 standalone archive', () => {
    ...
    try {
      ...
      const archive = path.join(outDir, 'atus-code-win-x64.zip');
      const extractDir = path.join(tmpDir, 'extract');
      mkdirSync(extractDir, { recursive: true });
      extractZipForTest(archive, extractDir);

      expect(existsSync(path.join(extractDir, 'atus-code'))).toBe(true);
      expect(
        existsSync(path.join(extractDir, 'atus-code', 'bin', 'atus.cmd')),
      ).toBe(true);
      expect(
        existsSync(path.join(extractDir, 'atus-code', 'node', 'node.exe')),
      ).toBe(true);
      expect(readScript(path.join(outDir, 'SHA256SUMS'))).toContain(
        'atus-code-win-x64.zip',
      );
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
      restoreMinimalDist(createdDist);
  ...
  it('syncs standalone and hosted installation assets during release', () => {
    ...
    expect(createReleaseStep).toContain('dist/standalone/atus-code-*');
    ...
    expect(ossWorkflow).toContain('releases/atus-code/${RELEASE_TAG}');
    expect(ossWorkflow).toContain('releases/atus-code/latest');
    expect(ossWorkflow).not.toContain(
      'upload_release_assets "releases/atus-code/latest"',
    );
    ...
    expect(syncStep).not.toContain('installation/install-atus-standalone.sh');
    const syncHostedStep = ossWorkflow.slice(
      syncHostedStepIndex,
      verifyHostedStepIndex,
    );
    expect(syncHostedStep).toContain(
      'dist/installation/install-atus-standalone.sh',
    );
    expect(syncHostedStep).toContain(
      'dist/installation/install-atus-standalone.bat',
    );
    expect(syncHostedStep).toContain(
      'dist/installation/install-atus-standalone.ps1',
    );
    expect(syncHostedStep).toContain(
      'dist/installation/uninstall-atus-standalone.sh',
    );
    expect(syncHostedStep).toContain(
      'dist/installation/uninstall-atus-standalone.ps1',
    );
    ...
    expect(ossWorkflow).toContain(
      'npm run verify:installation-release -- --base-url "${ALIYUN_OSS_PUBLIC_BASE_URL}/releases/atus-code/${RELEASE_TAG}"',
    );
    expect(ossWorkflow).toContain(
      'latest_version="$(curl -fsSL --connect-timeout 15 --max-time 300 "${ALIYUN_OSS_PUBLIC_BASE_URL}/releases/atus-code/latest/VERSION" | tr -d',
    );
    expect(ossWorkflow).not.toContain(
      'npm run verify:installation-release -- --base-url "${ALIYUN_OSS_PUBLIC_BASE_URL}/releases/atus-code/latest"',
    );
  ...
  it('does not whitelist internal planning documents in gitignore', () => {
    ...
    expect(gitignore).not.toContain('!.atus-code/design/');
    expect(gitignore).not.toContain('!.atus-code/e2e-tests/');
  });
  ...
  it('documents optional native module parity for standalone installs', () => {
    ...
    expect(guide).toContain('installation/install-atus-standalone.sh');
    expect(guide).toContain('installation/install-atus-standalone.bat');
    expect(guide).toContain('installation/install-atus-standalone.ps1');
    expect(guide).toContain('installation/uninstall-atus-standalone.sh');
    expect(guide).toContain('installation/uninstall-atus-standalone.ps1');
  ...
  it('provides standalone uninstall scripts that clean install-owned files only', () => {
    const uninstallShellSource = readScript(
      'scripts/installation/uninstall-atus-standalone.sh',
    );
    const uninstallPowerShellSource = readScript(
      'scripts/installation/uninstall-atus-standalone.ps1',
    );
    ...
    expect(uninstallShellSource).toContain('ATUS_UNINSTALL_PURGE');
    ...
    expect(uninstallPowerShellSource).toContain(
      'atus code current-session shim',
    );
    expect(uninstallPowerShellSource).toContain('ATUS_UNINSTALL_PURGE');
    ...
    expect(uninstallPowerShellSource).not.toMatch(
      /\$installWasManaged = Test-AtusStandaloneInstallDir[^\n]*\n\nRemove-CurrentCmdPathShim\n\nif \(\$installWasManaged\)/,
    );
  });
});
...
describe('Linux/macOS installer end-to-end', { timeout: 15000 }, () => {
  itOnUnix(
    ...
    () => {
      const createdDist = ensureMinimalDist();
      const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-install-test-'));
      ...
      try {
        ...
        expect(existsSync(path.join(installRoot, 'bin', 'atus'))).toBe(true);
        expect(
          existsSync(
            path.join(installRoot, 'lib', 'atus-code', 'node', 'bin', 'node'),
          ),
        ).toBe(true);
        expect(readScript(path.join(home, '.atus-code', 'source.json'))).toContain(
          '"source": "smoke"',
        );

        const version = execFileSync(path.join(installRoot, 'bin', 'atus'), [
          '--version',
        ])
          .toString()
          .trim();
        expect(version).toBe('0.0.0-smoke');
        expect(output).toContain('Installing atus code version: latest');
        ...
        expect(output).toContain('atuscode.github.io/atus-code');
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            '      printf "v0.0.0-smoke\\n"',
            '    fi ;;',
            '  */releases/atus-code/v0.0.0-smoke/atus-code-linux-x64.tar.gz)',
            '    cp "$ATUS_FAKE_ARCHIVE" "$dest" ;;',
            '  */releases/atus-code/v0.0.0-smoke/SHA256SUMS)',
            '    cp "$ATUS_FAKE_SHA256SUMS" "$dest" ;;',
            '  *)',
        ...
        const output = execFileSync(
          ...
          ],
          {
            env: {
              ...
              ATUS_FAKE_ARCHIVE: archive,
              ATUS_FAKE_SHA256SUMS: checksumFile,
              ATUS_FAKE_CURL_LOG: curlLog,
              ATUS_INSTALL_ROOT: installRoot,
            },
            stdio: 'pipe',
          },
        ).toString();
        ...
        expect(curlUrls).toContain('/releases/atus-code/latest/VERSION');
        ...
        expect(curlUrls).not.toContain(
          '/releases/atus-code/latest/atus-code-linux-x64.tar.gz',
        );
        expect(output).toContain('Downloading atus-code-linux-x64.tar.gz');
      } finally {
        rmSync(tmpDir, { recursive: true, force: true });
        restoreMinimalDist(createdDist);
      }
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            '    */releases/atus-code/v0.0.0-smoke/atus-code-linux-x64.tar.gz)',
            '      exit 22 ;;',
            '    */releases/download/v0.0.0-smoke/atus-code-linux-x64.tar.gz)',
            '      exit 0 ;;',
            '    *)',
  ...
  itOnUnix('uninstalls standalone files while preserving user config', () => {
    const createdDist = ensureMinimalDist();
    const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-uninstall-test-'));
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          'before',
          '# atus code PATH block begin',
          `export PATH='${installRoot}/bin':$PATH`,
          '# atus code PATH block end',
          'after',
        ].join('\n') + '\n',
      );
      const atusDir = path.join(home, '.atus-code');
      const sourceJson = path.join(atusDir, 'source.json');
      const settingsJson = path.join(atusDir, 'settings.json');
      writeFileSync(settingsJson, '{"theme":"dark"}\n');

      runUnixUninstaller(installRoot, home);

      expect(existsSync(path.join(installRoot, 'lib', 'atus-code'))).toBe(
        false,
      );
      expect(existsSync(path.join(installRoot, 'bin', 'atus'))).toBe(false);
      expect(readScript(rcFile)).toBe('before\nafter\n');
      expect(existsSync(sourceJson)).toBe(true);
      expect(existsSync(settingsJson)).toBe(true);

      runUnixUninstaller(installRoot, home, { ATUS_UNINSTALL_PURGE: '1' });
    ...
    () => {
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            'before',
            '# Added by atus-code installer (multi-atus shadow fix)   ',
            `export PATH='${installRoot}/bin':$PATH`,
            'middle',
            '# Added by atus-code installer (multi-atus shadow fix)',
            'echo keep-me',
  ...
  itOnUnix(
    'warns when an existing atus could shadow the standalone install',
    () => {
      const createdDist = ensureMinimalDist();
      const tmpDir = mkdtempSync(path.join(tmpdir(), 'atus-install-test-'));
      ...
      try {
        ...
        const existingQwen = path.join(fakeBin, 'atus');
        const installRoot = path.join(tmpDir, 'install');
        const home = path.join(tmpDir, 'home');
        ...
        const installedBin = path.join(installRoot, 'bin', 'atus');
        const bashrc = readScript(path.join(home, '.bashrc'));

        expect(output).toContain('installed successfully, to start:');
        expect(bashrc).toContain('# atus code PATH block begin');
        ...
        const resolvedQwen = execFileSync(
          'bash',
          ['-c', 'source "${HOME}/.bashrc"; command -v atus'],
          {
            env: {
              ...process.env,
              HOME: home,
  ...
  itOnUnix(
    ...
    () => {
      ...
      try {
        ...
        const installedBin = path.join(installBinDir, 'atus');
        ...
        expect(bashrcContents).toContain('# atus code PATH block begin');
        expect(
  ...
  itOnUnix('shell-quotes custom install paths in the generated wrapper', () => {
    ...
    try {
      ...
      const installLibDir = path.join(
        installRoot,
        'lib',
        'atus-code$(touch atus-pwned)',
      );

      runUnixInstaller(archive, installRoot, home, 'standalone', {
        ATUS_INSTALL_LIB_DIR: installLibDir,
      });

      const version = execFileSync(
        path.join(installRoot, 'bin', 'atus'),
        ['--version'],
        {
          cwd: tmpDir,
        },
        ...
        .trim();
      expect(version).toBe('0.0.0-smoke');
      expect(existsSync(path.join(tmpDir, 'atus-pwned'))).toBe(false);
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
      restoreMinimalDist(createdDist);
    }
  ...
  itOnUnix(
    ...
    () => {
      ...
      try {
        ...
        const installRoot = path.join(tmpDir, 'install');
        const home = path.join(tmpDir, 'home');
        const marker = path.join(tmpDir, 'atus-pwned');
        const unsafeBinDir = path.join(
          installRoot,
          'bin path $(touch atus-pwned)',
        );

        mkdirSync(fakeBin, { recursive: true });
        writeFileSync(path.join(fakeBin, 'atus'), '#!/usr/bin/env sh\n');
        chmodSync(path.join(fakeBin, 'atus'), 0o755);

        runUnixInstaller(archive, installRoot, home, 'standalone', {
          PATH: `${fakeBin}:${process.env.PATH}`,
          SHELL: '/bin/bash',
          ATUS_INSTALL_BIN_DIR: unsafeBinDir,
        });
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            '  shift',
            'done',
            'printf "%s %s %s\\n" "$url" "$is_head" "$is_range" >> "$ATUS_FAKE_CURL_LOG"',
            'case "$url" in',
            '  */atus-code-linux-x64.tar.gz)',
            '    if [ "$is_head" = "1" ]; then exit 22; fi',
            '    if [ "$is_range" = "1" ]; then : > "${dest:-/dev/null}"; exit 0; fi',
            '    cp "$ATUS_FAKE_ARCHIVE" "$dest"; exit 0 ;;',
            '  */SHA256SUMS)',
            '    cp "$ATUS_FAKE_SHA256SUMS" "$dest"; exit 0 ;;',
            '  *)',
            '    echo "unexpected url: $url" >&2',
            '    exit 22 ;;',
            'esac',
        ...
        const curlUrls = readScript(curlLog);
        expect(curlUrls).toContain('atus-code-linux-x64.tar.gz 1 0');
        expect(curlUrls).toContain('atus-code-linux-x64.tar.gz 0 1');
        expect(output).toContain('Downloading atus-code-linux-x64.tar.gz');
        expect(output).not.toContain('Falling back to npm installation');
      } finally {
        rmSync(tmpDir, { recursive: true, force: true });
  ...
  itOnUnix(
    ...
    () => {
      ...
      try {
        ...
        runUnixInstaller(archive, installRoot, home, 'standalone', {
          SHELL: '/bin/bash',
          ATUS_INSTALL_BIN_DIR: firstBinDir,
        });
        runUnixInstaller(archive, installRoot, home, 'standalone', {
          SHELL: '/bin/bash',
          ATUS_INSTALL_BIN_DIR: secondBinDir,
        });
  ...
  itOnUnix('rejects empty standalone archives', () => {
    ...
    try {
      const archive = path.join(tmpDir, 'atus-code-linux-x64.tar.gz');
  ...
  itOnUnix('backs up and overwrites a non-managed install directory', () => {
    ...
    try {
      const archive = packageFakeStandalone(tmpDir);
      const installRoot = path.join(tmpDir, 'install');
      const installDir = path.join(installRoot, 'lib', 'atus-code');
      ...
      expect(output).toContain('not a atus code standalone install');
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            'case "$1" in',
            '  -v) echo 10.0.0 ;;',
            '  prefix) echo "$ATUS_FAKE_NPM_PREFIX" ;;',
            '  install) printf "%s\\n" "$*" > "$ATUS_FAKE_NPM_LOG" ;;',
            'esac',
            'exit 0',
            '',
        ...
        writeFileSync(
          path.join(fakeBin, 'atus'),
          '#!/usr/bin/env sh\necho 0.0.0-npm\n',
        );
        for (const command of ['curl', 'node', 'npm', 'atus']) {
          chmodSync(path.join(fakeBin, command), 0o755);
        }
        ...
        const output = execFileSync(
          ...
          [
            ...
            'detect',
            '--base-url',
            'https://example.invalid/atus-code',
            '--source',
            'smoke',
          ],
          {
            env: {
              ...
              HOME: home,
              PATH: `${fakeBin}:${process.env.PATH}`,
              ATUS_FAKE_NPM_LOG: npmLog,
              ATUS_FAKE_NPM_PREFIX: path.join(tmpDir, 'npm-prefix'),
            },
            stdio: 'pipe',
          },
        ).toString();
        ...
        expect(readScript(npmLog)).toContain(
          'install -g @atus-code/atus-code@latest --registry',
        );
      } finally {
        rmSync(tmpDir, { recursive: true, force: true });
      }
  ...
  itOnUnix('passes pinned versions through to npm fallback', () => {
    ...
    try {
      ...
      expect(readScript(npmLog)).toContain(
        'install -g @atus-code/atus-code@0.15.10 --registry',
      );
    } finally {
      rmSync(tmpDir, { recursive: true, force: true });
    }
  ...
  itOnWindows(
    ...
    () => {
      ...
      try {
        ...
        expect(existsSync(path.join(installRoot, 'bin', 'atus.cmd'))).toBe(
          true,
        );
        expect(
          existsSync(path.join(installRoot, 'atus-code', 'node', 'node.exe')),
        ).toBe(true);
        expect(readScript(path.join(home, '.atus-code', 'source.json'))).toContain(
          '"source": "smoke"',
        );
        ...
        const version = runWindowsCommand(
          `call "${path.join(installRoot, 'bin', 'atus.cmd')}" --version`,
          { USERPROFILE: home },
        )
          .toString()
          .trim();
    ...
    try {
      ...
      expect(() =>
        runWindowsInstaller(
          ...
          'standalone',
          {
            ATUS_INSTALL_ROOT: `${path.join(tmpDir, 'install')}" & echo pwned > "${marker}" & "`,
          },
        ),
    ...
    () => {
      ...
      try {
        ...
        const output = runWindowsCommand(
          [
            `call "${path.resolve('scripts/installation/install-atus-standalone.bat')}"`,
            '--method',
            'standalone',
            '--mirror',
            'aliyun',
          ...
          ].join(' '),
          {
            ...
            ATUS_FAKE_SHA256SUMS: checksumFile,
            ATUS_FAKE_CURL_LOG: curlLog,
            ATUS_INSTALL_CURL_EXE: fakeCurl,
        ...
        expect(curlUrls).toContain('/releases/atus-code/latest/VERSION');
        expect(curlUrls).toContain(
          '/releases/atus-code/v0.0.0/atus-code-win-x64.zip',
        );
        expect(curlUrls).toContain('/releases/atus-code/v0.0.0/SHA256SUMS');
        expect(curlUrls).not.toContain(
          '/releases/atus-code/latest/atus-code-win-x64.zip',
        );
        expect(output).toContain('Downloading atus-code-win-x64.zip');
        expect(existsSync(path.join(installRoot, 'bin', 'atus.cmd'))).toBe(
          true,
        );
      } finally {
      ...
      try {
        ...
        const output = runWindowsCommand(
          ...
          ].join(' '),
          {
            ...
            ATUS_INSTALL_ROOT: path.join(tmpDir, 'install'),
            ATUS_FAKE_NPM_LOG: npmLog,
            ATUS_FAKE_NPM_PREFIX: path.join(tmpDir, 'npm-prefix'),
            ...prependWindowsPath(fakeBin),
            PROCESSOR_ARCHITECTURE: 'ARM64',
...
describe('Windows PowerShell uninstaller end-to-end', () => {
  itOnWindows('prints help without deleting standalone files', () => {
    ...
    try {
      const installRoot = path.join(tmpDir, 'install');
      const installDir = path.join(installRoot, 'atus-code');
      const home = path.join(tmpDir, 'home');
  ...
  itOnWindows('purges the source marker while preserving other config', () => {
    ...
    try {
      ...
      const installBinDir = path.join(installRoot, 'bin');
      const home = path.join(tmpDir, 'home');
      const atusConfigDir = path.join(home, '.atus-code');
      const sourceMarker = path.join(atusConfigDir, 'source.json');
      const settingsFile = path.join(atusConfigDir, 'settings.json');

      createFakeWindowsStandaloneInstall(installRoot);
      mkdirSync(atusConfigDir, { recursive: true });
      ...
      expect(existsSync(path.join(installBinDir, 'atus.cmd'))).toBe(false);
...
function ensureMinimalDist() {
  ...
  const backupPath = existsSync(distPath)
    ? path.join(
        ...
        `atus-dist-backup-${process.pid}-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}`,
      )
    : null;
  ...
  writeFileSync(path.join(distPath, 'cli.js'), 'console.log("atus");\n');
  writeFileSync(path.join(distPath, 'chunks/index.js'), 'export {};\n');
  writeFileSync(
    path.join(distPath, 'package.json'),
    JSON.stringify({ name: '@atus-code/atus-code', version: '0.0.0' }),
  );
...
function createFakeWindowsStandaloneArchive(tmpDir) {
  const packageRoot = path.join(tmpDir, 'atus-code');
  const outDir = path.join(tmpDir, 'out');
...
function createFakeWindowsStandaloneInstall(installRoot) {
  ...
  writeFileSync(
    path.join(installDir, 'bin', 'atus.cmd'),
    ['@echo off', 'echo 0.0.0-smoke', ''].join('\r\n'),
  );
  writeFileSync(path.join(installDir, 'node', 'node.exe'), 'fake node.exe\n');
  writeFileSync(
    path.join(installBinDir, 'atus.cmd'),
    ['@echo off', `"${path.join(installDir, 'bin', 'atus.cmd')}" %*`, ''].join(
      '\r\n',
    ),
  );
}
...
function createFakeWindowsNpmTools(fakeBin) {
  ...
  writeFileSync(
    ...
    [
      ...
      'if "%~1"=="prefix" echo %ATUS_FAKE_NPM_PREFIX% & exit /b 0',
      'if "%~1"=="install" echo %* > "%ATUS_FAKE_NPM_LOG%" & exit /b 0',
      'exit /b 0',
      '',
    ].join('\r\n'),
  );
  writeFileSync(
    path.join(fakeBin, 'atus.cmd'),
...
function createFakeWindowsCurlCommand(fakeBin) {
  ...
  writeFileSync(
    ...
    [
      ...
      'goto parse_args',
      ':done_parse',
      '>>"%ATUS_FAKE_CURL_LOG%" echo(!url!',
      'if "!url!"=="" echo missing url or destination 1>&2 & exit /b 2',
      'if "!destination!"=="" echo missing url or destination 1>&2 & exit /b 2',
      'echo(!url! | findstr /I /C:"/releases/atus-code/latest/VERSION" >nul && (',
      '  > "!destination!" echo 0.0.0',
      '  exit /b 0',
      ')',
      'echo(!url! | findstr /I /C:"/releases/atus-code/v0.0.0/atus-code-win-x64.zip" >nul && (',
      '  copy /Y "%ATUS_FAKE_ARCHIVE%" "!destination!" >nul',
      '  exit /b 0',
      ')',
      'echo(!url! | findstr /I /C:"/releases/atus-code/v0.0.0/SHA256SUMS" >nul && (',
      '  copy /Y "%ATUS_FAKE_SHA256SUMS%" "!destination!" >nul',
      '  exit /b 0',
      ')',
  ...
  if (process.platform === 'win32') {
    execFileSync(
      ...
      [
        ...
        'Bypass',
        '-Command',
        'Compress-Archive -LiteralPath $env:ATUS_TEST_ZIP_ENTRY -DestinationPath $env:ATUS_TEST_ZIP_ARCHIVE -Force',
      ],
      {
        env: {
          ...process.env,
          ATUS_TEST_ZIP_ENTRY: path.join(cwd, entry),
          ATUS_TEST_ZIP_ARCHIVE: archive,
        },
        stdio: 'ignore',
      },
    );
  ...
  if (process.platform === 'win32') {
    execFileSync(
      ...
      [
        ...
        'Bypass',
        '-Command',
        'Expand-Archive -LiteralPath $env:ATUS_TEST_ZIP_ARCHIVE -DestinationPath $env:ATUS_TEST_ZIP_DESTINATION -Force',
      ],
      {
        env: {
          ...process.env,
          ATUS_TEST_ZIP_ARCHIVE: archive,
          ATUS_TEST_ZIP_DESTINATION: destination,
        },
        stdio: 'ignore',
      },
    );
...
function packageFakeStandalone(tmpDir, nodeArchiveOptions = {}) {
  ...
  return path.join(outDir, 'atus-code-linux-x64.tar.gz');
}
...
function createSymlinkStandaloneArchive(tmpDir) {
  const packageRoot = path.join(tmpDir, 'malicious', 'atus-code');
  mkdirSync(path.join(packageRoot, 'bin'), { recursive: true });
  mkdirSync(path.join(packageRoot, 'node', 'bin'), { recursive: true });
  symlinkSync('/usr/bin/env', path.join(packageRoot, 'bin', 'atus'));
  writeFileSync(
  ...
  writeFileSync(
    path.join(packageRoot, 'manifest.json'),
    JSON.stringify({ name: '@atus-code/atus-code', target: 'linux-x64' }),
  );

  const outDir = path.join(tmpDir, 'out');
  mkdirSync(outDir, { recursive: true });
  const archive = path.join(outDir, 'atus-code-linux-x64.tar.gz');
  execFileSync(
    'tar',
    ['-czf', archive, '-C', path.dirname(packageRoot), 'atus-code'],
    {
      env: { ...process.env, LC_ALL: 'C' },
      stdio: 'ignore',
    },
...
function createTraversalStandaloneArchive(tmpDir) {
  ...
  const packageRoot = path.join(maliciousRoot, 'atus-code');
  mkdirSync(path.join(packageRoot, 'bin'), { recursive: true });
  mkdirSync(path.join(packageRoot, 'node', 'bin'), { recursive: true });
  writeFileSync(
    path.join(packageRoot, 'bin', 'atus'),
    '#!/usr/bin/env sh\necho 0.0.0-smoke\n',
  );
  chmodSync(path.join(packageRoot, 'bin', 'atus'), 0o755);
  writeFileSync(
    path.join(packageRoot, 'node', 'bin', 'node'),
    '#!/usr/bin/env sh\necho 0.0.0-smoke\n',
  );
  ...
  writeFileSync(path.join(tmpDir, 'atus-slip'), 'path traversal\n');

  const outDir = path.join(tmpDir, 'out');
  mkdirSync(outDir, { recursive: true });
  const archive = path.join(outDir, 'atus-code-linux-x64.zip');
  execFileSync('zip', ['-qr', archive, 'atus-code', '../atus-slip'], {
    cwd: maliciousRoot,
    stdio: 'ignore',
  });
  writeChecksumFile(outDir, path.basename(archive));
