...
describe('parseUploadArgs', () => {
  ...
  it('parses required options and asset list', () => {
    ...
    expect(args).toMatchObject({
      ...
      prefix: 'releases/atus-code/v1.2.3',
...
describe('uploadAssets (integration)', () => {
  ...
  it('spawns ossutil with the expected cp arguments per asset', async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-upload-'));
    try {
      ...
      uploadAssets(
        {
          assets,
          bucket: 'atus-test-bucket',
          config: configPath,
          prefix: 'releases/atus-code/v0.0.0',
        },
      ...
      expect(log).toContain(
        `oss://atus-test-bucket/releases/atus-code/v0.0.0/a.tar.gz`,
      );
      expect(log).toContain(
        `oss://atus-test-bucket/releases/atus-code/v0.0.0/b.zip`,
      );
  ...
  it('aggregates failures from ossutil non-zero exits', async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-upload-fail-'));
    try {
