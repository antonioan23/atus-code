...
function makeFixture(): {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'atus-check-i18n-'));
  tempDirs.push(root);
...
describe('checkI18n', () => {
  ...
  it('detects the unused-keys JSON flag from argv or env', () => {
    ...
    const originalEnv = process.env['ATUS_CHECK_I18N_WRITE_UNUSED_KEYS'];

    try {
      process.argv = ['node', 'check-i18n.ts'];
      delete process.env['ATUS_CHECK_I18N_WRITE_UNUSED_KEYS'];
      ...
      process.argv = ['node', 'check-i18n.ts'];
      process.env['ATUS_CHECK_I18N_WRITE_UNUSED_KEYS'] = '1';
      expect(shouldWriteUnusedKeysJson()).toBe(true);
    } finally {
      ...
      if (originalEnv === undefined) {
        ...
        process.env['ATUS_CHECK_I18N_WRITE_UNUSED_KEYS'] = originalEnv;
      }
    }
  });
});
