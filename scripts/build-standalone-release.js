...
const RELEASE_TARGETS = [
  {
    atusTarget: 'darwin-arm64',
    nodeTarget: 'darwin-arm64',
    nodeArchiveExtension: 'tar.gz',
  },
  {
    atusTarget: 'darwin-x64',
    nodeTarget: 'darwin-x64',
    nodeArchiveExtension: 'tar.gz',
  },
  {
    atusTarget: 'linux-arm64',
    nodeTarget: 'linux-arm64',
    nodeArchiveExtension: 'tar.xz',
  },
  {
    atusTarget: 'linux-x64',
    nodeTarget: 'linux-x64',
    nodeArchiveExtension: 'tar.xz',
  },
  { atusTarget: 'win-x64', nodeTarget: 'win-x64', nodeArchiveExtension: 'zip' },
];
...
async function main() {
  ...
  const runtimeDir = fs.mkdtempSync(
    path.join(runtimeParent, 'atus-node-runtime-'),
  );
...
function assertStandaloneOutput(outDir) {
  ...
  const expectedArchiveNames = RELEASE_TARGETS.map(
    ({ atusTarget }) =>
      `atus-code-${atusTarget}.${atusTarget === 'win-x64' ? 'zip' : 'tar.gz'}`,
  ).sort();
