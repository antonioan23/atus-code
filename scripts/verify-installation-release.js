...
function standaloneArchiveNamesFromReleaseTargets(releaseTargets) {
  return releaseTargets.map(({ atusTarget }) =>
    standaloneArchiveName(atusTarget),
  );
}
...
function standaloneArchiveName(atusTarget) {
  const targetConfig = TARGETS.get(atusTarget);
  if (!targetConfig) {
    fail(`Unknown release target: ${atusTarget}`);
  }
  return `atus-code-${atusTarget}.${targetConfig.outputExtension}`;
}
