...
const HOSTED_INSTALLATION_ASSETS = [
  {
    sourcePath: ['scripts', 'installation', 'install-atus-standalone.sh'],
    output: 'install-atus-standalone.sh',
    mode: 0o755,
  },
  {
    sourcePath: ['scripts', 'installation', 'install-atus-standalone.bat'],
    output: 'install-atus-standalone.bat',
    lineEndings: 'crlf',
  },
  {
    sourcePath: ['scripts', 'installation', 'install-atus-standalone.ps1'],
    output: 'install-atus-standalone.ps1',
  },
  {
    sourcePath: ['scripts', 'installation', 'uninstall-atus-standalone.sh'],
    output: 'uninstall-atus-standalone.sh',
    mode: 0o755,
  },
  {
    sourcePath: ['scripts', 'installation', 'uninstall-atus-standalone.ps1'],
    output: 'uninstall-atus-standalone.ps1',
  },
];
...
const HOSTED_INSTALLER_BEHAVIOR_PATTERNS = {
  'install-atus-standalone.sh': [
    {
      name: 'ATUS_INSTALL_VERSION',
      pattern: /ATUS_INSTALL_VERSION/,
    },
    {
      name: '--version parser',
  ...
  'install-atus-standalone.ps1': [
    {
      ...
      pattern: /& \$atusInstallerPath @args/,
    },
    {
    ...
    {
      name: 'ATUS_INSTALL_VERSION documentation',
      pattern: /ATUS_INSTALL_VERSION/,
    },
  ],
  'uninstall-atus-standalone.sh': [
    ...
    },
    {
      ...
      pattern: /ATUS_UNINSTALL_PURGE/,
    },
  ],
  'uninstall-atus-standalone.ps1': [
    {
      name: 'standalone directory guard',
      pattern: /Test-AtusStandaloneInstallDir/,
    },
    {
      name: 'PATH cleanup',
...
const HOSTED_INSTALLER_DEFAULT_VERSION_PATTERNS = {
  'install-atus-standalone.sh':
    /VERSION\s*=\s*"\$\{ATUS_INSTALL_VERSION:-latest\}"/,
  'install-atus-standalone.bat': /set\s+"VERSION=latest"/,
};
// install-atus-standalone.ps1 is a shim that downloads the .bat and forwards
...
// specific version (either by assigning $env:ATUS_INSTALL_VERSION or by
...
const HOSTED_INSTALLER_FORBIDDEN_PATTERNS = {
  'install-atus-standalone.ps1': [
    {
      name: 'no hardcoded ATUS_INSTALL_VERSION assignment',
      pattern: /^\s*\$env:ATUS_INSTALL_VERSION\s*=/m,
    },
    {
      name: 'no hardcoded --version prepended to forwarded args',
      pattern:
        /^\s*&\s+\$atusInstallerPath\s+(?:'--version'|"--version"|--version)/m,
    },
  ],
};
 ...
 * @param {string} assetName - Logical asset name (e.g. 'install-atus-standalone.sh').
 * @param {string} version - Release version to stamp (e.g. 'v1.2.3' or '1.2.3').
 */
function stampVersionInAsset(filePath, assetName, version) {
  const replacements = {
    'install-atus-standalone.sh': {
      from: 'VERSION="${ATUS_INSTALL_VERSION:-latest}"',
      to: `VERSION="\${ATUS_INSTALL_VERSION:-${version}}"`,
    },
    'install-atus-standalone.bat': {
