...
async function main() {
  ...
  const outputName = `atus-code-${target}.${targetConfig.outputExtension}`;
  const outputPath = path.join(outDir, outputName);
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-standalone-'));

  try {
    const packageRoot = path.join(tempRoot, 'atus-code');
...
function printUsage() {
  console.log(`atus code standalone package builder
...
Options:
  ...
  --version VERSION      atus code version. Defaults to package.json version.
  ...
  if (process.platform === 'win32') {
    run(
      ...
      [
        ...
        'Bypass',
        '-Command',
        'Expand-Archive -LiteralPath $env:ATUS_NODE_ARCHIVE -DestinationPath $env:ATUS_EXTRACT_DIR -Force',
      ],
      {
        env: {
          ...process.env,
          ATUS_NODE_ARCHIVE: nodeArchive,
          ATUS_EXTRACT_DIR: extractDir,
        },
      },
    );
    return;
...
`;
  const unixShimPath = path.join(binDir, 'atus');
...
`;
  fs.writeFileSync(path.join(binDir, 'atus.cmd'), windowsShim);
}
...
function createArchive(outputExtension, outputPath, cwd) {
  ...
  run('tar', ['-czf', outputPath, '-C', cwd, 'atus-code']);
}
...
function createZipArchive(outputPath, cwd) {
  if (process.platform === 'win32') {
    run(
      ...
      [
        ...
        'Bypass',
        '-Command',
        'Compress-Archive -LiteralPath $env:ATUS_PACKAGE_ROOT -DestinationPath $env:ATUS_OUTPUT_PATH -Force',
      ],
      {
        env: {
          ...process.env,
          ATUS_PACKAGE_ROOT: path.join(cwd, 'atus-code'),
          ATUS_OUTPUT_PATH: outputPath,
        },
      },
    );
    return;
  ...
  run('zip', ['-qr', outputPath, 'atus-code'], { cwd });
}
...
async function writeSha256Sums(outDir) {
  ...
  if (entries.length === 0) {
    fail(
      `No atus-code archives found in ${outDir}; refusing to write empty SHA256SUMS.`,
    );
  }
