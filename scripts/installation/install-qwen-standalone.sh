...
# atus code Installation Script
# Installs atus code from a standalone archive when available, with npm fallback.
# This script intentionally does not install Node.js or change npm config.
#
# Usage:
#   install-atus-standalone.sh --source [github|npm|internal|local-build]
#   install-atus-standalone.sh --method [detect|standalone|npm]
...
SOURCE="unknown"
METHOD="${ATUS_INSTALL_METHOD:-}"
MIRROR="${ATUS_INSTALL_MIRROR:-auto}"
BASE_URL="${ATUS_INSTALL_BASE_URL:-}"
ARCHIVE_PATH="${ATUS_INSTALL_ARCHIVE:-}"
VERSION="${ATUS_INSTALL_VERSION:-latest}"
NO_MODIFY_PATH="${ATUS_NO_MODIFY_PATH:-0}"
NPM_REGISTRY="${ATUS_NPM_REGISTRY:-https://registry.npmmirror.com}"
INSTALL_ROOT="${ATUS_INSTALL_ROOT:-${HOME:-}/.local}"
if [[ -n "${ATUS_INSTALL_LIB_DIR:-}" ]]; then
    INSTALL_LIB_DIR="${ATUS_INSTALL_LIB_DIR}"
    INSTALL_LIB_PARENT="$(dirname "${INSTALL_LIB_DIR}")"
else
    INSTALL_LIB_PARENT="${ATUS_INSTALL_LIB_PARENT:-${INSTALL_ROOT}/lib}"
    INSTALL_LIB_DIR="${INSTALL_LIB_PARENT}/atus-code"
fi
INSTALL_BIN_DIR="${ATUS_INSTALL_BIN_DIR:-${INSTALL_ROOT}/bin}"
...
validate_github_repo() {
    local github_repo="${ATUS_INSTALL_GITHUB_REPO:-atuscode/atus-code}"
    if [[ "${github_repo}" =~ ^[A-Za-z0-9._-]+/[A-Za-z0-9._-]+$ ]]; then
        return 0
    fi

    log_error "ATUS_INSTALL_GITHUB_REPO must be in owner/repo format."
    exit 1
}
...
validate_options() {
    ...
    validate_version
    validate_github_repo
    validate_install_path "${INSTALL_ROOT}" "ATUS_INSTALL_ROOT"
    validate_install_path "${INSTALL_LIB_PARENT}" "ATUS_INSTALL_LIB_PARENT"
    validate_install_path "${INSTALL_LIB_DIR}" "ATUS_INSTALL_LIB_DIR"
    validate_install_path "${INSTALL_BIN_DIR}" "ATUS_INSTALL_BIN_DIR"
    validate_source
}
...
print_header() {
    echo ""
    echo "Installing atus code version: $(display_install_version)"
    echo ""
}
...
create_source_json() {
    ...
    local atus_dir="${HOME}/.atus-code"
    mkdir -p "${atus_dir}"

    local escaped_source
    escaped_source=$(printf '%s' "${SOURCE}" | sed 's/\\/\\\\/g; s/"/\\"/g')

    cat > "${atus_dir}/source.json" <<EOF
{
...
# When a shadowing 'atus' is detected, append a PATH prepend to the user's
...
maybe_update_shell_path() {
    ...
    local begin_marker="# atus code PATH block begin"
    local end_marker="# atus code PATH block end"
...
aliyun_base_url_for_version() {
    local version_path="$1"
    echo "https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/releases/atus-code/${version_path}"
}

aliyun_latest_version_url() {
    echo "https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/releases/atus-code/latest/VERSION"
}
...
race_mirror_head() {
    ...
    local oss_url="$3"
    local tmpdir
    if ! tmpdir=$(mktemp -d -t atus-mirror.XXXXXX 2>/dev/null); then
...
ensure_managed_install_dir() {
    ...
    log_warning "${install_dir} exists but is not a atus code standalone install."
...
is_qwen_standalone_install_dir() {
    ...
    grep -Eq '"name"[[:space:]]*:[[:space:]]*"@atus-code/atus-code"' "${manifest_path}" 2>/dev/null || return 1
    grep -Eq '"target"[[:space:]]*:[[:space:]]*"(darwin|linux)-(arm64|x64)"' "${manifest_path}" 2>/dev/null || return 1
    [[ -f "${install_dir}/bin/atus" && ! -L "${install_dir}/bin/atus" && -x "${install_dir}/bin/atus" ]] || return 1
...
write_unix_wrapper() {
    ...
    quoted_qwen_bin=$(shell_quote "${atus_bin}")
...
install_standalone() {
    ...
    if [[ -n "${ARCHIVE_PATH}" ]]; then
        ...
        archive_name="atus-code-${target}.${archive_extension}"
        ...
        if [[ -n "${github_fallback_base_url}" && "${requested_version_path}" == "latest" ]]; then
            local aliyun_release_base="https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/releases/atus-code/"
    ...
    if [[ ! -f "${extract_dir}/atus-code/bin/atus" || -L "${extract_dir}/atus-code/bin/atus" || ! -x "${extract_dir}/atus-code/bin/atus" ]]; then
        log_error "Archive does not contain atus-code/bin/atus."
        rm -rf "${temp_dir}"
        return 1
    fi

    if [[ ! -f "${extract_dir}/atus-code/node/bin/node" || -L "${extract_dir}/atus-code/node/bin/node" || ! -x "${extract_dir}/atus-code/node/bin/node" ]]; then
        log_error "Archive does not contain executable atus-code/node/bin/node."
        rm -rf "${temp_dir}"
        return 1
    fi
    ...
    local wrapper_tmp="${INSTALL_BIN_DIR}/atus.new"
    ...
    mv "${extract_dir}/atus-code" "${new_install_dir}"

    if ! write_unix_wrapper "${wrapper_tmp}" "${INSTALL_LIB_DIR}/bin/atus"; then
        rm -rf "${temp_dir}" "${new_install_dir}" "${wrapper_tmp}"
        log_error "Failed to create atus wrapper in ${INSTALL_BIN_DIR}."
        return 1
    fi
    ...
    if ! mv -f "${wrapper_tmp}" "${INSTALL_BIN_DIR}/atus"; then
...
npm_package_spec() {
    if [[ "${VERSION}" == "latest" ]]; then
        echo "@atus-code/atus-code@latest"
        return 0
    fi

    local npm_version="${VERSION#v}"
    echo "@atus-code/atus-code@${npm_version}"
}
...
print_final_instructions() {
    ...
    local installed_bin=""
    local standalone_uninstall_url="https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/uninstall-atus-standalone.sh"
    if [[ -n "${install_bin_dir}" ]]; then
        installed_bin="${install_bin_dir}/atus"
        export PATH="${install_bin_dir}:${PATH}"
    fi

    # Detect shadowing atus executables
    local other_qwens=""
    ...
    elif command_exists atus; then
        installed_version=$(atus --version 2>/dev/null || echo "unknown")
    fi
    ...
    if [[ "${PATH_UPDATE_APPLIED:-0}" == "1" && -n "${rc_name}" ]]; then
        echo -e "${MUTED}Successfully added${NC} atus ${MUTED}to \$PATH in${NC} ${rc_name}"
    fi

    echo ""
    echo -e "${MUTED}atus code ${installed_version} installed successfully, to start:${NC}"
    echo ""
    echo -e "cd <project>  ${MUTED}# Open directory${NC}"
    echo -e "atus          ${MUTED}# Run command${NC}"
    echo ""
    echo -e "${MUTED}For more information visit ${NC}https://atuscode.github.io/atus-code"
    echo ""
}

main() {
    if [[ -z "${HOME:-}" ]]; then
        log_error "HOME is not set; cannot determine where to install atus code."
        exit 1
    fi

    # Discover all atus executables on disk BEFORE we install, so the
    ...
    PRE_INSTALL_QWENS=$(
        {
            ...
            for dir in $PATH; do
                [[ -z "${dir}" ]] && continue
                [[ -x "${dir}/atus" ]] && echo "${dir}/atus"
            done
            for candidate in \
                "${HOME}/.opencode/bin/atus" \
                "${HOME}/.bun/bin/atus" \
                "${HOME}/.cargo/bin/atus" \
                "${HOME}/.deno/bin/atus" \
                "${HOME}/.volta/bin/atus" \
                "${HOME}/.fnm/bin/atus" \
                "${HOME}/.local/bin/atus" \
                "${HOME}/Library/pnpm/atus" \
                "/usr/local/bin/atus" \
                "/opt/homebrew/bin/atus"; do
                [[ -x "${candidate}" ]] && echo "${candidate}"
            done
            if command_exists npm; then
                local npm_prefix
                npm_prefix=$(npm prefix -g 2>/dev/null || true)
                if [[ -n "${npm_prefix}" && -x "${npm_prefix}/bin/atus" ]]; then
                    echo "${npm_prefix}/bin/atus"
                fi
            fi
        } 2>/dev/null | sort -u
    )
