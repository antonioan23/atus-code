...
#
# Usage:
#   install-atus-with-source.sh --source [github|npm|internal|local-build]
#   install-atus-with-source.sh --method [detect|standalone|npm]
...
Options:
  ...
  --method METHOD          Install method: detect, standalone, or npm.
                           Defaults to ATUS_INSTALL_METHOD or detect.
  --mirror MIRROR          Standalone archive mirror: github or aliyun.
                           Defaults to ATUS_INSTALL_MIRROR or github.
  ...
  --registry REGISTRY      npm registry to use for npm fallback.
                           Defaults to ATUS_NPM_REGISTRY or https://registry.npmmirror.com
  -h, --help               Show this help message.

Examples:
  curl -fsSL https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-atus.sh | bash
  curl -fsSL https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-atus.sh | bash -s -- --source github
  curl -fsSL https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/installation/install-atus.sh | bash -s -- --method standalone
  ./install-atus-with-source.sh --archive ./atus-code-linux-x64.tar.gz
EOF
}

SOURCE="unknown"
METHOD="${ATUS_INSTALL_METHOD:-}"
MIRROR="${ATUS_INSTALL_MIRROR:-github}"
...
print_header() {
    echo "=========================================="
    echo "   atus code Installation Script"
    echo "=========================================="
    echo ""
...
print_node_help() {
    echo ""
    echo "Node.js 20 or newer is required before installing atus code with npm."
    echo ""
...
EOF
    ...
    log_success "Installation source saved to ~/.atus-code/source.json"
}
...
standalone_base_url() {
    ...
    if [[ "${version_path}" == "latest" ]]; then
        echo "https://github.com/atuscode/atus-code/releases/latest/download"
        return 0
    fi

    echo "https://github.com/atuscode/atus-code/releases/download/${version_path}"
}
...
ensure_managed_install_dir() {
    ...
    log_error "${install_dir} exists but is not a atus code standalone install."
...
install_standalone() {
    ...
    log_success "atus code standalone archive installed successfully."
...
install_npm() {
    ...
    if command_exists atus; then
        local atus_version
        atus_version=$(atus --version 2>/dev/null || echo "unknown")
        log_info "Existing atus code detected: ${atus_version}"
    ...
    local install_cmd=(
        ...
        install
        -g
        @atus-code/atus-code@latest
        --registry
        "${NPM_REGISTRY}"
    )

    log_info "Running: npm install -g @atus-code/atus-code@latest --registry ${NPM_REGISTRY}"
    if "${install_cmd[@]}"; then
        log_success "atus code installed successfully."
        create_source_json
        return 0
    fi

    log_error "Failed to install atus code."
    echo ""
    ...
    echo "  npm install -g @atus-code/atus-code@latest --registry ${NPM_REGISTRY}"
    return 1
}
...
print_final_instructions() {
    ...
    if command_exists atus; then
        local atus_version
        atus_version=$(atus --version 2>/dev/null || echo "unknown")
        log_success "atus code is ready to use: ${atus_version}"
        echo ""
        echo "You can now run: atus"
        echo ""
        log_info "Run atus in your project directory to start an interactive session."
        return 0
    fi

    log_warning "atus code was installed, but atus is not on PATH in this shell."
    echo ""
    echo "Restart your terminal, then run: atus"
    if [[ -n "${install_bin_dir}" ]]; then
        echo ""
