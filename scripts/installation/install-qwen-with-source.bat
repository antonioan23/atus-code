...
echo.
echo Usage: install-atus-with-source.bat [OPTIONS]
echo.
echo Options:
...
echo                            Defaults to ATUS_NPM_REGISTRY or https://registry.npmmirror.com
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$unsafe = [char[]](10,13,33,34,37,38,60,62,94,96,124); foreach ($name in 'METHOD','MIRROR','BASE_URL','ARCHIVE_PATH','VERSION','NPM_REGISTRY','INSTALL_BASE','INSTALL_DIR','INSTALL_BIN_DIR','SOURCE') { $value = [Environment]::GetEnvironmentVariable('ATUS_VALIDATE_' + $name); if ($null -ne $value -and $value.IndexOfAny($unsafe) -ge 0) { exit 1 } }"
...
if /i "!MIRROR!"=="aliyun" (
    set "STANDALONE_BASE_URL=https://atus-code-assets.oss-cn-hangzhou.aliyuncs.com/releases/atus-code/!VERSION_PATH!"
    exit /b 0
)

if /i "!VERSION_PATH!"=="latest" (
    set "STANDALONE_BASE_URL=https://github.com/atuscode/atus-code/releases/latest/download"
    exit /b 0
)

set "STANDALONE_BASE_URL=https://github.com/atuscode/atus-code/releases/download/!VERSION_PATH!"
exit /b 0

:UrlExists
set "ATUS_CHECK_URL=%~1"
powershell -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $request = [Net.WebRequest]::Create($env:ATUS_CHECK_URL); $request.Method = 'HEAD'; try { $response = $request.GetResponse(); $response.Close(); exit 0 } catch { exit 1 }" >nul 2>&1
set "PS_STATUS=%ERRORLEVEL%"
set "ATUS_CHECK_URL="
exit /b %PS_STATUS%
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference = 'Stop'; try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $client = New-Object Net.WebClient; $client.DownloadFile($env:ATUS_DOWNLOAD_URL, $env:ATUS_DOWNLOAD_DEST); exit 0 } catch { exit 1 }"
set "PS_STATUS=%ERRORLEVEL%"
...
if "!CHECKSUM_FILE!"=="" (
    ...
    if /i "!CHECKSUM_FILE:~0,8!"=="https://" (
        set "REQUIRE_CHECKSUM=1"
        set "TEMP_CHECKSUM=%TEMP%\atus-code-checksums-%RANDOM%%RANDOM%.txt"
...
if not "!ARCHIVE_PATH!"=="" (
    ...
    set "ARCHIVE_NAME=atus-code-win-x64.zip"
    ...
    set "TEMP_DIR=%TEMP%\atus-code-install-%RANDOM%%RANDOM%"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Expand-Archive -LiteralPath $env:ATUS_ARCHIVE_FILE -DestinationPath $env:ATUS_EXTRACT_DIR -Force"
set "PS_STATUS=!ERRORLEVEL!"
...
echo SUCCESS: Atus Code standalone archive installed successfully.
...
echo ERROR: !MANAGED_DIR! exists but is not a Atus Code standalone install.
...
where atus >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    for /f "delims=" %%i in ('atus --version 2^>nul') do set "ATUS_VERSION=%%i"
    echo INFO: Existing Atus Code detected: !ATUS_VERSION!
    echo INFO: Upgrading to the latest version.
)

echo INFO: Running: npm install -g @atus-code/atus-code@latest --registry !NPM_REGISTRY!
call npm install -g @atus-code/atus-code@latest --registry "!NPM_REGISTRY!"
if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to install Atus Code.
    echo.
...
echo SUCCESS: Atus Code installed successfully.
call :CreateSourceJson
exit /b 0
...
if not exist "!ATUS_DIR!" mkdir "!ATUS_DIR!"
...
echo SUCCESS: Installation source saved to !USERPROFILE!\.atus-code\source.json
exit /b 0
...
if %ERRORLEVEL% EQU 0 (
    for /f "delims=" %%i in ('atus --version 2^>nul') do set "ATUS_VERSION=%%i"
    echo SUCCESS: Atus Code is ready to use: !ATUS_VERSION!
    echo.
    echo You can now run: atus
    echo.
    echo INFO: Run atus in your project directory to start an interactive session.
    exit /b 0
)

echo WARNING: Atus Code was installed, but atus is not on PATH in this prompt.
echo.
echo Restart your command prompt, then run: atus
if not "!EXTRA_BIN!"=="" (
    echo.
