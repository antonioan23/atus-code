# Installation Guide for Atus Code with Source Tracking

This guide describes the source-tracking installation scripts for Atus Code.
...
- They do not edit npm config. Standalone installs may update the shell profile
  or user PATH so the generated `atus` shim is discoverable.
- They do not start `atus` automatically after installation.
- They store source information in `~/.atus-code/source.json` or
  `%USERPROFILE%\.atus-code\source.json` when `--source` is provided.
...
- Linux/macOS: `install-atus-standalone.sh`
- Windows: `install-atus-standalone.ps1`
- Linux/macOS uninstall: `uninstall-atus-standalone.sh`
- Windows uninstall: `uninstall-atus-standalone.ps1`

## Release Artifacts

GitHub releases publish these standalone archives:

- `atus-code-darwin-arm64.tar.gz`
- `atus-code-darwin-x64.tar.gz`
- `atus-code-linux-arm64.tar.gz`
- `atus-code-linux-x64.tar.gz`
- `atus-code-win-x64.zip`
- `SHA256SUMS`

The new standalone-first installer scripts (`install-atus-standalone.sh`,
`install-atus-standalone.ps1`) are not republished per release. They are served
...
existing production `install-atus.sh` / `install-atus.bat` OSS objects during
the staged rollout.

Hosted installer assets are staged separately from GitHub Release archives:

- `install-atus-standalone.sh` is the Linux/macOS hosted entrypoint.
- `install-atus-standalone.ps1` is the Windows hosted entrypoint for `irm | iex`.
- `install-atus-standalone.bat` is the Windows installer implementation used by
  `install-atus-standalone.ps1` and can also be downloaded and run directly.
- `uninstall-atus-standalone.sh` removes Linux/macOS standalone installs.
- `uninstall-atus-standalone.ps1` removes Windows standalone installs.

The global standalone-suffixed OSS entrypoints are maintained under
`installation/install-atus-standalone.sh`,
`installation/install-atus-standalone.ps1`,
`installation/install-atus-standalone.bat`,
`installation/uninstall-atus-standalone.sh`, and
`installation/uninstall-atus-standalone.ps1`.
...
The staged `install-atus-standalone.sh`, `install-atus-standalone.ps1`,
`install-atus-standalone.bat`, `uninstall-atus-standalone.sh`, and
`uninstall-atus-standalone.ps1` files map to the standalone-suffixed hosted URLs
...
`releases/atus-code/latest/VERSION` first, then downloading the matching
versioned release directory. Use `--version` or `ATUS_INSTALL_VERSION` to pin a
...
```bash
bash install-atus-standalone.sh --method standalone
bash install-atus-standalone.sh --method npm
```

```bat
install-atus-standalone.bat --method standalone
install-atus-standalone.bat --method npm
```

## Optional Native Modules

The standalone archives bundle Atus Code and a private Node.js runtime. They do
not currently install npm optional native modules such as `node-pty` and
`@teddyzhu/clipboard`. Atus Code is designed to degrade when these optional
...
bash install-atus-standalone.sh

# Record a source value
bash install-atus-standalone.sh --source github

# Use npm explicitly
bash install-atus-standalone.sh --method npm --registry https://registry.npmjs.org

# Use the Aliyun standalone mirror
bash install-atus-standalone.sh --mirror aliyun

# Install an offline archive
# SHA256SUMS must be in the same directory.
bash install-atus-standalone.sh --archive ./atus-code-linux-x64.tar.gz
```

Standalone installs to:

- Runtime: `~/.local/lib/atus-code`
- Shim: `~/.local/bin/atus`

Override with `ATUS_INSTALL_ROOT`, `ATUS_INSTALL_LIB_PARENT`,
`ATUS_INSTALL_LIB_DIR`, or `ATUS_INSTALL_BIN_DIR` when needed.
...
The uninstaller removes only the standalone runtime, generated `atus` wrapper,
and installer-managed shell PATH block. It preserves `~/.atus-code` by default. Set
`ATUS_UNINSTALL_PURGE=1` to remove `~/.atus-code/source.json`; other config and auth
...
install-atus-standalone.bat --source github

REM Use npm explicitly
install-atus-standalone.bat --method npm --registry https://registry.npmjs.org

REM Use the Aliyun standalone mirror
install-atus-standalone.bat --mirror aliyun

REM Install an offline archive
REM SHA256SUMS must be in the same directory.
install-atus-standalone.bat --archive atus-code-win-x64.zip
```

Standalone installs to:

- Runtime: `%LOCALAPPDATA%\atus-code\atus-code`
- Shim: `%LOCALAPPDATA%\atus-code\bin\atus.cmd`

Override with `ATUS_INSTALL_ROOT`, `ATUS_INSTALL_LIB_DIR`, or
`ATUS_INSTALL_BIN_DIR` when needed.

Restart the terminal if `atus` is not immediately available on PATH.
...
The uninstaller removes only the standalone runtime, generated `atus.cmd`
wrapper, user PATH entry, and the current-session `cmd.exe` shim created by the
hosted PowerShell installer. It preserves `%USERPROFILE%\.atus-code` by default. Set
`ATUS_UNINSTALL_PURGE=1` to remove `%USERPROFILE%\.atus-code\source.json`; other
...
- `ATUS_INSTALL_ARCHIVE`
- `ATUS_INSTALL_VERSION`
- `ATUS_NPM_REGISTRY`

Use `--base-url` for private mirrors. The URL must contain
`atus-code-<target>` archives and `SHA256SUMS` in the same directory. Custom
base URLs must use `https://`.

For Aliyun OSS/CDN, release publishing uploads byte-identical artifacts to the
versioned directory, for example `releases/atus-code/vX.Y.Z/`. Stable releases
also update the small `releases/atus-code/latest/VERSION` pointer used by the
...
- Linux/macOS: `~/.atus-code/source.json`
- Windows: `%USERPROFILE%\.atus-code\source.json`
...
Homebrew users can also install Atus Code with:
...
```bash
npm install -g @atus-code/atus-code@latest --registry https://registry.npmmirror.com
```

### atus Is Not on PATH After Installation
