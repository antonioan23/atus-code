...
# Removes files owned by install-atus-standalone.sh and preserves user config.
...
Options:
  --purge       Also remove the installer source marker at ~/.atus-code/source.json.
                Other atus code config and auth files are preserved.
  -h, --help    Show this help message.

Environment:
  ATUS_INSTALL_ROOT       Install root. Defaults to ~/.local.
  ATUS_INSTALL_LIB_DIR    Standalone runtime directory.
  ATUS_INSTALL_BIN_DIR    Wrapper directory.
  ATUS_UNINSTALL_PURGE=1  Same as --purge.
EOF
}

PURGE="${ATUS_UNINSTALL_PURGE:-0}"

if [[ -z "${HOME:-}" && -z "${ATUS_INSTALL_ROOT:-}" ]]; then
    log_error "HOME is not set. Set ATUS_INSTALL_ROOT to the standalone install root."
    exit 1
fi
...
remove_install_wrapper() {
    local wrapper_path="${INSTALL_BIN_DIR}/atus"
    local atus_bin="${INSTALL_LIB_DIR}/bin/atus"
    ...
    if ! grep -qF "${atus_bin}" "${wrapper_path}" 2>/dev/null &&
...
remove_shell_path_entry() {
    ...
    local legacy_marker="# Added by atus-code installer (multi-atus shadow fix)"
    local rc_files=()
    local rc_file
    ...
    for rc_file in "${rc_files[@]}"; do
        ...
        local temp_file
        temp_file=$(mktemp "${rc_file}.atus-code-uninstall.XXXXXX") || {
        ...
        ' "${rc_file}" > "${temp_file}" && mv "${temp_file}" "${rc_file}" || {
            rm -f "${temp_file}"
            log_warning "Could not remove atus code PATH entry from ${rc_file}."
            continue
        }

        log_success "Removed atus code PATH entry from ${rc_file}"
    done
}
...
remove_source_marker() {
    local source_json="${HOME:-}/.atus-code/source.json"

    if [[ "${PURGE}" != "1" ]]; then
        log_info "Preserving ${HOME:-~}/.atus-code (set ATUS_UNINSTALL_PURGE=1 to remove source.json)."
        return 0
    fi
    ...
    remove_empty_dir "${HOME}/.atus-code"
}

validate_options

echo "atus code Standalone Uninstaller"
echo ""
...
if is_qwen_standalone_install_dir "${INSTALL_LIB_DIR}"; then
    ...
    log_warning "${INSTALL_LIB_DIR} exists but is not a atus code standalone install; skipping."
else
...
if [[ "${install_was_managed}" == "1" ]]; then
    ...
    log_info "Leaving ${INSTALL_BIN_DIR}/atus unchanged because no managed standalone runtime was removed."
fi

remove_shell_path_entry
remove_source_marker

log_success "atus code standalone install removed."
