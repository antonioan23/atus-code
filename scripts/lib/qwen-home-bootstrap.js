...
/**
 * Pre-resolves ATUS_HOME / ATUS_RUNTIME_DIR from home-scoped `.env` files so
 ...
 * config dir via `~/.env` while these scripts still read `~/.atus-code/...`,
...
export function bootstrapHomeEnv() {
  ...
  const initialQwenDir = initialQwenHome
    ...
    : path.join(os.homedir(), '.atus-code');
...
function readEnvInto(file) {
  ...
  try {
    ...
    for (const key of ['ATUS_HOME', 'ATUS_RUNTIME_DIR']) {
