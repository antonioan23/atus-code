...
// Priority: ATUS_RUNTIME_DIR > ATUS_HOME > ~/.atus-code
function getRuntimeBaseDir() {
  const runtimeDir = process.env.ATUS_RUNTIME_DIR;
  if (runtimeDir) {
    return resolvePath(runtimeDir);
  }
  const homeEnv = process.env.ATUS_HOME;
  if (homeEnv) {
    return resolvePath(homeEnv);
  }
  return path.join(os.homedir(), '.atus-code');
}

// Project-level .atus-code directory in the workspace
const WORKSPACE_QWEN_DIR = path.join(projectRoot, '.atus-code');
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `atus-code-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: atus-code-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'atus-code-telemetry-'));
