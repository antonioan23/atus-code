 ...
 *
 *   # macOS / BSD `script`:
 *   script -q /tmp/atus.before.raw node dist/cli.js --yolo
 *   # …drive a SubAgent scenario, then exit atus with /quit or Ctrl-D
 *
 *   # Linux / util-linux `script`:
 *   script -q -c 'node dist/cli.js --yolo' /tmp/atus.before.raw
 *
 ...
 *   tmux pipe-pane -t flicker -o 'cat > /tmp/atus.before.raw'
 *   tmux attach -t flicker
 *
 * Then:
 *
 *   node scripts/measure-flicker.mjs /tmp/atus.before.raw
 *   node scripts/measure-flicker.mjs /tmp/atus.after.raw /tmp/atus.before.raw
 *
...
if (process.env.ATUS_FLICKER_VERBOSE) {
