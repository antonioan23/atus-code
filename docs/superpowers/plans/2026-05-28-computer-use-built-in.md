...
packages/core/src/tools/computer-use/
  ...
  install-state.ts                  # ~/.atus-code/computer-use/installed.json read/write
...
Expected: PASS (if there's no test file, run `npm run build -- --filter @atus-code/atus-code-core` to typecheck)
...
describe('computer-use schemas', () => {
  ...
  it('each tool name matches the upstream convention (no computer_use__ prefix)', () => {
    ...
    // The computer_use__ prefix lives on the atus-code-facing wrapper.
...
Create `packages/core/src/tools/computer-use/schemas.ts`. The schemas below are MVP — they reflect upstream's tool surface and parameter naming. The `sync-computer-use-schemas.ts` script (Task 13) will regenerate this file from a live upstream snapshot in CI before each atus-code release.
 ...
 * the `computer_use__` prefix is added by the atus-code-facing wrapper in
...
Run: `npm run build -- --filter @atus-code/atus-code-core --filter @atus-code/atus-code`
Expected: PASS
 ...
 * stays alive until `stop()` or atus-code exits. State (element_index
...
export class ComputerUseClient {
  ...
  static shared(): ComputerUseClient {
    if (!ComputerUseClient.singleton) {
      ComputerUseClient.singleton = new ComputerUseClient({
        packageSpec:
          process.env['ATUS_COMPUTER_USE_PACKAGE'] ??
  ...
  private async doStart(): Promise<void> {
    ...
    try {
      ...
      const client = new Client(
        { name: 'atus-code-computer-use', version: '1.0.0' },
        { capabilities: {} },
      );
  ...
  /**
   * Call a tool by upstream name (NOT the atus-code-facing
...
describe('ComputerUseTool', () => {
  ...
  it('exposes atus-facing name with computer_use__ prefix', () => {
...
> {
  constructor(
    ...
    const atusName = `computer_use__${upstreamName}`;
    super(
      atusName,
      atusName, // displayName == name; no MCP branding in UI
      schema.description,
      Kind.Other,
      schema.parameterSchema,
...
npm run build -- --filter @atus-code/atus-code-core
```
...
- [ ] **Step 2: Build atus-code**

Run: `npm run build`
Expected: PASS.

- [ ] **Step 3: Launch atus-code and test discovery**

Start atus-code, then ask the model: _"Use the ToolSearch tool with query 'click computer use' to find any desktop automation tools available."_
...
Persisted at `~/.atus-code/computer-use/installed.json`:
...
describe('install-state', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'atus-cu-test-'));
  });
...
export function installStatePathFor(home: string = homedir()): string {
  return join(home, '.atus-code', 'computer-use', 'installed.json');
}
...
git commit -m "feat(computer-use): persist install approval state under ~/.atus-code"
```
...
describe('runBootstrap', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'atus-cu-bs-'));
    deps = {
      homeDir: tmpHome,
 ...
 * atus-code's BaseDeclarativeTool exposes a `shouldConfirmExecute()`
...
export interface BootstrapDeps {
   ...
   * true if approved. Implementation may use the atus-code confirm
...
function defaultDeps(): BootstrapDeps {
  return {
    homeDir: homedir(),
    packageSpec:
      process.env['ATUS_COMPUTER_USE_PACKAGE'] ?? 'open-computer-use@latest',
    platform: process.platform,
    promptInstallApproval: async (spec) => {
      // v0 fallback: stderr prompt + stdin read. Replace with
      // atus-code's standard confirm pathway when wired in.
      ...
      // atus-code confirm system. For headless / SDK contexts the
      // default is to refuse — explicit user opt-in required.
      return process.env['ATUS_COMPUTER_USE_AUTO_APPROVE'] === '1';
    },
    spawnDoctor: () => {
...
### Task 11: Wire the real `promptInstallApproval` to atus-code's confirm system
...
```bash
rm -rf ~/.atus-code/computer-use
```

Launch atus-code and ask a computer-use question. Confirm the install prompt appears in the chosen UX (confirm dialog or stderr) and that approving it persists state correctly.

- [ ] **Step 3: Commit**

```bash
git add -A
git commit -m "feat(computer-use): wire install approval to atus-code confirm UX"
```
...
```bash
npm run build
# launch atus-code, ask a computer-use question
```
...
Runs as part of atus-code release prep. Spawns `npx -y open-computer-use@<pin> mcp`, sends `tools/list`, regenerates `schemas.ts`.
...
async function main(): Promise<void> {
  ...
  const client = new Client(
    { name: 'atus-code-schema-sync', version: '1.0.0' },
    { capabilities: {} },
  );
  await client.connect(transport);
...
- Idle timeout for the MCP server process (resource savings; v0 keeps it alive until atus-code exits).
