...
const sample: WorktreeSession = {
  slug: 'my-feature',
  worktreePath: '/repo/.atus-code/worktrees/my-feature',
...
async function seedSession(cfg: Config, slug: string) {
  await writeWorktreeSession(
    ...
    {
      slug,
      worktreePath: `/repo/.atus-code/worktrees/${slug}`,
...
注意：`readWorktreeSession` 和 `WorktreeSession` 需要从 `@atus-code/atus-code-core` 导出，需要同时在 `packages/core/src/index.ts` 中新增导出：
...
字段名和 claude-code 保持一致，方便用户在 atus-code 和 claude-code 之间复用 statusline 脚本。
  ...
  it('shows loading state initially', () => {
    const { lastFrame } = render(
      <WorktreeExitDialog
        ...
        worktreePath="/tmp/repo/.atus-code/worktrees/my-feature"
