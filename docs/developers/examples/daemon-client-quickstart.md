...
A minimal end-to-end example: start a `atus serve` daemon in another terminal, then drive it from a Node script with the SDK's `DaemonClient`. See also: [Daemon mode user guide](../../users/atus-serve.md) and [HTTP protocol reference](../atus-serve-protocol.md).
...
```bash
cd your-project/
atus serve --port 4170
# → atus serve listening on http://127.0.0.1:4170 (mode=http-bridge, workspace=/path/to/your-project)
```

Per [#3803](https://github.com/atuscode/atus-code/issues/3803) §02 each daemon binds to one workspace at boot (the current `cwd`, or override with `--workspace /path/to/dir`). The daemon's bound path is advertised on `/capabilities.workspaceCwd` so clients can pre-flight check + omit `cwd` from `POST /session`.
...
```ts
import { DaemonClient, type DaemonEvent } from '@atus-code/sdk';

const client = new DaemonClient({
  baseUrl: 'http://127.0.0.1:4170',
  // token: process.env.ATUS_SERVER_TOKEN, // required for non-loopback binds
});
...
loopback; start the daemon with `--token` or `ATUS_SERVER_TOKEN` before using
them.
...
Two clients pointed at the **same daemon** end up on the same session. Per #3803 §02 each daemon is bound to ONE workspace at boot, so the daemon launched as `atus serve --workspace /work/repo` (or `cd /work/repo && atus serve`) is what both clients connect to:

```ts
// Daemon was launched as `atus serve --workspace /work/repo` so
...
```ts
import { DaemonHttpError } from '@atus-code/sdk';
...
- [HTTP protocol reference](../atus-serve-protocol.md) — full route spec with status codes
- [Daemon mode user guide](../../users/atus-serve.md) — operator-side docs
