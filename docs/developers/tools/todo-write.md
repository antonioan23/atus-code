...
This document describes the `todo_write` tool for atus code.
...
## How to use `todo_write` with atus code
...
The tool stores todo lists in your home directory (`~/.atus-code/todos/`) with session-specific files, so each coding session maintains its own task list.
