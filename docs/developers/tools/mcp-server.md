# MCP servers with atus code

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with atus code.
...
atus code integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
atus code uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
atus code supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
```bash
atus mcp add --transport sse remote-server https://api.example.com/sse/ \
...
- **`redirectUri`** (string): Custom redirect URI. **Critical for remote deployments**: Defaults to `http://localhost:7777/oauth/callback`. When running atus code on remote/cloud servers, set this to a publicly accessible URL (e.g., `https://your-server.com/oauth/callback`). Can be configured via `atus mcp add --oauth-redirect-uri` or directly in settings.json.
...
- **Stored securely** in `~/.atus-code/mcp-oauth-tokens.json`
...
When atus code starts, it performs MCP server discovery through the following detailed process:
...
When atus code receives this response, it will:
...
In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within atus code. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="atus code" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "atus code" reverent
```
...
## Managing MCP Servers with `atus mcp`
...
### Adding a Server (`atus mcp add`)

The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.atus-code/settings.json` or the project config `.atus-code/settings.json` file.

**Command:**

```bash
atus mcp add [options] <name> <commandOrUrl> [args...]
```
...
- `--oauth-redirect-uri`: OAuth redirect URI (e.g., `https://your-server.com/oauth/callback`). Defaults to `http://localhost:7777/oauth/callback` for local setups. **Important for remote deployments**: When running atus code on remote/cloud servers, set this to a publicly accessible URL.
...
```bash
# Basic syntax
atus mcp add <name> <command> [args...]

# Example: Adding a local server
atus mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
atus mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
atus mcp add --transport http <name> <url>

# Example: Adding an HTTP server
atus mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
atus mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
atus mcp add --transport sse <name> <url>

# Example: Adding an SSE server
atus mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
atus mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"

# Example: Adding an OAuth-enabled SSE server
atus mcp add --transport sse oauth-server https://api.example.com/sse/ \
...
### Managing Servers (`atus mcp`)

To view and manage all MCP servers currently configured, use the `manage` command or simply `atus mcp`. This opens an interactive TUI dialog where you can:
...
### Removing a Server (`atus mcp remove`)
...
```bash
atus mcp remove <name>
```

**Example:**

```bash
atus mcp remove my-server
```
