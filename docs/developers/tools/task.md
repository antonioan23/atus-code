...
This document describes the `task` tool for atus code.
...
## How to use `task` with atus code
...
You can view available subagents by using the `/agents` command in atus code.
...
Subagents are configured through atus code's agent configuration system. Use the `/agents` command to:
