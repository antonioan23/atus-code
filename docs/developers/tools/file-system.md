# atus code file system tools

atus code provides a comprehensive suite of tools for interacting with the local file system. These tools allow the model to read from, write to, list, search, and modify files and directories, all under your control and typically with confirmation for sensitive operations.
...
- **Behavior:**
  ...
  - Respects .gitignore and .atusignore patterns by default.
...
- **Behavior:**
  ...
  - Respects .gitignore and .atusignore patterns.
...
When reading files, atus code detects the file's encoding using a multi-step strategy:
...
Set it in `.atus-code/settings.json` or `~/.atus-code/settings.json`:
...
These file system tools provide a foundation for atus code to understand and interact with your local project context.
