...
This document describes the `monitor` tool for atus code.
...
- `directory` (string, optional): An absolute path to run the command in. Must
  ...
  omitted, atus code uses the project root.

## How to use `monitor` with atus code
...
- **Auto-stop behavior:** Monitors stop automatically when they reach
  ...
  closed. When a monitor stops, atus code sends `SIGTERM` to the command's
  process group and escalates to `SIGKILL` after about 200 ms. On Windows, it
  uses `taskkill /f /t`. If the atus code process itself is hard-killed,
...
- **Concurrency limit:** atus code allows up to 16 running monitors per CLI
