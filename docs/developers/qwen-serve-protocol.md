# `atus serve` HTTP protocol reference

Stage 1 of the [atus-code daemon design](https://github.com/atuscode/atus-code/issues/3803). All routes live under the daemon's base URL (default `http://127.0.0.1:4170`).

## Authentication

When the daemon was started with `--token` or `ATUS_SERVER_TOKEN`, **every route except `/health` on loopback binds** must carry:
...
`mcp_guardrail_events` (issue [#4175](https://github.com/atuscode/atus-code/issues/4175) PR 14b) advertises the typed SSE push events that surface MCP budget state crossings without a poll loop. Two frame types arrive on `GET /session/:id/events`:
...
**MCP client guardrails (issue [#4175](https://github.com/atuscode/atus-code/issues/4175) PR 14).** Post-PR-14 daemons extend the payload with four additive fields and one workspace-level cell:
...
```json
{
  ...
  "workspaceCwd": "/canonical/path",
  "initialized": true,
  "current": { "authType": "atus", "modelId": "qwen3(atus)" },
  "providers": [
    {
      "kind": "model_provider",
      "status": "ok",
...
`DASHSCOPE_API_KEY`, `OPENROUTER_API_KEY`, and `ATUS_SERVER_TOKEN`. Other
...
```json
{
  ...
  "cells": [
    ...
    },
    {
      ...
      "status": "ok",
      "locality": "daemon",
      "detail": { "path": "/usr/local/bin/atus", "source": "process.argv[1]" }
    },
    {
      "kind": "workspace_dir",
      "status": "ok",
...
- `missing_binary` — Node version below required, missing `ATUS_CLI_ENTRY`,
...
Issue [#4175](https://github.com/atuscode/atus-code/issues/4175) Wave 4 PR 17 adds four mutation control routes that let remote clients change runtime posture without touching the daemon host's CLI. All four:

- Are gated by the **strict** mutation gate from PR 15. A daemon configured without a bearer token rejects them with `401 {code: 'token_required'}`. Configure `--token` (or `ATUS_SERVER_TOKEN`) before opting in.
...
Capability tag: `session_approval_mode_control`. Bridge → ACP extMethod `atus/control/session/approval_mode`.
...
Capability tag: `workspace_mcp_restart`. Bridge → ACP extMethod `atus/control/workspace/mcp/restart`.
...
- Send `Last-Event-ID: <n>` to replay events with `id > n` from the per-session ring (default depth **8000**, tunable via `atus serve --event-ring-size <n>`)
...
Capability tag: `auth_device_flow` (always advertised). Supported providers in v1: `atus-oauth`.

**Runtime locality.** The daemon never spawns a browser — even if it can. The client decides whether to call `open(verificationUri)` locally; on a headless pod (the canonical Mode B deployment) the user opens the URL on whatever device they have a browser on. See `docs/users/atus-serve.md` for the recommended UX.
...
```json
{
  ...
  "verificationUriComplete": "https://chat.atus-code.ai/api/v1/oauth2/device?user_code=USER-1",
...
```json
{
  ...
  "supportedDeviceFlowProviders": ["atus-oauth"]
}
```
...
| Var                 | Purpose                                                                                                                                                             |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ATUS_SERVER_TOKEN` | Bearer token. Stripped of leading/trailing whitespace at boot.                                                                                                      |
| `SKIP_LLM_TESTS`    | Set to `1` to **skip** LLM-required integration tests in `integration-tests/cli/atus-serve-streaming.test.ts` (default-on for CI envs that lack provider API keys). |
...
| `integration-tests/cli/atus-serve-routes.test.ts`    | 18 cases, no LLM                                                                                           |
| `integration-tests/cli/atus-serve-streaming.test.ts` | 3 cases, real `atus --acp` child (skipped when `SKIP_LLM_TESTS=1`)                                         |
