...
Learn how to enable and setup OpenTelemetry for atus code.
...
observability framework — atus code's observability system provides:
...
All telemetry behavior is controlled through your `.atus-code/settings.json` file.
These settings can be overridden by environment variables or CLI flags.

| Setting                          | Environment Variable                               | CLI Flag                                                 | Description                                                                                                                          | Values            | Default                 |
| -------------------------------- | -------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------- | ----------------------- |
| `enabled`                        | `ATUS_TELEMETRY_ENABLED`                           | `--telemetry` / `--no-telemetry`                         | Enable or disable telemetry                                                                                                          | `true`/`false`    | `false`                 |
| `target`                         | `ATUS_TELEMETRY_TARGET`                            | `--telemetry-target <local\|gcp>` _(deprecated)_         | Informational destination label; does not control exporter routing — set `otlpEndpoint` or `outfile` to configure where data is sent | `"gcp"`/`"local"` | `"local"`               |
| `otlpEndpoint`                   | `ATUS_TELEMETRY_OTLP_ENDPOINT`                     | `--telemetry-otlp-endpoint <URL>`                        | OTLP collector endpoint                                                                                                              | URL string        | `http://localhost:4317` |
| `otlpProtocol`                   | `ATUS_TELEMETRY_OTLP_PROTOCOL`                     | `--telemetry-otlp-protocol <grpc\|http>`                 | OTLP transport protocol                                                                                                              | `"grpc"`/`"http"` | `"grpc"`                |
| `otlpTracesEndpoint`             | `ATUS_TELEMETRY_OTLP_TRACES_ENDPOINT`              | -                                                        | Per-signal endpoint override for traces (HTTP only)                                                                                  | URL string        | -                       |
| `otlpLogsEndpoint`               | `ATUS_TELEMETRY_OTLP_LOGS_ENDPOINT`                | -                                                        | Per-signal endpoint override for logs (HTTP only)                                                                                    | URL string        | -                       |
| `otlpMetricsEndpoint`            | `ATUS_TELEMETRY_OTLP_METRICS_ENDPOINT`             | -                                                        | Per-signal endpoint override for metrics (HTTP only)                                                                                 | URL string        | -                       |
| `outfile`                        | `ATUS_TELEMETRY_OUTFILE`                           | `--telemetry-outfile <path>`                             | Save telemetry to file (overrides OTLP export)                                                                                       | file path         | -                       |
| `logPrompts`                     | `ATUS_TELEMETRY_LOG_PROMPTS`                       | `--telemetry-log-prompts` / `--no-telemetry-log-prompts` | Include prompts in telemetry logs                                                                                                    | `true`/`false`    | `true`                  |
| `includeSensitiveSpanAttributes` | `ATUS_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES` | -                                                        | Include user prompts, system prompts, tool I/O, and model output as native span attributes (in addition to log-to-span bridge spans) | `true`/`false`    | `false`                 |
...
| `metrics.includeSessionId`       | `ATUS_TELEMETRY_METRICS_INCLUDE_SESSION_ID`        | -                                                        | Include `session.id` on metric data points. **Disabled by default** to protect metric backends from time-series fan-out.             | `true`/`false`    | `false`                 |
...
1. **Native span attributes (`atus-code.interaction`, `api.generateContent*`,
...
independently of this setting. AtusLogger does not include `response_text`.

**HTTP OTLP signal routing:** When using HTTP protocol (`otlpProtocol: "http"`),
atus code automatically appends signal-specific paths (`/v1/traces`, `/v1/logs`,
...
The `ATUS_TELEMETRY_OTLP_*` variants take precedence over the `OTEL_*` variants.
...
2. `telemetry.resourceAttributes` in `.atus-code/settings.json` (overrides env on
   key conflict)
...
```bash
export OTEL_SERVICE_NAME=atus-code-ci
```

**Fleet baseline (`~/.atus-code/settings.json`) + per-host override:**
...
to the debug log file (`~/.atus-code/log/otel-*.log`), **not** the console, so the
behavior can look like silent failure.

If a custom resource attribute isn't appearing on exported telemetry:

1. Check `~/.atus-code/log/otel-*.log` for lines matching `cannot override` (reserved
   key dropped), `Skipping malformed` (bad env var pair), or `must be a string`
   (non-string settings value).
2. Verify the env var is set in the atus-code process's environment (not just
...
To prevent this, atus code keeps high-cardinality attributes off metric data
...
`ATUS_TELEMETRY_METRICS_INCLUDE_SESSION_ID=true`) re-attaches `session.id` to
...
```bash
export ATUS_TELEMETRY_METRICS_INCLUDE_SESSION_ID=true
```
...
When telemetry is enabled, atus code registers `UndiciInstrumentation`
...
which would themselves be uploaded, causing an infinite loop. atus code's
...
client-side correlation data atus-code writes **into outbound LLM API
...
When `false` (default), atus code installs a no-op `TextMapPropagator` on
...
`X-Atus-Code-Session-Id` and `X-Atus-Code-Request-Id` are **not part of
...
To view atus code telemetry in Alibaba Cloud Managed Service for
OpenTelemetry, configure atus code to export to the OTLP endpoint
provided by ARMS.

Setting `"target": "gcp"` alone does not configure the export
destination. If `otlpEndpoint` is not set, atus code still defaults to
...
1. Enable telemetry in your `.atus-code/settings.json` and set the OTLP
   endpoint:
   ...
   **Option B: HTTP protocol with per-signal endpoints** (for backends
   ...
   > per-signal overrides), atus code appends standard OTLP paths
...
3. Run atus code and send prompts.
...
1. Enable telemetry in your `.atus-code/settings.json`:

   ```json
   {
     "telemetry": {
       "enabled": true,
       "outfile": ".atus-code/telemetry.log"
     }
   }
   ```
...
2. Run atus code and send prompts.
3. View logs and metrics in the specified file (e.g., `.atus-code/telemetry.log`).
...
1. Run the automation script:
   ...
   - Save logs/metrics to `~/.atus-code/tmp/<projectHash>/otel/collector.log`
...
Logs are timestamped records of specific events. The following events are logged for atus code:

- `atus-code.config`: This event occurs once at startup with the CLI's configuration.
  - **Attributes**:
    - `model` (string)
...
- `atus-code.user_prompt`: This event occurs when a user submits a prompt.
  - **Attributes**:
    - `prompt_length` (int)
    - `prompt_id` (string)
...
- `atus-code.tool_call`: This event occurs for each function call.
  - **Attributes**:
    - `function_name`
    - `function_args`
    - `duration_ms`
...
- `atus-code.file_operation`: This event occurs for each file operation.
  - **Attributes**:
    - `tool_name` (string)
...
- `atus-code.api_request`: This event occurs when making a request to Qwen API.
  - **Attributes**:
    - `model`
    - `request_text` (if applicable)

- `atus-code.api_error`: This event occurs if the API request fails.
  - **Attributes**:
    - `model`
    - `error`
    - `error_type`
...
- `atus-code.api_response`: This event occurs upon receiving a response from Qwen API.
  - **Attributes**:
    - `model`
    - `status_code`
    - `duration_ms`
...
- `atus-code.tool_output_truncated`: This event occurs when the output of a tool call is too large and gets truncated.
  - **Attributes**:
    - `tool_name` (string)
...
- `atus-code.malformed_json_response`: This event occurs when a `generateJson` response from Qwen API cannot be parsed as a json.
  - **Attributes**:
    - `model`

- `atus-code.flash_fallback`: This event occurs when atus code switches to flash as fallback.
  - **Attributes**:
    - `auth_type`

- `atus-code.slash_command`: This event occurs when a user executes a slash command.
  - **Attributes**:
    - `command` (string)
    - `subcommand` (string, if applicable)

- `atus-code.extension_enable`: This event occurs when an extension is enabled
- `atus-code.extension_install`: This event occurs when an extension is installed
  - **Attributes**:
    - `extension_name` (string)
    - `extension_version` (string)
    - `extension_source` (string)
    - `status` (string)
- `atus-code.extension_uninstall`: This event occurs when an extension is uninstalled

### Metrics

Metrics are numerical measurements of behavior over time. The following metrics are collected for atus code (metric names remain `atus-code.*` for compatibility):

- `atus-code.session.count` (Counter, Int): Incremented once per CLI startup.

- `atus-code.tool.call.count` (Counter, Int): Counts tool calls.
  - **Attributes**:
    - `function_name`
    - `success` (boolean)
...
- `atus-code.tool.call.latency` (Histogram, ms): Measures tool call latency.
  - **Attributes**:
    - `function_name`
    - `decision` (string: "accept", "reject", or "modify", if applicable)

- `atus-code.api.request.count` (Counter, Int): Counts all API requests.
  - **Attributes**:
    - `model`
    - `status_code`
    - `error_type` (if applicable)

- `atus-code.api.request.latency` (Histogram, ms): Measures API request latency.
  - **Attributes**:
    - `model`

- `atus-code.token.usage` (Counter, Int): Counts the number of tokens used.
  - **Attributes**:
    - `model`
    - `type` (string: "input", "output", "thought", or "cache")

- `atus-code.file.operation.count` (Counter, Int): Counts file operations.
  - **Attributes**:
...
- `atus-code.chat_compression` (Counter, Int): Counts chat compression operations
  - **Attributes**:
