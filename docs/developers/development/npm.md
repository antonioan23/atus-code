...
This monorepo contains two main packages: `@atus-code/atus-code` and `@atus-code/atus-code-core`.

## `@atus-code/atus-code`

This is the main package for atus code. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@atus-code/atus-code-core`. This means that whether a user installs the package with `npm install -g @atus-code/atus-code` or runs it directly with `npx @atus-code/atus-code`, they are using this single, self-contained executable.

## `@atus-code/atus-code-core`
...
Releases are managed through the [release.yml](https://github.com/atuscode/atus-code/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
# Preview
npm install -g @atus-code/atus-code@preview

# Nightly
npm install -g @atus-code/atus-code@nightly
```
...
- `npx -y @atus-code/atus-code@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @atus-code/atus-code@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @atus-code/atus-code && npm uninstall -g @atus-code/atus-code && npm cache clean --force &&  npm install @atus-code/atus-code@<version>`
...
1.  Go to the [Actions tab](https://github.com/atuscode/atus-code/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/atus-code-0.1.6.tgz`).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @atus-code/atus-code`.
