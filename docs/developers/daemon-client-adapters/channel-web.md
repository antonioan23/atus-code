...
Let channel adapters and web chat backends consume `atus serve` through
`DaemonSessionClient` while keeping existing channel ACP subprocess behavior as
the default.

This draft covers server-side clients only:

- Channel bot backend -> `atus serve`
- Web browser -> web backend / BFF -> `atus serve`
...
```bash
ATUS_CHANNEL_DAEMON_URL=http://127.0.0.1:4170 atus channel start telegram
```

Web backend:

```bash
ATUS_WEB_DAEMON_URL=http://127.0.0.1:4170 atus web-chat-backend
```
...
The bridge is not wired into `atus channel start` yet. Existing Telegram,
...
- Smoke-test one single-user channel backend against local `atus serve`.
