...
extension host to `atus serve` through `DaemonSessionClient`.
...
```json
{
  "atus-code.experimentalDaemon.enabled": true,
  "atus-code.experimentalDaemon.url": "http://127.0.0.1:4170",
  "atus-code.experimentalDaemon.token": ""
}
```
...
```bash
ATUS_IDE_DAEMON_URL=http://127.0.0.1:4170 code .
```
...
```text
AtusAgentManager
  current default -> AcpConnection -> atus --acp child
  experimental    -> DaemonIdeConnection -> atus serve HTTP/SSE
```
...
adapter spike. It is not wired into the default `AtusAgentManager` path yet, so
...
- Smoke-test local extension host against `atus serve`:
