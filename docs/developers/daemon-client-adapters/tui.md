...
Add a flag-gated TUI transport that talks to `atus serve` through
...
```bash
ATUS_DAEMON_URL=http://127.0.0.1:4170 atus --experimental-daemon-tui
```

Optional:

```bash
ATUS_DAEMON_TOKEN=... ATUS_DAEMON_WORKSPACE=/repo atus --experimental-daemon-tui
```

The CLI should refuse this mode unless both are true:

- `ATUS_DAEMON_URL` or `--daemon-url` is set.
...
- If the daemon is unavailable, the experimental path fails before starting the
  TUI and tells the user to run `atus serve`.
...
- Smoke-test against a local `atus serve`:
