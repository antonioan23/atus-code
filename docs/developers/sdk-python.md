...
`atus-code-sdk` is an experimental Python SDK for atus code. v1 targets the
existing `stream-json` CLI protocol and keeps the transport surface small and
testable.

## Scope

- Package name: `atus-code-sdk`
- Import path: `atus_code_sdk`
- Runtime requirement: Python `>=3.10`
- CLI dependency: external `atus` executable is required in v1
...
```bash
pip install atus-code-sdk
```

For preview releases:

```bash
pip install --pre atus-code-sdk
```

If `atus` is not on `PATH`, pass `path_to_qwen_executable` explicitly.
...
```python
from atus_code_sdk import is_sdk_result_message, query_sync
...
| `path_to_qwen_executable`  | `str`                                                      | `atus`, an explicit binary path, or a `.js` CLI bundle.                                                         |
...
| `auth_type`                | `openai`, `anthropic`, `atus-oauth`, `gemini`, `vertex-ai` | Authentication mode passed to the CLI.                                                                          |
...
from atus_code_sdk import SDKUserMessage, is_sdk_result_message, query
...
async def main():
    async with query(
        ...
        await result.set_model("atus-plus")
...
try:
    with query_sync("Say hello", {"path_to_qwen_executable": "atus"}) as result:
        for message in result:
            if is_sdk_result_message(message):
                if message.get("is_error"):
                    error = message.get("error") or {}
    ...
    print(f"atus exited with {exc.exit_code}: {exc}")
```
...
- Verify `atus --version` works in the target environment
...
- Check that the target `atus` version supports `--input-format stream-json`
...
- `npm run smoke:sdk:python -- --atus atus`

## Real E2E Smoke

For a real runtime check (actual `atus` process + real model call), run from
