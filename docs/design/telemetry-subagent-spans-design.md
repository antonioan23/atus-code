...
> Issue #3731 — Phase 3 of hierarchical session tracing. Adds a `atus-code.subagent` span so subagent invocations get isolated, queryable trace structure instead of interleaving silently under the parent `atus-code.interaction` span.
>
> Builds on Phase 1 (#4126), Phase 1.5 (#4302), and Phase 2 (#4321).

## Problem

Today every `AgentTool.execute` invocation runs under the parent's `atus-code.interaction` span. Three pathologies:
...
2. **No span for the subagent boundary itself.** There's a `atus-code.subagent_execution` LogRecord (emitted from `agent-headless.ts:268,329`) bridged to a span of the same name via `LogToSpanProcessor`, but it's a stand-alone marker, not a parent that nests the subagent's LLM / tool / hook spans underneath.
...
| `SubagentExecutionEvent` LogRecord | `agent-headless.ts:268,329` → `loggers.ts:773` → 3 downstreams (LogToSpanProcessor span bridge + AtusLogger RUM + `recordSubagentExecutionMetrics`)                                              | LogRecord stays; downstreams depend on it                   |
...
- **Migrating the `atus-code.subagent_execution` LogRecord onto the new span as span events.** RUM and metrics are tightly coupled to the LogRecord; deferred to a follow-up that can renegotiate all 3 consumers together.
    ...
    links: [
      {
        ...
        attributes: { 'atus-code.link.kind': 'invoker' },
      },
    ],
...
function ttlFor(ctx: SpanContext): number {
  if (
    ctx.type === 'subagent' &&
    ctx.attributes['atus-code.subagent.invocation_kind'] !== 'foreground'
  ) {
...
```ts
{
  'atus-code.span.ttl_expired': true,
  'atus-code.span.duration_ms': age,
  'atus-code.subagent.status': 'aborted',
  'atus-code.subagent.terminate_reason': 'ttl_swept',
}
```
...
| ---------------------------------------------------------------------------------- | ------------------------------------------------- | --------------------------------------------------------------------------------------- |
| OTel LogRecord → `LogToSpanProcessor` → bridge span `atus-code.subagent_execution` | `loggers.ts:773` → `log-to-span-processor.ts:346` | **Skip this bridge** for the subagent event — new `atus-code.subagent` span replaces it |
| AtusLogger RUM ingestion (Aliyun internal stats)                                   | `atus-logger.ts:573-574`                          | Keep — RUM doesn't see OTel spans, only LogRecords                                      |
...
const skipBridge = new Set<string>([
  EVENT_SUBAGENT_EXECUTION, // covered by native atus-code.subagent span
]);
if (skipBridge.has(eventName)) return;
```

**Trace consumer impact**: dashboards that filter on span name `atus-code.subagent_execution` start returning zero results. They should be updated to `atus-code.subagent`. Note this in release notes.

**Why not delete the LogRecord**: it's the input to RUM and metrics. Deleting it is a 3-system refactor; out of scope here.

**Why not keep both**: trace would show two spans per subagent (`atus-code.subagent` + `atus-code.subagent_execution`) carrying overlapping info — confusing for operators reading traces, duplicate span volume.

### D5 — Span name + attrs: hybrid spec compliance, vendor-prefixed for extensions

**Span name**: `atus-code.subagent` (matches Phase 1/2 codebase convention: `atus-code.interaction`, `atus-code.tool`, `atus-code.hook`, …).

OTel GenAI spec says the canonical span name is `invoke_agent {gen_ai.agent.name}` — but **also** says "individual GenAI systems/frameworks MAY specify different span name formats." We use our own name and set `gen_ai.operation.name='invoke_agent'` so spec-aware tooling still identifies the span. Operators reading our trace tree see consistent `atus-code.*` naming.
...
| **Required spec**                                                | `gen_ai.provider.name='atus-code'`              | literal                                                              | spec-required; ambiguous for in-process agents (spec wrote it for LLM provider). Setting to `'atus-code'` is the most honest interpretation                                      |
| **Required (dual-emit)**                                         | `gen_ai.agent.id` + `atus-code.subagent.id`     | `agentContext.agentId`                                               | dual-emit until spec reaches Stable; remove vendor key later                                                                                                                     |
| **Required (dual-emit)**                                         | `gen_ai.agent.name` + `atus-code.subagent.name` | `agentConfig.subagentType` (e.g. `Explore`, `code-reviewer`, `fork`) | same dual-emit                                                                                                                                                                   |
...
| **Vendor**                                                       | `atus-code.subagent.invocation_kind`            | `'foreground'` ❘ `'fork'` ❘ `'background'`                           | drives TTL + traceId strategy                                                                                                                                                    |
| **Vendor**                                                       | `atus-code.subagent.is_built_in`                | bool                                                                 | dashboard filter                                                                                                                                                                 |
| **Vendor**                                                       | `atus-code.subagent.parent_agent_id`            | parent ALS `agentId`                                                 | for nested subagents + cross-trace lineage                                                                                                                                       |
| **Vendor**                                                       | `atus-code.subagent.depth`                      | parent depth + 1 (top = 0)                                           | recursion-bug detector                                                                                                                                                           |
| **Vendor**                                                       | `atus-code.subagent.invoking_request_id`        | from `agentContext`                                                  | request-level correlation                                                                                                                                                        |
...
| **End-of-span vendor**                                           | `atus-code.subagent.status`                     | `'completed'` ❘ `'failed'` ❘ `'cancelled'` ❘ `'aborted'`             | finer than OTel SpanStatus (which is OK / ERROR / UNSET)                                                                                                                         |
| **End-of-span vendor**                                           | `atus-code.subagent.terminate_reason`           | from `SubagentExecutionEvent.terminate_reason`                       | e.g. `task_complete`, `max_iterations`, `user_abort`, `ttl_swept`                                                                                                                |
| **End-of-span vendor**                                           | `atus-code.subagent.result_summary_present`     | bool                                                                 | "did subagent produce output" — bounded                                                                                                                                          |
...
**Why `atus-code.subagent.*` (not `atus.subagent.*`)**: every existing vendor-prefixed key in `constants.ts` uses `atus-code.*` (`atus-code.user_prompt`, `atus-code.tool_call`, etc.). Internal consistency > OTel naming-convention preference, since operators query ARMS by prefix.
...
**~10-15 attrs per span** (depending on invocation kind, failure, nesting). Same order as `atus-code.tool`.
...
A new tiny accessor `getCurrentAgentDepth(): number` returns `agentContextStorage.getStore()?.depth ?? 0` — used by `startSubagentSpan` to populate `atus-code.subagent.depth`.
...
// (set atus-code.subagent.depth = depth in startSubagentSpan args)
...
Three concurrent subagent invocations from one user prompt (model emits 3 AGENT tool_use blocks → `coreToolScheduler.runConcurrently` runs 3 `executeSingleToolCall` in parallel; each opens its own `atus-code.tool` span per Phase 2):

```
atus-code.interaction                         [traceId=T0]
├─ atus-code.tool [agent call #A]
│  └─ atus-code.subagent (A, foreground)     [traceId=T0, child]
│     ├─ atus-code.llm_request
│     └─ atus-code.tool [...]
│        └─ atus-code.tool.execution
├─ atus-code.tool [agent call #B]
│  └─ atus-code.subagent (B, foreground)     [traceId=T0, child]
│     └─ atus-code.llm_request
└─ atus-code.tool [agent call #C]
   └─ atus-code.subagent (C, fork)           [traceId=T1, linked root]
      └─ atus-code.llm_request                [traceId=T1]
...
Fork (C) is a separate trace — its child spans inherit `traceId=T1` even when emitted across multiple subsequent interactions of the parent session. ARMS query by `session.id` returns both T0 and T1; the Link from T1's root → C's invoking `atus-code.tool` span provides explicit navigation.
...
| `packages/core/src/telemetry/log-to-span-processor.ts`      | Skip-list to bypass bridging `atus-code.subagent_execution`                                                                                                                                   | +6      |
...
| `endSubagentSpan dual-emits gen_ai.agent.id + atus-code.subagent.id`         | Spec-compliance dual-emit                                       |
...
| `LogToSpanProcessor skips atus-code.subagent_execution but still RUM-emits`  | Bridge skip works                                               |
...
| Fork still alive when parent session ends                                                                               | 4h TTL fires; sentinel attrs `atus-code.span.ttl_expired:true`, `atus-code.subagent.terminate_reason='ttl_swept'`, `status='aborted'`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
For atus-code's current default (per `tracer.ts:shouldForceSampled()` — parentbased + always_on else always_on), every span is sampled, so the divergence doesn't bite. For deployments using probabilistic samplers (e.g. `traceidratio=0.1`), this means:
...
- Independent of Phase 4 (LLM request decomposition / TTFT). Phase 4 attaches to `atus-code.llm_request` spans regardless of whether they're under a subagent or an interaction. Recommend Phase 3 before Phase 4 so Phase 4's per-attempt metrics can be aggregated per-subagent.

## Open questions

1. **`gen_ai.provider.name`**: spec requires it but writes the description for LLM provider, not agent framework. Setting to `'atus-code'` is best interpretation; if a future spec revision adds an `agent.provider.name` variant we should switch.
2. **Span name `atus-code.subagent` vs spec `invoke_agent {name}`**: chose internal consistency. If GenAI-aware tooling adoption grows and `invoke_agent ${name}` becomes critical for auto-discovery, we can switch — span name is the most rebrandable thing in OTel.
