...
symlink planted in `~/.atus-code/projects/<proj>/chats/` can't redirect a
...
4. `!config.isInteractive()` → skip. Headless `atus -p` / CI never spends
   fast-model tokens on a one-shot session.
5. `autoTitleDisabledByEnv()` → skip. `ATUS_DISABLE_AUTO_TITLE=1`
   explicit opt-out.
...
| `ATUS_DISABLE_AUTO_TITLE=1` | unset   | Opt out of the auto trigger without unsetting `fastModel`. `/rename --auto` still works on request. |
...
| Non-interactive auto-titling                | `atus -p` / CI scripts throw the session away; fast-model tokens for a title no one will ever resume is pure waste.                         |
