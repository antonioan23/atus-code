...
> 基于 2026-05-13 对 atus-code origin/main 的复核

## 现状

atus-code 已具备 tracing 基础设施：
...
### atus-code 现有架构 vs claude-code

#### 根本差异：两套断裂的 span 创建路径

这是 atus-code 当前最关键的架构问题：
...
atus-code 的 `session-tracing.ts` 已经实现了与 claude-code **几乎相同的 parent 解析模式**：

```typescript
// atus-code session-tracing.ts (已有但未用)
...
**关键差异：** atus-code 的 `executeSingleToolCall` 入口检查 `toolCall.status !== 'scheduled'` 才继续——也就是说调用到这里时审批已经完成。Tool span 的 `withSpan` 包不住审批等待。
...
| 现有记录方式  | OTel span + Perfetto span           | `HookCallEvent` → `AtusLogger` (无 OTel)                             |
...
atus-code 的 `startToolExecutionSpan(parentToolSpan)` / `endToolExecutionSpan(span, metadata)` 已经完整实现，只需在 `executeSingleToolCall` 中调用：
...
注意：atus-code 的 `startToolExecutionSpan` 接收显式 `parentToolSpan` 参数，而 claude-code 的是从 `toolContext` ALS 隐式获取。这不影响功能，只是风格差异。如果引入 `toolContext` ALS，可以统一改为隐式获取。
...
- 短期：沿用 atus-code 现有的 `subagentNameContext` + 事件日志方案
...
atus-code 当前在 `loggingContentGenerator.ts` 中用 `withSpan('api.generateContent', ...)` 和 `startSpanWithContext('api.generateContentStream', ...)`。
