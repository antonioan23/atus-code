...
> External messaging integrations for atus code — interact with an agent from Telegram, WeChat, and more.
>
> User documentation: [Channels Overview](../../users/features/channels/overview.md).

## Overview

A **channel** connects an external messaging platform to a atus code agent. Configured in `settings.json`, managed via `atus channel` subcommands, multi-user (each user gets an isolated ACP session).
...
One `atus-code --acp` process with multiple ACP sessions. Scope per channel: **`user`** (default), **`thread`**, or **`single`**. Routing keys namespaced as `<channelName>:<key>`.
...
External plugins are **extensions** managed by `ExtensionManager`, declared in `atus-extension.json`:
...
Loading sequence at `atus channel start`: load settings → register built-ins → scan extensions → dynamic import + validate → register (reject collisions) → validate config → `createChannel()` → `connect()`.
...
```bash
# Channels
atus channel start [name]                     # start all or one channel
atus channel stop                             # stop running service
atus channel status                           # show channels, sessions, uptime
atus channel pairing list <ch>                # pending pairing requests
atus channel pairing approve <ch> <code>      # approve a request

# Extensions
atus extensions install <path-or-package>     # install
atus extensions link <local-path>             # symlink for dev
atus extensions list                          # show installed
atus extensions remove <name>                 # uninstall
```
...
├── telegram/                # @atus-code/channel-telegram
├── weixin/                  # @atus-code/channel-weixin
└── dingtalk/                # @atus-code/channel-dingtalk
```
...
- **`atus channel doctor`** — config validation, env vars, bot tokens, network checks
- **`atus channel status --probe`** — real connectivity checks per channel
...
- **Community plugin template** — `create-atus-channel` scaffolding tool
- **Plugin registry/discovery** — `atus extensions search`, version compatibility
