...
claude-code addresses (1) with `stripImagesFromMessages`. atus-code has
...
- **Large-paste externalization to a paste cache.** An earlier draft
  ...
  `~/.atus-code/paste-cache/<sha>.txt` and substituting a placeholder. We
...
| Concern                          | atus-code today                                      | claude-code reference                                            | Target after this change                                            |
| -------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------- |
...
atus-code attaches tool-returned media on `functionResponse.parts`
...
Plus an env override for ops/debug: `ATUS_IMAGE_TOKEN_ESTIMATE`.
...
(unsafe). For non-VL models (Qwen3-Coder, the atus-code default) the
