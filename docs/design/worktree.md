...
atus-code 目前仅有面向 Arena 多模型对比场景的内部 worktree 实现（`GitWorktreeService`），用户无法在普通会话中使用 worktree 隔离工作，AgentTool 也不支持为 subagent 创建隔离的 worktree 环境。

目标是将 worktree 做成通用能力，支持用户会话级隔离和 Agent 级隔离，同时保证现有 Arena 功能体验完全不变。

## 现状对比

| 功能                              | atus-code       | claude-code | 阶段    |
| --------------------------------- | --------------- | ----------- | ------- |
...
| tmux 集成                         | ❌              | ✅          | Future  |
| Arena 多模型 worktree 隔离        | ✅（atus 独有） | ❌          | —       |
| 脏状态覆盖（stash + copy）        | ✅              | ✅          | —       |
| Baseline commit 追踪              | ✅（atus 独有） | ❌          | —       |
...
```
{git 仓库根}/.atus-code/worktrees/{slug}
```
...
Arena 的 worktree 路径由 `agents.arena.worktreeBaseDir` 控制，默认 `~/.atus-code/arena`（`ArenaManager.ts:125`），与通用路径完全独立，不做任何改动。
...
3. 调用 `GitWorktreeService` 创建 worktree，路径为 `.atus-code/worktrees/{slug}`
...
| CLI 启动标志   | `atus --worktree my-feature`                             | Phase D  |
...
- Agent 启动时创建临时 worktree（slug：`agent-{7hex}`，路径：`.atus-code/worktrees/agent-{7hex}`）
- Agent 结束后：无变更则自动删除；有变更则保留，将路径和分支返回在结果中
- 过期 worktree 自动清理：扫描 `.atus-code/worktrees/`，匹配 `agent-{7hex}` 模式，超过 30 天且无未推送提交则删除，fail-closed 策略
...
- review skill（`SKILL.md`）：review 使用独立机制（路径 `.atus-code/tmp/review-pr-<n>`，通过 `atus review fetch-pr` 命令创建），与通用 worktree 路径和机制完全不同，不存在混淆
...
| 形式                      | 行为                                                 |
| ------------------------- | ---------------------------------------------------- |
| `atus --worktree`         | bare flag，自动生成 slug（`{形容词}-{名词}-{6hex}`） |
| `atus --worktree my-name` | 显式 slug，沿用 `EnterWorktreeTool` 的 slug 校验规则 |
| `atus --worktree=my-name` | 等价于上一种                                         |

不提供短别名 `-w`（atus-code 短别名只保留给最高频参数，避免命名冲突）。
...
- 分支：`worktree-pr-<N>`（沿用 atus-code 现有 `worktree-<slug>` 命名规则；不采用 claude-code 的 `pr-<N>` 直接命名，避免与本地 `pr-<N>` 分支冲突）
...
1. **`--worktree-keep-on-exit`？** claude-code 没有，atus-code 是否需要一个 CLI flag 让 Exit Dialog 默认选 keep？建议**先不加**，等用户反馈。
