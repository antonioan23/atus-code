...
atus code supports multiple API protocols through `authType` / `modelProviders` keys, including `openai`, `anthropic`, and `gemini`. Therefore, the custom setup wizard should start by asking users to select the protocol, then collect endpoint, key, and model information for that protocol.
...
This keeps the custom API key setup inside atus code, reduces the need to manually edit `settings.json`, and makes the final configuration transparent by showing the generated JSON before saving.
...
https://atuscode.github.io/atus-code-docs/en/users/configuration/model-providers/

Esc to go back
```

This requires users to leave the CLI, read documentation, understand `settings.json`, manually configure `modelProviders`, choose an `envKey`, add API keys, and then return to atus code. Users have reported that this flow is difficult and disconnected from the rest of the `/auth` experience.
...
Custom API key setup should offer a similar guided experience, while also respecting that atus code supports multiple provider protocols.
...
2. Support the main protocols atus code supports in `modelProviders`: `openai`, `anthropic`, and `gemini`.
...
│                                                              │
│ Model IDs: atus/qwen3-coder,openai/gpt-4.1_                  │
│                                                              │
│ Enter to continue, Esc to go back                            │
└──────────────────────────────────────────────────────────────┘
```
...
```text
Input:
atus/qwen3-coder, openai/gpt-4.1, atus/qwen3-coder

Normalized:
atus/qwen3-coder, openai/gpt-4.1
```
...
│ {                                                            │
│   "env": {                                                   │
│     "ATUS_CUSTOM_API_KEY_OPENAI_HTTPS_OPENROUTER_AI_API_V1":│
│       "sk-••••••••••••••••"                                  │
│   },                                                         │
│   "modelProviders": {                                        │
│     "openai": [                                              │
│       {                                                      │
│         "id": "atus/qwen3-coder",                           │
│         "name": "atus/qwen3-coder",                         │
│         "baseUrl": "https://openrouter.ai/api/v1",          │
│         "envKey": "ATUS_CUSTOM_API_KEY_OPENAI_HTTPS_OPENROUTER_AI_API_V1"│
│       }                                                      │
│     ]                                                        │
│   },                                                         │
│   "security": {                                              │
...
{
  "env": {
    "ATUS_CUSTOM_API_KEY_ANTHROPIC_HTTPS_API_ANTHROPIC_COM_V1": "sk-••••"
  },
  "modelProviders": {
    "anthropic": [
      {
        ...
        "envKey": "ATUS_CUSTOM_API_KEY_ANTHROPIC_HTTPS_API_ANTHROPIC_COM_V1"
      }
    ]
  },
  "security": {
...
```text
ATUS_CUSTOM_API_KEY_${PROTOCOL}_${NORMALIZED_BASE_URL}
```
...
-> ATUS_CUSTOM_API_KEY_OPENAI_HTTPS_API_OPENAI_COM_V1
...
-> ATUS_CUSTOM_API_KEY_ANTHROPIC_HTTPS_API_ANTHROPIC_COM_V1

Protocol: gemini
Base URL: https://generativelanguage.googleapis.com
-> ATUS_CUSTOM_API_KEY_GEMINI_HTTPS_GENERATIVELANGUAGE_GOOGLEAPIS_COM

Protocol: openai
Base URL: http://localhost:11434/v1
-> ATUS_CUSTOM_API_KEY_OPENAI_HTTP_LOCALHOST_11434_V1
```
...
return ATUS_CUSTOM_API_KEY_${NORMALIZED_PROTOCOL}_${NORMALIZED_BASE_URL}
```
...
function generateCustomApiKeyEnvKey(protocol: string, baseUrl: string): string {
  ...
  return `ATUS_CUSTOM_API_KEY_${normalize(protocol)}_${normalize(baseUrl)}`;
}
```
...
Model IDs: atus/qwen3-coder,openai/gpt-4.1
```
...
{
  "env": {
    "ATUS_CUSTOM_API_KEY_OPENAI_HTTPS_OPENROUTER_AI_API_V1": "sk-or-v1-xxx"
  },
  "modelProviders": {
    "openai": [
      {
...
```text
generatedEnvKey = ATUS_CUSTOM_API_KEY_${PROTOCOL}_${NORMALIZED_BASE_URL}
```
