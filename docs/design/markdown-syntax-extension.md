...
the image path with `ATUS_CODE_MERMAID_IMAGE_RENDERING=1`, then provide
`@mermaid-js/mermaid-cli` by installing `mmdc` on `PATH` or by setting
`ATUS_CODE_MERMAID_MMD_CLI` to the binary path. For ad-hoc local verification,
`ATUS_CODE_MERMAID_ALLOW_NPX=1` allows the renderer to invoke
...
`ATUS_CODE_MERMAID_ALLOW_LOCAL_RENDERERS=1` is set. Terminal protocol selection
can be forced with `ATUS_CODE_MERMAID_IMAGE_PROTOCOL=kitty|iterm2|off`.
