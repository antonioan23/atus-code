...
`~/.atus-code/debug/<sessionId>.txt` (`latest.txt` symlinks to the current
session); disable via `ATUS_DEBUG_LOG_FILE=0`.
...
| `ATUS_CODE_ENABLE_AWAY_SUMMARY` env var          | Claude Code uses it to keep the feature on when telemetry is disabled; atus code's current telemetry model doesn't need this.            |
