# atus code Agent Loop RT 优化技术方案

## 1. 背景与问题定义

### 1.1 现状

atus code 的 Agent Loop 为严格串行模型：
...
| 并发 prevalidate 共享 fd / 资源争抢        | 低     | `ATUS_CODE_MAX_TOOL_CONCURRENCY` 已限并发上限（默认 10）               |
...
atus code 是本地 CLI，**没有运行时下发能力**——传统"5% / 25% / 100% 灰度"不适用。采用**阶段性 release 推进**：
...
| 位置     | 内容                                                                                             | 何时可能变                |
| -------- | ------------------------------------------------------------------------------------------------ | ------------------------- |
| L190     | `process.env['ATUS_SYSTEM_MD']` 决定 basePrompt 来源（默认 vs 用户 system.md）                   | 进程内不变                |
...
| L871     | `process.env['ATUS_CODE_TOOL_CALL_STYLE']` 决定 tool call 风格（atus-coder / atus-vl / general） | 进程内不变                |
...
L207-209：若设置了 `ATUS_SYSTEM_MD` env，**每次** `getCoreSystemPrompt` 都会 `fs.readFileSync(systemMdPath)`：
...
- 但每轮 LLM 调用都有一次同步 IO（默认 `.atus-code/system.md`，网络挂载文件会更慢）
