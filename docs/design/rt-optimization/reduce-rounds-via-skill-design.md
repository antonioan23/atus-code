...
- **atus-logger 链路通畅**（§4.1.1b）：skill_launch 事件能同时落到 OTLP 和 atus-logger 两条管道
...
**前提**：基础设施已就绪 — `tools/tools.ts:818` 的 `CONCURRENCY_SAFE_KINDS` + `coreToolScheduler` 的 `partitionToolCalls` 已经能并发执行同 batch 内的 read/search/fetch 工具。**差的只是模型主动发起并发 tool_calls 的意愿**，atus-coder 默认偏串行。
...
#### 4.1.1b atus-logger 链路修复（前置）

补 `prompt_id` 之前要先解决一个 **既存的链路断点**：`packages/core/src/telemetry/atus-logger/atus-logger.ts:908` 定义了 `logSkillLaunchEvent(event)` 方法，但**全仓库无任何调用方** —— `loggers.ts:958` 的 `logSkillLaunch` 直接走 `logs.getLogger(SERVICE_NAME).emit()` 这条 OTLP 路径，绕过了 atus-logger。

后果：

- OTLP 路径上的 skill_launch 事件能到 OTLP collector（已工作），但 atus-logger 那条专用上报链路目前是死的
- 如果 telemetry backend 是从 atus-logger 消费（而非 OTLP），skill_launch 事件**完全不上报**
- §4.1.2 离线 SQL 派生 `SkillFollowupRecord` 依赖 skill_launch 事件落库 —— **必须先验证现在 skill_launch 在 backend 是否可见**

修复方向二选一：

- **A**（推荐）在 `loggers.ts:958` 的 `logSkillLaunch` 里加一行 `AtusLogger.getInstance(config)?.logSkillLaunchEvent(event)`，对齐 `logToolCall` 的 `loggers.ts:230` 写法
- **B** 确认 backend 只从 OTLP 消费，把 atus-logger 里的 `logSkillLaunchEvent` 标 `@deprecated` 或删除

**为什么只补 AtusLogger 一条路径，不对齐 `logToolCall` 的 4 条全路径**：
...
3. `AtusLogger.getInstance(config)?.logToolCallEvent(...)` — atus-logger 后端遥测
4. OTLP `logger.emit(...)` — OpenTelemetry

skill_launch 是**纯后端遥测事件**，不需要在 UI 上展示（用户已经看到 SkillTool 的 returnDisplay）、也不需要进 ChatRecording 的 turn 历史（skill 内部的工具调用已经各自被 recordUiTelemetryEvent 记录）。因此只补第 3 条（AtusLogger），保留第 4 条（OTLP），跳过 1/2 是有意的，不是遗漏。

**字段透传细节**：`loggers.ts:961-966` 用 `{ ...event }` spread 自动透传新字段（`prompt_id` 加进 `SkillLaunchEvent` 后这条路自动生效），但 `atus-logger.ts:908` 的 `logSkillLaunchEvent` 内部如果显式解构 `event.skill_name` / `event.success`，新字段不会自动纳入，需手动同步。
...
3. **Layer 3 模型可能不听并发指导** — atus-coder 训练数据偏串行；A/B 数据可能显示 prompt 改动无效，作为已知失败模式
...
8. **`atus-logger.ts:908` `logSkillLaunchEvent` 当前是死代码** — 仓库内无任何调用方，§4.1.1b 已列前置修复
...
| **P0**   | atus-logger 链路修复（§4.1.1b 前置）                                   | 0.5d                  | skill_launch 事件可见性确认    | 验证 §0.1 第 1 条                       |
...
| `packages/core/src/telemetry/loggers.ts`                 | `logSkillLaunch`（走 OTLP；缺 atus-logger 转发）              | L958                              |
| `packages/core/src/telemetry/loggers.ts`                 | `logToolCall`（双路径：OTLP + atus-logger，作为修复样板）     | L220, L230                        |
| `packages/core/src/telemetry/atus-logger/atus-logger.ts` | `logSkillLaunchEvent`（**当前死代码**，§4.1.1b 前置修复目标） | L908                              |
...
| `.atus-code/skills/`                                          | 各 skill 定义目录（Layer 2 改造对象）                         | 目录                              |
