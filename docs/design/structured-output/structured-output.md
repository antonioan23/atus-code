...
In headless runs (`atus -p`, piped stdin, or positional prompt), let
...
- `ToolCallEvent.function_args` (telemetry) — covers OTLP exports,
  AtusLogger, ui-telemetry, and the chat-recording UI event mirror.
- `redactStructuredOutputArgsForRecording` (used by
  ...
  `~/.atus-code/projects/<sanitized-cwd>/chats/<sessionId>.jsonl`.
...
- SDK protocol additions (Python / TypeScript / Java SDKs exposing a
  ...
  [PR #4001](https://github.com/atuscode/atus-code/pull/4001) (closed
