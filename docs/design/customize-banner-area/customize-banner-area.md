...
> data (version, auth, model, working directory) that makes atus code
> debuggable and trustworthy.

## Overview

The atus code CLI prints a banner at startup containing a QWEN ASCII logo
...
- **White-label / third-party brand integration**: enterprises and teams
  embedding atus code into their own products want to display their brand
  identity rather than the default "atus code".
...
This is tracked by [issue #3005](https://github.com/atuscode/atus-code/issues/3005).
...
│   │                      │         │                                     │  │
│   │  ███ QWEN ASCII ███  │         │  ① Title:    >_ atus code (vX.Y.Z)  │  │
│   │  ███   ART ART  ███  │         │  ② Subtitle: «blank, or override»   │  │
│   │  ███ QWEN ASCII ███  │         │  ③ Status:   Qwen OAuth | atus-…    │  │
...
- **B. Info panel** — a bordered box containing four rows. The second
  ...
  - **B①** Title: `>_ atus code (vX.Y.Z)` — brand text + version suffix.
...
| **B①. Title — brand text** (`>_ atus code`) | Hard-coded in `Header.tsx`          | **Replaceable**                 | Brand surface. The leading `>_` glyph is part of the existing brand; if a user wants it gone, they simply omit it from `customBannerTitle`.                                                                  |
...
(`~/.atus-code/settings.json`) and workspace-level (`.atus-code/settings.json` in
...
1. Each `{ path }` entry resolves against the file that declared it
   (workspace `.atus-code/` vs. user `~/.atus-code/`); the merged view alone would
...
│ Qwen OAuth | atus-coder ( /model to change)             │  ← B③ status
...
- **Workspace settings**: relative paths resolve against the workspace
  `.atus-code/` directory.
- **User settings**: relative paths resolve against `~/.atus-code/`.
...
1. Save `settings.json` and start a fresh `atus` session — banner
...
3. If something does not appear as expected, look at
   `~/.atus-code/debug/<sessionId>.txt` (the symlink `latest.txt` points to
...
2. **Resolve each tier**. For each `AsciiArtSource`:
   ...
   - If it is `{ path }`, read the file synchronously with `O_NOFOLLOW`
     ...
     workspace `.atus-code/`, user settings against `~/.atus-code/`. Read failure
...
<Text bold color={theme.text.accent}>
  {customBannerTitle ? customBannerTitle : '>_ atus code'}
</Text>
…
{customBannerSubtitle ? (
...
1. `~/.atus-code/settings.json` with `customBannerTitle: "Acme CLI"` and an
   inline `customAsciiArt` string → `atus` shows the new title and art;
...
3. `hideBanner: true` → `atus` starts with no banner; tips and chat
   render normally.
4. `customAsciiArt: { "path": "./brand.txt" }` in a workspace
   `settings.json`, with `brand.txt` next to it in `.atus-code/` → loads
...
7. Point `path` at a missing file → CLI starts; `[BANNER]` warn
   appears in `~/.atus-code/debug/<sessionId>.txt`; default art renders.
