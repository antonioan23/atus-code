...
atus code CLI 启动时会在终端顶部打印一个 Banner，包含 QWEN ASCII
Logo 和一个带边框的信息面板。多种真实场景需要对这一区域进行控制：

- **白标 / 第三方品牌集成**：将 atus code 嵌入企业或团队自有产品时，
  需要展示自家品牌而非默认的 "atus code"。
...
对应 issue：[#3005](https://github.com/atuscode/atus-code/issues/3005)。
...
│   │                      │         │                                     │  │
│   │  ███ QWEN ASCII ███  │         │  ① 标题：    >_ atus code (vX.Y.Z)  │  │
│   │  ███   ART ART  ███  │         │  ② 副标题：  «空白行 / 自定义覆盖» │  │
│   │  ███ QWEN ASCII ███  │         │  ③ 状态：    Qwen OAuth | atus-…    │  │
│   │                      │         │  ④ 路径：    ~/projects/example     │  │
│   └──────── A ───────────┘         └──────────────── B ──────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
- **B. 信息面板** —— 带边框的信息盒，共四行。第二行默认是空白视觉
  ...
  - **B①** 标题：`>_ atus code (vX.Y.Z)` —— 品牌文字 + 版本号后缀。
...
| **B①. 标题文字**（`>_ atus code`） | `Header.tsx` 硬编码                  | **可替换**              | 品牌区域。开头的 `>_` 字符是现有品牌的一部分；如不需要，用户在 `customBannerTitle` 中省略即可。                                                          |
...
（`~/.atus-code/settings.json`）和工作区级（项目根目录的
`.atus-code/settings.json`），按标准合并优先级生效（workspace 覆盖
...
1. 每个 `{ path }` 项相对于声明它的那个文件解析（workspace `.atus-code/`
   vs. user `~/.atus-code/`）；只看 merged 视图就丢失了 scope 信息。
...
│ Qwen OAuth | atus-coder ( /model 切换)                  │  ← B③ 状态
...
- **工作区级设置**：相对路径相对于 workspace 的 `.atus-code/` 目录。
- **用户级设置**：相对路径相对于 `~/.atus-code/`。
- 绝对路径直接使用。
...
1. 保存 `settings.json`，重新启动 `atus` —— Banner 解析仅在启动时
   运行一次。
2. 调整终端宽度，确认 `small` / `large` 切换符合预期，并且在极窄
   宽度下 Logo 列正确隐藏。
3. 若结果与预期不符，查看
   `~/.atus-code/debug/<sessionId>.txt`（`latest.txt` 软链指向当前
...
2. **逐档解析**。对每个 `AsciiArtSource`：
   ...
   - `{ path }`：同步读取，使用 `O_NOFOLLOW` 防御软链劫持
     ...
     设置相对 workspace `.atus-code/`，user 设置相对 `~/.atus-code/`。
...
7. `path` 指向不存在的文件 → CLI 正常启动；
   `~/.atus-code/debug/<sessionId>.txt` 出现 `[BANNER]` warn；
   渲染默认 art。
