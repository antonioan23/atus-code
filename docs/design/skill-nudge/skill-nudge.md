...
本文档描述在 AtusCode 现有 Memory-Dream 架构基础上，增加 **AutoSkill** 能力的设计方案。
...
| **写入目标** | `${projectRoot}/.atus-code/memory/`   | `${projectRoot}/.atus-code/skills/` |
...
1. **无专用写入工具**：skill review agent 直接使用通用的 `read_file`、`write_file`、`edit` 工具操作 `.atus-code/skills/`，不引入 `skill_manage` 专用工具。主会话同理——用户若要手动维护 skill，也使用相同的通用工具。
2. **技能变动检测代替工具计数重置**：参照 memory extract 检测 `memory_tool` 调用的方式，系统检测主会话中是否有任何写操作落在 `.atus-code/skills/` 目录下。若有，说明用户本轮已主动操作 skill，session 结束时跳过自动 skill review。
...
5. **写保护边界明确**：review agent 的权限管理器将 `write_file`、`edit` 限制在 `${projectRoot}/.atus-code/skills/` 内，不能触碰 user / extension / bundled 层。
...
```
每次工具调用完成
  if (工具调用的目标路径在 ${projectRoot}/.atus-code/skills/ 下):
...
> **为何用技能变动检测而非工具名检测？**
> 不再有专用的 `skill_manage` 工具，主会话和 review agent 都使用通用的 `write_file`/`edit`。因此检测维度从"是否调用了某个专用工具"转为"是否有写操作落在 `.atus-code/skills/` 目录"，语义更准确：只要用户本轮已主动操作过 skill 文件，就跳过自动 review。

> **为何用工具调用次数而非对话轮次？**
> 工具调用次数反映任务复杂度——一个用户消息可能触发 1 次或 30 次工具调用。高工具密度意味着试错、调整策略等行为更多，产生可复用经验的概率也更高。阈值 20 比 Hermes 的 10 更保守，原因是 AtusCode 工具调用粒度通常更细（如逐行 edit）。
...
| `ls`         | 扫描 `.atus-code/skills/` 目录结构         | 无限制                                                                      |
| `write_file` | 创建新 skill 文件                     | 仅限 `${projectRoot}/.atus-code/skills/` 内                                      |
| `edit`       | 修改已有 skill 内容                   | 仅限 `${projectRoot}/.atus-code/skills/` 内，且目标文件须含 `source: auto-skill` |
...
**主会话中的工具访问**：主 agent 不限制对 `.atus-code/skills/` 的读写——用户可以通过正常的 `write_file`/`edit` 指令管理 skill。此类操作会触发 `skillsModifiedInSession = true`，导致 session 结束时跳过自动 skill review。
...
write_file:   仅限 ${projectRoot}/.atus-code/skills/ 路径下的文件（创建新 skill）
edit:         仅限 ${projectRoot}/.atus-code/skills/ 内，且目标文件含 source: auto-skill
```
...
Skills are saved to the current project (.atus-code/skills/).
Use write_file to create a new skill, edit to update an existing auto-skill.
Each skill lives at .atus-code/skills/<name>/SKILL.md with YAML frontmatter:
...
export interface ScheduleSkillReviewParams {
  ...
  skillsModified: boolean; // 本次会话是否有写操作落在 .atus-code/skills/
  config?: Config;
  enabled?: boolean;
  threshold?: number;
  maxTurns?: number;
...
会话进行中
  agent 主循环
    ...
    └─ 若写操作目标路径在 ${projectRoot}/.atus-code/skills/ 下
         → skillsModifiedInSession = true

会话结束（sessionEnd 事件）
  ├─ scheduleExtract(params)
  │     └─ [现有逻辑：fork extraction agent → 写 .atus-code/memory/]
  │
...
下次会话
  ...
  → 扫描 .atus-code/skills/ 发现新建 skill
...
自动提炼的 skill 写入 `${projectRoot}/.atus-code/skills/<name>/SKILL.md`，格式与现有 SkillManager 完全兼容：
...
| 写入项目外路径                       | `write_file`/`edit` 权限限制在 `${projectRoot}/.atus-code/skills/` 内；`assertRealProjectSkillPath` 拒绝 symlink 穿越 |
| 提炼出含注入风险的内容               | 复用现有内容安全扫描逻辑                                                                                         |
| review agent 删除 skill              | review agent 工具集不含删除操作（无 `rm`、无 `shell` 写操作）；system prompt 明确禁止删除                        |
| 主会话手动操作 skill 后仍触发 review | `skillsModifiedInSession` 检测：主会话有写操作落在 `.atus-code/skills/` 则跳过 review                                 |
...
在 AtusCode config 中新增以下配置项（可选，有默认值）：
...
对应 QWEN.md / `~/.atus-code/config.json` 配置示例：
...
功能实现完成后，按照 `.atus-code/skills/e2e-testing/SKILL.md` 的流程，先执行 `npm run build && npm run bundle`，再使用本地构建产物 `node dist/cli.js` 进行端到端验证。
...
- 断言 `.atus-code/skills/` 未新增 `source: auto-skill` skill；JSON 流中不应出现对 `.atus-code/skills/` 的写操作。

### 2. 达到阈值后触发 skill review

- 使用临时项目目录运行 headless 模式（`AUTO_SKILL_THRESHOLD` 硬编码为 20，可在测试夹具中调低）。
- 发送一个需要多次工具调用并包含可复用流程的任务。
- 断言会话结束后调度了 skill review；若模型判断值得保存，`.atus-code/skills/<name>/SKILL.md` 被创建，且 frontmatter 包含 `source: auto-skill`。
- 若模型判断 `Nothing to save.`，断言流程正常结束且没有权限错误。

### 3. 主会话操作 skill 后跳过 review

- 构造一次会话，在工具调用达到阈值的同时，通过 `write_file` 或 `edit` 写入 `.atus-code/skills/` 下的文件（模拟用户手动管理 skill）。
...
- 断言写入被拒绝，错误信息指向只能写入 `${projectRoot}/.atus-code/skills/`。
- 断言允许写入 `${projectRoot}/.atus-code/skills/<name>/SKILL.md`。

### 5. `auto-skill` 标识保护用户创建的 skill

- 在 `.atus-code/skills/` 中预置一个无 `source: auto-skill` 的用户创建 skill。
...
- 在 `.atus-code/skills/` 下创建一个指向项目外目录的 symlink。
...
现有 SkillManager
  ├─ listSkills()            ← 不变（自动发现 .atus-code/skills/ 下新增文件）
  └─ loadSkill()             ← 不变

现有文件工具（read_file / write_file / edit）
  ├─ 主会话中：用户可通过这些工具手动管理 skill
  │   └─ 写操作落在 .atus-code/skills/ → skillsModifiedInSession = true
...
SkillManager 的读取侧（`listSkills`、`loadSkill`）完全不需要修改——review agent 写入 `${projectRoot}/.atus-code/skills/` 后，`SkillManager` 通过现有的 `chokidar` 文件监听自动感知变化，调用 `notifyChangeListeners()` 触发缓存刷新，下次对话自然可以在 system prompt 中看到新 skill。
