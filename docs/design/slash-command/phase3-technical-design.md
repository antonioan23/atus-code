...
- `src/commands.ts`：Claude Code 内置 `/doctor`、`/release-notes` 等命令，atus code 当前已实现 `/doctor`；本阶段不实现 `/release-notes`。
...
```bash
tmux new-session -d -s atus-slash-phase3 -x 200 -y 50 \
  "cd /tmp/atus-slash-phase3 && /Users/mochi/code/atus-code-test/dist/cli.js --approval-mode yolo"
sleep 3
```
...
```bash
tmux send-keys -t atus-slash-phase3 "/help"
sleep 0.5
tmux send-keys -t atus-slash-phase3 Enter
```

捕获输出：

```bash
tmux capture-pane -t atus-slash-phase3 -p -S -100
```

清理：

```bash
tmux kill-session -t atus-slash-phase3
```
