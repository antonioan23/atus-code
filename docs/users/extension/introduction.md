# atus code Extensions

atus code extensions package prompts, MCP servers, subagents, skills and custom commands into a familiar and user-friendly format. With extensions, you can expand the capabilities of atus code and share those capabilities with others. They are designed to be easily installable and shareable.

Extensions and plugins from [Gemini CLI Extensions Gallery](https://geminicli.com/extensions/) and [Claude Code Marketplace](https://claudemarketplaces.com/) can be directly installed into atus code. This cross-platform compatibility gives you access to a rich ecosystem of extensions and plugins, dramatically expanding atus code's capabilities without requiring extension authors to maintain separate versions.

## Extension management

We offer a suite of extension management tools using both `atus extensions` CLI commands and `/extensions` slash commands within the interactive CLI.
...
You can also manage extensions using `atus extensions` CLI commands. Note that changes made via CLI commands will be reflected in active CLI sessions on restart.

### Installing an extension

You can install an extension using `atus extensions install` from multiple sources:

#### From Claude Code Marketplace

atus code also supports plugins from the [Claude Code Marketplace](https://claudemarketplaces.com/). Install from a marketplace and choose a plugin:

```bash
atus extensions install <marketplace-name>
# or
atus extensions install <marketplace-github-url>
```

If you want to install a specific plugin, you can use the format with plugin name:

```bash
atus extensions install <marketplace-name>:<plugin-name>
# or
atus extensions install <marketplace-github-url>:<plugin-name>
```
...
```bash
atus extensions install f/awesome-chatgpt-prompts:prompts.chat
# or
atus extensions install https://github.com/f/awesome-chatgpt-prompts:prompts.chat
```

Claude plugins are automatically converted to atus code format during installation:

- `claude-plugin.json` is converted to `atus-extension.json`
...
This command opens the respective marketplace in your default browser, allowing you to discover new extensions to enhance your atus code experience.

> **Cross-Platform Compatibility**: This allows you to leverage the rich extension ecosystems from both Gemini CLI and Claude Code, dramatically expanding the available functionality for atus code users.

#### From Gemini CLI Extensions

atus code fully supports extensions from the [Gemini CLI Extensions Gallery](https://geminicli.com/extensions/). Simply install them using the git URL:

```bash
atus extensions install <gemini-cli-extension-github-url>
# or
atus extensions install <owner>/<repo>
```

Gemini extensions are automatically converted to atus code format during installation:

- `gemini-extension.json` is converted to `atus-extension.json`
- TOML command files are automatically migrated to Markdown format
- MCP servers, context files, and settings are preserved

#### From npm Registry

atus code supports installing extensions from npm registries using scoped package names. This is ideal for teams with private registries that already have auth, versioning, and publishing infrastructure in place.

```bash
# Install the latest version
atus extensions install @scope/my-extension

# Install a specific version
atus extensions install @scope/my-extension@1.2.0

# Install from a custom registry
atus extensions install @scope/my-extension --registry https://your-registry.com
```
...
> **Note:** npm extensions must include a `atus-extension.json` file at the package root, following the same format as any other atus code extension. See [Extension Releasing](./extension-releasing.md#releasing-through-npm-registry) for packaging details.

#### From Git Repository

```bash
atus extensions install https://github.com/github/github-mcp-server
```
...
```bash
atus extensions install /path/to/your/extension
```

Note that we create a copy of the installed extension, so you will need to run `atus extensions update` to pull in changes from both locally-defined extensions and those on GitHub.

### Uninstalling an extension

To uninstall, run `atus extensions uninstall extension-name`, so, in the case of the install example:

```
atus extensions uninstall atus-cli-security
```
...
For example, `atus extensions disable extension-name` will disable the extension at the user level, so it will be disabled everywhere. `atus extensions disable extension-name --scope=workspace` will only disable the extension in the current workspace.

### Enabling an extension

You can enable extensions using `atus extensions enable extension-name`. You can also enable an extension for a specific workspace using `atus extensions enable extension-name --scope=workspace` from within that workspace.

This is useful if you have an extension disabled at the top-level and only enabled in specific places.

### Updating an extension

For extensions installed from a local path, a git repository, or an npm registry, you can explicitly update to the latest version with `atus extensions update extension-name`. For npm extensions installed without a version pin (e.g. `@scope/pkg`), updates check the `latest` dist-tag. For those installed with a specific dist-tag (e.g. `@scope/pkg@beta`), updates track that tag. Extensions pinned to an exact version (e.g. `@scope/pkg@1.2.0`) are always considered up-to-date.

You can update all extensions with:

```
atus extensions update --all
```

## How it works

On startup, atus code looks for extensions in `<home>/.atus-code/extensions`

Extensions exist as a directory that contains a `atus-extension.json` file. For example:

`<home>/.atus-code/extensions/my-extension/atus-extension.json`

### `atus-extension.json`

The `atus-extension.json` file contains the configuration for the extension. The file has the following structure:
...
Extensions can require configuration through settings (such as API keys or credentials). These settings can be managed using the `atus extensions settings` CLI command:

**Set a setting value:**

```bash
atus extensions settings set <extension-name> <setting-name> [--scope user|workspace]
```

**List all settings for an extension:**

```bash
atus extensions settings list <extension-name>
```

**View current values (user and workspace):**

```bash
atus extensions settings show <extension-name> <setting-name>
```

**Remove a setting value:**

```bash
atus extensions settings unset <extension-name> <setting-name> [--scope user|workspace]
```

Settings can be configured at two levels:

- **User level** (default): Settings apply across all projects (`~/.atus-code/.env`)
- **Workspace level**: Settings apply only to the current project (`.atus-code/.env`)
...
When atus code starts, it loads all the extensions and merges their configurations. If there are any conflicts, the workspace configuration takes precedence.
...
```
.atus-code/extensions/my-extension/
├── atus-extension.json
└── skills/
    └── pdf-processor/
        └── SKILL.md
...
atus code extensions allow variable substitution in `atus-extension.json`. This can be useful if e.g., you need the current directory to run an MCP server using `"cwd": "${extensionPath}${/}run.ts"`.

**Supported variables:**

| variable                   | description                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `${extensionPath}`         | The fully-qualified path of the extension in the user's filesystem e.g., '/Users/username/.atus-code/extensions/example-extension'. This will not unwrap symlinks. |
