...
Subagents are specialized AI assistants that handle specific types of tasks within atus code. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
In addition to named subagents, atus code supports **implicit forking** — when the AI omits the `subagent_type` parameter, it triggers a fork that inherits the parent's full conversation context.
...
- **Project-level**: `.atus-code/agents/` (highest precedence)
- **User-level**: `~/.atus-code/agents/` (fallback)
...
To see which extensions provide subagents, check the extension's `atus-extension.json` file for an `agents` field.
...
- `glm-5`: Use that model ID. atus code first checks the main conversation's
...
When the selector resolves to another auth type, atus code creates a dedicated
...
atus code proactively delegates tasks based on:
