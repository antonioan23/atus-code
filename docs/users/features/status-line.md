...
Add a `statusLine` object under the `ui` key in `~/.atus-code/settings.json`:
...
| ---------------------- | ------- | ------------------------------------------------------------------ |
| `model-with-reasoning` | Yes     | Current model name with reasoning level (e.g. `atus-3-235b high`)  |
...
| `atus-version`         |         | atus code version (e.g. `v0.14.1`)                                 |
...
```
atus-3-235b high | main | Context 65.7% left | /home/user/project | Context 34.3% used
```
...
│                                                               │
│ Preview                                                       │
│ atus-3-235b high | main | Context 65.7% left                 │
│                                                               │
...
```json
{
  ...
  "worktree": {
    "name": "fix-auth",
    "path": "/home/user/project/.atus-code/worktrees/fix-auth",
    "branch": "fix-auth",
...
| `version`                             | string           | atus code version                                                                  |
...
For longer commands, save a script file at `~/.atus-code/statusline-command.sh`:
...
```json
{
  "ui": {
    "statusLine": {
      "type": "command",
      "command": "bash ~/.atus-code/statusline-command.sh"
    }
  }
}
```
...
- **Shell**: Commands run via `/bin/sh` on macOS/Linux. On Windows, `cmd.exe` is used by default — wrap POSIX commands with `bash -c "..."` or point to a bash script (e.g. `bash ~/.atus-code/statusline-command.sh`).
