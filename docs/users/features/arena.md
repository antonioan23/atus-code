...
1. **Worktree setup**: atus code creates isolated Git worktrees for each agent at `~/.atus-code/arena/<session-id>/worktrees/<model-name>/`. Each worktree mirrors your current working directory state exactly — including staged changes, unstaged changes, and untracked files.
...
{
  "arena": {
    "worktreeBaseDir": "~/.atus-code/arena",
...
| :------------------------ | :--------------------------------- | :-------------- |
| `arena.worktreeBaseDir`   | Base directory for arena worktrees | `~/.atus-code/arena` |
...
- Ensure you have write access to the worktree base directory (`~/.atus-code/arena/` by default)
...
Agent Arena is one of several planned multi-agent modes in atus code. **Agent Team** and **Agent Swarm** are not yet implemented — the table below describes their intended design for reference.
...
- **Manual parallel sessions**: Run multiple atus code sessions yourself in separate terminals with [Git worktrees](https://git-scm.com/docs/git-worktree) for full manual control
