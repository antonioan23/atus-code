...
Headless mode allows you to run atus code programmatically from command line
scripts and automation tools without any interactive UI. This is ideal for
scripting, automation, CI/CD pipelines, and building AI-powered tools.

## Overview

The headless mode provides a headless interface to atus code that:
...
```bash
atus --prompt "What is machine learning?"
```

### Stdin Input

Pipe input to atus code from your terminal:

```bash
echo "Explain this code" | atus
```

### Combining with File Input

Read from files and process with atus code:

```bash
cat README.md | atus --prompt "Summarize this documentation"
```
...
atus --continue -p "Run the tests again and summarize failures"

# Resume a specific session ID directly (no UI)
atus --resume 123e4567-e89b-12d3-a456-426614174000 -p "Apply the follow-up refactor"
```

> [!note]
>
> - Session data is project-scoped JSONL under `~/.atus-code/projects/<sanitized-cwd>/chats`.
...
Use `--system-prompt` to replace atus code's built-in main-session prompt for the current run:

```bash
atus -p "Review this patch" --system-prompt "You are a terse release reviewer. Report only blocking issues."
```
...
```bash
atus -p "Review this patch" --append-system-prompt "Be terse and focus on concrete findings."
```

You can combine both flags when you want a custom base prompt plus an extra run-specific instruction:

```bash
atus -p "Summarize this repository" \
...
atus code supports multiple output formats for different use cases:
...
```bash
atus -p "What is the capital of France?"
```
...
```bash
atus -p "What is the capital of France?" --output-format json
```
...
```bash
atus -p "Explain TypeScript" --output-format stream-json
```
...
```bash
atus -p "Write a Python script" --output-format stream-json --include-partial-messages
```

### Input Format

The `--input-format` parameter controls how atus code consumes input from standard input:
...
```bash
# Save to file
atus -p "Explain Docker" > docker-explanation.txt
atus -p "Explain Docker" --output-format json > docker-explanation.json

# Append to file
atus -p "Add more details" >> docker-explanation.txt

# Pipe to other tools
atus -p "What is Kubernetes?" --output-format json | jq '.response'
atus -p "Explain microservices" | wc -w
atus -p "List programming languages" | grep -i "python"

# Stream-JSON output for real-time processing
atus -p "Explain Docker" --output-format stream-json | jq '.type'
atus -p "Write code" --output-format stream-json --include-partial-messages | jq '.event.type'
```
...
| ---------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| `--prompt`, `-p`             | Run in headless mode                                                     | `atus -p "query"`                                                        |
| `--output-format`, `-o`      | Specify output format (text, json, stream-json)                          | `atus -p "query" --output-format json`                                   |
| `--input-format`             | Specify input format (text, stream-json)                                 | `atus --input-format text --output-format stream-json`                   |
| `--include-partial-messages` | Include partial messages in stream-json output                           | `atus -p "query" --output-format stream-json --include-partial-messages` |
| `--system-prompt`            | Override the main session system prompt for this run                     | `atus -p "query" --system-prompt "You are a terse reviewer."`            |
| `--append-system-prompt`     | Append extra instructions to the main session system prompt for this run | `atus -p "query" --append-system-prompt "Focus on concrete findings."`   |
| `--debug`, `-d`              | Enable debug mode                                                        | `atus -p "query" --debug`                                                |
| `--all-files`, `-a`          | Include all files in context                                             | `atus -p "query" --all-files`                                            |
| `--include-directories`      | Include additional directories                                           | `atus -p "query" --include-directories src,docs`                         |
| `--yolo`, `-y`               | Auto-approve all actions                                                 | `atus -p "query" --yolo`                                                 |
| `--approval-mode`            | Set approval mode                                                        | `atus -p "query" --approval-mode auto_edit`                              |
| `--continue`                 | Resume the most recent session for this project                          | `atus --continue -p "Pick up where we left off"`                         |
| `--resume [sessionId]`       | Resume a specific session (or choose interactively)                      | `atus --resume 123e... -p "Finish the refactor"`                         |
| `--max-session-turns`        | Cap the number of user/model/tool turns in the run                       | `atus -p "..." --max-session-turns 30`                                   |
| `--max-wall-time`            | Wall-clock budget; accepts `90` (s), `30s`, `5m`, `1h`, `1.5h`           | `atus -p "..." --max-wall-time 10m`                                      |
| `--max-tool-calls`           | Cumulative tool-call budget for the run                                  | `atus -p "..." --max-tool-calls 50`                                      |
...
- **`atus serve` / ACP sessions:** the daemon ACP session path does NOT currently consult `--max-wall-time` / `--max-tool-calls` from settings.json. These budgets only apply to single-shot `atus -p` runs and to `--input-format stream-json` sessions. (`atus serve` does emit the YOLO-no-sandbox warning at boot if `tools.approvalMode: 'yolo'` is set in settings.)

### Recommended combinations

- **Trusted, isolated environment (ephemeral CI runner, container):** `atus -p "..." --yolo --max-session-turns N --max-wall-time 10m --output-format json`. Pin a turn budget and a wall-clock budget so a stuck agent can't burn through your CI minutes, and capture `--output-format json` for post-run usage / tool-call auditing.
- **Local machine or shared infra:** also pass `--sandbox` (or set `ATUS_SANDBOX=1`) so shell / write / edit tools run inside the sandbox image.
- **Long-running CI with retry-on-rate-limit:** combine `ATUS_CODE_UNATTENDED_RETRY=1` with `--max-wall-time`. The retry env keeps the run alive past transient 429 / 529 responses; the wall-clock budget ensures a persistently-failing provider can't extend the job indefinitely.
...
```bash
cat src/auth.py | atus -p "Review this authentication code for security issues" > security-review.txt
```

### Generate commit messages

```bash
result=$(git diff --cached | atus -p "Write a concise commit message for these changes" --output-format json)
...
```bash
result=$(cat api/routes.js | atus -p "Generate OpenAPI spec for these routes" --output-format json)
...
for file in src/*.py; do
    echo "Analyzing $file..."
    result=$(cat "$file" | atus -p "Find potential bugs and suggest improvements" --output-format json)
...
```bash
result=$(git diff origin/main...HEAD | atus -p "Review these changes for bugs, security issues, and code quality" --output-format json)
...
```bash
grep "ERROR" /var/log/app.log | tail -20 | atus -p "Analyze these errors and suggest root cause and fixes" > error-analysis.txt
```

### Release notes generation

```bash
result=$(git log --oneline v1.0.0..HEAD | atus -p "Generate release notes from these commits" --output-format json)
...
```bash
result=$(atus -p "Explain this database schema" --include-directories db --output-format json)
...
When atus code runs in CI/CD pipelines or as a background daemon, a brief API outage (rate limiting or overload) should not kill a multi-hour task. **Persistent retry mode** makes atus code retry transient API errors indefinitely until the service recovers.
...
Set the `ATUS_CODE_UNATTENDED_RETRY` environment variable to `true` or `1` (strict match, case-sensitive):

```bash
export ATUS_CODE_UNATTENDED_RETRY=1
```

> [!important]
> Persistent retry requires an **explicit opt-in**. `CI=true` alone does **not** activate it — silently turning a fast-fail CI job into an infinite-wait job would be dangerous. Always set `ATUS_CODE_UNATTENDED_RETRY` explicitly in your pipeline configuration.
...
- name: Automated code review
  env:
    ATUS_CODE_UNATTENDED_RETRY: '1'
  run: |
    atus -p "Review all files in src/ for security issues" \
      --output-format json \
      --yolo > review.json
```
...
```bash
export ATUS_CODE_UNATTENDED_RETRY=1
atus -p "Migrate all callback-style functions to async/await in src/" --yolo
```

#### Background daemon

```bash
ATUS_CODE_UNATTENDED_RETRY=1 nohup atus -p "Audit all dependencies for known CVEs" \
...
```
[atus-code] Waiting for API capacity... attempt 3, retry in 45s
[atus-code] Waiting for API capacity... attempt 3, retry in 15s
```
