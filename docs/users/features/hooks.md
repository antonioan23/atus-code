...
Hooks are user-defined scripts or programs that are automatically executed by atus code at predefined points in the application flow. They allow users to:
...
atus code supports four hook executor types:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$ATUS_PROJECT_DIR/.atus-code/hooks/security-check.sh",
            "name": "security-check",
            "timeout": 10000
          }
        ]
...
4. atus code processes the decision and continues or blocks execution accordingly
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "You are evaluating whether atus code should stop working. Context: $ARGUMENTS\n\nAnalyze the conversation and determine if:\n1. All user-requested tasks are complete\n2. Any errors need to be addressed\n3. Follow-up work is needed\n\nRespond with JSON: {\"ok\": true} to allow stopping, or {\"ok\": false, \"reason\": \"your explanation\"} to continue working.",
            "timeout": 30
          }
        ]
      }
...
When `ok` is `false`, atus code will continue working and use the `reason` as context for the next response.
...
Hooks fire at specific points during a atus code session. Different events support different matchers to filter trigger conditions.
...
{
  "decision": "block",
  "reason": "Must be provided when atus code is blocked from stopping"
}
```
...
```bash
#!/bin/bash
# ~/.atus-code/hooks/todo-validator.sh
    ...
    "TodoCreated": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.atus-code/hooks/todo-validator.sh",
            "name": "todo-validator",
            "timeout": 5000
          }
        ]
...
```bash
#!/bin/bash
# ~/.atus-code/hooks/todo-completion-validator.sh
    ...
    "TodoCompleted": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.atus-code/hooks/todo-completion-validator.sh",
...
Hooks are configured in atus code settings, typically in `.atus-code/settings.json` or user configuration files:
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$ATUS_PROJECT_DIR/.atus-code/hooks/run-tests-async.sh",
            "async": true,
            "timeout": 300000
          }
        ]
...
# Log the operation
echo "INFO: Tool $TOOL_NAME executed safely at $(date)" >> /var/log/atus-security.log
...
Configure in `.atus-code/settings.json`:
