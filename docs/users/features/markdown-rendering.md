...
atus code renders common Markdown structures directly in the TUI so model
...
To start atus code in raw mode by default, set `ui.renderMode`:
...
1. If enabled and supported, atus code asks Mermaid CLI (`mmdc`) to render the
...
3. Otherwise, atus code falls back to a terminal wireframe or compact text
   preview.
4. If a Mermaid diagram type cannot be previewed, atus code shows the original
...
```bash
ATUS_CODE_MERMAID_IMAGE_RENDERING=1 atus
```
...
| ------------------------------------------- | ----------------------------------------------------------------------------------- |
| `ATUS_CODE_MERMAID_IMAGE_RENDERING=1`       | Enables external Mermaid image rendering.                                           |
| `ATUS_CODE_DISABLE_MERMAID_IMAGES=1`        | Disables Mermaid image rendering even when enabled elsewhere.                       |
| `ATUS_CODE_MERMAID_IMAGE_PROTOCOL=kitty`    | Forces Kitty protocol output. Useful for terminals such as Kitty and Ghostty.       |
| `ATUS_CODE_MERMAID_IMAGE_PROTOCOL=iterm2`   | Requests iTerm2 inline images. Interactive TUI rendering falls back to text/ANSI.   |
| `ATUS_CODE_MERMAID_IMAGE_PROTOCOL=off`      | Disables terminal image protocols and allows text or `chafa` fallback.              |
| `ATUS_CODE_MERMAID_MMD_CLI=/path/to/mmdc`   | Uses a specific Mermaid CLI executable.                                             |
| `ATUS_CODE_MERMAID_ALLOW_NPX=1`             | Allows atus code to run `npx @mermaid-js/mermaid-cli` when `mmdc` is not installed. |
| `ATUS_CODE_MERMAID_ALLOW_LOCAL_RENDERERS=1` | Allows project-local renderer binaries under `node_modules/.bin`.                   |
| `ATUS_CODE_MERMAID_RENDER_WIDTH=1200`       | Overrides the PNG render width.                                                     |
| `ATUS_CODE_MERMAID_RENDER_TIMEOUT_MS=10000` | Overrides the external render timeout, capped at 60000 ms.                          |
| `ATUS_CODE_MERMAID_CELL_ASPECT_RATIO=0.5`   | Adjusts image row fitting for terminal font cell geometry.                          |

The first image render can be slow, especially when `npx` needs to resolve or
download Mermaid CLI. During streaming, atus code shows a bounded text preview
...
atus code supports basic inline and block LaTeX rendering in the terminal:
