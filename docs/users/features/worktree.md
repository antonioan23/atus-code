...
atus --worktree=#4174
atus --worktree https://github.com/atuscode/atus-code/pull/4174

# Continue a previous --worktree session — re-attaches to the existing dir
atus --resume <session-id> --worktree=my-feature
```

> **Tip — bare `--worktree` followed by a positional prompt is ambiguous.** Because `--worktree` takes an optional value, `atus --worktree "say hi"` makes yargs consume `"say hi"` as the slug (and reject it because of the space). Use one of:
>
> - `atus --worktree=my-feature "say hi"` (always works — explicit slug via `=`)
> - `atus "say hi" --worktree` (positional first, flag at the end → auto slug)
> - `atus --worktree --approval-mode yolo "say hi"` (any flag between them anchors the bare form)

> **Tip — `atus --resume --worktree foo` (no session ID) shows an empty picker on first use.** The picker scopes to the chosen worktree's session storage; sessions started outside that worktree are not listed. To resume a session that was started inside `foo`, use `atus --resume <id> --worktree foo` directly — the CLI re-attaches to the existing `foo/` directory rather than re-creating it.
...
Alternatively, ask atus code in plain language to create a worktree from inside an existing session:

```text
> start a worktree called experiment-a
Worktree experiment-a created on branch worktree-experiment-a
.atus-code/worktrees/experiment-a
```

From this point on, the model routes every file edit and shell command through `.atus-code/worktrees/experiment-a/`. Your original working directory is untouched.
...
Kept worktree experiment-a at .atus-code/worktrees/experiment-a
```
...
Every Qwen-managed worktree is placed under your project's `.atus-code` directory:

```
<repoRoot>/.atus-code/worktrees/<slug>/         # Working directory
...
```json
{
  "worktree": {
    "name": "experiment-a",
    "path": "/path/to/repo/.atus-code/worktrees/experiment-a",
...
The `agent` tool accepts an optional `isolation: "worktree"` parameter. When set, atus code creates an ephemeral worktree at `<repoRoot>/.atus-code/worktrees/agent-<7hex>/` before the sub-agent starts, and:
...
- **Has changes** → the worktree is preserved; its path and branch are appended to the agent's result, e.g.
  ```
  …agent output…
  [worktree preserved: /path/to/.atus-code/worktrees/agent-3f2a1b9 (branch worktree-agent-3f2a1b9)]
  ```
...
```jsonc
// ~/.atus-code/settings.json or <repo>/.atus-code/settings.json
{
  "worktree": {
...
- `agents.arena.worktreeBaseDir` — controls **Agent Arena** worktree placement (default `~/.atus-code/arena`). Does not affect general-purpose worktrees, which always live under `<repoRoot>/.atus-code/worktrees/`.
...
- The current working directory is already inside `.atus-code/worktrees/` (no nested worktrees).
...
```bash
atus --worktree                                               # auto-generate slug
atus --worktree my-feature                                    # explicit slug
atus --worktree=my-feature                                    # = form
atus --worktree=#123                                          # PR reference
atus --worktree https://github.com/owner/repo/pull/123        # PR URL
```
