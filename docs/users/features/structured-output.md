...
Headless only — works with `atus -p`, a positional prompt, or a prompt
piped via stdin.

## Quick start

```bash
atus --prompt "Summarize the changes in HEAD with risk_level" \
  --json-schema '{
    "type": "object",
    "properties": {
      "summary":    { "type": "string" },
...
That makes `$(atus --json-schema …) || exit 1` capture patterns safe
...
```bash
# Inline JSON literal
atus -p "…" --json-schema '{"type":"object", "properties":{…}}'

# Read from a file
atus -p "…" --json-schema @./schemas/summary.json
```
...
- The `ToolCallEvent` telemetry path (OTLP exports, AtusLogger,
  ui-telemetry stream, chat-recording UI event mirror).
- The on-disk chat-recording JSONL at
  `~/.atus-code/projects/<sanitized-cwd>/chats/<sessionId>.jsonl` (re-fed
...
```bash
RESULT=$(atus --prompt "Audit this diff and rate its risk." \
