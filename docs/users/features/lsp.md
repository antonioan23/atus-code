...
atus code provides native Language Server Protocol (LSP) support, enabling advanced code intelligence features like go-to-definition, find references, diagnostics, and code actions. This integration allows the AI agent to understand your code more deeply and provide more accurate assistance.

## Overview

LSP support in atus code works by connecting to language servers that understand your code. Once you configure servers via `.lsp.json` (or extensions), atus code can start them and use them to:
...
LSP is an experimental feature in atus code. To enable it, use the `--experimental-lsp` command line flag:

```bash
atus --experimental-lsp
```

LSP servers are configuration-driven. You must define them in `.lsp.json` (or via extensions) for atus code to start them.
...
atus code exposes LSP functionality through the unified `lsp` tool. Here are the available operations:
...
1. **Verify `--experimental-lsp` flag**: Make sure you're using the flag when starting atus code
...
5. **Check logs**: Start atus code with `--debug`, then search for LSP-related entries in the debug log (see Debugging section below)
...
LSP does not have a separate debug flag. Use atus code's normal debug mode together with the LSP feature flag:

```bash
atus --experimental-lsp --debug
```
...
rg "LSP|Native LSP|clangd|connection closed" ~/.atus-code/debug/latest
# Or, without ripgrep:
grep -E "LSP|Native LSP|clangd|connection closed" ~/.atus-code/debug/latest

# If ATUS_RUNTIME_DIR is configured
rg "LSP|Native LSP|clangd|connection closed" "$ATUS_RUNTIME_DIR/debug/latest"
```
...
Extensions can provide LSP server configurations through the `lspServers` field in their `plugin.json`. This can be either an inline object or a path to a `.lsp.json` file. atus code loads these configs when the extension is enabled. The format is the same language-keyed layout used in project `.lsp.json` files.
...
Use the `--experimental-lsp` flag when starting atus code:
...
Start atus code with LSP and debug mode enabled:
...
```bash
# Default runtime directory
rg "LSP|Native LSP|<server-name>" ~/.atus-code/debug/latest
# Or:
grep -E "LSP|Native LSP|<server-name>" ~/.atus-code/debug/latest

# If ATUS_RUNTIME_DIR is configured
rg "LSP|Native LSP|<server-name>" "$ATUS_RUNTIME_DIR/debug/latest"
```

LSP uses atus code's normal `--debug` mode; there is no separate LSP debug flag.
