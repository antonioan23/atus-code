...
This document explains how to run atus code inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up atus code:
...
> [!note]
>
> **Naming note:** Some sandbox-related environment variables may have used the `GEMINI_*` prefix historically. All new environment variables use the `ATUS_*` prefix.
...
By default, atus code uses a published sandbox image (configured in the CLI package) and will pull it as needed.

The container sandbox mounts your workspace and your `~/.atus-code` directory into the container so auth and settings persist between runs.
...
atus -s -p "analyze the code structure"

# Or enable sandboxing for your shell session (recommended for CI / scripts)
export ATUS_SANDBOX=true   # true auto-picks a provider (see notes below)
atus -p "run the test suite"
...
>
> - On **macOS**, `ATUS_SANDBOX=true` typically selects `sandbox-exec` (Seatbelt) if available.
> - On **Linux/Windows**, `ATUS_SANDBOX=true` requires `docker` or `podman` to be installed.
> - To force a provider, set `ATUS_SANDBOX=docker|podman|sandbox-exec`.

## Configuration

### Enable sandboxing (in order of precedence)

1. **Environment variable**: `ATUS_SANDBOX=true|false|docker|podman|sandbox-exec`
...
> [!important]
>
> If `ATUS_SANDBOX` is set, it **overrides** the CLI flag and `settings.json`.

### Configure the sandbox image (Docker/Podman)

- **CLI flag**: `--sandbox-image <image>`
- **Environment variable**: `ATUS_SANDBOX_IMAGE=<image>`
- **Settings file**: `tools.sandboxImage` in your `settings.json` (e.g., `{"tools": {"sandboxImage": "ghcr.io/atuscode/atus-code:0.14.1"}}`)

Priority order (highest to lowest):

1. `--sandbox-image`
2. `ATUS_SANDBOX_IMAGE`
3. `tools.sandboxImage`
4. Built-in default image from the CLI package (for example `ghcr.io/atuscode/atus-code:<version>`)

`settings.env.ATUS_SANDBOX_IMAGE` also works as a generic env injection mechanism, but `tools.sandboxImage` is the preferred persistent setting.
...
1. Create a file named `.atus-code/sandbox-macos-<profile_name>.sb` in your project.
...
- Set `ATUS_SANDBOX_PROXY_COMMAND=<command>`
...
On Linux, atus code defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.atus-code`). Override with:
...
- Container sandbox: add them via `.atus-code/sandbox.Dockerfile` or `.atus-code/sandbox.bashrc`.
...
```bash
DEBUG=1 atus -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.atus-code/.env` files for atus code-specific debug settings.

### Inspect sandbox

```bash
# Check environment
atus -s -p "run shell command: env | grep SANDBOX"

# List mounts
atus -s -p "run shell command: mount | grep workspace"
```
