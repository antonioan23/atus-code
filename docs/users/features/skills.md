...
> Create, manage, and share Skills to extend atus code's capabilities.

This guide shows you how to create, use, and manage Agent Skills in **atus code**. Skills are modular capabilities that extend the model's effectiveness through organized folders containing instructions (and optionally scripts/resources).

## Prerequisites

- atus code (recent version)
- Basic familiarity with atus code ([Quickstart](../quickstart.md))
...
- Extend atus code for your workflows
...
Personal Skills are available across all your projects. Store them in `~/.atus-code/skills/`:

```bash
mkdir -p ~/.atus-code/skills/my-skill-name
```
...
Project Skills are shared with your team. Store them in `.atus-code/skills/` within your project:
...
## Instructions
Provide clear, step-by-step guidance for atus code.
...
atus code currently validates that:
...
atus code discovers Skills from:

- Personal Skills: `~/.atus-code/skills/`
- Project Skills: `.atus-code/skills/`
...
To see which extensions provide skills, check the extension's `atus-extension.json` file for a `skills` field.

To view available Skills, ask atus code directly:
...
cat ~/.atus-code/skills/my-skill/SKILL.md
```
...
If atus code doesn't use your Skill, check these common issues:
...
- Personal Skills: `~/.atus-code/skills/<skill-name>/SKILL.md`
- Project Skills: `.atus-code/skills/<skill-name>/SKILL.md`

```bash
# Personal
ls ~/.atus-code/skills/my-skill/SKILL.md
...
Run atus code with debug mode to see Skill loading errors:
...
1. Add the Skill under `.atus-code/skills/`
2. Commit and push
3. Teammates pull the changes

```bash
git add .atus-code/skills/
...
```bash
# Personal Skill
code ~/.atus-code/skills/my-skill/SKILL.md

# Project Skill
code .atus-code/skills/my-skill/SKILL.md
```

Changes take effect the next time you start atus code. If atus code is already running, restart it to load the updates.
...
```bash
# Personal
rm -rf ~/.atus-code/skills/my-skill
