...
When reviewing a PR, `/review` creates a temporary git worktree (`.atus-code/tmp/review-pr-<number>`) instead of switching your current branch. This means:
...
- Model attribution footer on each comment (e.g., _— qwen3-coder via atus code /review_)
...
1. `.atus-code/review-rules.md` (atus code native)
...
Example `.atus-code/review-rules.md`:
...
Cache is stored in `.atus-code/review-cache/` and tracks both the commit SHA and model ID. Make sure this directory is in your `.gitignore` (a broader rule like `.atus-code/*` also works). If the cached commit was rebased away, it falls back to a full review.

## Review Reports

For same-repo reviews, results are saved as a Markdown file in your project's `.atus-code/reviews/` directory (cross-repo lightweight reviews skip report persistence):

```
.atus-code/reviews/2026-04-06-143022-pr-123.md
.atus-code/reviews/2026-04-06-150510-local.md
```
