...
This document details all commands supported by atus code, helping you efficiently manage sessions, customize the interface, and control its behavior.

atus code commands are triggered through specific prefixes and fall into three categories:

| Prefix Type                | Function Description                                | Typical Use Case                                                 |
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------- |
| Slash Commands (`/`)       | Meta-level control of atus code itself              | Managing sessions, modifying settings, getting help              |
...
Slash commands are used to manage atus code sessions, interface, and basic behavior.
...
| `/qc-helper` | Answer questions about atus code usage and configuration            | `/qc-helper how do I configure MCP?`              |
...
| `/bug`          | Submit issue about atus code                    | `/bug Button click unresponsive` |
...
Use `/auth` inside a atus code session to configure authentication. Use `/doctor` to inspect the current authentication and environment status.
...
> [!note]
>
> The standalone `atus auth` CLI command has been removed. Legacy invocations such as `atus auth status` print a removal notice with migration guidance. See the [Authentication](../configuration/auth) page for full details.
...
Exclamation commands allow you to execute system commands directly within atus code.
...
Environment Variables: Commands executed via `!` will set the `ATUS_CODE=1` environment variable.
...
| Global Commands  | `~/.atus-code/commands/`                        | Available in all projects              | Low      | Personal frequently used commands, cross-project use |
| Project Commands | `<project root directory>/.atus-code/commands/` | Project-specific, version-controllable | High     | Team sharing, project-specific commands              |
...
| ---------------------------------------- | ----------------- | --------------------- |
| `~/.atus-code/commands/test.md`               | `/test`           | `/test Parameter`     |
| `<project>/.atus-code/commands/git/commit.md` | `/git:commit`     | `/git:commit Message` |
...
| ----------------------------- | ----------------------------------------- |
| 1. Create directory structure | `mkdir -p ~/.atus-code/commands/refactor`      |
| 2. Create command file        | `touch ~/.atus-code/commands/refactor/pure.md` |
