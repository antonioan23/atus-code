...
atus code includes a contextual tips system that helps you discover features and stay aware of session state.

## Startup Tips

Each time you launch atus code, a tip is shown in the header area. Tips are selected by priority first, then rotated across sessions using LRU (least-recently-used) scheduling among tips of the same priority, so you see a different tip each time.
...
During a conversation, atus code monitors your context window usage and shows tips when action may be needed:
...
Tip display history is persisted at `~/.atus-code/tip_history.json`. This file tracks:
...
To hide all tips (both startup and post-response), set `ui.hideTips` to `true` in `~/.atus-code/settings.json`:
