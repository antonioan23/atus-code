...
You can extend the channel system with custom platform adapters packaged as [extensions](../../extension/introduction). This lets you connect atus code to any messaging platform, webhook, or custom transport.

## How It Works

Channel plugins are loaded at startup from active extensions. When `atus channel start` runs, it:

1. Scans all enabled extensions for `channels` entries in their `atus-extension.json`
...
atus extensions install /path/to/my-channel-extension

# Or link it for development (changes are reflected immediately)
atus extensions link /path/to/my-channel-extension
```

## Configuring a Custom Channel

Add a channel entry to `~/.atus-code/settings.json` using the custom type provided by the extension:
