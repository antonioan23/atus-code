...
This guide covers setting up a atus code channel on WeChat via the official iLink Bot API.
...
```bash
atus channel configure-weixin
```

This will display a QR code URL. Scan it with your WeChat mobile app to authenticate. Your credentials are saved to `~/.atus-code/channels/weixin/account.json`.

### 2. Configure the channel

Add the channel to `~/.atus-code/settings.json`:
...
- **Session expiry** — If you see "Session expired (errcode -14)" in the logs, your WeChat login has expired. Stop the channel and re-run `atus channel configure-weixin` to log in again.
...
Run `atus channel configure-weixin` to log in via QR code first.

### "Session expired (errcode -14)"

Your WeChat login session has expired. Stop the channel and run `atus channel configure-weixin` again.

### Bot doesn't respond

- Check the terminal output for errors
- Verify the channel is running (`atus channel start my-weixin`)
