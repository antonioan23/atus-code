...
Every atus code session starts with a fresh context window. Two mechanisms carry knowledge across sessions so you don't have to re-explain yourself every time:
...
| ----------------------------- | ------------------------------------------------ |
| `~/.atus-code/QWEN.md`             | You, across all your projects                    |
| `QWEN.md` in the project root | Your whole team (commit it to source control)    |
| `.atus-code/QWEN.local.md`         | Only you, only in this project (keep out of git) |
...
#### When to use `.atus-code/QWEN.local.md`
...
**You must gitignore it yourself.** Although `.atus-code/` is often treated as a local directory, atus-code does not generate a `.gitignore` for you, and some projects commit `.atus-code/settings.json`. Add this line to your `.gitignore` (or to your global git ignore):

```
.atus-code/QWEN.local.md
```
...
Auto-memory files live at `~/.atus-code/projects/<project>/memory/`. All branches and worktrees of the same repository share the same memory folder, so what Qwen learns in one branch is available in others.
...
You can also set them in `~/.atus-code/settings.json` (applies to all projects) or `.atus-code/settings.json` (this project only):
...
- Open your personal QWEN.md (`~/.atus-code/QWEN.md`)
...
Open `/memory` to see which files are loaded. If your file isn't listed, Qwen can't see it — make sure it's in the project root or `~/.atus-code/`.
