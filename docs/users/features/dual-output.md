...
Dual Output is a sidecar mode for the interactive TUI: while atus code keeps
...
Embed atus code inside the IDE. The TUI runs in the editor's integrated
...
A CI job runs atus code with a task prompt. The human sees the TUI in the
...
Integration tests spawn atus code headlessly, drive it with `--input-file`
...
At first glance `--json-fd` looks sufficient — the caller spawns atus code
...
```ts
const child = spawn('atus', ['--json-fd', '3'], {
...
const pty = spawn(
  ...
  [
    '--json-file',
    '/tmp/atus-events.jsonl',
    '--input-file',
    '/tmp/atus-input.jsonl',
  ],
  { cols: 120, rows: 40 },
);
```
...
Run atus code with all three channels enabled:

```bash
mkfifo /tmp/atus-events.jsonl /tmp/atus-input.jsonl
atus \
  --json-file /tmp/atus-events.jsonl \
  --input-file /tmp/atus-input.jsonl
```

In a second terminal, tail the event stream:

```bash
cat /tmp/atus-events.jsonl
```

In a third terminal, push a prompt into the running TUI:

```bash
echo '{"type":"submit","text":"Explain this repo"}' >> /tmp/atus-input.jsonl
```

The prompt appears in the TUI exactly as if the user typed it, and the
streaming response is mirrored on `/tmp/atus-events.jsonl`.
...
A typical embedding parent process spawns atus code with both channels:

```ts
import { spawn } from 'node:child_process';
import { openSync } from 'node:fs';

const eventsFd = openSync('/tmp/atus-events.jsonl', 'w');
const child = spawn(
  'atus',
  ['--json-fd', '3', '--input-file', '/tmp/atus-input.jsonl'],
  { stdio: ['inherit', 'inherit', 'inherit', eventsFd] },
);
```
...
appending JSONL lines to `/tmp/atus-input.jsonl`.
...
```jsonc
// ~/.atus-code/settings.json  (user-level)
// or <workspace>/.atus-code/settings.json  (workspace-level)
{
  "dualOutput": {
    "jsonFile": "/tmp/atus-events.jsonl",
    "inputFile": "/tmp/atus-input.jsonl",
  },
}
```
...
next atus code launch, since the bridge is constructed once during
startup.
...
```bash
# Terminal A
mkfifo /tmp/atus-events.jsonl
cat /tmp/atus-events.jsonl | jq -c 'select(.type != "stream_event") | {type, subtype}'

# Terminal B
atus --json-file /tmp/atus-events.jsonl
...
```bash
# Terminal A
touch /tmp/atus-in.jsonl
atus --input-file /tmp/atus-in.jsonl
...
mkfifo /tmp/atus-out.jsonl
touch /tmp/atus-in.jsonl
(cat /tmp/atus-out.jsonl \
  | jq -c 'select(.type == "control_request")') &

# Terminal B
atus --json-file /tmp/atus-out.jsonl --input-file /tmp/atus-in.jsonl
...
The most realistic shape: a parent process spawns atus code, tails
...
const events = join(tmpdir(), `atus-events-${process.pid}.jsonl`);
const input = join(tmpdir(), `atus-input-${process.pid}.jsonl`);
writeFileSync(events, '');
writeFileSync(input, '');

const child = spawn('atus', ['--json-file', events, '--input-file', input], {
  stdio: 'inherit',
});
...
# atus code TUI opens in the current terminal; the embedder logs
# handshake + turn-end + session_end events to the parent's stdout.
```

### POC 5 — capability handshake feature detection

Older atus code versions won't emit `protocol_version`. Treat the field
...
rl.on('line', (line) => {
  ...
  if (ev.type === 'system' && ev.subtype === 'session_start') {
    ...
    if (v < 1) {
      console.error(
        'atus-code dual output is present but protocol < 1; ' +
          'falling back to best-effort behavior',
      );
    } else {
      console.log('atus-code dual output protocol v' + v);
    }
  }
});
```
