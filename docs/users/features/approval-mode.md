...
atus code offers five distinct permission modes that allow you to flexibly control how AI interacts with your code and system based on task complexity and risk level.
...
> [!tip]
>
> You can quickly cycle through modes during a session using **Shift+Tab** (or **Tab** on Windows). The terminal status bar shows your current mode, so you always know what permissions atus code has.

> The cycle order is: **plan → default → auto-edit → auto → yolo → plan → ...**

## 1. Use Plan Mode for safe code analysis

Plan Mode instructs atus code to create a plan by analyzing the codebase with **read-only** operations, perfect for exploring codebases, planning complex changes, or reviewing code safely.

### When to use Plan Mode

- **Multi-step implementation**: When your feature requires making edits to many files
- **Code exploration**: When you want to research the codebase thoroughly before changing anything
- **Interactive development**: When you want to iterate on the direction with atus code
...
atus code enters Plan Mode and analyzes the current implementation to create a comprehensive plan. Refine with follow-ups:
...
Ask Permissions Mode is the standard way to work with atus code. In this mode, you maintain full control over all potentially risky operations - atus code will ask for your approval before making any file changes or executing shell commands.
...
- **Learning and teaching**: When you want to understand each step atus code is taking
...
Ask Permissions Mode is the initial mode when you start atus code. If you've changed modes and want to return to Ask Permissions Mode, use:
...
```
atus --prompt "Analyze this code for potential bugs"
```
...
atus code will analyze your codebase and propose a plan. It will then ask for approval before:
...
Auto-Edit Mode instructs atus code to automatically approve file edits while requiring manual approval for shell commands, ideal for accelerating development workflows while maintaining system safety.
...
1. You ask atus code to refactor a function
...
atus code makes the file edits (in-workspace edits skip the classifier),
...
YOLO Mode grants atus code the highest permissions, automatically approving all tool calls including file editing and shell commands.
...
atus --prompt "Run the test suite, fix all failing tests, then commit changes"
...
During a atus code session, use **Shift+Tab**​ (or **Tab** on Windows) to quickly cycle through the four modes:
...
```
// Project-level: ./.atus-code/settings.json
// User-level: ~/.atus-code/settings.json
{
  "permissions": {
