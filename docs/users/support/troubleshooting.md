...
- **Error: `Qwen OAuth free tier was discontinued on 2026-04-15`**
  ...
  - **Solution:** Switch to a different authentication method. Run `atus` → `/auth` and choose one of:
...
- **Error: `Device authorization flow failed: fetch failed`**
  - **Cause:** Node.js could not reach Qwen OAuth endpoints (often a proxy or SSL/TLS trust issue). When available, atus code will also print the underlying error cause (for example: `UNABLE_TO_VERIFY_LEAF_SIGNATURE`). Note: this error is specific to the legacy Qwen OAuth flow.
  - **Solution:**
    - If you are still using Qwen OAuth, switch to API Key or Coding Plan via `/auth`.
    - If you are behind a proxy, set it via `atus --proxy <url>` (or the `proxy` setting in `settings.json`).
...
- **Issue: Unable to display UI after authentication failure**
  ...
  - **Solution:** Clear the `security.auth.selectedType` configuration item in your `settings.json` file:
    - Open `~/.atus-code/settings.json` (or `./.atus-code/settings.json` for project-specific settings)
    - Remove the `security.auth.selectedType` field
    - Restart the CLI to allow it to prompt for authentication again

## Frequently asked questions (FAQs)

- **Q: How do I update atus code to the latest version?**
  - A: If you installed atus code with the standalone installer, rerun the standalone install command. If you installed it globally via `npm`, update it using the command `npm install -g @atus-code/atus-code@latest`. If you compiled it from source, pull the latest changes from the repository, and then rebuild using the command `npm run build`.

- **Q: Where are the atus code configuration or settings files stored?**
  - A: The atus code configuration is stored in two `settings.json` files:
    1. In your home directory: `~/.atus-code/settings.json`.
    2. In your project's root directory: `./.atus-code/settings.json`.

    Refer to [atus code Configuration](../configuration/settings) for more details.
...
- **Error: Command not found (when attempting to run atus code with `atus`).**
  - **Cause:** The CLI is not correctly installed or it is not in your system's `PATH`.
  - **Solution:**
    The update depends on how you installed atus code:
    - If you installed `atus` with the standalone installer, rerun the standalone install command and then open a new terminal.
    - If you installed `atus` globally, check that your `npm` global binary directory is in your `PATH`. You can update using the command `npm install -g @atus-code/atus-code@latest`.
    - If you are running `atus` from source, ensure you are using the correct command to invoke it (e.g. `node packages/cli/dist/index.js ...`). To update, pull the latest changes from the repository, and then rebuild using the command `npm run build`.
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, atus code may attempt operations that are restricted by your sandbox configuration, such as writing outside the project directory or system temp directory.
...
- **atus code is not running in interactive mode in "CI" environments**
  - **Issue:** atus code does not enter interactive mode (no prompt appears) if an environment variable starting with `CI_` (e.g. `CI_TOKEN`) is set. This is because the `is-in-ci` package, used by the underlying UI framework, detects these variables and assumes a non-interactive CI environment.
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`, `CONTINUOUS_INTEGRATION`, or any environment variable with a `CI_` prefix. When any of these are found, it signals that the environment is non-interactive, which prevents the CLI from starting in its interactive mode.
  - **Solution:** If the `CI_` prefixed variable is not needed for the CLI to function, you can temporarily unset it for the command. e.g. `env -u CI_TOKEN atus`
...
- **DEBUG mode not working from project .env file**
  ...
  - **Solution:** Use a `.atus-code/.env` file instead, or configure the `advanced.excludedEnvVars` setting in your `settings.json` to exclude fewer variables.
...
- Restart the integrated terminal after installing the extension so it inherits:
  - `ATUS_CODE_IDE_WORKSPACE_PATH`
  - `ATUS_CODE_IDE_SERVER_PORT`
- If running in a container, verify `host.docker.internal` resolves. Otherwise, map the host appropriately.
- Reinstall the companion with `/ide install` and use “atus code: Run” in the Command Palette to verify it launches.

## Exit Codes

The atus code uses specific exit codes to indicate the reason for termination. This is especially useful for scripting and automation.
...
If you encounter an issue that was not covered here in this _Troubleshooting guide_, consider searching the atus code [Issue tracker on GitHub](https://github.com/atuscode/atus-code/issues). If you can't find an issue similar to yours, consider creating a new GitHub Issue with a detailed description. Pull requests are also welcome!
