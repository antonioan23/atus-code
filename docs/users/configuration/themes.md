...
atus code supports a variety of themes to customize its color scheme and appearance. You can change the theme to suit your preferences via the `/theme` command or `"theme":` configuration setting.

## Available Themes

atus code comes with a selection of pre-defined themes, which you can list using the `/theme` command within the CLI:
...
Selected themes are saved in atus code's [configuration](../configuration/settings) so your preference is remembered across sessions.

---

## Auto Theme Detection

When the theme is set to `"auto"` (or left unset), atus code automatically detects whether your terminal uses a dark or light background and selects the matching Qwen theme (`Qwen Dark` or `Qwen Light`).
...
atus code uses multiple detection methods in a fallback chain. At startup (async path), the order is:
...
atus code allows you to create your own custom color themes by specifying them in your `settings.json` file. This gives you full control over the color palette used in the CLI.
...
- Select your custom theme using the `/theme` command in atus code. Your custom theme will appear in the theme selection dialog.
