...
atus code supports three authentication methods. Pick the one that matches how you want to run the CLI:

- **Qwen OAuth**: sign in with your `atus.ai` account in a browser. **Free tier discontinued on 2026-04-15** — switch to another method.
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure atus code to use the dedicated endpoint and your subscription API key.
...
Enter `atus` in the terminal to launch atus code, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.
...
For CI, containers, or scripts, configure Coding Plan with environment variables or `settings.json` instead of the removed `atus auth coding-plan` command.
...
If you prefer to skip the interactive `/auth` flow, add the following to `~/.atus-code/settings.json`:
...
The simplest way to get started with API Key authentication is to put everything in a single `~/.atus-code/settings.json` file. Here's a complete, ready-to-use example:
...
| `security.auth.selectedType` | Tells atus code which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when atus code starts. Must match one of the `id` values in your `modelProviders`.                                |

After saving the file, just run `atus` — no interactive `/auth` setup needed.
...
The key concept is **Model Providers** (`modelProviders`): atus code supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.atus-code/settings.json`, then switch between them at runtime with the `/model` command.
...
#### Step 1: Configure models and providers in `~/.atus-code/settings.json`
...
> [!important]
>
> It is recommended to define `modelProviders` in the user-scope `~/.atus-code/settings.json` to avoid merge conflicts between project and user settings.

Edit `~/.atus-code/settings.json` (create it if it doesn't exist). You can mix multiple protocols in a single file — here is a multi-provider example showing just the `modelProviders` section:
...
atus code reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
atus code auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.atus-code/.env` (preferred — keeps atus code variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.atus-code/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with atus code behavior.

**3. `settings.json` → `env` field (lowest priority)**

You can also define API keys directly in `~/.atus-code/settings.json` under the `env` key. These are loaded as the **lowest-priority fallback** — only applied when a variable is not already set by the system environment or `.env` files.
...
After launching atus code, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
atus --model "qwen3-coder-plus"

# In another terminal

atus --model "qwen3.5-plus"
```

## Removed `atus auth` CLI command

The standalone `atus auth` CLI command has been removed. Use these replacements instead:

| Previous use case                | Replacement                                                                                 |
| -------------------------------- | ------------------------------------------------------------------------------------------- |
| Interactive authentication setup | Run `atus`, then use `/auth`                                                                |
...
| API-key or custom provider setup | Configure `~/.atus-code/settings.json`, `.env`, or provider-specific environment variables       |
| Check current authentication     | Run `/doctor` inside atus code                                                              |
| OAuth browser flow               | Run `atus` interactively and use `/auth`; OAuth cannot be configured with env vars alone    |

Legacy invocations such as `atus auth status` now print a removal notice with these migration paths.

## Security notes

- Don't commit API keys to version control.
- Prefer `.atus-code/.env` for project-local secrets (and keep it out of git).
