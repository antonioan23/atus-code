...
atus code allows you to configure multiple model providers through the `modelProviders` setting in your `settings.json`. This enables you to switch between different AI models and providers using the `/model` command.
...
| `atus-oauth` | Qwen OAuth (hard-coded, cannot be overridden in `modelProviders`)                       |
...
atus code uses the following official SDKs to send requests to each provider:
...
| `atus-oauth` | [`openai`](https://www.npmjs.com/package/openai) with custom provider (DashScope-compatible)    |
...
> [!note]
>
> The `extra_body` parameter is **only supported for OpenAI-compatible providers** (`openai`, `atus-oauth`). It is ignored for Anthropic, and Gemini providers.
...
When you authenticate with an Alibaba Cloud Coding Plan API key using the `/auth` command, atus code automatically configures the following models:
...
2. Run the `/auth` command in atus code
...
Coding Plan model configurations are versioned. When atus code detects a newer version of the model template, you will be prompted to update. Accepting the update will:
...
| Default / computed         | Falls back to `AuthType.ATUS_OAUTH` | Built-in default (OpenAI ⇒ `qwen3-coder-plus`)  | —                                                   | —                                                    | —                      | `Config.getProxy()` if configured |
...
```json
// User settings (~/.atus-code/settings.json)
{
  "model": {
    "generationConfig": {
      "timeout": 30000,
...
atus code distinguishes between two types of model configurations:
...
When you configure a model without using `modelProviders`, atus code automatically creates a RuntimeModelSnapshot to preserve your configuration:

```bash
# This creates a RuntimeModelSnapshot with ID: $runtime|openai|my-custom-model
atus --auth-type openai --model my-custom-model --openaiApiKey $KEY --openaiBaseUrl https://api.example.com/v1
```
