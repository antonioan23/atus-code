...
`/Users/mochi/code/atus-code/.claude/worktrees/tender-jemison-037f0a/dist/cli.js`.
...
- **Local build (Phase 6 verification)**: `node /Users/mochi/code/atus-code/.claude/worktrees/tender-jemison-037f0a/dist/cli.js`
- **Phase 4 dry-run baseline**: globally installed `atus`

For dry-runs the globally installed `atus` is expected to fail Groups A / E / F
...
The globally installed `atus` may predate PR #4073 (Phase A+B, merged 2026-05-14)
...
```bash
TEST_DIR=$(mktemp -d -t atus-wt-phd-XXXXXX)
...
QWEN="node /Users/mochi/code/atus-code/.claude/worktrees/tender-jemison-037f0a/dist/cli.js"
```

PR-ref tests (Group F) additionally require a checked-out clone of a public
GitHub repo with at least one merged PR. Use this repo (atus-code itself) as
...
jq -e '.[] | select(.type=="system" and .subtype=="init") | .cwd | test("/\\.atus-code/worktrees/[a-z]+-[a-z]+-[0-9a-f]{6}$")' < /tmp/a1.out

ls -d "$TEST_DIR/.atus-code/worktrees/"*
```

**Expected (post-impl):**

- `worktree_started` event with `.data.notice` containing the auto slug
- Init `.cwd` ends with `.atus-code/worktrees/<auto-slug>`
- Exactly one worktree directory under `.atus-code/worktrees/`
...
ls -d "$TEST_DIR/.atus-code/worktrees/my-feature"
...
ls "$TEST_DIR/.atus-code/worktrees/" 2>/dev/null
```
...
- `.atus-code/worktrees/` directory does not exist (worktree creation never started)
...
SIDECAR=~/.atus-code/projects/$PROJECT_ID/chats/$SESSION_ID.worktree.json
...
- `slug = "b1-test"`
- `worktreePath` ends with `.atus-code/worktrees/b1-test`
...
**Expected (post-impl):** the `pwd` output equals `$TEST_DIR/.atus-code/worktrees/b2-test`.
...
rm -rf "$TEST_DIR/.atus-code/worktrees/c2"   # simulate user-deleted dir
...
ls -d "$TEST_DIR/.atus-code/worktrees/d2-test"   # still exists
...
ls "$TEST_DIR/.atus-code/worktrees/d3-test" 2>/dev/null && echo "FAIL: dir exists"
git -C "$TEST_DIR" branch | grep "worktree-d3-test" && echo "FAIL: branch exists"
test ! -f ~/.atus-code/projects/$PROJECT_ID/chats/$SESSION_ID.worktree.json && echo "PASS: sidecar gone"
```
...
mkdir -p "$TEST_DIR/.atus-code"
cat > "$TEST_DIR/.atus-code/settings.json" <<'EOF'
{
  "worktree": {
...
ls -la "$TEST_DIR/.atus-code/worktrees/e1-test/node_modules"
readlink "$TEST_DIR/.atus-code/worktrees/e1-test/node_modules"
```
...
readlink "$TEST_DIR/.atus-code/worktrees/e2-test/node_modules"
```
...
ls -la "$TEST_DIR/.atus-code/worktrees/"agent-*/node_modules 2>/dev/null | head -3
```
...
ls -d "$TEST_DIR/.atus-code/worktrees/e4-test"
ls "$TEST_DIR/.atus-code/worktrees/e4-test/does-not-exist" 2>/dev/null && echo "UNEXPECTED"
```
...
mkdir -p "$TEST_DIR/.atus-code/worktrees/e5-test/node_modules"
echo "preexisting" > "$TEST_DIR/.atus-code/worktrees/e5-test/node_modules/.marker"
...
test -f "$TEST_DIR/.atus-code/worktrees/e5-test/node_modules/.marker" && echo "PASS: not overwritten"
```
...
ls "$TEST_DIR/.atus-code/worktrees/e6-test/" | head -10
```
...
```bash
# Use atus-code itself as the test repo
TEST_DIR=$(mktemp -d -t atus-wt-phd-pr-XXXXXX)
TEST_DIR=$(cd "$TEST_DIR" && pwd -P)
cd "$TEST_DIR"
git clone --depth 1 https://github.com/atuscode/atus-code.git .
PROJECT_ID=$(node -e "console.log(process.argv[1].replace(/[^a-zA-Z0-9]/g,'-'))" "$TEST_DIR")
```
...
ls -d "$TEST_DIR/.atus-code/worktrees/pr-4174"
git -C "$TEST_DIR/.atus-code/worktrees/pr-4174" rev-parse --abbrev-ref HEAD
```
...
```bash
$QWEN --worktree "https://github.com/atuscode/atus-code/pull/4174" "say hi" \
...
readlink "$TEST_DIR/.atus-code/worktrees/pr-4174/node_modules"
```
...
# File survived
cat "$TEST_DIR/.atus-code/worktrees/g1-test/work.txt"
...
### Phase 4 dry-run — baseline `atus` v0.15.11 (2026-05-20)
...
**Binary**: `node /Users/mochi/code/atus-code/.claude/worktrees/tender-jemison-037f0a/dist/cli.js`
**Date**: 2026-05-20
**Scope**: Groups A, B, C, E, F, G (6 parallel `test-engineer` agents)

| Group                              | Result                    | Notes                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1 (bare flag)                     | ✅ (with doc tip)         | yargs consumes the next positional as the slug value when user passes `atus --worktree "say hi"`; quickstart now tells users to use `=` form or put the prompt before the flag. Auto-slug feature itself confirmed via `atus --worktree --approval-mode yolo "say hi"` → slug `bright-elm-8a4c12`, init `.cwd` ends with `.atus-code/worktrees/<auto-slug>`.                                     |
| A2 (explicit slug)                 | ✅                        | dir `.atus-code/worktrees/my-feature` + branch `worktree-my-feature`                                                                                                                                                                                                                                                                                                                             |
...
**Binary**: `atus` (globally installed, v0.15.11 at `/Users/mochi/.nvm/versions/node/v22.21.1/bin/atus`)
**Override**: `QWEN="atus"`
