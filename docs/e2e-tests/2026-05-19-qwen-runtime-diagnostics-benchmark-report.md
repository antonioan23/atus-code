# atus code Runtime Diagnostics Benchmark Report

Date: 2026-05-19

## Scope

This run repeats the previous atus code benchmark shapes with the new opt-in
runtime diagnostics enabled. It only tests atus code, not Claude Code.
...
`ATUS_CODE_PROFILE_RUNTIME=1` and a temporary CLI home. Global MCP servers and
hooks were not loaded for this benchmark.

Important caveat: these absolute RSS numbers are lower than the previous
PATH-resolved `atus` runs because this run used `node dist/cli.js` from the
...
intentional variable was whether atus code loaded a stripped temporary CLI home
or the normal user config.

| CLI                 | Config mode     | Total tokens | Tree RSS peak | Root RSS peak | Process count peak | Runtime diagnostics |
| ------------------- | --------------- | -----------: | ------------: | ------------: | -----------------: | ------------------- |
| PATH `atus`         | stripped config |       33,965 |     542.4 MiB |     249.9 MiB |                  3 | no                  |
| local `dist/cli.js` | stripped config |       47,281 |     455.2 MiB |     214.2 MiB |                  4 | yes                 |
| PATH `atus`         | normal config   |       97,615 |   1,099.9 MiB |     250.1 MiB |                  6 | no                  |
...
| ------------------- | -------- | ---: | ----: | --------: | ------------: | ------------: | -----------------: |
| PATH `atus`         | normal   | 5.5s |     1 |         0 |   1,021.3 MiB |     251.5 MiB |                  5 |
| PATH `atus`         | `--bare` | 2.4s |     1 |         0 |     525.7 MiB |     246.4 MiB |                  2 |
...
benchmark, a follow-up used temporary `ATUS_HOME` directories with generated
...
| ------------------- | -------------------- | ----------: | ----: | ------------: | ------------: | -----------------: |
| PATH `atus`         | full                 |           4 |    46 |   1,017.4 MiB |     249.8 MiB |                  5 |
| PATH `atus`         | MCP disabled         |           0 |    17 |     548.7 MiB |     252.4 MiB |                  2 |
| PATH `atus`         | hooks disabled       |           4 |    46 |   1,003.8 MiB |     246.4 MiB |                  5 |
| PATH `atus`         | MCP + hooks disabled |           0 |    17 |     542.5 MiB |     248.0 MiB |                  2 |
...
2. The local bundle run peaks around 0.36-0.41 GiB, not the earlier
   ...
   both PATH `atus` and local `dist/cli.js`, with the extra memory coming from
...
| ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| [`#4309`](https://github.com/atuscode/atus-code/issues/4309)                                                             | User reports 5.84 GiB memory usage / 7.02 GiB warning with YOLO mode and DeepSeek backend; increasing Node memory to 8 GiB did not remove the symptom | Long autonomous tool loops can retain enough state that simply raising old-space limit is not a root fix                         |
| [`#4149`](https://github.com/atuscode/atus-code/issues/4149)                                                             | Multiple reports show `Ineffective mark-compacts near heap limit`, including 4 GiB and much larger heap-limit cases                                   | A large fraction of heap is reachable application state, not immediately collectible garbage                                     |
| [`#4116`](https://github.com/atuscode/atus-code/issues/4116)                                                             | OOM occurred while context display was around 9.5%; analysis points to `structuredClone`, UI history, Ink static tree, and large context windows      | Token usage can be low while JS heap pressure is high; token threshold alone is not a reliable memory guard                      |
| [`#4167`](https://github.com/atuscode/atus-code/issues/4167)                                                             | User says the crash happened while compressing; analysis identifies compression peak memory as a distinct shape                                       | Compression can itself create a peak when heap is already high, especially if history is cloned/stringified around the same time |
| [`#2128`](https://github.com/atuscode/atus-code/issues/2128)                                                             | Report identifies unbounded UI history, retained file diffs / terminal output, string-width caches, and checkpoint serialization                      | Interactive TUI long sessions may retain memory outside model history and outside non-interactive benchmarks                     |
| [`#2562`](https://github.com/atuscode/atus-code/issues/2562)                                                             | Report focuses on `GeminiChat.getHistory()` deep-cloning full history in long sessions                                                                | Full-history cloning can amplify memory peaks and should be measured separately from retained steady-state size                  |
| [`#4185`](https://github.com/atuscode/atus-code/issues/4185)                                                             | Tracks V8 heap pressure exceeding limit before token-based compaction runs                                                                            | Heap-pressure guard is necessary, but it only mitigates symptoms if retained data remains large                                  |
| [`#4184`](https://github.com/atuscode/atus-code/issues/4184)                                                             | Proposes diagnostics and offload/preview for large retained tool results                                                                              | Large tool output may be bounded for model requests while still retained in local hot memory                                     |
| [`#4186`](https://github.com/atuscode/atus-code/pull/4186)                                                               | Merged heap-pressure auto-compaction safety net and O(1) last-history access for `nextSpeakerChecker`                                                 | Covers part of heap-pressure and clone amplification, but does not claim to solve all OOM classes                                |
| [`#4127`](https://github.com/atuscode/atus-code/pull/4127), [`#4168`](https://github.com/atuscode/atus-code/pull/4168)     | Open compaction-threshold PRs; one uses fixed heap thresholds, the other redesigns token thresholds and compression behavior                          | Useful related work, but long-task testing must verify whether heap, token, and compression signals line up in real runs         |
| [`#3000`](https://github.com/atuscode/atus-code/issues/3000), [`#4183`](https://github.com/atuscode/atus-code/issues/4183) | Diagnostic roadmap calls out `/doctor memory`, heap snapshot, and bounded memory timeline                                                             | Snapshot/timeline support is needed to move from RSS attribution to retained-object attribution                                  |
...
- Installed atus code `0.15.11`, model `atus-latest-series-invite-beta-v28`.
...
After the initial model matrix, an additional atus code-only run tested
...
5. The DeepSeek run reinforces the model-choice conclusion: provider/model
   ...
   but the local bundle RSS peak remains dominated by atus code runtime shape
...
| ----------------------- | ----------------------------- |
| Duration                | 87.0 min                      |
| atus code version       | 0.15.10                       |
| Model                   | atus-latest-series beta model |
| API responses           | 380                           |
