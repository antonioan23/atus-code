...
`/Users/mochi/code/atus-code/.claude/worktrees/romantic-burnell-b6e48c/dist/cli.js`.

Phase C delivers:

- **Task 1, 3, 4** — `WorktreeSession` sidecar JSON file at
  `~/.atus-code/tmp/<projectHash>/chats/<sessionId>.worktree.json`
...
- **Local build**: `node /Users/mochi/code/atus-code/.claude/worktrees/romantic-burnell-b6e48c/dist/cli.js`
- **Baseline (for pre-impl comparison if needed)**: globally installed `atus`
...
```bash
TEST_DIR=$(mktemp -d -t atus-wt-phc-XXXXXX)
...
`QWEN=/Users/mochi/code/atus-code/.claude/worktrees/romantic-burnell-b6e48c/dist/cli.js`
...
SIDECAR=~/.atus-code/projects/$PROJECT_ID/chats/$SESSION.worktree.json
...
- `slug` = "a1-test"
- `worktreePath` ends with `.atus-code/worktrees/a1-test`
...
test ! -d "$TEST_DIR/.atus-code/worktrees/a3-test" && echo "PASS: worktree dir removed"
```
...
HOOKS_PATH=$(git -C "$TEST_DIR/.atus-code/worktrees/b1-test" config --local core.hooksPath)
...
HOOKS_PATH=$(git -C "$TEST_DIR/.atus-code/worktrees/b2-test" config --local core.hooksPath)
...
#!/bin/sh
echo "hook-fired" > /tmp/atus-wt-hook-marker
EOF
...
WT="$TEST_DIR/.atus-code/worktrees/b3-test"
echo "x" > "$WT/file.txt"
git -C "$WT" add file.txt
rm -f /tmp/atus-wt-hook-marker
git -C "$WT" commit -m "trigger hook" 2>&1
test -f /tmp/atus-wt-hook-marker && echo "PASS: hook fired" || echo "FAIL: hook did not fire"
rm -f /tmp/atus-wt-hook-marker
```

**Expected:** `/tmp/atus-wt-hook-marker` exists after the commit.
...
rm -rf "$TEST_DIR/.atus-code/worktrees/c2-test"
...
WT="$TEST_DIR/.atus-code/worktrees/e2-test"
echo "new" > "$WT/new.txt"
...
test -d "$TEST_DIR/.atus-code/worktrees/e3-test" && echo "worktree intact" || echo "FAIL: worktree gone"
...
test -d "$TEST_DIR/.atus-code/worktrees/e4-test" && echo "PASS: worktree preserved" || \
...
test ! -d "$TEST_DIR/.atus-code/worktrees/e5-test" && echo "PASS: worktree removed" || \
...
WT="$TEST_DIR/.atus-code/worktrees/f1-feature"
...
SETTINGS_DIR=~/.atus-code
SETTINGS_FILE=$SETTINGS_DIR/settings.json
cp -f "$SETTINGS_FILE" /tmp/atus-settings-backup.json 2>/dev/null || true
mkdir -p "$SETTINGS_DIR"
SL_SCRIPT=/tmp/atus-wt-statusline.sh
cat > $SL_SCRIPT <<'EOF'
#!/bin/sh
INPUT=$(cat)
echo "$INPUT" > /tmp/atus-wt-statusline-input.json
...
cat /tmp/atus-wt-statusline-input.json | jq '.worktree.name, .worktree.path, .worktree.branch'
...
# Restore settings
cp -f /tmp/atus-settings-backup.json "$SETTINGS_FILE" 2>/dev/null || rm -f "$SETTINGS_FILE"
```

**Expected:**

- `/tmp/atus-wt-statusline-input.json` has `.worktree.name == "f2-test"`, `.path`, `.branch` set
