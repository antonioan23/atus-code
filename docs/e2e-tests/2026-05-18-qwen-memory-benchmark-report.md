# atus code Runtime Memory Benchmark Report

Date: 2026-05-18

## Summary

This report records local memory benchmarks for atus code runtime behavior. It
compares atus code across models and compares atus code with Claude Code on the
...
- atus code process-tree RSS peak: about `852-1062 MiB` (`0.83-1.04 GiB`).
- Claude Code process-tree RSS peak: about `279-366 MiB` (`0.27-0.36 GiB`).
- atus code was about `2.3x-3.6x` higher in the tested
...
| atus code version                             | `0.15.11`                                  |
| atus code binary                              | PATH-resolved `atus` binary                |
...
Process-tree RSS is used as the headline metric because atus code launches a
...
1. atus code PR review memory run.
2. atus code model comparison run.
3. Strict atus code vs Claude Code comparison with `pai/glm-5`.
4. atus code vs Claude Code, two CLIs by two models.
5. atus code vs Claude Code, five-case matrix.
...
The current data is strong enough to say that atus code has a higher runtime
...
The leading explanation is a atus code runtime/path difference rather than a
...
- the gap reproduces in small PR and code-navigation tasks, not only in large
  diff tasks;
- atus code repeatedly sends or accounts for more tokens than Claude Code for
  similar work;
- atus code's largest observed component is the child Node/Qwen worker process,
...
1. **Task-time runtime cost first**: atus code likely initializes or retains
...
2. **Context/tool-result volume second**: atus code appears to carry larger
...
- 2 CLIs: atus code and Claude Code.
...
The same matrix also showed atus code using more model-side tokens in every
tested case.
...
- atus code used more total tokens than Claude Code in every matrix cell.
...
| -------------- | ----------- | --------------------: |
| `pai/glm-5`    | atus code   |            1000.7 MiB |
| `pai/glm-5`    | Claude Code |             349.0 MiB |
| `qwen3.6-plus` | atus code   |            1095.8 MiB |
...
1. atus code has a higher non-interactive task-time process footprint than
...
4. Context/tool-result handling is still a likely contributor. atus code used
...
- Claude Code and atus code use different runtime implementations and protocol
...
current RSS comparison can be connected to internal atus code state.
