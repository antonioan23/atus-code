...
Baseline binary: globally installed `atus` (0.15.10).
Local build binary: `node /Users/mochi/code/atus-code/.claude/worktrees/trusting-euclid-6fdfb9/bundle/atus.js`.
...
```bash
<atus> "say hello" --approval-mode yolo --output-format json 2>/dev/null \
...
```bash
<atus> "create a new git worktree using the enter_worktree tool" \
  --approval-mode yolo --output-format json 2>/dev/null > /tmp/a2.json
# Check worktree dir created
ls -la .atus-code/worktrees/ | grep -v "^\." | wc -l
# Should have a directory matching the auto-generated slug pattern
```

**Pre-implementation:** model says it can't find the tool; no `.atus-code/worktrees/` directory.
**Post-implementation:** `.atus-code/worktrees/<slug>` exists with auto-generated slug (format: `{adj}-{noun}-{4hex}`).
...
```bash
<atus> "use the enter_worktree tool with name='my-feature' to create a worktree" \
  --approval-mode yolo --output-format json 2>/dev/null
ls .atus-code/worktrees/my-feature/
git branch | grep worktree-my-feature
```

**Pre-implementation:** tool unknown.
**Post-implementation:** `.atus-code/worktrees/my-feature/` directory exists; branch `worktree-my-feature` exists.
...
```bash
<atus> "use enter_worktree with name='../../../etc' to create a worktree" \
  --approval-mode yolo --output-format json 2>/dev/null \
...
```bash
<atus> "create a worktree named 'temp-keep' using enter_worktree, then immediately exit it with action='keep' using exit_worktree" \
  --approval-mode yolo --output-format json 2>/dev/null > /tmp/b1.json
# Directory should still exist (keep preserves it)
ls -d .atus-code/worktrees/temp-keep
...
```bash
<atus> "create a worktree named 'temp-remove' using enter_worktree, then immediately exit it with action='remove' using exit_worktree" \
  --approval-mode yolo --output-format json 2>/dev/null
ls -d .atus-code/worktrees/temp-remove 2>&1
...
```bash
tmux new-session -d -s wt-test-b3 -x 200 -y 50 "cd $TEST_DIR && <atus> --approval-mode yolo"
sleep 3
...
echo "dirty" > "$TEST_DIR/.atus-code/worktrees/dirty-test/dirty.txt"
...
```bash
SESSION_ID=$(<atus> "create a worktree named 'persist-test' using enter_worktree" \
  --approval-mode yolo --output-format json 2>/dev/null \
  | jq -r 'select(.type=="system") | .session_id' | head -1)
# Check session storage for worktree state
find ~/.atus-code -name "*${SESSION_ID}*" 2>/dev/null | head
grep -l "persist-test" ~/.atus-code/projects/*/sessions/*.json 2>/dev/null || \
  grep -rl "worktreeSession\|persist-test" ~/.atus-code/projects/ 2>/dev/null | head -5
```
...
```bash
<atus> "spawn an agent using the agent tool with isolation='worktree' to run 'echo hello'" \
  --approval-mode yolo --output-format json 2>/dev/null \
  | jq 'select(.type=="assistant") | .message.content[] | select(.type=="tool_use" and .name=="agent") | .input'
# Check that .atus-code/worktrees/ contains an agent-* slug during execution
```
...
```bash
ls .atus-code/worktrees/ > /tmp/d2-before.txt 2>/dev/null
<atus> "spawn an agent with isolation='worktree' to list files in the current directory using ls" \
  --approval-mode yolo --output-format json 2>/dev/null
ls .atus-code/worktrees/ > /tmp/d2-after.txt 2>/dev/null
...
```bash
<atus> "spawn an agent with isolation='worktree' to write 'test content' to a new file called test.txt" \
  --approval-mode yolo --output-format json 2>/dev/null > /tmp/d3.json
# Worktree should be preserved with the change
ls .atus-code/worktrees/agent-* 2>/dev/null
ls .atus-code/worktrees/agent-*/test.txt 2>/dev/null
...
**Post-implementation:** `.atus-code/worktrees/agent-<7hex>/test.txt` exists; agent result mentions worktree path and branch.
...
E2E spot check (optional): manually `touch -t 200001010000 .atus-code/worktrees/agent-aabcdef0` and invoke cleanup; verify removal.

## Test Group F: Arena compatibility (no regression)

### F1: Arena worktree path unchanged

**Steps:** Run an Arena session (separate from EnterWorktree); verify it still creates worktrees under `~/.atus-code/arena/<sessionId>/worktrees/` and not under `.atus-code/worktrees/`.

```bash
# Setup: requires Arena-enabled config. Detailed steps depend on Arena CLI invocation.
# Pre-implementation: arena worktrees are under ~/.atus-code/arena/.
...
| A2    | error/no-op        | `.atus-code/worktrees/<auto-slug>` created                |
| A3    | error/no-op        | `.atus-code/worktrees/my-feature` created, branch present |
...
| A3    | ✅                                   | `.atus-code/worktrees/my-feature` created, branch `worktree-my-feature` present                                                                                            |
