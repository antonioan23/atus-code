...
多个 issue（#4309, #4276, #4185, #4315, #4322, #2868）报告 atus-code 在长会话中出现 V8 heap OOM crash：
...
| 工作目录                 | atus-code monorepo (3538 .ts files, 1.26M lines)             |
...
- 使用临时 `ATUS_HOME`，禁用 MCP 和 hooks，避免本地全局配置污染
- 只用进程外采样统计 process-tree RSS

| CLI                  | 结果 |   时长 | Tree RSS 峰值 | Root RSS 峰值 | Worker RSS 峰值 | 备注                                                        |
| -------------------- | ---- | -----: | ------------: | ------------: | --------------: | ----------------------------------------------------------- |
| installed `atus`     | 成功 | 167.3s |     838.0 MiB |     230.2 MiB |       566.3 MiB | 第一次 fresh run 遇到模型服务端错误，未纳入结论；retry 成功 |
...
while true; do
  ...
  ATUS_PID=$(ps aux | grep "dist/index.js" | grep -v grep | awk '{print $2}' | sort -rn | head -1)
  RSS=$(ps -o rss= -p $ATUS_PID 2>/dev/null)
...
# 3. 启动 atus (128K context model, 512MB heap)
SESSION="oom-test"
