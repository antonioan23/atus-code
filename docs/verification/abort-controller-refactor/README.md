...
```sh
tmux new-session -d -s atus-verify-XX
tmux pipe-pane -t atus-verify-XX -o "cat >> $LOGDIR/XX-name.log"
tmux send-keys -t atus-verify-XX "cd /path/to/your/test/workspace && exec node $WT/packages/cli/dist/index.js" C-m
tmux attach -t atus-verify-XX
```

Then drive the session manually per the matrix below. Hit `C-b d` to detach
when done; `tmux kill-session -t atus-verify-XX` to stop the pane.
...
- **Setup:** `NODE_OPTIONS=--trace-warnings DEBUG=1 atus`.
...
- **Setup:** `atus --prompt "do a long task"`; send `SIGINT` from outside via `kill -INT <pid>`.
...
- **Setup:** `atus --inspect`, attach Chrome devtools.
