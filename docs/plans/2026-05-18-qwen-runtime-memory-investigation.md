# atus code Runtime Memory Investigation Plan

Date: 2026-05-18

## Context

Local benchmarks show atus code using substantially more process-tree RSS than
Claude Code for similar non-interactive CLI task shapes. The latest five-case
matrix found atus code peaking around `0.83-1.04 GiB` while Claude Code stayed
...
- atus code has been compared across multiple models.
- atus code and Claude Code have been compared on the same task shapes where
...
- `docs/e2e-tests/2026-05-18-atus-memory-benchmark-report.md`
...
- atus code used more tokens than Claude Code in every tested matrix cell.
...
The current data points toward a atus code runtime/path issue more than a model
provider issue.

The strongest current inference is:

> atus code appears to carry a high non-interactive CLI task execution
...
problem is that atus code initializes or retains heavier state for the same
...
1. **Process and module startup/execution cost**: atus code may initialize more
   runtime, tools, UI/session infrastructure, or provider machinery than needed
   for non-interactive CLI tasks.
2. **History and context assembly**: atus code may retain or construct larger
...
2. Add or extend local diagnostic output so atus code can report:
...
history loading. The implementation should still follow atus code's own
...
The intended claim is narrower: atus code shows a consistent local RSS gap in
